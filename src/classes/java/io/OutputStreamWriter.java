 ...
 * could be any OutputStream). We shouldn't create a very short living JPF
 * byte[] object per write() call, since that kills the heap. We shouldn't use
 * an explicit lock since that blows up the state space. That leaves us with
 * atomic sections, but it's not really safe since clients might use their own
 * OutputStream classes with synchronized write() methods. At least this would
 * not go unnoticed
 */
public class OutputStreamWriter extends Writer {
	...
	public OutputStreamWriter(OutputStream os, Charset cs) {
		...
		throw new UnsupportedOperationException(
				"OutputStreamWriter model does not fully implement this constructor");
	}

...
/**
 * MJI adapter for java.lang.String, based on jdk 1.7.0_07 source. Adapted by
 * frank.
 * 
 * We have to model java.lang.String since it changed its implementation between
 * Java 1.6 and Java 1.7, and JPF is initializing string objects internally
 * without doing roundtrips (for performance reasons), i.e. we need to be able
 * to rely on fields of the String implementation.
 * 
...
public final class String implements java.io.Serializable, Comparable<String>,
	...
	native public void getChars(int srcBegin, int srcEnd, char dst[],
			int dstBegin);

	@Deprecated
	native public void getBytes(int srcBegin, int srcEnd, byte dst[],
			int dstBegin);
	...
	/**
	 * we can't turn this into a native method at top level since it would
	 * require a bunch of round trips
	 */
	public boolean contentEquals(CharSequence charSequence) {
		...
		if (charSequence instanceof AbstractStringBuilder) {
			return equals0(value,
					((AbstractStringBuilder) charSequence).getValue(),
					value.length);
		}
	...
	public String replace(CharSequence target, CharSequence other) {
		// No CharSequence model
		int PATTERN = 0x10;
		Matcher pattern = Pattern.compile(target.toString(), PATTERN).matcher(
				this);

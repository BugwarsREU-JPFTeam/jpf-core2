...
/**
 * our model of java.lang.ThreadGroup, which we need since the VM relies on
 * certain fields during bootstrapping
 * 
 * Note that we don't have a native peer, which would help with atomic container
 * updates, because most of these methods are called by the VM and we don't want
 * to introduce a dependency from VM to the peers. This is esp. true since we
 * would have to dig out & cast the peer, call it, and then go backwards
 * extracting the *Info objects we already had in the calling context.
 * 
 * If we ever want to introduce a peer, it could just delegate to the respective
 * ThreadInfo methods.
 */
public class ThreadGroup implements Thread.UncaughtExceptionHandler {
	private final ThreadGroup parent;

	// --- those are package visible, so we better keep names and access
	// modifiers
	...
	// group updates are just class internal, whereas thread updates happen from
	// java.lang.Thread and hence need
	// package visibility
		...
		if (!destroyed) {
			for (int i = 0; i < ngroups; i++) {
				if (groups[i] == childGroup) {
					...
					// check if its time for a suicide in case we are a daemon
					// group
					if (daemon) {
						if ((nthreads == 0) && (nUnstartedThreads == 0)
								&& (ngroups == 0)) {
							destroy();
						}
					}
	...
	// thread add/remove happens from the native peer, to avoid additional CGs
	// during creation/termination
	// since the ThreadGroup is per se a shared object. We just include these
	// methods here as a specification
	// for ThreadInfo

	/**
	 * called from the Thread ctor, i.e. before it is started NOTE - this is
	 * implemented as a native method to avoid shared field CGs
	 */
	...
	/**
	 * this is called during Thread.start() NOTE - this is implemented as a
	 * native method to avoid shared field CGs
	 */
		...
		if (!destroyed) {
			for (int i = 0; i < nthreads; i++) {
				if (threads[i] == terminatedThread) {
					...
					// OpenJDK does this no matter if the group was destroyed,
					// but that looks like a bug
					// that could lead to multiple destroys and notifyAll()
					// signals
					if (nthreads == 0) {
						notifyAll();
					}
	...
	/**
	 * this sets the maxPriority to the min of the argument and the parents
	 * maxPriority and raises maxPriority for all our threads that have a lower
	 * one
	 */
	public final synchronized void setMaxPriority(int newMaxPriority) {
		...
		if (newMaxPriority >= Thread.MIN_PRIORITY
				&& newMaxPriority <= Thread.MAX_PRIORITY) {
			maxPriority = (parent != null) ? Math.min(parent.maxPriority,
					newMaxPriority) : newMaxPriority;
	...
	/**
	 * copy threads into provided list kind of a misnomer
	 */
	...
	private synchronized int enumerate(ThreadGroup[] dst, int idx,
			boolean recurse) {
		int n = 0;
	...
	// we handle this in ThreadInfo, but again this is a public method that
	...
	public void uncaughtException(Thread t, Throwable x) {
		if (parent != null) { // if we have a parent, delegate
			...
			Thread.UncaughtExceptionHandler xh = Thread

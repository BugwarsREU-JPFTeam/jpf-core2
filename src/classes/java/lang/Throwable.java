 ...
 * 
 * the main reason for having one is to defer the stacktrace creation, which we
 * don't want if all an application does is intercepting a gazillion of
 * exceptions like NoSuchMethodException
 */
public class Throwable {

	int[] snapshot; // this is an internal array of { globalMethodId, pcOffset }
					// pairs

	protected Throwable cause; // in case this is a wrapper exception (like
								// InvocationTargetException)

	protected String detailMessage; // where we keep the context specific info

	protected StackTraceElement[] stackTrace; // only set on demand, if
												// getStackTrace() is called

	public Throwable() {
		try { // Use Class.forName() instead of new StackTraceElement() since
				// the latter creates garbage.
			Class.forName("java.lang.StackTraceElement"); // Force this class to
															// load here instead
															// of in
															// createStackTrace().
	...
	int getStackTraceDepth() {
		return (snapshot.length / 2); // snapshot stores the methodId and pc for
										// each StackFrame
	}
}

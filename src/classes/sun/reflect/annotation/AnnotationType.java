...
/**
 * this is a placeholder for a Java 6 class, which we only have here to support
 * both Java 1.5 and 6 with the same set of env/ classes
 * 
 * this is Java only, so it's a drag we have to add this, but since it is
 * outside java.* and doesn't refer to Java 6 stuff outside the
 * sun.misc.SharedSecrets we bite the bullet and add it (for now)
 * 
...
public class AnnotationType {
	...
	public static synchronized AnnotationType getInstance(
		...
		AnnotationType at = SharedSecrets.getJavaLangAccess()
	...
	private AnnotationType(final Class<?> annoCls) {
		...
		if ((annoCls != Retention.class) && (annoCls != Inherited.class)) { // don't
																			// get
																			// recursive

...
public class Crossing {
	...
	void solve() {
		...
		while (!haveAllPersonsCrossed()) {
			...
			if (bridge.arePersonsOnBridge()) {
				...
				// Verify.ignoreIf(elapsedTime > 17); //with this DFS will also
				// find error
	...
	void printCrossingState() {
		System.out
				.printf("%20s %s====%s %-20s : elapsed time=%d\n",
						personsOnSide(Side.LEFT), torchSymbol(Side.LEFT),
						torchSymbol(Side.RIGHT), personsOnSide(Side.RIGHT),
						elapsedTime);
	}

	public static void main(String[] args) {
		Person[] persons = { new Person("Bill", 1), new Person("Xoe", 2),
				new Person("Sue", 5), new Person("Joe", 10) };

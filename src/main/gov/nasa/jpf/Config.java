 ...
 * and robust.
 * 
 * Except of JPF and Config itself, all JPF classes are loaded by a Classloader
 * that is constucted by Config (e.g. by collecting jars from known/configured
 * locations), i.e. we SHOULD NOT rely on any 3rd party libraries within Config.
 * The class should be as autarkical as possible.
 * 
 * 
 * PROPERTY SOURCES ----------------
 * 
 * (1) one site.properties - this file specifies the location of the jpf-core
 * and installed extensions, like:
 * 
 * jpf-core = /Users/pcmehlitz/projects/jpf/jpf-core ... jpf-numeric =
 * /Users/pcmehlitz/projects/jpf/jpf-numeric ... extensions = ${jpf-core}
 * 
 * Each key/directory that is in site.properties is used to locate a
 * corresponding project property (jpf.properties) file
 * 
 * (2) any number of jpf.properties project properties files - each directory
 * entry in the 'extensions' list is checked for a jpf.properties file, which is
 * automatically loaded if found. Project properties mostly contain path
 ...
 * specific JPF run, esp. listener and target/target.args. app properties can be
 * specified as the sole JPF argument, i.e. instead of a SUT classname .. target
 * = x.Y.MySystemUnderTest target.args = one,two .. listener = z.MyListener
 * 
 * (4) commandline properties - all start with '+', they can override all other
 * props
 * 
 * 
 * LOOKUP ORDER ------------ property lookup property type : spec : default
 * ----------------:-----------------------:---------- | site : +site :
 * "${user.home}/[.]jpf/site.properties" | : : | project : 'extensions' value :
 * set in site.properties | : : | app : +app : - | : : v cmdline : +<key>=<val>
 * : -
 * 
...
public class Config extends Properties {
	...
	public static final String PATH_SEPARATOR = ","; // the default for
														// automatic appends
	...
	// it seems bad design to keep ClassLoader management in a glorified
	// Properties object,
	// but a lot of what Config does is to resolve configured types, for which
	// we need
	...
	// non-property/option command line args (starting from the first arg that
	// is not prepened by '-','+')
	String[] freeArgs;
	...
	public Config(String[] cmdLineArgs) {
		...
		// we need the app properties (*.jpf) pathname upfront because it might
		// define 'site'
		...
		// --- get the project properties from current dir + site configured
		// extensions
		...
		// note that global path collection now happens from initClassLoader(),
		// to
		// accommodate for deferred project initialization when explicitly
	...
	String getSitePropertiesLocation(String[] args, String appPropPath) {
		...
		if (path == null) {
			...
			// NOTE: we might want to drop this in the future because it
			// constitutes
				...
				if (len > keyLen + 2) {
					if (a.charAt(0) == '+' && a.charAt(keyLen + 1) == '=') {
						if (a.substring(1, keyLen + 1).equals(key)) {
							String val = expandString(key,
									a.substring(keyLen + 2));
	...
	/*
	 * if the first freeArg is a JPF application property filename, use this as
	 * targetArg and set the "jpf.app" property accordingly
	 */
		...
		try {
			...
			if (is != null) {
				try {
					...
					log("error closing input stream for: " + url + " : "
							+ iox1.getMessage());
				}
			}
		}
	...
	/**
	 * add the current dir to the list of JPF components. Note: this includes
	 * the core, so that we maintain the general principle that the enclosing
	 * project takes precedence (imagine the opposite: if we want to test a
	 * certain feature that is overridden by another extension we don't know
	 * about)
	 */
	 ...
	 * the not-so-obvious part is that it re-orders already present files to
	...
	/*
	 * argument syntax: {'+'<key>['='<val>'] | '-'<driver-arg>} {<free-arg>}
	 * 
	 * (1) null cmdLineArgs are ignored (2) all config cmdLineArgs start with
	 * '+' (3) if '=' is ommitted, a 'true' value is assumed (4) if <val> is
	 * ommitted, a 'null' value is assumed (5) no spaces around '=' (6) all '-'
	 * driver-cmdLineArgs are ignored
	 */
	...
	/*
	 * this does not include the '+' prefix, just the <key>[=[<value>]]
	 */
	...
	protected String normalize(String v) {
		...
		// trim leading and trailing blanks (at least Java 1.4.2 does not take
		// care of trailing blanks)
		v = v.trim();
	...
	void includeProjectPropertyFile(String projectId) {
		...
		if (projectPath != null) {
			...
			if (projectProps.isFile()) {
				...
				throw exception("project properties not found: "
						+ projectProps.getAbsolutePath());
			}

		} else {
			throw exception("unknown project id (check site.properties): "
					+ projectId);
		}
	}
	...
	public Object put(Object keyObject, Object valueObject) {
		...
		if (key.charAt(0) == KEY_PREFIX) {
			...
			// finally, a real key/value pair to add (or remove) - expand and
			// store
	...
	protected void processPseudoProperty(String key, String value) {
		if (REQUIRES_KEY.equals(key)) {
			...
			// check if corresponding jpf.properties has already been loaded. If
			// yes, skip
	...
	private void append0(String key, String oldValue, String a, String b,
		...
		if (a != null) {
			if (b != null) {
				...
				if (oldValue == a) { // using reference compare is intentional
										// here
					return; // no change
				} else {
					newValue = a;
	...
	protected String append(String key, String value) {
		return append(key, value, LIST_SEPARATOR); // append with our standard
													// list separator
	}

	/**
	 * check if we have a key.index entry. If not, fall back to key This
	 * simplifies clients that can have process id indexed properties
	 */
	...
	public JPFClassLoader initClassLoader(ClassLoader parent) {
		...
		// explicit collectGlobalPath() calls because (a) this could not
		// preserve
		// initial path settings, and (b) setting it *after* the JPFClassLoader
		// got
		// installed won't work (would have to add URLs explicitly, or would
		// have
		// to create a new JPFClassLoader, which then conflicts with classes
		// already
		...
		JPFClassLoader cl;
		if (parent instanceof JPFClassLoader) { // no need to create a new one,
												// just initialize
	...
	/**
	 * has to be called if 'native_classpath' gets explicitly changed USE WITH
	 * CARE - if this is messed up, it is hard to debug
	 */
	...
	public void jpfRunTerminated() {
		if (changeListeners != null) {
			// note we can't use the standard list iterator here because the
			// sole purpose
			// of having this notification is to remove the listener from the
			// list during its enumeration
			// which would give us ConcurrentModificationExceptions
			ArrayList<ConfigChangeListener> list = (ArrayList<ConfigChangeListener>) changeListeners
					.clone();
	...
	/**
	 * return any command line args that are not options or properties (this
	 * usually contains the application class and arguments)
	 */
	...
	/*
	 * target and its associated keys (target.args, target.entry) are now just
	 * ordinary key/value pairs and only here as convenience methods for JPF
	 * drivers/shells so that you don't have to remember the key names
	 * 
	...
	/**
	 * for a given <baseKey>, check if there are corresponding values for keys
	 * <baseKey>.0 ... <baseKey>.<maxSize> If a value is found, store it in an
	 * array at the respective index
	 * 
	 * @param baseKey
	 *            String with base key without trailing '.'
	 * @param maxSize
	 *            maximum size of returned value array
	...
	public int[] getIntArray(String key) throws JPFConfigException {
		...
		if (v != null) {
			...
			try {
				...
				throw new JPFConfigException("illegal int[] element in '" + key
						+ "' = \"" + sa[i] + '"');
			}
		} else {
			return null;
				...
				for (int i = a.length - 1; i >= 0; i--, m *= 60) {
					try {
						...
						throw new JPFConfigException(
								"illegal duration element in '" + key
										+ "' = \"" + v + '"');
					}
				}
		...
		if (v != null) {
			if (MAX.equals(v)) {
				...
				try {
					...
					throw new JPFConfigException("illegal int element in '"
							+ key + "' = \"" + v + '"');
				}
			}
		}
		...
		if (v != null) {
			if (MAX.equals(v)) {
				...
				try {
					...
					throw new JPFConfigException("illegal long element in '"
							+ key + "' = \"" + v + '"');
				}
			}
		}
	...
	public long[] getLongArray(String key) throws JPFConfigException {
		...
		if (v != null) {
			...
			try {
				...
				throw new JPFConfigException("illegal long[] element in " + key
						+ " = " + sa[i]);
			}
		} else {
			return null;
	...
	public float getFloat(String key, float defValue) {
		...
		if (v != null) {
			try {
				...
				throw new JPFConfigException("illegal float element in '" + key
						+ "' = \"" + v + '"');
			}
		}
	...
	public float[] getFloatArray(String key) throws JPFConfigException {
		...
		if (v != null) {
			...
			try {
				...
				throw new JPFConfigException("illegal float[] element in "
						+ key + " = " + sa[i]);
			}
		} else {
			return null;
	...
	public double getDouble(String key, double defValue) {
		...
		if (v != null) {
			try {
				...
				throw new JPFConfigException("illegal double element in '"
						+ key + "' = \"" + v + '"');
			}
		}
	...
	public <T extends Enum<T>> T getEnum(String key, T[] values, T defValue) {
		...
		if (v != null) {
			...
			throw new JPFConfigException("unknown enum value for " + key
					+ " = " + v);
	 ...
	 * can have a 'M' or 'k' postfix, values have to be positive integers
	 * (decimal notation)
	 */
	public long getMemorySize(String key, long defValue) {
		...
		if (v != null) {
			...
			try {
				...
				throw new JPFConfigException("illegal memory size element in '"
						+ key + "' = \"" + v + '"');
			}
		}
	...
	/**
	 * return an [optional] id part of a property value (all that follows the
	 * first '@')
	 */
		...
		if ((v != null) && (v.length() > 0)) {
			...
			try {
				...
				throw new JPFConfigException("class not found " + v
						+ " by classloader: " + loader);
			} catch (ExceptionInInitializerError ix) {
				throw new JPFConfigException("class initialization of " + v
						+ " failed: " + ix, ix);
			}
		}
	...
	public <T> Class<? extends T> getClass(String key, Class<T> type)
		...
		if (cls != null) {
			if (type.isAssignableFrom(cls)) {
				...
				throw new JPFConfigException("classname entry for: \"" + key
			...
			for (int i = 0; i < n; i++) {
				...
				if (clsName != null && clsName.length() > 0) {
					try {
						...
						throw new JPFConfigException("class initialization of "
								+ v[i] + " failed: " + ix, ix);
					}
				}
			}
	 ...
	 * the same prefix, e.g.
	 * 
	 * shell.panels = config,site shell.panels.site = .shell.panels.SitePanel
	 * shell.panels.config = .shell.panels.ConfigPanel ...
	 * 
	 * note that we specify default class names, not classes, so that the
	 * classes get loaded through our own loader at call time (they might not be
	 * visible to our caller)
	 */
	public <T> T[] getGroupInstances(String keyPrefix, String keyPostfix,
			Class<T> type, String... defaultClsNames) throws JPFConfigException {
		...
		if (ids.length > 0) {
			...
			for (int i = 0; i < arr.length; i++) {
				...
				if (arr[i] == null) {
					exception("cannot instantiate default type "
							+ defaultClsNames[i]);
				}
			}
	...
	public <T> T getInstance(String key, Class<T> type, String defClsName)
			throws JPFConfigException {
	...
	public <T> T getInstance(String key, Class<T> type, Object arg1, Object arg2)
			throws JPFConfigException {
	...
	public <T> T getEssentialInstance(String key, Class<T> type)
			throws JPFConfigException {
		Class<?>[] argTypes = { Config.class };
		Object[] args = { this };
	 ...
	 */
	public <T> T getEssentialInstance(String key, Class<T> type, Object arg1,
			Object arg2) throws JPFConfigException {
		Class<?>[] argTypes = new Class<?>[2];
		argTypes[0] = arg1.getClass();
		argTypes[1] = arg2.getClass();
	...
	public <T> T getInstance(String id, String clsName, Class<T> type,
			Class<?>[] argTypes, Object[] args) throws JPFConfigException {
	...
	/**
	 * this is our private instantiation workhorse - try to instantiate an
	 * object of class 'cls' by using the following ordered set of ctors 1.
	 * <cls>( <argTypes>) 2. <cls>(Config) 3. <cls>() if all of that fails, or
	 * there was a 'type' provided the instantiated object does not comply with,
	 * return null
	 */
	<T> T getInstance(String key, Class<?> cls, Class<T> type,
			Class<?>[] argTypes, Object[] args, String id)
			throws JPFConfigException {
		Object o = null;
		...
		if (id != null) { // check first if we already have this one
		...
		if (id != null) { // add to singletons (in case it's not already in
							// there)
			singletons.put(id, o);
		}
	 ...
	 * separated lists in JPF property files, but still can use quoted ';' if we
	 * absolutely have to specify Java signatures. On the other hand, we can't
	 * quote with '\' because that would make Windows paths even more terrible.
	...
	/**
	 * collect all the
	 * <project>.{native_classpath,classpath,sourcepath,peer_packages
	 * ,native_libraries} and append them to the global settings
	 * 
	 * NOTE - this is now called from within initClassLoader, which should only
	 * happen once and is the first time we really need the global paths.
	 * 
	 * <2do> this is Ok for native_classpath and native_libraries, but we should
	 * probably do classpath, sourcepath and peer_packages separately (they can
	 * be collected later)
	 */
	public void collectGlobalPaths() {

		// note - this is in the order of entry, i.e. reflects priorities
		// we have to process this in reverse order so that later entries are
		// prioritized
			...
			if (elements != null) {
				for (String e : elements) {
					if (e != null && e.length() > 0) {
						...
						// if this entry is not an absolute path, or doesn't
						// start with
						// the project path, prepend the project path
						if (!(absPath.matcher(e).matches())
								&& !e.startsWith(pathPrefix)) {
							e = pathPrefix + e;
						}
	 ...
	 * 
	 * if a matching key has an IGNORE_VALUE value ("-"), the entry is *not*
	 * promoted (we need this to override promoted keys)
	 */
	public void promotePropertyCategory(String keyPrefix) {
		int prefixLen = keyPrefix.length();

		// HashTable does not support adding elements while iterating over the
		// entrySet
	...
	protected void notifyPropertyChangeListeners(String key, String oldValue,
			String newValue) {
		if (changeListeners != null) {
	...
	public void print(PrintWriter pw) {
		...
		// just how much do you have to do to get a printout with keys in
		// alphabetical order :<
	...
	public void printStatus(Logger log) {
		...
		for (Object src : sources) {
			if (src instanceof File) {
				log.config("configuration source " + idx++ + " : "
						+ getSourceName(src));
			}
		}
	}

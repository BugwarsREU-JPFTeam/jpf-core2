...
public class JPF implements Runnable {
	...
	class ConfigListener implements ConfigChangeListener {
		...
		public void propertyChanged(Config config, String key, String oldValue,
			...
			if ("listener".equals(key)) {
				...
				Class<?>[] argTypes = { Config.class, JPF.class }; // Many
																	// listeners
																	// have 2
																	// parameter
																	// constructors
				...
				if (newListeners != null) {
					for (String clsName : newListeners) {
						try {
							JPFListener newListener = config.getInstance(
									"listener", clsName, JPFListener.class,
									argTypes, args);
							addListener(newListener);
							logger.info("config changed, added listener "
									+ clsName);

						} catch (JPFConfigException cfx) {
							logger.warning("listener change failed: "
									+ cfx.getMessage());
						}
					}
				}
	...
	/**
	 * a list of listeners that get automatically added from VM, Search or
	...
	/**
	 * use this one to get a Logger that is initialized via our Config
	 * mechanism. Note that our own Loggers do NOT pass
	 */
	...
	public static void start(Config conf, String[] args) {
		...
		if (shell != null) {
			...
			try {
				...
				/**
				 * Throwable cause = x.getCause(); if ((cause != null) &&
				 * conf.getBoolean("pass_exceptions", false)) {
				 * cause.fillInStackTrace(); throw cause; }
				 **/
	 ...
	 * object might not be really usable). If you directly use getSearch() /
	 * getVM(), check for null
	 */
	...
	private void initialize() {
		...
		memoryReserve = new byte[config.getInt("jpf.memory_reserve", 64 * 1024)]; // in
																					// bytes
		...
		try {
			...
			vm = config.getEssentialInstance("vm.class", VM.class, vmArgTypes,
					vmArgs);
			...
			// although the Reporter will always be notified last, this has to
			// be set
			// first so that it can register utility listeners like Statistics
			// that
	...
	void addListeners() {
		...
		// first listeners that were automatically added from VM, Search and
		// Reporter initialization
		if (pendingVMListeners != null) {
		...
		List<JPFListener> listeners = config.getInstances("listener",
				JPFListener.class, argTypes, args);
		if (listeners != null) {
	...
	public <T extends Publisher> boolean addPublisherExtension(Class<T> pCls,
	...
	/**
	 * return the search object. This can be null if the initialization has
	 * failed
	 */
	 ...
	 * options, so we just have to check for any '-' option prior to the target
	 * class name
	 */
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i] != null) {
				if (args[i].charAt(0) == '-') {
					...
					// this is supposed to be the target class name - everything
					// that follows
	...
	/**
	 * find the value of an arg that is either specific as "-key=value" or as
	 * "-key value". If not found, the supplied defValue is returned
	 */
	static String getArg(String[] args, String pattern, String defValue,
			boolean consume) {
		String s = defValue;
	...
	/**
	 * return a Config object that holds the JPF options. This first loads the
	 * properties from a (potentially configured) properties file, and then
	 * overlays all command line arguments that are key/value pairs
	 */
	...
	public void run() {
		...
		if (isRunnable()) {
			try {
				...
				// try to get memory back before we do anything that makes it
				// worse
				// (note that we even try to avoid calls here, we are on thin
				// ice)

				// NOTE - we don't try to recover at this point (that is what we
				// do
				// if we fall below search.min_free within search()), we only
				// want to
				...
				// see if we can reclaim some memory before logging or printing
				// statistics
				for (int i = 0; i < 10; i++) {
					rt.gc();
				...
				// that's questionable, but we might want to see statistics /
				// coverage
				// to know how far we got. We don't inform any other listeners
				// though
				// if it throws an exception we bail - we can't handle it w/o
				// memory
				try {
					search.notifySearchConstraintHit("JPF out of memory");
					search.error(new NoOutOfMemoryErrorProperty()); // JUnit
																	// tests
																	// will
																	// succeed
																	// if OOM
					...
					throw new JPFListenerException(
							"exception during out-of-memory termination", t);
				}
	 ...
	 * whole Java process via System.exit). While this is basically a bad
	 * non-local goto exception, it seems to be the least of evils given the
	 * current JPF structure, and the need to terminate w/o exiting the whole
	 * Java process. If we just do a System.exit(), we couldn't use JPF in an
	 * embedded context
	 */

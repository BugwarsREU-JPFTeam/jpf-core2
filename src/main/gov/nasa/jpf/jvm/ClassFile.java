...
/**
 * class to read and dissect Java classfile contents (as specified by the Java
 * VM spec
...
public class ClassFile extends BinaryClassSource {
	...
	String requestedTypeName; // the type name that caused this classfile to be
								// loaded

	// the const pool
	int[] cpPos; // cpPos[i] holds data start index for cp_entry i (0 is unused)
	Object[] cpValue; // cpValue[i] hold the String/Integer/Float/Double
						// associated with corresponding cp_entries
	...
	/**
	 * set classfile data. This is mainly provided to allow on-the-fly classfile
	 * instrumentation with 3rd party libraries
	 * 
	...
	/**
	 * return the typename this classfile gets loaded for <2do> this is going
	 * away
	 */
	...
	protected final static String[] stdFieldAttrs = { CONST_VALUE_ATTR,
			SYNTHETIC_ATTR, DEPRECATED_ATTR, SIGNATURE_ATTR,
			RUNTIME_INVISIBLE_ANNOTATIONS_ATTR,
	...
	protected final static String[] stdMethodAttrs = { CODE_ATTR,
			EXCEPTIONS_ATTR, SYNTHETIC_ATTR, DEPRECATED_ATTR, SIGNATURE_ATTR,
			RUNTIME_INVISIBLE_ANNOTATIONS_ATTR,
			RUNTIME_VISIBLE_ANNOTATIONS_ATTR,
			RUNTIME_INVISIBLE_PARAMETER_ANNOTATIONS_ATTR,
			RUNTIME_VISIBLE_PARAMETER_ANNOTATIONS_ATTR, ANNOTATIONDEFAULT_ATTR };
	...
	protected final static String[] stdCodeAttrs = { LINE_NUMBER_TABLE_ATTR,
			LOCAL_VAR_TABLE_ATTR };
	...
	protected final static String[] stdClassAttrs = { SOURCE_FILE_ATTR,
			DEPRECATED_ATTR, INNER_CLASSES_ATTR, DEPRECATED_ATTR,
			SIGNATURE_ATTR, RUNTIME_INVISIBLE_ANNOTATIONS_ATTR,
			RUNTIME_VISIBLE_ANNOTATIONS_ATTR, ENCLOSING_METHOD_ATTR };
	...
	public String getSourceFileAttribute(int dataPos) {
		// SourceFile_attribute { u2 attr_name_idx; u4 attr_length; u2
		// sourcefile_idx<utf8>; }
	...
	public final int readI4() {
		...
		return (data[idx++] << 24) | ((data[idx++] & 0xff) << 16)
				| ((data[idx++] & 0xff) << 8) | (data[idx] & 0xff);
	}

	// --- reader notifications
	private void setClass(ClassFileReader reader, String clsName,
			String superClsName, int flags, int cpCount)
	...
	private void setInterface(ClassFileReader reader, int ifcIndex,
			String ifcName) {
		int p = pos;
	...
	private void setField(ClassFileReader reader, int fieldIndex,
			int accessFlags, String name, String descriptor) {
		int p = pos;
		reader.setField(this, fieldIndex, accessFlags, name, descriptor);
		pos = p;
	}

	private void setFieldAttributeCount(ClassFileReader reader, int fieldIndex,
			int attrCount) {
		int p = pos;
	...
	private void setFieldAttribute(ClassFileReader reader, int fieldIndex,
			int attrIndex, String name, int attrLength) {
	...
	private void setConstantValue(ClassFileReader reader, Object tag,
			Object value) {
		int p = pos;
	...
	private void setMethod(ClassFileReader reader, int methodIndex,
			int accessFlags, String name, String descriptor) {
		int p = pos;
	...
	private void setMethodAttributeCount(ClassFileReader reader,
			int methodIndex, int attrCount) {
		int p = pos;
		reader.setMethodAttributeCount(this, methodIndex, attrCount);
		pos = p;
	}

	private void setMethodAttribute(ClassFileReader reader, int methodIndex,
			int attrIndex, String name, int attrLength) {
		int p = pos + attrLength;
		reader.setMethodAttribute(this, methodIndex, attrIndex, name,
				attrLength);
		pos = p;
	}
	...
	private void setExceptionCount(ClassFileReader reader, Object tag,
			int exceptionCount) {
		int p = pos;
	...
	private void setException(ClassFileReader reader, Object tag,
			int exceptionIndex, String exceptionType) {
		int p = pos;
		reader.setException(this, tag, exceptionIndex, exceptionType);
		pos = p;
	}

	private void setCode(ClassFileReader reader, Object tag, int maxStack,
			int maxLocals, int codeLength) {
		int p = pos + codeLength;
		reader.setCode(this, tag, maxStack, maxLocals, codeLength);
		pos = p;
	}

	private void setExceptionTableCount(ClassFileReader reader, Object tag,
	...
	private void setExceptionTableEntry(ClassFileReader reader, Object tag,
			int exceptionIndex, int startPc, int endPc, int handlerPc,
			String catchType) {
		int p = pos;
		reader.setExceptionHandler(this, tag, exceptionIndex, startPc, endPc,
				handlerPc, catchType);
		pos = p;
	}
	...
	private void setCodeAttributeCount(ClassFileReader reader, Object tag,
			int attrCount) {
		int p = pos;
	...
	private void setCodeAttribute(ClassFileReader reader, Object tag,
			int attrIndex, String name, int attrLength) {
		int p = pos + attrLength;
	...
	private void setLineNumberTableCount(ClassFileReader reader, Object tag,
			int lineNumberCount) {
		int p = pos;
	...
	private void setLineNumber(ClassFileReader reader, Object tag,
			int lineIndex, int lineNumber, int startPc) {
		int p = pos;
	...
	private void setLocalVarTableCount(ClassFileReader reader, Object tag,
			int localVarCount) {
		int p = pos;
	...
	private void setLocalVar(ClassFileReader reader, Object tag,
			int localVarIndex, String varName, String descriptor,
			int scopeStartPc, int scopeEndPc, int slotIndex) {
		int p = pos;
		reader.setLocalVar(this, tag, localVarIndex, varName, descriptor,
	...
	private void setClassAttribute(ClassFileReader reader, int attrIndex,
			String name, int attrLength) {
		int p = pos + attrLength;
	...
	private void setSourceFile(ClassFileReader reader, Object tag,
			String pathName) {
		int p = pos;
	...
	private void setInnerClassCount(ClassFileReader reader, Object tag,
			int innerClsCount) {
		int p = pos;
	...
	private void setInnerClass(ClassFileReader reader, Object tag,
			int innerClsIndex, String outerName, String innerName,
			String innerSimpleName, int accessFlags) {
		int p = pos;
		reader.setInnerClass(this, tag, innerClsIndex, outerName, innerName,
				innerSimpleName, accessFlags);
		pos = p;
	}

	private void setEnclosingMethod(ClassFileReader reader, Object tag,
			String enclosingClass, String enclosedMethod, String descriptor) {
		int p = pos;
		reader.setEnclosingMethod(this, tag, enclosingClass, enclosedMethod,
				descriptor);
		pos = p;
	}
	...
	private void setAnnotationCount(ClassFileReader reader, Object tag,
			int annotationCount) {
		int p = pos;
	...
	private void setAnnotation(ClassFileReader reader, Object tag,
			int annotationIndex, String annotationType) {
		int p = pos;
		reader.setAnnotation(this, tag, annotationIndex, annotationType);
		pos = p;
	}

	private void setAnnotationValueCount(ClassFileReader reader, Object tag,
			int annotationIndex, int nValuePairs) {
		int p = pos;
		reader.setAnnotationValueCount(this, tag, annotationIndex, nValuePairs);
		pos = p;
	}

	private void setPrimitiveAnnotationValue(ClassFileReader reader,
			Object tag, int annotationIndex, int valueIndex,
			String elementName, int arrayIndex, Object val) {
		int p = pos;
		reader.setPrimitiveAnnotationValue(this, tag, annotationIndex,
				valueIndex, elementName, arrayIndex, val);
		pos = p;
	}

	private void setStringAnnotationValue(ClassFileReader reader, Object tag,
			int annotationIndex, int valueIndex, String elementName,
			int arrayIndex, String s) {
		int p = pos;
		reader.setStringAnnotationValue(this, tag, annotationIndex, valueIndex,
				elementName, arrayIndex, s);
		pos = p;
	}

	private void setClassAnnotationValue(ClassFileReader reader, Object tag,
			int annotationIndex, int valueIndex, String elementName,
			int arrayIndex, String typeName) {
		int p = pos;
		reader.setClassAnnotationValue(this, tag, annotationIndex, valueIndex,
				elementName, arrayIndex, typeName);
		pos = p;
	}

	private void setEnumAnnotationValue(ClassFileReader reader, Object tag,
			int annotationIndex, int valueIndex, String elementName,
			int arrayIndex, String enumType, String enumValue) {
		int p = pos;
		reader.setEnumAnnotationValue(this, tag, annotationIndex, valueIndex,
				elementName, arrayIndex, enumType, enumValue);
		pos = p;
	}

	private void setAnnotationValueElementCount(ClassFileReader reader,
			Object tag, int annotationIndex, int valueIndex,
			String elementName, int elementCount) {
		int p = pos;
		reader.setAnnotationValueElementCount(this, tag, annotationIndex,
				valueIndex, elementName, elementCount);
		pos = p;
	}

	private void setAnnotationValueElementsDone(ClassFileReader reader,
			Object tag, int annotationIndex, int valueIndex, String elementName) {
		int p = pos;
		reader.setAnnotationValueElementsDone(this, tag, annotationIndex,
				valueIndex, elementName);
		pos = p;
	}

	public void setAnnotationValuesDone(ClassFileReader reader, Object tag,
			int annotationIndex) {
		int p = pos;
	...
	private void setParameterCount(ClassFileReader reader, Object tag,
			int parameterCount) {
		int p = pos;
	...
	private void setParameterAnnotationCount(ClassFileReader reader,
			Object tag, int paramIndex, int annotationCount) {
		int p = pos;
		reader.setParameterAnnotationCount(this, tag, paramIndex,
				annotationCount);
		pos = p;
	}

	private void setParameterAnnotation(ClassFileReader reader, Object tag,
			int annotationIndex, String annotationType) {
		int p = pos;
		reader.setParameterAnnotation(this, tag, annotationIndex,
				annotationType);
		pos = p;
	}

	private void setParameterAnnotationsDone(ClassFileReader reader,
			Object tag, int paramIndex) {
		int p = pos;
	...
	public void setSignature(ClassFileReader reader, Object tag,
			String signature) {
		int p = pos;
	 ...
	 * /ClassFile.doc.html#74353 u4 magic; // 0xcafebabe u2 minor_version; u2
	 * major_version;
	 * 
	 * u2 constant_pool_count; cp_entry constant_pool[constant_pool_count-1]; u2
	 * access_flags;
	 * 
	...
	public void parse(ClassFileReader reader) throws ClassParseException {
		...
		try {
			...
			// nothing, just a control exception to shortcut the classfile
			// parsing
		}
	}
	...
	public static String readModifiedUTF8String(byte[] data, int pos, int len)
			throws ClassParseException {

		int n = 0; // the number of chars in buf
		char[] buf = new char[len]; // it can't be more, but it can be less
									// chars
	...
	// the protected methods are called automatically, the public parse..Attr()
	// methods
	// are called optionally from the corresponding
	// ClassFileReader.set..Attribute() method.
	// Note that these calls have to provide the ClassFileReader as an argument
	// because
	// we might actually switch to another reader (e.g. MethodInfos for
	// parseCodeAttr)
	...
	protected void parseCp(int cpCount) throws ClassParseException {
		...
		// --- first pass: store data index values and convert non-delegating
		// constant values
		...
		for (int i = 1; i < cpCount; i++) {
			...
			case CONSTANT_UTF8: // utf8_info { u1 tag; u2 length; u1
								// bytes[length]; }
				dataIdx[i] = j++;
			...
			case CONSTANT_INTEGER: // Integer_info { u1 tag; u4 bytes; }
				...
				int iVal = (data[j++] & 0xff) << 24 | (data[j++] & 0xff) << 16
						| (data[j++] & 0xff) << 8 | (data[j++] & 0xff);
			...
			case CONSTANT_FLOAT: // Float_info { u1 tag; u4 bytes; }
				...
				int iBits = (data[j++] & 0xff) << 24 | (data[j++] & 0xff) << 16
						| (data[j++] & 0xff) << 8 | (data[j++] & 0xff);
				float fVal = Float.intBitsToFloat(iBits);
				values[i] = new Float(fVal);
				break;

			case CONSTANT_LONG: // Long_info { u1 tag; u4 high_bytes; u4
								// low_bytes; }
				dataIdx[i] = j++;
				long lVal = (data[j++] & 0xffL) << 56
						| (data[j++] & 0xffL) << 48 | (data[j++] & 0xffL) << 40
						| (data[j++] & 0xffL) << 32 | (data[j++] & 0xffL) << 24
						| (data[j++] & 0xffL) << 16 | (data[j++] & 0xffL) << 8
						| (data[j++] & 0xffL);
			...
			case CONSTANT_DOUBLE: // Double_info { u1 tag; u4 high_bytes; u4
									// low_bytes; }
				dataIdx[i] = j++;

				long lBits = (data[j++] & 0xffL) << 56
						| (data[j++] & 0xffL) << 48 | (data[j++] & 0xffL) << 40
						| (data[j++] & 0xffL) << 32 | (data[j++] & 0xffL) << 24
						| (data[j++] & 0xffL) << 16 | (data[j++] & 0xffL) << 8
						| (data[j++] & 0xffL);
			...
			case CONSTANT_STRING: // String_info { u1 tag; u2
									// string_index<utf8>; }
				dataIdx[i] = j;
			...
			case FIELD_REF: // Fieldref_info { u1 tag; u2 class_index; u2
							// name_and_type_index; }
				dataIdx[i] = j;
			...
			case METHOD_REF: // Methodref_info { u1 tag; u2 class_index; u2
								// name_and_type_index; }
				dataIdx[i] = j;
			...
			case INTERFACE_METHOD_REF: // InterfaceMethodref_info { u1 tag; u2
			...
			case NAME_AND_TYPE: // NameAndType_info { u1 tag; u2
								// name_index<utf8>; u2 descriptor_index<utf8>;
								// }
				dataIdx[i] = j;
	...
	protected void parseFieldAttributes(ClassFileReader reader, int fieldIdx,
			int attrCount) {
		...
		for (int i = 0; i < attrCount; i++) {
			...
			int attrLength = readI4(); // actually U4, but we don't support 2GB
										// attributes
	 ...
	 * 
	 * ConstantValue {u2 attrName<utf8>; u4 attrLength; u2
	 * constIndex<class|string|int|float|long|double> }
	 * 
	...
	protected void parseMethodAttributes(ClassFileReader reader, int methodIdx,
			int attrCount) {
	...
	/**
	 * (optionally) called by reader from within the setMethodAttribute()
	 * notification This means we have recursive notification since this is a
	 * variable length attribute that has variable length attributes
	 * 
	 * Code_attribute { u2 attr_name_index<utf8>; u4 attr_length; u2 max_stack;
	 * u2 max_locals; u4 code_length; u1 code[code_length]; u2
	 * exception_table_length; { u2 start_pc; u2 end_pc; u2 handler_pc; u2
	 * catch_type<class_entry>; } exception_table[exception_table_length]; u2
	...
	public void parseCodeAttr(ClassFileReader reader, Object tag) {
		...
		for (int i = 0; i < exceptionCount; i++) {
			...
			setExceptionTableEntry(reader, tag, i, startPc, endPc, handlerPc,
					catchType);
		}
	...
	protected void parseCodeAttrAttributes(ClassFileReader reader, Object tag,
			int attrCount) {
	...
	/**
	 * optionally called from ClassFileReader.setCodeAttribute() to parse
	 * LineNumberTables LineNumberTable { u2 attrName; u4 attrLength; u2
	 * lineCount; { u2 startPc; u2 lineNumber; } [lineCount] }; pos is at
	 * lineCount
	 */
	...
	/**
	 * optionally called from ClassFileReader.setCodeAttribute() to parse
	 * LocalVarTables LocalVarTableTable { u2 attrName; u4 attrLength; u2
	 * localVarCount; { u2 startPc; u2 lineNumber; } [lineCount] }; pos is at
	 * localVarCount
	 */
	public void parseLocalVarTableAttr(ClassFileReader reader, Object tag) {
		...
		for (int i = 0; i < localVarCount; i++) {
			...
			setLocalVar(reader, tag, i, varName, descriptor, startPc, startPc
					+ length - 1, slotIndex);
		}
	...
	/**
	 * (optionally) called by ClassFileReader from within setClassAttribute()
	 * notification
	 * 
	 ...
	 * notification
	 * 
	 * InnerClass { u2 nameIdx<utf8>; u4 length; u2 classCount; { u2
	 * innerCls<cls>; u2 outerCls<cls>; u2 innerName<utf8>; u2 innerAccessFlags;
	 * } classes[classCount] }
	 * 
	...
	public void parseInnerClassesAttr(ClassFileReader reader, Object tag) {
		...
		for (int i = 0; i < innerClsCount; i++) {
			...
			cpIdx = readU2();
			String innerSimpleName = (cpIdx != 0) ? (String) cpValue[cpIdx]
					: null;
			int accessFlags = readU2();

			setInnerClass(reader, tag, i, outerClsName, innerClsName,
					innerSimpleName, accessFlags);
		}
	...
	/**
	 * EnclosingMethod_attribute { u2 attribute_name_index; u4 attribute_length;
	 * u2 class_index -> Class_info { u1 tag; u2 name_index->utf8 } u2
	 * method_index -> NameAndType_info { u1 tag; u2 name_index->utf8; u2
	...
	public void parseEnclosingMethodAttr(ClassFileReader reader, Object tag) {
		...
		// check if this is inside a method - we also get EnclosingMethod_infos
		// for
		...
		setEnclosingMethod(reader, tag, enclosingClass, enclosedMethod,
				descriptor);
	}
	...
	/*
	 * element_value { u1 tag; union { u2 const_value_index; { u2
	 * type_name_index; u2 const_name_index; } enum_const_value; u2
	 * class_info_index; annotation annotation_value; { u2 num_values;
	 * element_value values[num_values]; } array_value; } value; } valid tags
	 * are primitve type codes B,C,D,F,I,J,S,Z plus: 's'=String, 'e'=enum,
	 * 'c'=class, '@'=annotation, '['=array
	 */
	void parseAnnotationValue(ClassFileReader reader, Object tag,
			int annotationIndex, int valueIndex, String elementName,
			int arrayIndex) {
		int cpIdx;
		Object val;
		...
		switch (t) {
		case 'Z':
			// booleans have to be treated differently since there is no
			// CONSTANT_Boolean, i.e. values are
			// stored as CONSTANT_Integer in the constpool, i.e. the cpValue
			// doesn't have the right type
			cpIdx = readU2();
			val = cpValue[cpIdx];
			val = Boolean.valueOf((Integer) val == 1);
			setPrimitiveAnnotationValue(reader, tag, annotationIndex,
					valueIndex, elementName, arrayIndex, val);
			break;
		...
		case 's':
			...
			setStringAnnotationValue(reader, tag, annotationIndex, valueIndex,
					elementName, arrayIndex, s);
			break;
		...
		case 'e':
			...
			setEnumAnnotationValue(reader, tag, annotationIndex, valueIndex,
					elementName, arrayIndex, enumTypeName, enumConstName);
			break;

		case 'c':
			cpIdx = readU2();
			String className = (String) cpValue[cpIdx];
			setClassAnnotationValue(reader, tag, annotationIndex, valueIndex,
					elementName, arrayIndex, className);
			break;
		...
		case '[':
			int arrayLen = readU2();
			setAnnotationValueElementCount(reader, tag, annotationIndex,
					valueIndex, elementName, arrayLen);
			for (int i = 0; i < arrayLen; i++) {
				parseAnnotationValue(reader, tag, annotationIndex, valueIndex,
						elementName, i);
			}
			setAnnotationValueElementsDone(reader, tag, annotationIndex,
					valueIndex, elementName);
			break;
		}
	}
	...
	/*
	 * annotation { u2 type_index; u2 num_element_value_pairs; { u2
	 * element_name_index; element_value value; }
	 * element_value_pairs[num_element_value_pairs] }
	 */
	void parseAnnotation(ClassFileReader reader, Object tag,
			int annotationIndex, boolean isParameterAnnotation) {
		int cpIdx = readU2();
		...
		for (int i = 0; i < nValuePairs; i++) {
			...
			parseAnnotationValue(reader, tag, annotationIndex, i, elementName,
					-1);
		}
	 ...
	 * 
	 * Runtime[In]VisibleAnnotations_attribute { u2 attribute_name_index; u4
	 * attribute_length; u2 num_annotations; << pos annotation
	 ...
	 * u4 attribute_length; u1 num_parameters; << pos { u2 num_annotations;
	...
	/**
	 * Signature_attribute { u2 attribute_name_index; u4 attr-length; u2
	 * signature-index << pos }
	 */
	...
	/**
	 * AnnotationDefault_attribute { u2 attribute_name_index; u4
	 * attribute_length; element_value default_value; << pos }
	 */
	...
	public void parseBytecode(JVMByteCodeReader reader, Object tag,
			int codeLength) {
		int localVarIndex;
		int cpIdx;
		int constVal;
	...
	// --- those can only be called from within a
	// JVMByteCodeReader.tableswitch() notification
	public void parseTableSwitchEntries(JVMByteCodeReader reader, int low,
			int high) {
	...
	public int getTableSwitchOffset(int low, int high, int defaultOffset,
			int val) {
		if (val < low || val > high) {
	...
	// --- those can only be called from within a
	// JVMByteCodeReader.lookupswitch() notification

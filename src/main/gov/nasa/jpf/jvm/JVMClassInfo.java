...
public class JVMClassInfo extends ClassInfo {
	...
	/**
	 * this is the inner class that does the actual ClassInfo initialization
	 * from ClassFile. It is an inner class so that (a) it can set ClassInfo
	 * fields, (b) it can extend ClassFileReaderAdapter, and (c) we don't
	 * clutter JVMClassInfo with fields that are only temporarily used during
	 * parsing
	 */
	class Initializer extends ClassFileReaderAdapter {
		...
		public Initializer(ClassFile cf, JVMCodeBuilder cb)
		...
		public void setClass(ClassFile cf, String clsName, String superClsName,
				int flags, int cpCount) throws ClassParseException {
			JVMClassInfo.this.setClass(clsName, superClsName, flags, cpCount);
		}

		@Override
		public void setClassAttribute(ClassFile cf, int attrIndex, String name,
				int attrLength) {
		...
		public void setInnerClass(ClassFile cf, Object tag, int innerClsIndex,
				String outerName, String innerName, String innerSimpleName,
				int accessFlags) {
			// Ok, this is a total mess - some names are in dot notation, others
			// use '/'
			// and to make it even more confusing, some InnerClass attributes
			// refer NOT
			// to the currently parsed class, so we have to check if we are the
			// outerName,
			// but then 'outerName' can also be null instead of our own name.
			// Oh, and there are also InnerClass attributes that have their own
			// name as inner names
			// (see java/lang/String$CaseInsensitiveComparator or ...System and
			// java/lang/System$1 for instance)
			if (outerName != null) {
			...
			if (!innerName.equals(name)) {
				...
				// this refers to ourself, and can be a force fight with
				// setEnclosingMethod
				if (outerName != null) { // only set if this is a direct member,
		...
		@Override
		public void setEnclosingMethod(ClassFile cf, Object tag,
				String enclosingClassName, String enclosingMethodName,
				String descriptor) {
			setEnclosingClass(enclosingClassName);

			if (enclosingMethodName != null) {
				JVMClassInfo.this.setEnclosingMethod(enclosingMethodName
						+ descriptor);
			}
		}
		...
		@Override
		public void setField(ClassFile cf, int fieldIndex, int accessFlags,
		...
		@Override
		public void setFieldAttribute(ClassFile cf, int fieldIndex,
		...
		@Override
		public void setMethodAttribute(ClassFile cf, int methodIndex,
				int attrIndex, String name, int attrLength) {
		...
		@Override
		public void setExceptionCount(ClassFile cf, Object tag,
				int exceptionCount) {
			exceptions = new String[exceptionCount];
		}

		@Override
		public void setException(ClassFile cf, Object tag, int exceptionIndex,
				String exceptionType) {
			exceptions[exceptionIndex] = Types
		...
		@Override
		public void setExceptionHandlerTableCount(ClassFile cf, Object tag,
				int exceptionTableCount) {
			handlers = new ExceptionHandler[exceptionTableCount];
		}

		@Override
		public void setExceptionHandler(ClassFile cf, Object tag,
				int handlerIndex, int startPc, int endPc, int handlerPc,
				String catchType) {
		...
		// --- current method code
		@Override
		public void setCode(ClassFile cf, Object tag, int maxStack,
		...
		@Override
		public void setCodeAttribute(ClassFile cf, Object tag, int attrIndex,
		...
		@Override
		public void setLineNumberTableCount(ClassFile cf, Object tag,
				int lineNumberCount) {
		...
		@Override
		public void setLineNumber(ClassFile cf, Object tag, int lineIndex,
		...
		@Override
		public void setLocalVarTableCount(ClassFile cf, Object tag,
				int localVarCount) {
		...
		public void setLocalVar(ClassFile cf, Object tag, int localVarIndex,
				String varName, String descriptor, int scopeStartPc,
				int scopeEndPc, int slotIndex) {
			LocalVarInfo lvi = new LocalVarInfo(varName, descriptor, "",
		...
		@Override
		public void setAnnotationCount(ClassFile cf, Object tag,
				int annotationCount) {
		...
		@Override
		public void setParameterCount(ClassFile cf, Object tag,
				int parameterCount) {
			parameterAnnotations = new AnnotationInfo[parameterCount][];
		}

		@Override
		public void setParameterAnnotationCount(ClassFile cf, Object tag,
				int paramIndex, int annotationCount) {
			annotations = new AnnotationInfo[annotationCount];
			parameterAnnotations[paramIndex] = annotations;
		}

		@Override
		public void setParameterAnnotation(ClassFile cf, Object tag,
				int annotationIndex, String annotationType) {
			curAi = getResolvedAnnotationInfo(Types
					.getClassNameFromTypeName(annotationType));
		...
		@Override
		public void setAnnotation(ClassFile cf, Object tag,
				int annotationIndex, String annotationType) {
			if (tag instanceof InfoObject) {
				curAi = getResolvedAnnotationInfo(Types
						.getClassNameFromTypeName(annotationType));
		...
		@Override
		public void setAnnotationValueCount(ClassFile cf, Object tag,
				int annotationIndex, int nValuePairs) {
			// if we have values, we need to clone the defined annotation so
			// that we can overwrite entries
			curAi = curAi.cloneForOverriddenValues();
			annotations[annotationIndex] = curAi;
		}

		@Override
		public void setPrimitiveAnnotationValue(ClassFile cf, Object tag,
				int annotationIndex, int valueIndex, String elementName,
				int arrayIndex, Object val) {
			if (arrayIndex >= 0) {
		...
		@Override
		public void setStringAnnotationValue(ClassFile cf, Object tag,
				int annotationIndex, int valueIndex, String elementName,
				int arrayIndex, String val) {
			if (arrayIndex >= 0) {
				values[arrayIndex] = val;
			} else {
				curAi.setClonedEntryValue(elementName, val);
		...
		@Override
		public void setClassAnnotationValue(ClassFile cf, Object tag,
				int annotationIndex, int valueIndex, String elementName,
		...
		@Override
		public void setEnumAnnotationValue(ClassFile cf, Object tag,
				int annotationIndex, int valueIndex, String elementName,
		...
		@Override
		public void setAnnotationValueElementCount(ClassFile cf, Object tag,
				int annotationIndex, int valueIndex, String elementName,
				int elementCount) {
		...
		@Override
		public void setAnnotationValueElementsDone(ClassFile cf, Object tag,
				int annotationIndex, int valueIndex, String elementName) {
	...
	JVMClassInfo(String name, ClassLoaderInfo cli, ClassFile cf, String srcUrl,
			JVMCodeBuilder cb) throws ClassParseException {
	...
	// concrete proxy ctor
	protected JVMClassInfo(ClassInfo ciAnnotation, String proxyName,
	...
	protected JVMCodeBuilder getSystemCodeBuilder(ClassFile cf, MethodInfo mi) {
		JVMSystemClassLoaderInfo sysCl = (JVMSystemClassLoaderInfo) ClassLoaderInfo
	...
	/**
	 * to be called from super proxy ctor this needs to be in the VM specific
	 * ClassInfo because we need to create code
	 */
	@Override
	...
	public StackFrame createStackFrame(ThreadInfo ti, MethodInfo callee) {
		...
		if (callee.isMJI()) {
			...
			JVMNativeStackFrame calleeFrame = new JVMNativeStackFrame(
					nativeCallee);
	...
	public DirectCallStackFrame createDirectCallStackFrame(ThreadInfo ti,
			MethodInfo miCallee, int nLocals) {
	...
	/**
	 * while this is a normal DirectCallStackFrame, it has different code which

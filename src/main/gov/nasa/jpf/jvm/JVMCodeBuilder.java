...
public class JVMCodeBuilder implements JVMByteCodeReader {
	...
	// <2do> these should use interface types to avoid hardwiring our own
	// instruction classes
	...
	public void getfield(String fieldName, String clsName,
	...
	public void getstatic(String fieldName, String clsName,
			String fieldDescriptor) {
		add(insnFactory.getstatic(fieldName, clsName, fieldDescriptor));
		pc += 3;
	}
	...
	public void invokeinterface(int cpInterfaceMethodRefIndex, int count,
			int zero) {
		String clsName = cf
				.interfaceMethodClassNameAt(cpInterfaceMethodRefIndex);
		String methodName = cf.interfaceMethodNameAt(cpInterfaceMethodRefIndex);
		String methodSignature = cf
				.interfaceMethodDescriptorAt(cpInterfaceMethodRefIndex);
	...
	public void invokespecial(String clsName, String methodName,
	...
	public void invokestatic(String clsName, String methodName,
			String methodSignature) {
		add(insnFactory.invokestatic(clsName, methodName, methodSignature));
		pc += 3;
	}
	...
	public void invokevirtual(String clsName, String methodName,
			String methodSignature) {
		add(insnFactory.invokevirtual(clsName, methodName, methodSignature));
		pc += 3;
	}
	...
	public void ldc_(int cpIntOrFloatOrStringOrClassIndex) {
		...
		case ClassFile.CONSTANT_INTEGER:
			add(insnFactory.ldc((Integer) v));
			break;
		case ClassFile.CONSTANT_FLOAT:
			add(insnFactory.ldc((Float) v));
			break;
		case ClassFile.CONSTANT_STRING:
			add(insnFactory.ldc((String) v, false));
			break;
		case ClassFile.CONSTANT_CLASS:
			add(insnFactory.ldc((String) v, true));
			break;
		}
		pc += 2;
	}
	...
	public void ldc_w_(int cpIntOrFloatOrStringOrClassIndex) {
		...
		switch (cf.getCpTag(cpIntOrFloatOrStringOrClassIndex)) {
		case ClassFile.CONSTANT_INTEGER:
			add(insnFactory.ldc_w((Integer) v));
			break;
		case ClassFile.CONSTANT_FLOAT:
			add(insnFactory.ldc_w((Float) v));
			break;
		case ClassFile.CONSTANT_STRING:
			add(insnFactory.ldc_w((String) v, false));
			break;
		case ClassFile.CONSTANT_CLASS:
			add(insnFactory.ldc_w((String) v, true));
			break;
		}
		pc += 3;
	}
	...
	public void lookupswitch(int defaultPcOffset, int nEntries) {
		Instruction insn = insnFactory.lookupswitch(pc + defaultPcOffset,
				nEntries);
		add(insn);
	...
	public void putfield(String fieldName, String clsName,
			String fieldDescriptor) {
		add(insnFactory.putfield(fieldName, clsName, fieldDescriptor));
		pc += 3;
	}
	...
	public void putstatic(String fieldName, String clsName,
			String fieldDescriptor) {
		add(insnFactory.putstatic(fieldName, clsName, fieldDescriptor));
		pc += 3;
	}
	...
	public void tableswitch(int defaultPcOffset, int low, int high) {
		Instruction insn = insnFactory.tableswitch(pc + defaultPcOffset, low,
				high);
		add(insn);
	...
	public void unknown(int bytecode) {
		throw new JPFException("unknown bytecode: "
				+ Integer.toHexString(bytecode));
	}

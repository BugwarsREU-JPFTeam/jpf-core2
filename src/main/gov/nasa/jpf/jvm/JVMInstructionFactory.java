 ...
 * 
 * this deliberately uses the abstract abstract public Instruction as return
 * type to allow different instruction hierarchies in extensions.
 * 
 * This shouldn't impose runtime overhead since mandatory parameters are now
 * passed in as factory method arguments. The only drawback is that the compiler
 * cannot check for abstract public Instruction class typos, but that seems less
 * important than allowing extension specific abstract public Instruction class
 * hierarchies
 * 
 * <2do> there are still direct references of LOOKUPSWITCH, TABLESWITCH. Once
 * these are removed, .jvm does not assume a particular abstract public
...
public abstract class JVMInstructionFactory implements Cloneable {
	...
	abstract public Instruction getfield(String fieldName, String clsName,
			String fieldDescriptor);

	abstract public Instruction getstatic(String fieldName, String clsName,
			String fieldDescriptor);
	...
	abstract public Instruction invokeinterface(String clsName,
			String methodName, String methodSignature);

	abstract public Instruction invokespecial(String clsName,
			String methodName, String methodSignature);

	abstract public Instruction invokestatic(String clsName, String methodName,
			String methodSignature);

	abstract public Instruction invokevirtual(String clsName,
			String methodName, String methodSignature);
	...
	abstract public Instruction putfield(String fieldName, String clsName,
			String fieldDescriptor);

	abstract public Instruction putstatic(String fieldName, String clsName,
			String fieldDescriptor);
	...
	abstract public Instruction tableswitch(int defaultTargetPc, int low,
			int high);

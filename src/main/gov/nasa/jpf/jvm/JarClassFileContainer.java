...
public class JarClassFileContainer extends JVMClassFileContainer {
	...
	public JarClassFileContainer(File file, String pathPrefix)
			throws IOException {
			...
			if (len > 0) {
				if (pathPrefix.charAt(0) == '/') {
					if (len == 1) {
						...
						pathPrefix = pathPrefix.substring(1); // skip the
																// heading '/'
						len--;
					}
				}
	...
	public ClassFileMatch getMatch(String clsName) throws ClassParseException {
		...
		if (e != null) {
			...
			try {
				...
				return new JVMClassFileMatch(clsName, getClassURL(clsName),
						data);
				...
				if (is != null) {
					try {
						...
						error("cannot close input stream for file "
								+ e.getName());
					}
				}
			}

...
public class INVOKECG extends JVMInstruction {
	...
	public Instruction execute(ThreadInfo ti) {
		...
		if (!ti.isFirstStepInsn()) {
			...
			InvocationCG cg = ti.getVM().getCurrentChoiceGenerator("INVOKECG",
					InvocationCG.class);
			assert (cg != null) : "no current InvocationCG";

			Invocation call = cg.getNextChoice();
			MethodInfo callee = call.getMethodInfo();
			JVMInstructionFactory insnFactory = JVMInstructionFactory
					.getFactory();
			...
			if (callee.isStatic()) {
				realInvoke = insnFactory.invokestatic(clsName, mthName,
						signature);
			} else {
				realInvoke = insnFactory.invokevirtual(clsName, mthName,
						signature);
			}

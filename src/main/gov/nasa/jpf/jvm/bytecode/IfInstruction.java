...
public abstract class IfInstruction extends JVMInstruction {
	...
	/**
	 * retrieve value of jump condition from operand stack (not ideal to have
	 * this public, but some listeners might need it for skipping the insn, plus
	 * we require it for subclass factorization)
	 */
	 ...
	 */
	protected Instruction executeBothBranches(SystemState ss, KernelState ks,
			ThreadInfo ti) {
		if (!ti.isFirstStepInsn()) {
			BooleanChoiceGenerator cg = new BooleanChoiceGenerator(ti.getVM()
					.getConfig(), "ifAll");
			if (ss.setNextChoiceGenerator(cg)) {
				...
				StackFrame frame = ti.getModifiableTopFrame();
				// some listener did override the CG, fallback to normal
				// operation
				conditionValue = popConditionValue(frame);
				if (conditionValue) {
					return getTarget();
			...
			BooleanChoiceGenerator cg = ss.getCurrentChoiceGenerator("ifAll",
					BooleanChoiceGenerator.class);
			assert (cg != null) : "no BooleanChoiceGenerator";

			StackFrame frame = ti.getModifiableTopFrame();
			popConditionValue(frame); // we are not interested in concrete
										// values

...
public abstract class InstanceFieldInstruction extends FieldInstruction {
	/**
	 * this is required for package external derived GET/PUTFIELDS that replace
	 * enter().
	 * 
	...
	protected InstanceFieldInstruction(String fieldName, String classType,
	...
	public FieldInfo getFieldInfo() {
		if (fi == null) {
			ClassInfo ci = ClassLoaderInfo
					.getCurrentResolvedClassInfo(className);
			if (ci != null) {
	...
	protected boolean isSchedulingRelevant(ThreadInfo ti, int objRef) {
		...
		if (fname.startsWith("this$")) {
			// that one is an automatically created outer object reference in an
			// inner class,
			// it can't be set. Unfortunately, we don't have an 'immutable'
			// attribute for
			// fields, just objects, so we can't push it into class load time
			// attributing.
		...
		if (mi.isInit() && ti.useUnsharedInit()) {
			// <2do> this should perhaps use the more expensive
			// ti.isCtorOnStack(objref) to cover
			// init methods called from the ctor
			return false;
		}

		// --- from here on, we know this is a shared object that can be
		// accessed concurrently
		if (ti.usePorSyncDetection()) {
			// this is a potentially more expensive test to identify fields
			// that are protected by locks, for which get/putXX should not
			// break.
	...
	protected boolean isNewPorFieldBoundary(ThreadInfo ti, FieldInfo fi,
			int objRef) {
		return (!ti.isFirstStepInsn()) && ti.usePorFieldBoundaries()
				&& isSchedulingRelevant(ti, objRef);
	}

	/**
	 * NOTE - the return value is *only* valid in a instructionExecuted()
	 * context, since the same instruction can be executed from different
	 * threads
	 */
	...
	public ElementInfo getLastElementInfo() {
		if (lastThis != MJIEnv.NULL) {
			return VM.getVM().getHeap().get(lastThis); // <2do> remove - should
														// be in clients
		}
	...
	/**
	 * this one can be used from a choiceGeneratorSet() or executeInstruction()
	 * context, since it peeks 'this' from the operand stack (enter didn't pop
	 * the value yet) it' less efficient than getLastElementInfo() from a
	 * instructionExecuted context
	 */
	@Override

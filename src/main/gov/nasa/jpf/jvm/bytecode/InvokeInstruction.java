...
public abstract class InvokeInstruction extends JVMInstruction {
	 ...
	 * Note that we can't directly resolve to MethodInfo objects because the
	 * corresponding class might not be loaded yet (has to be done on execution)
	 */
	...
	/**
	 * watch out - this is only const for static and special invocation all
	 * virtuals will use it only as a cache
	 */
	protected MethodInfo invokedMethod;

	protected Object[] arguments; // temporary cache for arg values (for
									// listeners)

	protected InvokeInstruction(String clsName, String methodName,
			String signature) {
	 ...
	 * denotes the target type info we have at the static point of the call,
	 * i.e. before dynamic dispatching
	 */
	...
	/**
	 * this is a little helper to find out about call argument values from
	 * listeners that don't want to dig through MethodInfos and Types. Reference
	 * arguments are returned as either ElementInfos or 'null', all others are
	 * boxed (i.e. a 'double' is returned as a 'Double' object). It goes without
	 * saying that this method can only be called during an executeInstruction()
	 * or instructionExecuted() notification for the corresponding
	 * InvokeInstruction We use the caller frame to retrieve the arguments
	 * (instead of the locals of the callee) since that works for both pre- and
	...
	/**
	 * check if there is any argument attr of the specified type (use this
	 * before using any of the more expensive retrievers)
	 */
	...
	/**
	 * do we have a reference argument that has an object attribute? less
	 * efficient, but still without object creation
	 */
	...
	// we get this from the caller because this works both for pre- and
	// post-exec
	// notifications, whereas retrieval from the callee frame of course only
	// works
	// post-exec
	Object[] getArgsFromCaller(ThreadInfo ti, StackFrame frame,
			MethodInfo callee) {
	...
	/**
	 * <2do> - this relies on same order of arguments and LocalVariableTable
	 * entries, which seems to hold for javac, but is not required by the VM
	 * spec, which only says that arguments are stored in consecutive slots
	 * starting at 0
	 */
	...
	protected boolean checkSyncCG(ElementInfo ei, ThreadInfo ti) {
		if (!ti.isFirstStepInsn()) {
			if (ei.getLockingThread() != ti) { // maybe its a recursive lock
				...
				if (ei.canLock(ti)) { // we can lock the object, check if we
										// need a CG
					if (ei.isShared()) {
					...
					ChoiceGenerator<?> cg = vm.getSchedulerFactory()
							.createSyncMethodEnterCG(ei, ti);
					vm.setMandatoryNextChoiceGenerator(cg,
							"blocking sync without CG");
					return true;
				}
			}
		}

...
/**
 * this is a property listener that turns thrown AssertionErrors into property
 * violations before they are caught (i.e. potentially change the stack).
 * Besides serving the purpose of eliminating the "catch(Throwable)" case, it
 * can be used in conjunction with "search.multiple_errors=true" to report
 * assertions but otherwise ignore them and go on searching the same path
 * (otherwise, multiple_errors would cause a backtrack)
 */
public class AssertionProperty extends PropertyListenerAdapter {

	static JPFLogger log = JPF
			.getLogger("gov.nasa.jpf.listener.AssertionProperty");

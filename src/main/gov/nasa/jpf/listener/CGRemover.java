...
/**
 * listener that removes CGs for specified locations, method calls or method
 * bodies
 * 
 ...
 * cgrm.thread.method_bodies = a.SomeClass.someMethod() // ..or a MethodSpec for
 * a body cgrm.thread.method_calls = b.A.foo(int) // ..or a MethodSpec for a
 * call
 * 
...
public class CGRemover extends ListenerAdapter {
	...
	static class Category {
		...
		boolean checkSpecification() {
			return cgClass != null
					&& (!locations.isEmpty() || !methodBodies.isEmpty() || !methodCalls
							.isEmpty());
		}
	}
		...
		for (String key : conf.getKeysStartingWith("cgrm.")) {
			...
			if (kc.length == 3) {
				...
				if ("cg_class".equals(k)) {
					...
					parseLocationSpecs(category.locations,
							conf.getStringArray(key));

				} else if ("method_bodies".equals(k)) {
					parseMethodSpecs(category.methodBodies,
							conf.getStringArray(key));

				} else if ("method_calls".equals(k)) {
					parseMethodSpecs(category.methodCalls,
							conf.getStringArray(key));
	...
	protected void processClass(ClassInfo ci, Category cat) {
		...
		for (LocationSpec loc : cat.locations) {
			if (loc.matchesFile(fname)) { // Ok, we have to dig out the
											// corresponding insns (if any)
				...
				if (insns != null) {
					...
					log.warning("no insns for location: ", loc, " in class: ",
							ci.getName());
				}
			}
		}
	...
	// this is where we turn Categories into MethodInfos and Instructions to
	// watch out for
	@Override
	...
	public void choiceGeneratorRegistered(VM vm, ChoiceGenerator<?> nextCG,
		...
		if (insn instanceof InvokeInstruction) {
			MethodInfo invokedMi = ((InvokeInstruction) insn)
					.getInvokedMethod();
			if (methodCalls != null) {

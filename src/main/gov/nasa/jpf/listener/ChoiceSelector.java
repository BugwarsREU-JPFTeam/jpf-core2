...
/**
 * this is a listener that only executes single choices until it detects that it
 * should start to search. If nothing is specified, this is pretty much a
 * simulator that randomly picks choices. Otherwise the user can give it any
 * combination of - a set of thread names - a set of method names - a start
 * search depth to turn on the search. If more than one condition is given, all
...
public class ChoiceSelector extends ListenerAdapter {
	...
	// those are our singleChoice end conditions (i.e. where we start the
	// search)
	StringSetMatcher threadSet; // we start when all threads in the set are
								// active
	...
	public ChoiceSelector(Config config, JPF jpf) {
		...
		threadSet = StringSetMatcher.getNonEmpty(config
				.getStringArray("choice.threads"));
		if (threadSet != null) {
			threadsAlive = false;
		}

		calls = StringSetMatcher.getNonEmpty(config
				.getStringArray("choice.calls"));
		callSeen = false;
	...
	public void choiceGeneratorAdvanced(VM vm, ChoiceGenerator<?> currentCG) {
		...
		if (trace != null) { // this is a replay
			...
			// <2do> maybe that should just be a warning, and then a single
			// choice
			assert currentCG.getClass().getName()
					.equals(trace.getCgClassName()) : "wrong choice generator class, expecting: "
					+ trace.getCgClassName()
					+ ", read: "
					+ currentCG.getClass().getName();
			...
			if (singleChoice) {
				if (n > 1) {
					...
					currentCG.select(r); // sets it done, so we never backtrack
											// into it
				}
			}
		}
	...
	public void executeInstruction(VM vm, ThreadInfo ti,
		...
		if (singleChoice && !callSeen && (calls != null)) {
			if (insnToExecute instanceof InvokeInstruction) {
				String mthName = ((InvokeInstruction) insnToExecute)
						.getInvokedMethod(ti).getBaseName();

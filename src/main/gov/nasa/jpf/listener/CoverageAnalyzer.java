 ...
 * 
 * The idea is to collect per-class/-method/-thread information about executed
 * instructions, and then analyze this deeper when it comes to report time (e.g.
 * branch coverage, basic blocks, ..)
 * 
 ...
 */
public class CoverageAnalyzer extends ListenerAdapter implements
		PublisherExtension {
	...
	static class MethodCoverage {
		...
		boolean getCoveredLines(BitSet executable, BitSet covered) {
			...
			if (branchTrue != null) {
				for (i = branchTrue.length() - 1; i >= 0; i--) {
					...
					if ((!cTrue || !cFalse)
							&& (inst[i] instanceof IfInstruction)) {
						insn.clear(i);
					}
				}
			}
			...
			for (i = inst.length - 1; i >= 0; i--) {
				line = inst[i].getLineNumber();
				if ((!insn.get(i)) && (line > 0)) { // TODO What do we do with
													// instructions that don't
													// have a line number. Is
													// this even possible?
					covered.clear(line);
				}
			}
		...
		BitSet getHandlerStarts() {
			...
			if (handler != null) {
				for (int i = 0; i < handler.length; i++) {
					Instruction hs = mi.getInstructionAt(handler[i]
							.getHandler());
		...
		BitSet getHandlers() {
			...
			// we hit a forward GOTO (or RETURN). If the insn following the goto
			// is the
				...
				if (!hs.isEmpty()) {
					for (int i = 0; i < code.length; i++) {
						...
						if (insn instanceof GOTO) {
							...
							if (!gotoInsn.isBackJump() && hs.get(i + 1)) { // jump
																			// around
																			// handler
								int handlerEnd = gotoInsn.getTarget()
										.getInstructionIndex();
								for (i++; i < handlerEnd; i++) {
									b.set(i);
								}
							}
						} else if (insn instanceof ReturnInstruction) { // everything
																		// else
																		// is
																		// handler
		...
		// what the condition values were. We are not interested in GOTOs and
		// exceptions
		BitSet getBranches() {
			if (branches == null) {
				Instruction[] code = mi.getInstructions();
			...
			if (basicBlocks == null) {
				...
				for (int i = 0; i < code.length; i++) {
					...
					if (insn instanceof IfInstruction) {
						...
						// hmm, this might be a bit too conservative, but who
						// says we
						// don't jump out of a caller into a handler, or even
						// that we
				...
				if (handlers != null) {
					for (int i = 0; i < handlers.length; i++) {
						Instruction tgt = mi.getInstructionAt(handlers[i]
								.getHandler());
						bb.set(tgt.getInstructionIndex());
					}
				}
				 ...
				 * System.out.println(mi.getFullName()); for (int i=0; i
				 * <code.length; i++) {
	...
	static class ClassCoverage {
		...
		void setLoaded(ClassInfo ci) {
			if (methods == null) {
				...
				for (MethodInfo mi : ci.getDeclaredMethodInfos()) {
					// <2do> what about MJI methods? we should report why we
					// don't cover them
		...
		boolean isCodeLoaded() {
			...
			try {
				...
				log.warning("CoverageAnalyzer problem: " + cie); // Just log the
																	// problem
																	// but don't
																	// fail. We
																	// still
	...
	static boolean loadedOnly; // means we only check loaded classes that are
								// not filtered
	static boolean showMethods; // do we want to show per-method coverage?
	static boolean showMethodBodies;
	static boolean excludeHandlers; // do we count the handlers in? (off-nominal
									// CF)
	...
	public CoverageAnalyzer(Config conf, JPF jpf) {
		includes = StringSetMatcher.getNonEmpty(conf
				.getStringArray("coverage.include"));
		excludes = StringSetMatcher.getNonEmpty(conf
				.getStringArray("coverage.exclude"));
	...
	void getCoverageCandidates() {
		...
		// multiple entries)
		// NOTE : this doesn't yet deal with ClassLoaders, but that's also true
		// for BCEL
	...
	void traverseJar(File jar) {
		try {
			...
			for (Enumeration<JarEntry> entries = jf.entries(); entries
					.hasMoreElements();) {
				...
				if (!e.isDirectory()) {
					...
					if (eName.endsWith(".class")) {
						if (e.getSize() > 0) {
							String clsName = eName.substring(0,
									eName.length() - 6);
	...
	HashMap<String, Integer> getGlobalRequirementsMethods() {
		...
		// it would be much better to do this with Class<?> instead of
		// ClassInfo, but
		// then we need a JPF- ClassLoader, since the path might be different.
		// As a
		// middle ground, we could use BCEL

		for (ClassCoverage cc : classes.values()) {
			ClassInfo ci = ClassLoaderInfo
					.getCurrentResolvedClassInfo(cc.className);
			for (MethodInfo mi : ci.getDeclaredMethodInfos()) {
	...
	private void computeCoverages(String packageFilter,
			List<Map.Entry<String, ClassCoverage>> clsEntries, Coverage cls,
			Coverage mth, Coverage branch, Coverage block, Coverage line,
			Coverage insn) {
	...
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction nextInsn,
		...
		if (mc != null) {
			...
			if (showRequirements) {
				if (executedInsn.getPosition() == 0) { // first insn in method,
														// check for
														// Requirements
					AnnotationInfo ai = getRequirementsAnnotation(mc
							.getMethodInfo());
					if (ai != null) {
	...
	public void choiceGeneratorSet(VM vm, ChoiceGenerator<?> newCG) {
		/***
		 * should be an option Instruction insn = vm.getLastInstruction();
	...
	class PublishConsole extends Publish {
		...
		void printCoverage(Coverage cov) {
			...
			if (nTotal <= 0) {
				...
				s = String.format("%.2f (%d/%d)", ((double) nCovered / nTotal),
						nCovered, nTotal);
			}
		...
		void printClassCoverages() {
			...
			computeCoverages("", clsEntries, clsCoverage, mthCoverage,
					branchCoverage, bbCoverage, lineCoverage, insnCoverage);
		...
		void printMethodCoverages(ClassCoverage cc) {
			...
			ArrayList<Map.Entry<MethodInfo, MethodCoverage>> mthEntries = Misc
					.createSortedEntryList(
							cc.methods,
							new Comparator<Map.Entry<MethodInfo, MethodCoverage>>() {
								...
								public int compare(
									...
									int a = o2.getValue().getCoveredInsn()
											.percent();
									int b = o1.getValue().getCoveredInsn()
											.percent();
			...
			for (Map.Entry<MethodInfo, MethodCoverage> e : mthEntries) {
				...
				if (showMethodBodies
						&& (!insnCoverage.isFullyCovered() || !branchCoverage
								.isFullyCovered())) {
		...
		void printBodyCoverage(MethodCoverage mc) {
			...
			lastStart = -1; // reset in case condition and branch are in same
							// line
			for (i = 0; i < code.length; i++) {
				if (branches.get(i)) {
					String prefix = "";
					...
					if (bTrue != null) { // means we have condition bit sets
						...
						if (cTrue) {
							prefix = cFalse ? "" : "F "; // covered or false
															// missing
						} else {
							prefix = cFalse ? "T " : "N "; // true or both
															// missing
						}
					} else {
		...
		void printSourceRange(Instruction[] code, BitSet handlers, int start,
			...
			printLocation(prefix, "at", code[start].getSourceLocation(),
					handlers.get(start) ? "x" : "");

			if (line != code[end].getLineNumber()) {
				printLocation(prefix, "..", code[end].getSourceLocation(),
						handlers.get(end) ? "x" : "");
			}
		...
		private void printLocation(String prefix, String at, String location,
				String suffix) {
	...
	public void publishFinished(Publisher publisher) {
		...
		if (clsEntries == null) {
			clsEntries = Misc.createSortedEntryList(classes,
					new Comparator<Map.Entry<String, ClassCoverage>>() {

 ...
 * it's own transition stack. this is still work in progress, analyzing the
 * trace can be much more elaborate (we just dump up to a max history size for
 * now)
 * 
 * <2do> DeadlockAnalyzer output can be confusing if a reorganizing ThreadList
 * is used (which reassigns thread ids)
 */
public class DeadlockAnalyzer extends ListenerAdapter {
	...
	static class ThreadOp {
		...
		ThreadOp(ThreadInfo ti, ElementInfo ei, OpType type) {
			this.ti = ti;
			this.ei = ei;
			insn = getReportInsn(ti); // we haven't had the executeInsn
										// notification yet
			opType = type;
			...
			for (ThreadInfo t : tlist) {
				if (ti == t) {
					if (opType == OpType.started || opType == OpType.terminated) {
						pw.print(String.format("   %1$s    ",
								opTypeMnemonic[opType.ordinal()]));
					} else {
						pw.print(String.format("%1$s:%2$-5x ",
								opTypeMnemonic[opType.ordinal()],
								ei.getObjectRef()));
					}
					// break;
	 ...
	 * completely on the basis of the recorded ThreadOps, i.e. don't rely on
	 * when this is called
	 */
			...
			for (ThreadOp tOp = trans; tOp != null; tOp = tOp.prevOp) {
				...
				if (ot == OpType.wait || ot == OpType.block) {
					if (!runnables.contains(oti) && !threads.contains(oti)) {
						HashMap<ElementInfo, ThreadInfo> map = (ot == OpType.block) ? blocks
								: waits;
						threads.add(oti);
						map.put(tOp.ei, oti);
						ops.add(tOp);
				...
				} else if (ot == OpType.notify || ot == OpType.notifyAll
						|| ot == OpType.lock) {
					HashMap<ElementInfo, ThreadInfo> map = (ot == OpType.lock) ? blocks
							: waits;
	...
	Collection<ThreadInfo> getThreadList() {
		...
		prevTrans: for (ThreadOp tOp = lastTransition; tOp != null; tOp = tOp.prevTransition) {
			i++;
	...
	/**
	 * this is the workhorse - filter which ops should be shown, and which are
	 * irrelevant for the deadlock
	 */
	boolean showOp(ThreadOp op, ThreadInfo[] tlist, boolean[] waitSeen,
			boolean[] notifySeen, boolean[] blockSeen, boolean[] lockSeen,
		...
		case lock:
			...
			// only report the last one if there is a thread that's currently
			// blocked on it
			for (int i = 0; i < tlist.length; i++) {
				if ((i != idx) && tlist[i].isBlocked()
						&& (tlist[i].getLockObject() == ei)) {
					if (!lockSeen[i]) {
						lockSeen[i] = true;
						return true;
					}
		...
		case wait:
			if (ti.isWaiting()) { // only show the last one if this is a waiting
									// thread
				if (!waitSeen[idx]) {
					waitSeen[idx] = true;
					return true;

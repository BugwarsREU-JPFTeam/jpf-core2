 ...
 * the case of Multiprocess applications. It distinguishes local choices from
 * global choices.
 * 
...
public class DistributedSimpleDot extends SimpleDot {
	...
	public DistributedSimpleDot(Config config, JPF jpf) {
		...
		startNodeAttrs = config.getString("dot.mp_start_node_attr",
				MP_START_NODE_ATTRS);
	}
	...
	public void stateAdvanced(Search search) {
		...
		String prcId = "P"
				+ Integer.toString(search.getVM()
						.getCurrentApplicationContext().getId());
		...
		if (search.isErrorState()) {
			...
			if (search.isEndState()) {
				printTransition(getStateId(lastId), getStateId(id), choice,
						lastInst);
	...
	protected String getNextCG() {
		...
		} else if (insn instanceof DIRECTCALLRETURN
				&& vm.getCurrentThread().getNextPC() == null) {
			return "return";
		}

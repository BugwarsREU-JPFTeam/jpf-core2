 ...
 * replacement for JPF, which is called by ExecTracker. ExecTracker is mostly a
 * VMListener of 'instructionExecuted' and a SearchListener of 'stateAdvanced'
 * and 'statehBacktracked'
 */
...
public class ExecTracker extends ListenerAdapter {
	...
	public ExecTracker(Config config) {
		/**
		 * @jpfoption et.print_insn : boolean - print executed bytecode
		 *            instructions (default=true).
		 */
		printInsn = config.getBoolean("et.print_insn", true);

		/**
		 * @jpfoption et.print_src : boolean - print source lines
		 *            (default=false).
		 */
		printSrc = config.getBoolean("et.print_src", false);

		/**
		 * @jpfoption et.print_mth : boolean - print executed method names
		 *            (default=false).
		 */
		printMth = config.getBoolean("et.print_mth", false);

		/**
		 * @jpfoption et.skip_init : boolean - do not log execution before
		 *            entering main() (default=true).
		 */
	...
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction nextInsn,
		...
		// that's pretty redundant to what is done in the ConsolePublisher, but
		// we don't want
		...
		if (printInsn) {
			...
			if (executedInsn instanceof InvokeInstruction) {
				MethodInfo callee = ((InvokeInstruction) executedInsn)
						.getInvokedMethod();
				if ((callee != null) && callee.isMJI()) { // Huhh? why do we
															// have to check
															// this?
					out.print(" [native]");
				}
	...
	public void threadStarted(VM vm, ThreadInfo ti) {
		out.println("\t\t # thread started: " + ti.getName() + " index: "
				+ ti.getId());
	}

	@Override
	public void threadTerminated(VM vm, ThreadInfo ti) {
		out.println("\t\t # thread terminated: " + ti.getName() + " index: "
				+ ti.getId());
	}
	...
	public void objectExposed(VM vm, ThreadInfo currentThread,
		...
		if (showShared) {
			out.println("\t\t # exposed " + exposedObject + " through shared "
					+ sharedObject);
		}
	}

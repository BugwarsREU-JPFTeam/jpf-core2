...
public class HeapTracker extends PropertyListenerAdapter {
	...
	public HeapTracker(Config config, JPF jpf) {
		...
		includes = StringSetMatcher.getNonEmpty(config
				.getStringArray("heap.include"));
		excludes = StringSetMatcher.getNonEmpty(config
				.getStringArray("heap.exclude"));
	...
	public boolean check(Search search, VM vm) {
		if (throwOutOfMemory) {
			...
			if ((maxLiveLimit >= 0)
					&& ((stat.nNew - stat.nReleased) > maxLiveLimit)) {
				return false;
			}
	...
	public String getErrorMessage() {
		return "heap limit exceeded: " + stat.heapSize + " > "
				+ maxHeapSizeLimit;
	}
	...
	public void publishFinished(Publisher publisher) {
		...
		if (showTypeStats) {
			...
			ArrayList<Map.Entry<String, TypeStat>> list = Misc
					.createSortedEntryList(typeStat,
							new Comparator<Map.Entry<String, TypeStat>>() {
								@Override
								public int compare(
										Map.Entry<String, TypeStat> e1,
										Map.Entry<String, TypeStat> e2) {
									return Integer.signum(e1.getValue().nAlloc
											- e2.getValue().nAlloc);
								}
							});
	...
	public void gcBegin(VM vm) {
		/**
		 * System.out.println(); System.out.println( "----- gc cycle: " +
		 * vm.getDynamicArea().getGcNumber() + ", state: " + vm.getStateId());
		 **/
	}
	...
	public void gcEnd(VM vm) {
		...
		if (pShared > pElemSharedMax)
			pElemSharedMax = pShared;
		if (pShared < pElemSharedMin)
			pElemSharedMin = pShared;
		...
		if (pImmutable > pElemImmutableMax)
			pElemImmutableMax = pImmutable;
		if (pImmutable < pElemImmutableMin)
			pElemImmutableMin = pImmutable;

		nElemAv = nElemTotal / nGcTotal;
		nReleasedAv = nReleasedTotal / nGcTotal;

		if (nReleased > nReleasedMax)
			nReleasedMax = nReleased;
		if (nReleased < nReleasedMin)
			nReleasedMin = nReleased;
	...
	static void printUsage() {
		System.out
				.println("HeapTracker - a JPF listener tool to report and check heap utilization");
		System.out
				.println("usage: java gov.nasa.jpf.tools.HeapTracker <jpf-options> <heapTracker-options> <class>");
		System.out
				.println("       +heap.size_limit=<num> : report property violation if heap exceeds <num> bytes");
		System.out
				.println("       +heap.live_limit=<num> : report property violation if more than <num> live objects");
		System.out
				.println("       +heap.classes=<regEx> : only report instances of classes matching <regEx>");
		System.out
				.println("       +heap.throw_exception=<bool>: throw a OutOfMemoryError instead of reporting property violation");
	}
}

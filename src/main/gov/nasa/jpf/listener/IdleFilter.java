 ...
 * might starve other threads or JPF. The most classical case is a "busy wait"
 * loop like
 * 
 ...
 * familiar with model checking would expect state matching. Only that without a
 * transition break, JPF has no reason to match states, so we have to
 ...
 * do terminate.
 * 
 * IdleFilter has two options: idle.max_backjumps : sets the number of backjumps
 * after which we break idle.action : what to do if max_backjumps are exceeded
 * in the same thread on the same location and stackframe warn : only print
 * warning for backjumps exceeding the max_backjumps break : break the
 * transition to allow state matching prune : unconditionally prune the search
 * jump : jump past the backjump (this is dangerous if the loop has side
 * effects)
 */
public class IdleFilter extends PropertyListenerAdapter {
	...
	static enum Action {
		JUMP, PRUNE, BREAK, YIELD, WARN
	}

	DynamicObjectArray<ThreadStat> threadStats = new DynamicObjectArray<ThreadStat>(
			4, 16);
	...
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction nextInsn,
		...
		if (executedInsn.isBackJump()) {
			...
			if ((loopStackDepth != ts.loopStackDepth)
					|| (loopPc != ts.loopStartPc)) {
				// new loop, reset
				ts.isCleared = false;
				...
				if (!ts.isCleared) {
					if (ts.backJumps > maxBackJumps) {
						...
						ti.reschedule("idleFilter"); // this breaks the
														// executePorStep loop
						...
						switch (action) {
						case JUMP:
							// pretty bold, we jump past the loop end and go on
							// from there
						...
						case PRUNE:
							// cut this sucker off - we declare this a visited
							// state
							vm.ignoreState();
						...
						case BREAK:
							// just break the transition and let the state
							// matching take over
							brokeTransition = true;
							ti.breakTransition("breakIdleLoop");

							log.warning("breaks transition on suspicious loop in thread: "
									+ ti.getName()
									+ "\n\tat "
									+ ci.getName()
									+ "."
						...
						case YIELD:
							...
							log.warning("yield on suspicious loop in thread: "
									+ ti.getName() + "\n\tat " + ci.getName()
									+ "." + mi.getName() + "(" + file + ":"
									+ line + ")");
						...
						case WARN:
							log.warning("detected suspicious loop in thread: "
									+ ti.getName() + "\n\tat " + ci.getName()
									+ "." + mi.getName() + "(" + file + ":"
									+ line + ")");
							break;
			...
			// if we call methods or set array elements inside the loop in
			// question,
			// we assume this is not an idle loop and terminate the checks
			// <2do> this is too restrictive - we should leave this to state
			// matching

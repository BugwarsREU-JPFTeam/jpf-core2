...
/**
 * A listener that tracks information about the stack depth of when a lock is
 * first acquired. If
 * 
 * Writing a test for this class is very difficult. Hence, a lot of asserts are
 * added.
 */
public class LockedStackDepth extends ListenerAdapter {
	private static final Logger s_logger = JPF.getLogger(LockedStackDepth.class
			.getName());
	...
	public int getLockedStackDepth(ElementInfo lock) {
		...
		if (s_logger.isLoggable(Level.INFO))
			s_logger.info("Depth = " + result + " | Lock Index = " + lockIndex
					+ " | Lock = " + lock);
	...
	public void objectLocked(VM vm, ThreadInfo thread, ElementInfo ei) {
		...
		if (m_state.containsKey(makeKey(lock))) // So that a breakpoint on the
												// next line will only get hit
												// if the assert will trigger.
	...
	public void stateAdvanced(Search search) {
		...
		if (!m_operations.containsKey(id)) // Don't overwrite the original chain
											// of Operations to get to the same
											// state. The original chain is more
	...
	public void stateProcessed(Search search) {
		...
		if (!(search instanceof DFSearch)) // Can't remove from m_operations
											// since Search could go back to the
											// state.
		...
		m_operations.remove(id); // DFSearch won't ever revisit this state. It
									// is safe to remove and allow for cleanup.
	...
	private void switchTo(Operation next) {
		...
		for (operation = next; operation != null; operation = operation
				.getParent()) // Go through all of the operations leading back
								// to the root.
		{
			m_index.put(operation, m_apply.size()); // Keep track of the index
													// into m_apply where
													// operation is found
		...
		for (operation = m_current; operation != null; operation = operation
				.getParent()) // Go through all of the operations leading back
								// to the root.
		{
			index = m_index.get(operation);

			if (index != null) // If a common ancestor is found, stop going
								// back.
				break;

			operation.revert(); // Revert the operation since it isn't common to
								// both states.
		}

		if (index == null)
			index = m_apply.size(); // No common ancestor found. Must apply all
									// of the operations.

		for (i = index; --i >= 0;)
			// Apply all of the operations required to get back to the "next"
			// state.
			m_apply.get(i).apply();
	...
	private class Operation {
		...
		private void log(String header) {
			...
			message = header + " " + subheader + " | Depth = " + depthStr
					+ " | Key = " + (m_key & ~THREAD_FLAG) + " | " + type;

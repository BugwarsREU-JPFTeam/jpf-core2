 ...
 * the program to hang. This property finds and outputs cycles in the states so
 * that they may be investigated.
 * 
 * The following might need to be added to listener property...
 * gov.nasa.jpf.tools.SimpleIdleFilter gov.nasa.jpf.tools.IdleFilter
 */
...
public class NoStateCycles extends PropertyListenerAdapter {
	...
	public NoStateCycles(Config config) {
		if (!config.getString("search.class").equals(
				"gov.nasa.jpf.search.DFSearch"))
			config.throwException("search.class must be gov.nasa.jpf.search.DFSearch"); // Or
																						// any
																						// class
																						// which
																						// does

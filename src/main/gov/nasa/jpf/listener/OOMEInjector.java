...
/**
 * simulator for OutOfMemoryErrors. This can be configured to either fire for a
 * specified location range (file:line) or specified types. Ranges are
 * transitive, i.e. everything called from within it should also trigger.
 * 
 * Since our only action is to inject OutOfMemoryErrors, we don't need to
...
public class OOMEInjector extends ListenerAdapter {
	...
	static OOME throwOOME = new OOME(); // we can reuse the same object as an
										// attribute
	...
	public void executeInstruction(VM vm, ThreadInfo ti,
		...
		if (insnToExecute instanceof AllocInstruction) {
			if (checkCallerForOOM(ti.getTopFrame(), insnToExecute)) {
				// we could use Heap.setOutOfMemory(true), but then we would
				// have to reset
				// if the app handles it so that it doesn't throw outside the
				// specified locations.
				// This would require more effort than throwing explicitly
				Instruction nextInsn = ti
						.createAndThrowException("java.lang.OutOfMemoryError");
	...
	@Override
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction insn,
		...
		if (executedInsn instanceof InvokeInstruction) {
			...
			if (frame.getPC() != executedInsn) { // means the call did succeed
				if (checkCallerForOOM(frame.getPrevious(), executedInsn)) {
					frame.addFrameAttr(throwOOME); // propagate caller OOME
													// context
				}
			}

...
/**
 * listener that keeps track of all allocations, method calls, field updates and
 * deallocations of instances of a set of types
 * 
 * we don't need a StateExtensionClient/Listener here because we only keep live,
 * typed object data. If we ever backtrack to a point where a registered object
 * didn't exist yet, this id can't be used again (for a relevant object) w/o
 * previous registration. Note this might cause some discarded objects to stay
 * around, but requires probably less memory than storing a collection for every
 * state id.
 */
public class ObjectTracker extends PropertyListenerAdapter {
	...
	public ObjectTracker(Config conf, JPF jpf) {
		...
		includes = StringSetMatcher.getNonEmpty(conf
				.getStringArray("ot.include"));
		excludes = StringSetMatcher.getNonEmpty(conf
				.getStringArray("ot.exclude"));
	...
	boolean checkShared(Record rec, ThreadInfo ti, InfoObject use,
			Instruction insn) {
		if (checkShared) {
			AnnotationInfo ai = rec.ei.getClassInfo().getAnnotation(
					"gov.nasa.jpf.NonShared");
	...
	boolean checkConst(Record rec, ThreadInfo ti, FieldInfo fi, Instruction insn) {
		if (checkConst) {
			AnnotationInfo ai = insn.getMethodInfo().getAnnotation(
					"gov.nasa.jpf.Const");
			if (ai != null) {
	...
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction nextInsn,
		...
		if (executedInsn instanceof VirtualInvocation) {
			...
			if (rec != null) {
				...
				if (logPut) {
					log(ti, "put %1$s.%2$s = <%3$d>", rec.ei, fi.getName(),
							storeInsn.getLastValue());
				}

 ...
 * 
 * NOTE - this listener initializes in two steps: (1) during listener
 * construction it builds a list of classes it has to monitor, and (2) during
 * class load time it further analyzes classes from this list to get the actual
 * target objects (FieldInfos and MethodInfos) so that instruction monitoring is
 * efficient enough.
 * 
 ...
 * 
 * # field getter example perturb.fields = altitude,... perturb.altitude.field =
 * x.y.MyClass.alt perturb.altitude.class = .perturb.IntOverUnder
 * perturb.altitude.location = MyClass.java:42 perturb.altitude.delta = 1
 * 
 * # method return value example perturb.returns = velocity,...
 * perturb.velocity.method = x.y.MyClass.computeVelocity()
 * perturb.velocity.class = .perturb.IntOverUnder perturb.velocity.delta = 50
 * 
 * # method parameter perturbation example perturb.params = foo, ...
 * perturb.foo.method = x.y.MyClass.send(int, float, boolean)
 * perturb.foo.location = MyClass.java:42 perturb.class =
 * .perturb.dataAbstractor
 * 
 */
...
public class Perturbator extends ListenerAdapter {
	...
	public static class Perturbation {
		...
		Class<? extends ChoiceGenerator<?>> cgType; // needs to be compatible
													// with field type
	...
	public static class FieldPerturbation extends Perturbation {
		...
		FieldPerturbation(FieldSpec fieldSpec, OperandPerturbator perturbator,
				String loc) {
	...
	public static class ReturnPerturbation extends Perturbation {
		...
		ReturnPerturbation(MethodSpec mthSpec, OperandPerturbator perturbator,
				String loc) {
			super(perturbator, loc);
	...
	public static class ParamsPerturbation extends Perturbation {
		...
		ParamsPerturbation(MethodSpec mthSpec, OperandPerturbator perturbator,
				String loc) {
			super(perturbator, loc);
	...
	public Perturbator(Config conf) {
		...
		// in the ctor we only find out which classname patterns we have to
		// watch
		// for, and store them in a list (together with their partially
		// initialized
		// Perturbation instances) that is to be checked upon classLoaded
		// notifications
		...
		String[] returnIds = conf
				.getCompactTrimmedStringArray("perturb.returns");
		for (String id : returnIds) {
			addToReturnWatchList(conf, id);
		}

		String[] paramsIds = conf
				.getCompactTrimmedStringArray("perturb.params");
	...
	protected void addToFieldWatchList(Config conf, String id) {
		...
		if (fs != null) {
			...
			if (fieldSpec != null) {
				...
				OperandPerturbator perturbator = conf.getInstance(keyPrefix
						+ ".class", OperandPerturbator.class, argTypes, args);
				if (perturbator != null) {
					String loc = conf.getString(keyPrefix + ".location");
					FieldPerturbation p = new FieldPerturbation(fieldSpec,
							perturbator, loc);
	...
	protected void addToReturnWatchList(Config conf, String id) {
		...
		if (ms != null) {
			...
			if (mthSpec != null) {
				...
				if (perturbator != null) {
					String loc = conf.getString(keyPrefix + ".location");
					ReturnPerturbation p = new ReturnPerturbation(mthSpec,
							perturbator, loc);
	...
	protected void addToParamsWatchList(Config conf, String id) {
		...
		if (ms != null) {
			...
			if (mthSpec != null) {
				...
				if (perturbator != null) {
					String loc = conf.getString(keyPrefix + ".location");
					ParamsPerturbation p = new ParamsPerturbation(mthSpec,
							perturbator, loc);
	...
	public void classLoaded(VM vm, ClassInfo loadedClass) {
		...
		for (FieldPerturbation p : fieldWatchList) {
			...
			if (fs.isMatchingType(loadedClass)) {
				addFieldPerturbations(p, loadedClass,
						loadedClass.getDeclaredInstanceFields());
				addFieldPerturbations(p, loadedClass,
						loadedClass.getDeclaredStaticFields());
			}
		}
		...
		for (ReturnPerturbation p : returnWatchList) {
			...
			if (ms.isMatchingType(loadedClass)) {
				for (MethodInfo mi : loadedClass.getDeclaredMethodInfos()) {
					if (ms.matches(mi)) {
						Class<? extends ChoiceGenerator<?>> returnCGType = mi
								.getReturnChoiceGeneratorType();
						Class<? extends ChoiceGenerator<?>> perturbatorCGType = p.perturbator
						...
						if (returnCGType.isAssignableFrom(perturbatorCGType)) {
							...
							log.warning("method "
									+ mi
									+ " not compatible with perturbator choice type "
									+ perturbatorCGType.getName());
						}
					}
				}
			}
		...
		for (ParamsPerturbation p : paramsWatchList) {
			...
			if (ms.isMatchingType(loadedClass)) {
				for (MethodInfo mi : loadedClass.getDeclaredMethodInfos()) {
					if (ms.matches(mi)) {
						// We simply associate the method with the parameters
						// perturbator
						Class<? extends ChoiceGenerator<?>> perturbatorCGType = p.perturbator
								.getChoiceGeneratorType();
	...
	protected void addFieldPerturbations(FieldPerturbation p, ClassInfo ci,
			FieldInfo[] fieldInfos) {
		for (FieldInfo fi : ci.getDeclaredInstanceFields()) {
			if (p.fieldSpec.matches(fi)) {
				Class<? extends ChoiceGenerator<?>> fieldCGType = fi
						.getChoiceGeneratorType();
				Class<? extends ChoiceGenerator<?>> perturbatorCGType = p.perturbator
						.getChoiceGeneratorType();
	...
	protected boolean isRelevantCallLocation(Instruction invokeInsn,
			Perturbation p) {
	...
	public void executeInstruction(VM vm, ThreadInfo ti,
		...
		if (insnToExecute instanceof GETFIELD) {
			FieldInfo fi = ((InstanceFieldInstruction) insnToExecute)
					.getFieldInfo();
			...
			if (e != null) { // managed field
				if (isMatchingInstructionLocation(e, insnToExecute)) {
					if (!ti.isFirstStepInsn()) {
						// save the current stackframe so that we can restore it
						// before
						// we re-enter
			...
			if (e != null && isRelevantCallLocation(ti, e)) {
				...
				if (!ti.isFirstStepInsn()) {
					// first time, create & set CG but DO NOT enter the insn
					// since it would
					// pop the callee stackframe and modify the caller
					// stackframe
					// note that we don't need to enter in order to get the
					// perturbation base
					// value because its already on the operand stack
					ChoiceGenerator<?> cg = e.perturbator
							.createChoiceGenerator("perturbReturn",
									ti.getTopFrame(), new Integer(0));
					if (ss.setNextChoiceGenerator(cg)) {
						ti.skipInstruction(insnToExecute);
					}
				} else {
					// re-executing, modify the operand stack top and enter
					ChoiceGenerator<?> cg = ss.getCurrentChoiceGenerator(
							"perturbReturn", e.cgType);
					if (cg != null) {
			...
			// first get the method info object corresponding to the invoked
			// method
			// We can't use getMethodInfo as the method returned may not be the
			// actual
			...
			if (e != null && isRelevantCallLocation(insnToExecute, e)) {
				...
				if (!ti.isFirstStepInsn()) {
					// first time, create and set CG and skip instruction as we
					// want the instruction
					// to be executed with the parameter choices we like instead
					// of the ones that
					// were passed in
					ChoiceGenerator<?> cg = e.perturbator
							.createChoiceGenerator(mi.getFullName(),
									ti.getTopFrame(), mi);
					// check if the cg returned is null. If it is then we don't
					// want to enter this
					// method as we are done exploring it
					if (cg != null) {
						log.info("--- Creating choice generator: "
								+ mi.getFullName() + " for thread: " + ti);
						if (ss.setNextChoiceGenerator(cg)) {
							ti.skipInstruction(insnToExecute);
						}
					}
					...
					ChoiceGenerator<?> cg = ss.getChoiceGenerator(mi
							.getFullName());
					if (cg != null) {
						log.info("--- Using choice generator: "
								+ mi.getFullName() + " in thread: " + ti);
						e.perturbator.perturb(cg, ti.getTopFrame());
					}
				}
			}
	...
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction nextInsn,
		...
		if (executedInsn instanceof GETFIELD) {
			FieldInfo fi = ((InstanceFieldInstruction) executedInsn)
					.getFieldInfo();
			FieldPerturbation p = perturbedFields.get(fi);
			if (p != null) {
				if (isMatchingInstructionLocation(p, executedInsn)) { // none or
																		// managed
																		// filePos
					StackFrame frame = ti.getTopFrame();
					SystemState ss = vm.getSystemState();

					if (ti.isFirstStepInsn()) { // retrieve value from CG and
												// replace it on operand stack
						ChoiceGenerator<?> cg = ss.getCurrentChoiceGenerator(
								"perturbGetField", p.cgType);
						if (cg != null) {
					...
					} else { // first time around, create&set the CG and
								// reexecute
						ChoiceGenerator<?> cg = p.perturbator
								.createChoiceGenerator("perturbGetField",
										frame, new Integer(0));
						if (ss.setNextChoiceGenerator(cg)) {
							assert savedFrame != null;
							// we could more efficiently restore the stackframe
							// to pre-exec state from last 'this' or classobject
							// ref, but then
	...
	protected boolean isMatchingInstructionLocation(Perturbation p,
			Instruction insn) {

...
/**
 * This is a Race Detection Algorithm that is precise in its calculation of
 * races, i.e. no false warnings. It exploits the fact that every thread choice
 * selection point could be due to a possible race. It just runs through all the
 * thread choices and checks whether there are more than one thread trying to
 * read & write to the same field of an object.
 * 
 * Current limitation is that it is only sound, i.e. will not miss a race, if
 * the sync-detection is switched off during model checking. This is due to the
 * fact that the sync-detection guesses that an acess is lock-protected when it
 * in reality might not be.
 * 
 * The listener also checks races for array elements, but in order to do so you
 * have to set "cg.threads.break_arrays=true" (note that it is false by default
 * because this can cause serious state explosion)
 * 
 * This algorithm came out of a discussion with Franck van Breugel and Sergey
 * Kulikov from the University of York. All credits for it goes to Franck and
 * Sergey, all the bugs are mine.
 * 
 * 
...
public class PreciseRaceDetector extends PropertyListenerAdapter {
	...
	static class Race {
		...
		boolean isRace() {
			return insn2 != null && ti1 != null && ti2 != null
					&& (!ti1.equals(ti2));
		}
	...
	static class FieldRace extends Race {
		...
		static Race check(Race prev, ThreadInfo ti, Instruction insn,
				ElementInfo ei, FieldInfo fi) {
			for (Race r = prev; r != null; r = r.prev) {
				if (r instanceof FieldRace) {
					FieldRace fr = (FieldRace) r;
					if (fr.ei == ei && fr.fi == fi) {
						if (!((FieldInstruction) fr.insn1).isRead()
								|| !((FieldInstruction) insn).isRead()) {
							fr.ti2 = ti;
							fr.insn2 = insn;
							return fr;
						}
	...
	static class ArrayElementRace extends Race {
		...
		static Race check(Race prev, ThreadInfo ti, Instruction insn,
				ElementInfo ei, int idx) {
			for (Race r = prev; r != null; r = r.prev) {
				if (r instanceof ArrayElementRace) {
					ArrayElementRace ar = (ArrayElementRace) r;
					if (ar.ei == ei && ar.idx == idx) {
						if (!((ArrayElementInstruction) ar.insn1).isRead()
								|| !((ArrayElementInstruction) insn).isRead()) {
							ar.ti2 = ti;
							ar.insn2 = insn;
							return ar;
						}
	...
	public PreciseRaceDetector(Config conf) {
		includes = StringSetMatcher.getNonEmpty(conf
				.getStringArray("race.include"));
		excludes = StringSetMatcher.getNonEmpty(conf
				.getStringArray("race.exclude"));
	}
		...
		for (int i = 0; i < threads.length; i++) {
			...
			if (StringSetMatcher.isMatch(mi.getBaseName(), includes, excludes)) {
				if (insn instanceof FieldInstruction) {
					...
					// these insns have been through their top half since they
					// created CGs, but they haven't
					// removed the operands from the stack
					int idx = ainsn.peekIndex(ti);

					candidate = ArrayElementRace.check(candidate, ti, ainsn,
							ei, idx);
				}
			}
	...
	// executed insns and don't rely on any past-exec info, PROVIDED that we
	// only
	...
	public void choiceGeneratorSet(VM vm, ChoiceGenerator<?> newCG) {
		...
		if (newCG instanceof ThreadChoiceFromSet) {
			ThreadInfo[] threads = ((ThreadChoiceFromSet) newCG)
					.getAllThreadChoices();
			checkRace(threads);
		}
	}

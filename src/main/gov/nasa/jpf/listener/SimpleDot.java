...
/**
 * an alternative Graphviz dot-file generator for simple,educational state
 * graphs except of creating funny wallpapers, it doesn't help much in real life
 * if the state count gets > 50, but for the small ones it's actually quite
 * readable. Good for papers.
 * 
 * normal states are labeled with their numeric ids, end states are double
 * circled. start, end and error states are color filled
 * 
 * edges have two labels: the choice value at the beginning and the CG cause at
 * the end. Only the first incoming edge into a state shows the CG cause
 * 
 * we only render one backtrack edge per from-state
 * 
 * <2do> GraphViz doesn't seem to handle color or fontname for head/tail labels
 * correctly
 */
public class SimpleDot extends ListenerAdapter {
	...
	public SimpleDot(Config config, JPF jpf) {
		...
		genericNodeAttrs = config
				.getString("dot.node_attr", GENERIC_NODE_ATTRS);
		genericEdgeAttrs = config
				.getString("dot.edge_attr", GENERIC_EDGE_ATTRS);
		newEdgeAttrs = config.getString("dot.new_edge_attr", NEW_EDGE_ATTRS);
		visitedEdgeAttrs = config.getString("dot.visited_edge_attr",
				VISITED_EDGE_ATTRS);
		startNodeAttrs = config.getString("dot.start_node_attr",
				START_NODE_ATTRS);
		endNodeAttrs = config.getString("dot.end_node_attr", END_NODE_ATTRS);
		errorNodeAttrs = config.getString("dot.error_node_attr",
				ERROR_NODE_ATTRS);
		backtrackEdgeAttrs = config.getString("dot.bt_edge_attr",
				BACKTRACK_EDGE_ATTRS);
		restoreEdgeAttrs = config.getString("dot.restore_edge_attr",
				RESTORED_EDGE_ATTRS);
	...
	void initialize(VM vm) {
		...
		try {
			...
			throw new JPFConfigException(
					"unable to open SimpleDot output file: " + fname);
		}
	...
	public void stateAdvanced(Search search) {
		...
		if (id < 0 || seenEdges.contains(edgeId)) {
			return; // skip the root state and property violations (reported
					// separately)
		}

		if (search.isErrorState()) {
			String eid = "e" + search.getNumberOfErrors();
			printTransition(getStateId(lastId), eid, getLastChoice(),
					getError(search));
			printErrorState(eid);
			lastErrorId = eid;
			...
			if (search.isEndState()) {
				printTransition(getStateId(lastId), getStateId(id),
						getLastChoice(), "return");
				printEndState(getStateId(id));
			} else {
				printTransition(getStateId(lastId), getStateId(id),
						getLastChoice(), getNextCG());
			}
	...
	protected String getNextCG() {
		ChoiceGenerator<?> cg = vm.getNextChoiceGenerator(); // that's the next
																// one
		...
		} else if (insn instanceof FieldInstruction) { // shared object field
														// access
	...
	protected String getFieldAccessCG(FieldInstruction insn) {
		...
		if (insn instanceof InstanceFieldInstruction) {
			...
			if (showTarget) {
				String clsName = ((StaticFieldInstruction) insn)
						.getLastClassName();
	...
	protected String getError(Search search) {
		...
		if (prop instanceof NoUncaughtExceptionsProperty) {
			ExceptionInfo xi = ((NoUncaughtExceptionsProperty) prop)
	...
	protected void printTransition(String fromState, String toState,
			String choiceVal, String cgCause) {
		pw.println();
		pw.print(fromState);
		pw.print(" -> ");
		pw.print(toState);

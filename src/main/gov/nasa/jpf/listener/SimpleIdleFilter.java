...
/**
 * This is the simple version of IdleFilter. This one simply breaks all
 * back-edges encountered to make sure JPF's partial-order reduction doesn't add
 * meaningless transitions forever. This is our dual of the cycle-proviso in
 * classic po-reduction theory.
 * 
 * One can set how many back-edges to consider before breaking, but by default
 * it is 1
 * 
 */
public class SimpleIdleFilter extends ListenerAdapter {
	...
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction nextInsn,
		...
		if (!executedInsn.isBackJump()) { // Put this test first for a
		...
		if ((loopStackDepth != ts.loopStackDepth) || (loopPc != ts.loopStartPc)) {
			...
			if (ts.backJumps > maxBackJumps) {
				ti.reschedule("idleFilter"); // this breaks the executePorStep
												// loop
			}
		}
	}

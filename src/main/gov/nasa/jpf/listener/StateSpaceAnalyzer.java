 ...
 * size, and to give hints of how to reduce it. The interesting part is that
 * this is a listener that doesn't work off traces, but needs to collect info up
 * to a point where we want it to report. That's state space or resource
 * related, i.e. a combination of
 * 
 * - number of transitions - memory consumption - elapsed time
 * 
 * once the limit is reached, we stop the search and report.
 * 
 * There are two parts we are interested in:
 * 
 * - what CGs do we have - what creates those CGs (thread,insn,source) = last
 * step insn
 */
public class StateSpaceAnalyzer extends ListenerAdapter implements
		PublisherExtension {
	...
	private final int m_maxOutputLines; // how many detail lines do we display
										// in the report
	...
	private void initGroupers(Config config) {
		...
		if (config.getStringArray("ssa.sort_order") == null) {
			...
			config.setProperty("ssa.sort_order2",
					"package,class,method,instruction,type");
		}
	...
	private CGGrouper initGrouper(Config config, String parameter,
			Map<String, CGAccessor> accessors) {
		CGGrouper grouper;
		CGAccessor list[];
		StringBuilder name;
		...
		for (i = 0; i < sortOrder.length; i++) {
			...
			if (list[i] == null) {
				config.exception("Unknown sort key: " + sortOrder[i]
						+ ".  Found in parameter: " + parameter);
			}
		}
	...
	public void choiceGeneratorSet(VM vm, ChoiceGenerator<?> newCG) {
		...
		// is actually used (which doesn't necessarily mean it produces a new
		// state,
		// but it got created from a new state)

		// The original code stored each choice generator in an ArrayList. For
		// long
		// running tests, this would grow and cause an OutOfMemoryError. Now,
		// the
	...
	private boolean shouldTerminate(Search search) {
		if ((m_maxStates >= 0)
				&& (search.getVM().getStateCount() >= m_maxStates)) {
			return (true);
		}
		...
		if ((m_maxMemory >= 0)
				&& (Runtime.getRuntime().totalMemory() >= m_maxMemory)) {
			return (true);
		}
	...
	public void publishFinished(Publisher publisher) {
		...
		if (publisher instanceof ConsolePublisher) {
			new PublishConsole((ConsolePublisher) publisher, groupers,
					m_maxOutputLines).publish();
		}
	}

	private enum CGType {

		FieldAccess, ObjectWait, ObjectNotify, SyncEnter, SyncExit, ThreadStart, ThreadTerminate, ThreadSuspend, ThreadResume, SyncCall, SyncReturn, AtomicOp, DataChoice
	}
	...
	private static class CGTypeAccessor implements CGAccessor {
		...
		private static boolean is(InvokeInstruction instruction,
		...
		private static CGType getType(ThreadChoiceGenerator generator,
			...
			if (generator.getThreadInfo().getStackDepth() <= 1) // The main
																// thread has 0
																// frames. Other
																// threads have
																// 1 frame.
		...
		public void sort() {
			...
			comparator = new Comparator<Object>() {
				...
				public int compare(Object value1, Object value2) {
					...
					return (node2.getChoiceCount() - node1.getChoiceCount()); // Sort
																				// descending
				}
			};
	...
	private static class PublishConsole extends Publish {
		...
		PublishConsole(ConsolePublisher publisher, CGGrouper[] groupers,
				int maxOutputLines) {

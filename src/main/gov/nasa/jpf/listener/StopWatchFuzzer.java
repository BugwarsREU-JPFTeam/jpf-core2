 ...
 * System.currentTimeMillis() obtained values, which are attribute tagged upon
 * return from the native method, propagated on LSUB (duration computation), and
 * finally used for LCMP interception if the tag attributes are present
 * 
 * long t1 = System.currentTimeMillis(); doSomeComputation(); long t2 =
 * System.currentTimeMillis(); if (t2 - t1 <= MAX_DURATION){ // all fine branch
 * } else { // catastrophic failure branch }
 * 
 * which boils down to a bytecode pattern like
 * 
 * invokestatic #2; // System.currentTimeMillis() <<< tag result with TimeVal
 * attr .. lsub <<< propagate if any of the operands has a TimeVal attr .. lcmp
 * <<< register CG and skip if any of the operands has TimeVal attr
 * 
 */
public class StopWatchFuzzer extends ListenerAdapter {
	...
	static TimeVal timeValAttr = new TimeVal(); // we don't need separate
												// instances
	...
	public void classLoaded(VM vm, ClassInfo ci) {
		if (miCurrentTimeMillis == null) {
			if (ci.getName().equals("java.lang.System")) {
				miCurrentTimeMillis = ci.getMethod("currentTimeMillis()J",
						false); // its got to be there
			}
		}
	}
	...
	public void executeInstruction(VM vm, ThreadInfo ti,
		...
		if (insnToExecute instanceof LSUB) { // propagate TimeVal attrs
			...
			if (frame.hasOperandAttr(1, TimeVal.class)
					|| frame.hasOperandAttr(3, TimeVal.class)) {
				// enter insn (this pops the 4 top operand slots and pushes the
				// long result
		...
		} else if (insnToExecute instanceof LCMP) { // create and set CG if
													// operand has TimeVal attr

			if (!ti.isFirstStepInsn()) { // this is the first time we see this
											// insn
				StackFrame frame = ti.getTopFrame();
				...
				if (frame.hasOperandAttr(1, TimeVal.class)
					...
					if (vm.setNextChoiceGenerator(cg)) {
						ti.skipInstruction(insnToExecute); // reexecute after
															// breaking the
															// transition
					}
				}

			} else { // it is the beginning of a transition, push the choice and
						// proceed
				IntChoiceFromSet cg = vm.getCurrentChoiceGenerator(CG_ID,

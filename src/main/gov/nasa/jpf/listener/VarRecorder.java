...
public class VarRecorder extends ListenerAdapter {
	...
	public VarRecorder(Config config) {
		includes = StringSetMatcher.getNonEmpty(config
				.getStringArray("var_recorder.include"));
		excludes = StringSetMatcher.getNonEmpty(config
				.getStringArray("var_recorder.exclude"));
	...
	private final void setComment(VM vm, ThreadInfo ti, String name,
			String operator, String value, boolean pending) {
		Step step;
		String comment;
	...
	private final boolean canRecord(VM vm, Instruction inst) {
		...
		if (lastClass != ci) {
			lastClass = ci;
			recordClass = StringSetMatcher.isMatch(ci.getName(), includes,
					excludes);
		}
	...
	// <2do> general purpose listeners should not use anonymous attribute types
	// such as String
	...
	private final static byte getTypeFromInstruction(ArrayInstruction inst) {
		...
		case 'A':
			return (Types.T_REFERENCE);
		case 'B':
			return (Types.T_BYTE); // Could be a boolean but it is better to
									// assume a byte.
		case 'C':
			return (Types.T_CHAR);
		case 'F':
	...
	private final static String decodeValue(byte type, int lo, int hi) {
		...
		case Types.T_BOOLEAN:
			return (String.valueOf(Types.intToBoolean(lo)));
		case Types.T_BYTE:
			return (String.valueOf(lo));
		case Types.T_CHAR:
			return (String.valueOf((char) lo));
		case Types.T_DOUBLE:
			return (String.valueOf(Types.intsToDouble(lo, hi)));
		case Types.T_FLOAT:
			return (String.valueOf(Types.intToFloat(lo)));
		case Types.T_INT:
			return (String.valueOf(lo));
		case Types.T_LONG:
			return (String.valueOf(Types.intsToLong(lo, hi)));
		case Types.T_SHORT:
			return (String.valueOf(lo));

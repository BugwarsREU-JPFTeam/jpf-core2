...
public class VarTracker extends ListenerAdapter {
	...
	public VarTracker(Config config, JPF jpf) {
		...
		includeVars = StringSetMatcher.getNonEmpty(config
				.getStringArray("vt.include"));
		excludeVars = StringSetMatcher.getNonEmpty(config.getStringArray(
				"vt.exclude", new String[] { "java.*", "javax.*" }));

		maxVars = config.getInt("vt.max_vars", 25);

		methodSpec = MethodSpec.createMethodSpec(config.getString("vt.methods",
				"!java.*.*"));
			...
			if (!queue.isEmpty()) {
				for (Iterator<VarChange> it = queue.iterator(); it.hasNext();) {
					...
					if (s == null) {
						...
						// no good - we should filter during reg (think of large
						// vectors or loop indices)
						if (s.lastState != stateId) { // count only once per new
														// state
							s.nChanges++;
							s.lastState = stateId;
						}
	...
	// <2do> - general purpose listeners should not use types such as String for
	// storing
	// attributes, there is no good way to make sure you retrieve your own
	// attributes
	@Override
	public void instructionExecuted(VM vm, ThreadInfo ti, Instruction nextInsn,
		...
		if (((((executedInsn instanceof GETFIELD) || (executedInsn instanceof GETSTATIC))) && ((FieldInstruction) executedInsn)
				.isReferenceField()) || (executedInsn instanceof ALOAD)) {
			// a little extra work - we need to keep track of variable names,
			// because
			// we cannot easily retrieve them in a subsequent xASTORE, which
			// follows
			...
			if (objRef != MJIEnv.NULL) {
				...
				if (ei.isArray()) {
					...
					// <2do> unfortunately, we can't filter here because we
					// don't know yet
					// how the array ref will be used (we would only need the
					// attr for
					// subsequent xASTOREs)
		...
		// here come the changes - note that we can't update the stats right
		// away,
		// because we don't know yet if the state this leads into has already
		// been
		// visited, and we want to detect only var changes that lead to *new*
		// states
		// (objective is to find out why we have new states)
			...
			if (isMethodRelevant(executedInsn.getMethodInfo())
					&& isVarRelevant(varId)) {
	...
	boolean isVarRelevant(String varId) {
		...
		// filter subsequent changes in the same transition (to avoid gazillions
		// of

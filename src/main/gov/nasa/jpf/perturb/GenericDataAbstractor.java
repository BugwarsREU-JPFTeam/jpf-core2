...
/**
 * This file implements a generic data abstraction module that can be used with
 * the Perturbator to execute a method with a choice of values for the method
 * parameters.
 * 
 * This module handles all basic types and selects values for them as follows:
 * int, short: random negative integer | 0 | random positive integer float:
 * random negative floating point | 0 | random positive floating point char: two
 * random values in [0, 255] and an explicit 0 boolean: true | false
 * 
 * An instance of this class is expected to be specialized for each method that
 * we want perturbed. The cstor creates a vector of valuations for the
 * parameters using all choices for each basic type such that each vector can be
 * written directly into the method stack frame for parameters
 * 
 */
...
public class GenericDataAbstractor implements OperandPerturbator {
	...
	public int[] populateChar(MethodInfo mi, String name) {
		int[] iVec = { Math.abs(randomizer.nextInt() % 255), 0,
				Math.abs(randomizer.nextInt() % 255) };
	...
	public int[] populateByte(MethodInfo mi, String name) {
		int[] iVec = { Math.abs(randomizer.nextInt() % 128), 0,
				-1 * Math.abs(randomizer.nextInt() % 127) };
	...
	public int[] populateInt(MethodInfo mi, String name) {
		int[] iVec = { Math.abs(randomizer.nextInt() % 100), 0,
				-1 * Math.abs(randomizer.nextInt() % 100) };
	...
	public int[] populateLong(MethodInfo mi, String name) {
		long[] lVec = { Math.abs(randomizer.nextLong() % 100), 0,
				-1 * Math.abs(randomizer.nextLong() % 100) };
	...
	public void populateValuations(StackFrame frame, int paramIndex,
			int dataIndex) {
	...
	public ChoiceGenerator<?> createChoiceGenerator(String id,
		...
		if (choices > 0) {
			// now create a choices vector which will be used to iterate over
			// the number of
			// parameter valuations we want to use. We set each element of the
			// vector simply
	...
	public boolean perturb(ChoiceGenerator<?> cg, StackFrame frame) {
		assert cg instanceof IntChoiceGenerator : "wrong choice generator type for GenericDataAbstractor: "
				+ cg.getClass().getName();

		int choice = ((IntChoiceGenerator) cg).getNextChoice();
		Valuation valuation = valuations.get(choice);

		// iterate over the number of operands and set the operand array to the
		// values
		...
		for (int j = 0; j < argSize; j++) { // j ranges over actual arguments
			if (!frame.isOperandRef(top - stackIdx)) {
				frame.setOperand(top - stackIdx++,
						valuation.getValuation()[val++], false);
				if (paramTypes[j] == Types.T_LONG
						|| paramTypes[j] == Types.T_DOUBLE) {
					frame.setOperand(top - stackIdx++,
							valuation.getValuation()[val++], false);
				}
			}

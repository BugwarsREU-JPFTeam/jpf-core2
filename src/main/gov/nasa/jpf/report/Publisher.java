 ...
 * also has to work for non-stream based reporting, i.e. within Eclipse (we
 * don't want to re-parse from text reports there)
 */
public abstract class Publisher {
	...
	/**
	 * to be initialized in openChannel NOTE - not all publishers need to have
	 * one
	 */
	...
	public void setItems(int category, String[] newTopics) {
		switch (category) {
		case START:
			startItems = newTopics;
			break;
		case PROBE:
			probeItems = newTopics;
			break;
		case TRANSITION:
			transitionItems = newTopics;
			break;
		case CONSTRAINT:
			constraintItems = newTopics;
			break;
		case PROPERTY_VIOLATION:
			propertyViolationItems = newTopics;
			break;
		case FINISHED:
			finishedItems = newTopics;
			break;
		default:
	...
	protected void setTopicItems(String name) {
		...
		transitionItems = conf.getStringArray(prefix + ".transition",
				transitionItems);
		probeItems = conf.getStringArray(prefix + ".probe", transitionItems);
		propertyViolationItems = conf.getStringArray(prefix
				+ ".property_violation", propertyViolationItems);
		constraintItems = conf.getStringArray(prefix + ".constraint",
				constraintItems);
		finishedItems = conf
				.getStringArray(prefix + ".finished", finishedItems);
	}
	...
	private static final List<PublisherExtension> EMPTY_LIST = new ArrayList<PublisherExtension>(
			0);
	...
	/**
	 * static public String _formatHMS (long t) { t /= 1000; // convert to sec
	 * 
	 * long s = t % 60; long h = t / 3600; long m = (t % 3600) / 60;
	 * 
	 * StringBuilder sb = new StringBuilder(); sb.append(h); sb.append(':'); if
	 * (m < 10){ sb.append('0'); } sb.append(m); sb.append(':'); if (s < 10){
	 * sb.append('0'); } sb.append(s);
	 * 
	 * return sb.toString(); }
	 **/
	...
	// --- if you have different preferences about when to report things,
	// override those

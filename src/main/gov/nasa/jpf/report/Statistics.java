...
/**
 * simple structure to hold statistics info created by Reporters/Publishers this
 * is kind of a second tier SearchListener, which does not explicitly have to be
 * registered
 * 
 * <2do> this should get generic and accessible enough to replace all the other
 * statistics collectors, otherwise there is too much redundancy. If users have
 * special requirements, they should subclass Statistics and set
 * jpf.report.statistics.class accordingly
 * 
 * Note that Statistics might be accessed by a background thread reporting JPF
 * progress, hence we have to synchronize
 */
	...
	public void choiceGeneratorSet(VM vm, ChoiceGenerator<?> newCG) {
		...
		if (cg instanceof ThreadChoiceGenerator) {
			...
			if (insn instanceof FieldInstruction) {
				...
				if (mi != null) {
					...
					if (ci != null) {
						if (ci.isObjectClassInfo()) {
							// its got to be either a wait or a notify since we
							// know the java.lang.Object methods
							signalCGs++;
				...
				breakTransitionCGs++; // e.g. max_transition_length or idleLoop
										// breakers
			}
		} else {
			dataCGs++;

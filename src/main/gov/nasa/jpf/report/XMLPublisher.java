...
public class XMLPublisher extends Publisher {
	...
	protected void publishJPFConfig() {
		...
		for (Map.Entry<Object, Object> e : eSet) {
			out.println("    <entry key=\"" + e.getKey() + "\" value=\""
					+ e.getValue() + "\"/>");
		}
		out.println("  </jpf-properties>");
	...
	protected void publishSuT() {
		...
		if (mainCls != null) {
			...
			if (mainPath != null) {
				...
				RepositoryEntry rep = RepositoryEntry
						.getRepositoryEntry(mainPath);
				if (rep != null) {
	...
	protected void publishTrace() {
		...
		for (Transition t : path) {
			...
			out.println("    <transition id=\"" + i++ + "\" thread=\""
					+ t.getThreadIndex() + "\">");
			out.println("      <cg class=\"" + cg.getClass().getName()
					+ "\" choice=\"" + cg.getProcessedNumberOfChoices()
					+ "\"/>");
			for (Step s : t) {
				out.print("      <insn src=\"" + s.getLocationString() + "\">");
				String insn = s.getInstruction().toString();
				if (insn.indexOf('<') >= 0) { // <init> and <clinit> clash with
												// XML
	...
	protected void publishSnapshot() {
		...
		for (ThreadInfo ti : vm.getLiveThreads()) {
			out.println("    <thread id=\"" + ti.getId() + "\" name=\""
					+ ti.getName() + "\" status=\"" + ti.getStateName() + "\">");
			// owned locks
			...
			for (StackFrame frame : ti) {
				if (!frame.isDirectCallFrame()) {
					out.println("      <frame>" + frame.getStackTraceInfo()
							+ "</frame>");
				}
			}
			out.println("    </thread>");

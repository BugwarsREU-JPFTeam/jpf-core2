...
public abstract class Search {
	 ...
	 * request a probe
	 * 
	 * This does not do the actual listener notification, it only stores the
	 * request, which is then processed from within JPFs inner execution loop.
	 * As a consequence, probeSearch() can be called async, and searchProbed()
	 * listeners don't have to bother with synchronization or inconsistent JPF
	 * states (notification happens from within JPFs main thread after a
	...
	public void error(Property property, Path path, ThreadList threadList) {
		...
		if (getAllErrors) {
			...
			try {
				...
				throw new JPFException("failed to clone error information: "
						+ cnsx);
			}
			done = false;
	...
	public void notifySearchProbed() {
		try {
			...
			throw new JPFListenerException(
					"exception during searchProbed() notification", t);
		}
	}

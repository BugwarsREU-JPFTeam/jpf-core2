...
/**
 * interface to register for notification by the Search object. Observer role in
 * same-name pattern
 */
public interface SearchListener extends JPFListener {

	/**
	 * got the next state Note - this will be notified before any potential
	 * propertyViolated, in which case the currentError will be already set
	 */
	...
	/**
	 * a previously generated state was restored (can be on a completely
	 * different path)
	 */
	void stateRestored(Search search);

	/**
	 * there was a probe request, e.g. from a periodical timer note this is
	 * called synchronously from within the JPF execution loop (after
	 * instruction execution)
	 */
	void searchProbed(Search search);

	/**
	 * JPF encountered a property violation. Note - this is always preceeded by
	 * a stateAdvanced
	 */
	...
	/**
	 * there was some contraint hit in the search, we back out could have been
	 * turned into a property, but usually is an attribute of the search, not
	 * the application
	 */

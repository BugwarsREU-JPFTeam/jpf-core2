 ...
 * aware of that each test (TestJPF.verifyX() invocation) uses its own Config
 * and JPF object, i.e. can have different path settings
 * 
...
public class RunTest extends Run {
	...
	public static void showUsage() {
		System.out
				.println("Usage: \"java [<vm-option>..] -jar ...RunTest.jar [<jpf-option>..] [<class> [<app-arg>..]]");
		System.out
		...
		System.out
				.println("               | -quiet : don't show System.out test output");
		System.out
				.println("               | +<key>=<value>  : add or override <key>/<value> pair to global config");
		System.out
				.println("               | +test.<key>=<value>  : add or override <key>/<value> pair in test config");
		System.out.println("  <class>      : application class name");
		System.out
				.println("  <methods>    : test methods of application class");
	}
	...
	public static void main(String[] args) {
		...
		JPFClassLoader cl = config.initClassLoader(RunTest.class
				.getClassLoader());
		...
		try {
			...
			error("class not found " + cnfx.getMessage()
					+ ", check native_classpath in jpf.properties");
			return;
		...
		List<Class<?>> testClasses = getTestClasses(cl, testJpfCls,
				testPathElements, testClsName);
			...
			try {
				try {
					try { // check if there is a main(String[]) method
						Method mainEntry = testCls.getDeclaredMethod("main",
								String[].class);
						mainEntry.invoke(null, (Object) testArgs);
														...
														// directly
						Method mainEntry = testJpfCls.getDeclaredMethod(
								"runTests", Class.class, String[].class);
						mainEntry.invoke(null,
								new Object[] { testCls, testArgs });
					}

					nPass++;

				} catch (NoSuchMethodException x) {
					error("no suitable main() or runTests() in "
							+ testCls.getName());
	...
	static Class<?> loadTestClass(JPFClassLoader cl, Class<?> testJpfCls,
			String testClsName) {
		try {
			...
			error("class not found " + cnfx.getMessage()
					+ ", check test_classpath in jpf.properties");
			return null;
		}
	}
	...
	static List<Class<?>> getTestClasses(JPFClassLoader cl,
			Class<?> testJpfCls, String[] testPathElements, String testClsName) {
		List<Class<?>> testClasses = new ArrayList<Class<?>>();

		if (!hasWildcard(testClsName)) { // that's simple, no need to look into
											// dirs
			Class<?> testCls = loadTestClass(cl, testJpfCls, testClsName);
			if (testCls == null) { // error if this was an explicit classname
				error("specified class name not found or no TestJPF derived class: "
						+ testClsName);
			}
			testClasses.add(testCls);

		} else { // we have to recursively look into the testPathElements for
					// potential test classes
			List<String> classFileList = getClassFileList(testPathElements,
					testClsName);
	...
	static void collectMatchingFiles(int nPrefix, File dir, List<String> list,
			String pattern) {
		for (File e : dir.listFiles()) {
			if (e.isDirectory()) {
				...
				if (pn.matches(pattern)) {
					String clsName = pn.substring(0, pn.length() - 6); // strip
																		// cp
																		// entry
																		// and
																		// ".class"
	...
	static List<String> getClassFileList(String[] testPathElements,
			String testClsPattern) {

...
public class AvailableBufferedInputStream extends InputStream {
	...
	static final int DEFAULT_BUFFER_SIZE = 16 * 1024; // Not private for testing
														// purposes
	...
	public int peek() throws IOException // Returns -1 if there is nothing to
											// read.
	{
		if (m_read >= m_end) {
			fill();
	...
	public void unread(int data) throws IOException {
		if (m_read <= 0) {
			...
			System.arraycopy(m_buffer, m_read, m_buffer, m_buffer.length
					- m_end, m_end);
	...
	private void fill() throws IOException {
		...
		if (m_end <= 0) // m_input.read(m_buffer, 0, 0) can be expensive. Don't
						// waste time.
			return;

		m_end = Math.min(m_end, m_buffer.length);
		m_end = m_input.read(m_buffer, 0, m_end);
		m_end = Math.max(m_end, 0); // Defend against a bug where
									// m_input.available() returning > 0 and
									// m_input.read() returning -1
	}

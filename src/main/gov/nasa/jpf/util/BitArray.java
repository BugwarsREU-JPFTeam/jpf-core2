...
public class BitArray {
	...
	public void fromBitSet(BitSet in) {
		...
		for (i = 0; i < max; i++) {
			int j = i << 3;
			data[i] = (byte) ((in.get(j + 0) ? 1 : 0) | (in.get(j + 1) ? 2 : 0)
					| (in.get(j + 2) ? 4 : 0) | (in.get(j + 3) ? 8 : 0)
					| (in.get(j + 4) ? 16 : 0) | (in.get(j + 5) ? 32 : 0)
					| (in.get(j + 6) ? 64 : 0) | (in.get(j + 7) ? 128 : 0));
		}
	...
	public final void set(int idx, boolean val) {
		if (idx >= length)
			throw new ArrayIndexOutOfBoundsException("" + idx + " >= " + length);
		if (val) {
			data[idx >> 3] |= (1 << (idx & 7));
		} else {
			data[idx >> 3] &= ~(1 << (idx & 7));

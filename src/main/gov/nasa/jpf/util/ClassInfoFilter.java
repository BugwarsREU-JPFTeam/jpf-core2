 ...
 * 
 * Filtering is based on include/exclude name patterns (e.g. for packages)
 * and/or on inheritance (both down- and upwards)
 */
public class ClassInfoFilter {

	// filter using an explicit set of class names (can be used for one-pass
	// load)
	protected StringSetMatcher includes; // included classes that should use
											// them
	protected StringSetMatcher excludes; // excluded classes (that should NOT
											// use them)
	...
	public boolean isPassing(ClassInfo ci) {
		if (ci == null) {
			...
			// better to instrument by default (until we have a better
			// mechanism)
			return true;

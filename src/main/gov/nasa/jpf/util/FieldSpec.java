...
/**
 * utility class that can match FieldInfos against specs. if the class or name
 * part are omitted, "*" is assumed a preceeding '!' means the match is inverted
 * 
 * spec examples "x.y.Foo.bar : field 'bar' in class 'x.y.Foo' "x.y.Foo+.bar :
 * all 'bar' fields in 'x.y.Foo' and all its supertypes "x.y.Foo.*" : all fields
 * of x.y.Foo "*.myData" : all fields names 'myData' "!x.y.*" : all fields of
 * types outside types in package x.y
 */

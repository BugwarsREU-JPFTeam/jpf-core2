...
/**
 * A hash map that holds int values associated with generic key objects. This is
 * a straight forward linked list hashmap.
 * 
...
public final class IntTable<E> implements Iterable<IntTable.Entry<E>>,
	...
	/**
	 * encapsulates an Entry in the table. changes to val will be reflected in
	 * the table.
	 */
			...
			for (int i = 0; i < tbl.length && j < nEntries; i++) {
				...
				if (e != null) {
					if (e.next == null) { // just one entry under this head
						...
						// we have to store in reverse order so that restore
						// preserves it
						// we do the revert here because storing happens once,
						// whereas restore can happen many times
						int n = 1;
	...
	Snapshot<E> lastSnapshot; // cache for the last snapshot (nulled once the
								// IntTable is changed)
	...
	// this is a deep copy (needs to be because entries are reused when growing
	// the table)
	@Override
	...
	/**
	 * gets the Entry associated with key, adding previous `size' if not yet
	 * bound.
	 */
	...
	/**
	 * returns an iterator over the entries. unpredictable behavior could result
	 * if using iterator after table is altered.
	 */
	@Override

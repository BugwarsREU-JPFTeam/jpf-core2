...
public final class IntVector implements Comparable<IntVector>, Cloneable {
	...
	public static void copy(IntVector src, int srcPos, IntVector dst,
			int dstPos, int len) {
		if (len == 0)
			return;
	...
	public static void copy(int[] src, int srcPos, IntVector dst, int dstPos,
			int len) {
		if (len == 0)
			return;
		dst.ensureSize(dstPos + len);
	...
	public static void copy(IntVector src, int srcPos, int[] dst, int dstPos,
			int len) {
		if (len == 0)
			return;
		src.ensureCapacity(srcPos + len);
	...
	public int compareTo(IntVector that) {
		if (that == null)
			return this.size; // consider null and 0-length the same

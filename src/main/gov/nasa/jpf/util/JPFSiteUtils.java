...
/**
 * utility class for JPF site configuration related functions. This is partially
 * redundant to Config since it is used during bootstrapping, and
 * gov.nasa.jpf.Config might not be in the classpath yet. It mostly differs in
 * terms of key/value expansion, which is only partially supported here.
 * 
 * NOTE - this class is not allowed to use any JPF specific types
 */
public class JPFSiteUtils {

	// --- preparse support - we need this if we use app properties to locat
	// lower level property files

	static Pattern keyValPattern = Pattern
			.compile("^[ \t]*([^# \t][^ \t]*)[ \t]*=[ \t]*(.+?)[ \t]*$");
	...
	/**
	 * this returns the contents of a config source in-order, without expanding
	 * values or keys
	 */
	public static List<Pair<String, String>> getRawEntries(Reader reader)
			throws IOException {
	...
	public static File getCoreDir(File siteProps) {
		if (siteProps != null) {
			String path = getMatchFromFile(siteProps.getAbsolutePath(),
					"jpf-core");
			if (path != null) {
	...
	static Pattern idPattern = Pattern
			.compile("^[ \t]*([^# \t][^ \t]*)[ \t]*=[ \t]*\\$\\{config_path\\}");
	...
	public static boolean isFreeArg(String a) {
		return ((a != null) && (a.length() > 0) && a.charAt(0) != '+' && a
				.charAt(0) != '-');
	}
	...
	public static File getSiteProperties(String[] args) {
		...
		// --- 2. check if the first free arg is an application property file
		// (*.jpf), and it contains a 'site=..' setting
		for (int i = 0; i < args.length; i++) {
			String a = args[i];
			if (isFreeArg(a)) {
				if (a.matches("[^+-].*\\.jpf")) {
	...
	/**
	 * locate the site.properties. Start with the current dir, go upwards until
	 * the user.home is reached. If site.properties isn't found there, look for
	 * '.jpf' and 'jpf' dirs within the home dir. If no site.properties is found
	 * there either, give up
	 */
	...
	public static String getGlobalSitePropertiesPath() {
		String userHome = System.getProperty("user.home");
		String globalPath = userHome + File.separator + ".jpf" + File.separator
				+ "site.properties";
		return globalPath;
	}
	...
	/**
	 * this returns a list of all the project ids in the 'extensions' entries
	 * (also handles accumulated 'extensions+=.." entries
	 */
	...
	public static boolean addProject(File siteProps, String projectId,
			File projectDir, boolean isExt) {
		List<Pair<String, String>> entries = getRawEntries(siteProps);
		List<String> extensions = getExtensions(entries);

		if ("jpf-core".equals(projectId)) { // jpf-core always has to be in the
											// extensions list
			isExt = true;
		}
		...
		try {
			...
			String projectPath = FileUtils.asCanonicalUserPathName(projectDir
					.getAbsolutePath());
			...
			for (Pair<String, String> e : entries) {
				if (!"extensions".equals(e._1)) {
					...
					if (projectId.equals(e._1)) {
						alreadyThere = true;
						// Hmm, not sure its best to use absolute pathnames here
						// (e.g. when doing local site installs)

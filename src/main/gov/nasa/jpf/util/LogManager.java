...
/**
 * this class is responsible for returning properly JPF-configured Loggers. It
 * is not supposed to be used directly by clients, but rather is a JPF
 * delegatee.
 * 
 * While we could modify/replace the standard java.util.logging facility at
 * various levels (own LogManager, own initialization class etc.), we choose the
 * approach to piggyback on it, because these mechanisms either require changing
 * system properties, rely on only partly documented features, or don't give us
 * the full functionality we need. By having our own log encapsulator, we could
 * also replace the underlying mechanism if we really want to
 */
public class LogManager {

	static HashMap<String, JPFLogger> loggers = new HashMap<String, JPFLogger>(); // our
																					// own
																					// set
	...
	/**
	 * note - this is not allowed to fail, since we couldn't log that. Hardcoded
	 * default values have to do in this case (make sure we catch the proper
	 * Config exceptions)
	 */
	public static void init(Config conf) {
		try {
			defaultLevel = Level.parse(conf.getString("log.level", "INFO")
					.toUpperCase());
		} catch (Throwable x) {
	...
	static Level getLevel(String name) {
		if (checkInclusion(activeSevere, name))
			return Level.SEVERE;
		if (checkInclusion(activeWarning, name))
			return Level.WARNING;
		if (checkInclusion(activeInfo, name))
			return Level.INFO;
		if (checkInclusion(activeConfig, name))
			return Level.CONFIG;
		if (checkInclusion(activeFine, name))
			return Level.FINE;
		if (checkInclusion(activeFiner, name))
			return Level.FINER;
		if (checkInclusion(activeFinest, name))
			return Level.FINEST;
	...
	public static JPFLogger getLogger(String name) {
		if (handler == null) {
			// <2do> this is only a band aid for missing LogManager.init(conf)
			// calls
		...
		if (logger == null) {
			// we haven't had this one yet - create and init a new one from the
			// host logging system
			Logger baseLogger = Logger.getLogger(name);
			baseLogger.setLevel(getLevel(name));
			baseLogger.addHandler(handler);
			baseLogger.setUseParentHandlers(false); // we don't want to pass
													// this up

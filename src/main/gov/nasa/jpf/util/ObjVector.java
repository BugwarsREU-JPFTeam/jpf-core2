 ...
 * supports dynamic growth on set() operations. While it supports list functions
 * such as append, ObjVector resembles mostly an array, i.e. is meant to be a
 * random-access collection
 * 
 * this collection does not keep a count of non-null elements, but does maintain
 * the highest set index as its size through set/add and remove operations. Note
 * that size only shrinks through remove operations, not by setting null values.
 * This means there is no guarantee that data[size-1] is not null. The converse
 * however is true - there is no non-null element at an index >= size.
 * 
 * @author pcd
 */
public class ObjVector<E> implements ReadOnlyObjList<E>, Cloneable {
	...
	/**
	 * remove all non-null elements between 'fromIdx' (inclusive) and 'toIdx'
	 * (exclusive) throw IndexOutOfBoundsException if index values are out of
	 * range
	 */
	...
	/**
	 * this is a block operation snapshot that stores chunks of original data
	 * with not more than DEFAULT_MAX_GAP consecutive null elements. Use this if
	...
	/**
	 * create a snapshot that doesn't store more than maxGap consecutive null
	 * values
	 */
	...
	/**
	 * snapshot that can mutate element values, but therefore can't use block
	 * operations. This is slower to store/restore, but can be more memory
	 * efficient if the elements are fragmented (lots of small holes in data)
	 */
	...
	public <T> void restore(MutatingSnapshot<E, T> snap,
	...
	/**
	 * iterator that goes over all elements regardless of value (i.e. also

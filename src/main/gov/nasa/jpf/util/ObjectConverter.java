...
public class ObjectConverter {
				...
				if (!fi.isReference()) {
					...
					try {
						...
						if (isArrayField(fi)) {
							...
							fieldJPFObjRef = JPFObjectFromJavaObject(env,
									fieldJavaObj);
						}
						...
						throw new JPFException(
								"JPF object creation failed, no such field: "
										+ fi.getFullName(), nsfx);
					} catch (IllegalAccessException iax) {
						throw new JPFException(
								"JPF object creation failed, illegal access: "
										+ fi.getFullName(), iax);
					}
				}
			}
	...
	private static void setJPFPrimitive(ElementInfo newObjEI, FieldInfo fi,
			Object javaObject) {
		try {
	...
	private static Field getField(String fieldName, Class javaClass)
	...
	private static int getJPFArrayRef(MJIEnv env, Object javaArr)
		...
		if (arrayElementClass == Character.TYPE) {
			...
			arrRef = env.newObjectArray(arrayElementClass.getCanonicalName(),
					javaArrLength);
	...
	private static void setJavaPrimitive(Object javaObject, ElementInfo ei,
			FieldInfo fi) throws NoSuchFieldException, IllegalAccessException {

 ...
 * 
 * Object attr; // either a single value or a list .. attr =
 * ObjectList.remove(attr, v);
 * 
 ...
 * user (and therefore could be just overwritten by simple assignments) YOU
 * SHOULD NOT DO THIS! Other extensions or JPF itself could rely on current
 * attributes. In case you have to change the whole list, use
 * set(oldAttrs,newAttr), which checks if there currently is a SystemAttribute
 * instance in the list, in which case it throws a JPFException unless the new
 * attibute value is also a gov.nasa.jpf.SystemAttribute instance. Use
 * forceSet(null) if you really have to remove lists with SystemAttributes
 * 
 * 
 * usage: Object attr; ... attr = AttrContainer.add( newAttr, attr);
 * 
 * MyAttr a = AttrContainer.getNext( MyAttr.class, attr);
 * 
 * attr = AttrContainer.remove( a, attr);
 * 
 * for (MyAttr a = ObjectList.getFirst(MyAttr.class, attr); a != null; a =
 * ObjectList.getNext(MyAttr.class, attr, a)) {..}
 * 
 */
public abstract class ObjectList {
	...
	public static class Iterator implements java.util.Iterator<Object>,
		...
		@Override
		public void remove() {
			// we can't remove since that would have to change the head field in
			// the client
	...
	static final TypedIterator<Object> emptyTypedIterator = new TypedIterator<Object>(
			null, Object.class);
	...
	/**
	 * this returns either the first value if there is only one element, or a
	 * Node list containing all the values in the order they are provided
	 * 
				...
				for (Node n = (Node) head; n != null; n = n.next) {
					if (n.data instanceof SystemAttribute) {
						throw new JPFException(
								"attempt to overwrite SystemAttribute with "
										+ newHead);
					}
				}
	...
	static Object cloneData(Object o) throws CloneNotSupportedException {
		if (o instanceof CloneableObject) {
			...
			try {
				...
				// since Class.getMethod() only returns publics, we don't have
				// to set accessible
				return m.invoke(o);

			} catch (NoSuchMethodException nsmx) {
				// since Object.clone() would throw it (unless this is a
				// Cloneable, in which
				// case there most probably is a public clone() and we would not
				// have
				// gotten here), there is no use trying to call it
				throw new CloneNotSupportedException("no public clone(): " + o);
			} catch (InvocationTargetException ix) {
				throw new RuntimeException("generic clone failed: " + o,
						ix.getCause());

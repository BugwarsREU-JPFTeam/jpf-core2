...
/**
 * utility to perform pathname expansion the following patterns are supported so
 * far:
 * 
 * (1) brace expansion ala bash: foo{Boo,Shoo} => fooBoo, fooShoo (this doesn't
 * check for existence, its simply lexical)
 * 
...
public class PathnameExpander {
	...
	private int initializeMatchList(ArrayList<File> list, String path) {
		if (path.isEmpty()) { // absolute pathname (ignoring drive letters for
								// now)
	...
	private void collectMatchingNames(ArrayList<File> list,
			ArrayList<File> newList, Pattern pattern) {
		for (File dir : list) {
			if (dir.isDirectory()) {
	...
	private void collectExistingFile(ArrayList<File> list,
			ArrayList<File> newList, String fname) {
		for (File dir : list) {
			if (dir.isDirectory()) {

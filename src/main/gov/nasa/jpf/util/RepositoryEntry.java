...
public class RepositoryEntry {
	...
	static RepositoryEntryFactory searchers[] = {
			new SvnRepositoryEntryFactory(), new HgRepositoryEntryFactory(),
	...
	public RepositoryEntry(String fileName, String repositoryType,
...
class SvnRepositoryEntryFactory implements RepositoryEntryFactory {
	...
	/*
	 * <2do> doesn't work on Windows, where the .svn/entries is apparently not
	...
	public RepositoryEntry getRepositoryEntry(String fullFileName) {
		...
		File fEntries = new File(dName + File.separatorChar + ".svn"
				+ File.separatorChar + "entries");
		if (fEntries.exists()) {
			String repository = "?";
			String revision = "?";
					...
					if (mRep.matches()) {
						...
						if (mRev.matches()) {
							...
							if (mName.matches() && mName.group(1).equals(fname)) {
								// Ok, got everything
								return new RepositoryEntry(fname, "svn",
										repository, revision);
							}
						}
					}
	...
	public RepositoryEntry getRepositoryEntry(String fullFileName) {
		...
		if (currentDir != null) {
			try {
				...
				return new RepositoryEntry(fullFileName, "hg", repoURL,
						revision);
			} catch (Exception ex) {
				return null;
			}
	...
	public RepositoryEntry getRepositoryEntry(String fullFileName) {
		...
		if (currentDir != null) {
			try {
				...
				// git doesn't has revision numbers so we will read last
				// revision's hash instead
				File gitHeadHash = new File(currentDir,
						".git/refs/heads/master");
				r = new BufferedReader(new FileReader(gitHeadHash));
				String revision = r.readLine();

				return new RepositoryEntry(fullFileName, "git", repoURL,
						revision);

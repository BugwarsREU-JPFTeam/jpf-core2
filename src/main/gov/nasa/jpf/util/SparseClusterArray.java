...
/**
 * A generic sparse reference array that assumes clusters, and more frequent
 ...
 * 
 * The result should find a compromise between the fast element access &
 * iteration of a simple, dense array, and the efficient memory storage of a
 * HashMap (if only it could avoid box objects).
 * 
...
public class SparseClusterArray<E> implements Iterable<E> {
	...
	public static final int N_ELEM = 1 << CHUNK_BITS; // 8 bits chunk index ->
														// 24 bits segment key
														// (3x8bits / 256 segs)
	protected static final int ELEM_MASK = 0xff;
	protected static final int BM_ENTRIES = N_ELEM / 64; // number of bitmap
															// long entries
	...
	public static final int MAX_CLUSTERS = CHUNK_SIZE; // max int with
														// CHUNK_BITS bits (8)
	public static final int MAX_CLUSTER_ENTRIES = 0xffffff; // max int with
															// 32-CHUNK_BITS
															// bits (24) =
															// 16,777,215
															// elements
	...
	protected static class Node {
		...
		// int minNextFree; // where to start looking for free elements, also
		// used to determine if Node is full
	}

	protected static class ChunkNode {
		public Chunk[] seg = new Chunk[N_SEG];
		// int minNextFree; // where to start looking for free elements, also
		// used to determine if ChunkNode is full
	}

	protected static class Chunk implements Cloneable { // with some extra info
														// to optimize in-chunk
														// access
		public int base, top;
		public Chunk next;
		public Object[] elements; // it's actually E[], but of course we can't
									// create arrays of a generic type
		public long[] bitmap;

		// int minNextFree; // where to start looking for free elements, also
		// used to determine if Chunk is full
		...
		public <E> Chunk deepCopy(Cloner<E> cloner)
				throws CloneNotSupportedException {
			Chunk nc = (Chunk) super.clone();

			E[] elem = (E[]) elements; // bad, but we have to cope with type
										// erasure
		...
		public T next() {
			...
			while (c != null) {
				...
				if (i >= 0) {
					...
					if (ret == null) {
						// try to recover from a concurrent modification, maybe
						// there is one left
						ret = c.elements[i];
						continue;
					} else {
	...
	/**
	 * be careful, this should only be used to get old stored elements during a
	 * Snapshot restore
	 */
	...
	public E get(int i) {
		...
		if (l3 != null && (l3.base == (i & CHUNK_BASEMASK))) { // cache
																// optimization
																// for
																// in-cluster
																// access
	...
	public void set(int i, E e) {
		...
		if (e != null) {
			...
			if (isSet) {
				...
				bm[u] &= ~(1L << v);
				nSet--;
				// <2do> discard upwards if chunk is empty ? (maybe as an
				// option)
			}
		}
	}
	...
	/**
	 * deep copy we need to do this depth first, right-to-left, to maintain the
	 * Chunk list ordering. We also compact during cloning, i.e. remove empty
	 * chunks and ChunkNodes/Nodes w/o descendants
	 */
						...
						if (cn != null) {
							...
							for (int k = 0, k1 = 0; k < cList.length; k++) {
								...
								if (c != null && !c.isEmpty()) {
									...
									// create the required ChunkNode/Node
									// instances
									if (newNode == null) {
										newNode = new Node();
										j1 = k1 = 0;
	...
	/**
	 * create a snapshot that can be used to restore a certain state of our
	 * array This is more suitable than cloning in case the array is very
	 * sparse, or the elements contain a lot of transient data we don't want to
	 * store
	 */
	...
	protected <T> void populateSnapshot(Snapshot<E, T> snap,

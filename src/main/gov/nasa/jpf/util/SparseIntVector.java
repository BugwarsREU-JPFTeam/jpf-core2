...
public class SparseIntVector implements Cloneable {
	...
	/**
	 * a simplistic snapshot implementation that stores set indices/values in
	 * order to save space
	 */
	...
	/**
	 * Creates a SimplePool that holds about 0.7 * 2**pow elements before first
	 * rehash.
	 */
	...
	public void set(int idx, int val) {
		...
		if ((count + 1) >= nextWipe) { // too full
			...
			/**
			 * // determine if size needs to be increased or just wipe null
			 * blocks int oldCount = count; count = 0; for (int i = 0; i <
			 * idxTable.length; i++) { //if (idxTable[i] != MIN_VALUE &&
			 * valTable[i] != defaultValue) { if (idxTable[i] != MIN_VALUE) {
			 * count++; } } if (count >= nextRehash) { pow++; // needs to be
			 * increased in size if (DEBUG) {
			 * System.out.println("Rehash to capacity: 2**" + pow); } } else {
			 * if (DEBUG) {
			 * System.out.println("Rehash reclaiming this many nulls: " +
			 * (oldCount - count)); } }
			 **/
			...
			for (int i = 0; i < oldLen; i++) {
				...
				while (idxTable[pos] != MIN_VALUE) { // we know enough slots
														// exist
					pos = (pos + delta) & mask;
				}
				idxTable[pos] = tidx;

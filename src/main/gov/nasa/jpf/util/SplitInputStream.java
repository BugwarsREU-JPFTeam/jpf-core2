...
public class SplitInputStream {
	static final int INITIAL_BUFFER_SIZE = 1024; // Recommended to be a power of
													// 2. If not, it will be
	...
	private long m_write; // Must hold m_dataGate lock to read. Must hold
							// m_dataGate and m_sourceGate to write.
	private int m_available; // Must hold m_dataGate lock to read. Must hold
								// m_dataGate and m_sourceGate to write.
	private int m_openStreams; // Must hold m_dataGate lock to access.
	private byte m_buffer[]; // Must hold m_dataGate lock to read. Must hold
								// m_sourceGate lock to write. The written data
								// doesn't become available until m_write is
								// updated.
	...
	public SplitInputStream(InputStream source, int streamCount, int initialSize) {
		...
		if (streamCount <= 0)
			throw new IllegalArgumentException("streamCount <= 0 : "
					+ streamCount);
	...
	private int read(int index, byte buffer[], int offset, int length)
			throws IOException {
		long position;
		int off;
	...
	private boolean fill(int index) throws IOException {
		...
		try {
			...
			write = m_write; // Capture the data in local variables so the
								// calculations can take place outside
								// m_dataLock.
			length = m_buffer.length;
			m_available = m_source.available();

			m_dataLock.unlock(); // Don't hold m_dataLock while blocked reading.
									// That way other Streams with data left to
									// read can do so.

			offsetWrite = getBufferOffset(write);
			offsetPosition = getBufferOffset(minPosition); // If the minPosition
															// advances while
															// not holding the
															// lock, no big
															// deal. It simply
			...
			} while (length == 0); // Guarantee that at least 1 byte is read or
			...
			if (!m_dataLock.isHeldByCurrentThread()) // Restore the lock state
														// when the method was
														// called.
				m_dataLock.lock();
		}
	...
	private boolean doLock(int index) {
		...
		/*
		 * m_sourceLock must be acquired before m_dataLock. Otherwise, there
		 * will be a deadlock. But, if we can tryLock() m_sourceLock while
		 * holding m_dataLock, this will save CPU time.
		 */
		...
		return (position == m_write); // Does the thread still need to read
										// data?
	}
	...
	private void expand() {
		...
		// Since we are doubling the length of the array, we simply have to
		// duplicate the contents.
		// This allows us to avoid figuring out which part of m_buffer is
		// actually holding data and dealing with wrapping.
	...
	private int available(int index) throws IOException {
		...
		sourceLock = m_sourceLock.tryLock(); // By putting this after locking
												// m_dataLock, the only way
												// tryLock() will fail is if a

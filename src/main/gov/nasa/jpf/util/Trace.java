 ...
 * 
 * we could register this as a listener itself, but since it usually is used
 * from a listener, we might as well just delegate from there
 */
public class Trace<T> extends ListenerAdapter implements Iterable<T> {
	...
	class TraceIterator implements Iterator<T> {
		...
		public void remove() {
			throw new UnsupportedOperationException(
					"TraceElement removal not supported");
		}
	}
	...
	public List<T> getOps() {
		// this is a rather braindead way around the limitation that we can't
		// explicitly

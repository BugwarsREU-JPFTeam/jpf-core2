...
/**
 * This is a simplified hash pool that does not support removal or numbering of
 * elements.
 */
public class WeakPool<E> {
	...
	/**
	 * Returns a pooled element matching e, which will be e if no match has been
	 * previously pooled.
	 */
	public E pool(E e) {
		if (e == null)
		...
		if (count >= nextWipe) { // too full
			// determine if size needs to be increased or just wipe unused weak
			// refs
			int oldCount = count;
			count = 0;
			...
			if (DEBUG && oldCount > count) {
				System.out.println("Weak references collected: "
						+ (oldCount - count));
			}
			if (count >= nextRehash) {
			...
			idx = code & mask;
			delta = (code >> (pow - 1)) | 1; // must be odd!
			while (table[idx] != null) { // we know enough slots exist & new
											// element
				idx = (idx + delta) & mask;
			}
		} else {

...
public class JSONLexer {
	...
	private void back() {
		if (backtracked) {
			throw new JPFException(
					"Tried to return twice. Posibly an error. Please report");
		}
		backtracked = true;
	}
	...
	private void error(String string) {
		throw new JPFException(string + " '" + (char) currentChar
				+ "' charCode = " + currentChar + "; in line " + lineNumber
				+ " pos " + symbolNumberInLine);
	}

	private boolean hexadecimalChar(int i) {
		return Character.isDigit(i) || (i <= 'F' && i >= 'A')
				|| (i <= 'f' && i >= 'a');
	}

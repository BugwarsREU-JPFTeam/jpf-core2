...
public class JSONObject {
	...
	private static final JPFLogger logger = JPF
			.getLogger("gov.nasa.jpf.util.json.JSONObject");
	...
	void addValue(String key, Value value) {
		if (keyValues.containsKey(key)) {
			throw new JPFException(
					"Attempt to add two nodes with the same key in JSON object");
		}
	...
	public void addCGCall(String key, CGCall cgCall) {
		if (cgCalls.containsKey(key)) {
			throw new JPFException(
					"Attempt to add two CG with the same key in JSON object");
		}
	 ...
	 * 
	 * NOTE - this currently does not support concrete field types that are
	 * subtypes of the respective field types
	 */
	...
	// NOTE - (pcm) before calling this method you have to make sure all
	// required
	// types are initialized

	public int fillObject(MJIEnv env, ClassInfo ci, ChoiceGenerator<?>[] cgs,
			String prefix) throws ClinitRequired {
			...
			for (FieldInfo fi : fields) {
				...
				if (val != null) {
					...
					if (!fi.isReference()) {
						...
						int newFieldRef = ObjectConverter
								.JPFObjectFromJavaObject(env, cgResult);
						ei.setReferenceField(fi, newFieldRef);
					}
				} else {
					logger.warning("Value for field ", fi.getFullName(),
							" isn't specified");
				}
			}
	...
	private void fillFromValue(FieldInfo fi, ElementInfo ei, Value val,
			MJIEnv env, ChoiceGenerator<?>[] cgs, String prefix) {
		String fieldName = fi.getName();
		...
		if (!fi.isReference()) {
			...
			if (isArrayType(fi.getType())) {
				int newArrRef = createArray(env, fi.getTypeClassInfo(), val,
						cgs, prefix + fieldName);
				...
				if (creator != null) {
					...
					if (jsonObj != null) {
						fieldRef = jsonObj.fillObject(env, ciField, cgs, prefix
								+ fieldName);
					}
	...
	public int createArray(MJIEnv env, ClassInfo ciArray, Value value,
			ChoiceGenerator<?>[] cgs, String prefix) {
			...
			for (int i = 0; i < vals.length; i++) {
				...
				if (creator != null) {
					...
					if (isArrayType(arrayElementType)) {
						newObjRef = createArray(env, ciElement, vals[i], cgs,
								prefix + "[" + i);
					} else {
						JSONObject jsonObj = vals[i].getObject();
						if (jsonObj != null) {
							newObjRef = jsonObj.fillObject(env, ciElement, cgs,
									prefix + "[" + i);
						} else {
	...
	/**
	 * This is method is used to set field of primitive type from CG result
	 * object
	 * 
	 * @param ei
	 *            - ElementInfo to set field in
	 * @param fi
	 *            - FieldInfo of a field we want to set
	 * @param cgResult
	 *            - result of CG call
	 */
	private void convertPrimititve(ElementInfo ei, FieldInfo fi, Object cgResult) {
		...
		if (primitiveName.equals("boolean") && cgResult instanceof Boolean) {
			...
			throw new JPFException("Can't convert "
					+ cgResult.getClass().getCanonicalName() + " to "
					+ primitiveName);
		}
	}
	 ...
	 * 
	 * @param cgs
	 *            - array of CG from current state
	 * @param id
	 *            - id of the CG that we search for

 ...
 * node. Parser read extended JSON grammar (http://json.org). Standard grammar
 * was extended by ability to set Choice Generator call as a value in JSON
 * object.
 * 
...
public class JSONParser {
	...
	/**
	 * Read next token from lexer output stream. If parser backtraced return
	...
	private Token next() {
		if (lastReadToken != null
				&& lastReadToken.getType() == Token.Type.DocumentEnd) {
	...
	private void back() {
		if (backtrack == 2) {
			throw new JPFException(
					"Attempt to bactrack three times. Posibly an error. Please report");
		}

		if (lastReadToken == null) {
			throw new JPFException(
					"Attempt to backtrack before starting to read token stream. Please report");
		}

		if (backtrack == 1 && prevReadToken == null) {
			throw new JPFException(
					"Attempt to backtrack twice when less then two tokens read. Please report");
		}
	...
	/**
	 * Read next token and check it's type. If type is wrong method throws
	 * exception else it returns read token
	 * 
	 * @param type
	 *            - type of the following token.
	...
	private JSONObject parseObject() {
		...
		if (t.getType() != Token.Type.ObjectEnd) {
			...
			while (true) {
				...
				t = next();
				// If next token is comma there is one more key-value pair to
				// read
	...
	private void error(String string) {
		throw new JPFException(string + "(" + lexer.getLineNumber() + ":"
				+ lexer.getCurrentPos() + ")");
	}
	...
	private Value parseValue() {
		...
		default:
			error("Unexpected token '" + t.getValue()
					+ "' during parsing JSON value");
			return null;
		}
	 ...
	 * 
	 * @param cgName
	 *            - name of called Choice Generator.

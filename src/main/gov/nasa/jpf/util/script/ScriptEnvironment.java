...
public abstract class ScriptEnvironment<CG extends ChoiceGenerator<?>>
	...
	class ActiveSnapshot implements Cloneable {
		...
		ActiveSnapshot advance(String[] activeStates, BitSet isReEntered) {
			...
			// --- add the new ones
			int skipped = 0;
			nextActive: for (int i = 0; i < activeStates.length; i++) {
				if (newActives[i] == null) {
					...
					if (sec != null) {
						...
						for (int j = 0; j < newActives.length; j++) {
							if (newActives[j] != null
									&& newActives[j].section == sec) {
								skipped++;
								continue nextActive;
							}
						}

						// check if it was processed by a prev state (superstate
						// section by a
						// common parent of a new and an old state - this is the
						// common case
						for (int j = 0; j < actives.length; j++) {
							// <2do> how do we handle state re-entering?
							if (actives[j].section == sec) {
								ActiveSequence as = new ActiveSequence(
										activeStates[i], sec, actives[j].intrp);
								newActives[i] = as;
								continue nextActive;
							}
						}
						...
						// it's a new one
						ActiveSequence as = new ActiveSequence(activeStates[i],
								sec, new SequenceInterpreter(sec));
						newActives[i] = as;

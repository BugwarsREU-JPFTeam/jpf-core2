 ...
 * 
 * <2do> this is awfully connected to ESParser - we should really make it more
 * generic (this is the reason why it was moved from the general
 * gov.nasa.jpf.util to this package)
 */
public class StringExpander {
	 ...
	 * void nextToken () {
	 * 
	 * int i = pos; int len = this.len;
	 * 
	 * if (i>=len){ token = EOS;
	 * 
	 * } else { if (src.charAt(i) == META_CHAR){ // symbol char c =
	 * src.charAt(++i); switch (c){ case CAT_START_CHAR: token = CAT_START;
	 * break; case CAT_END_CHAR: token = CAT_END; break;
	 * 
	 * case ALT_START_CHAR: token = ALT_START; break; case ALT_CHAR: token =
	 * ALT; break; case ALT_END_CHAR: token = ALT_END; break; default:
	 * error("illegal symbol: " + c); } pos += 2;
	 * 
	 * } else { // string literal int j = i + 1; for (; j < len && src.charAt(j)
	 * != META_CHAR; j++); pos = j; token = new Token(src.substring(i, j)); } }
	 * }
	 **/

...
public abstract class TestJPF implements JPFShell {
	 ...
	 */
	protected static String getSutClassName(String testClassName,
			String sutPackage) {
	...
	protected static List<Method> getMatchingMethods(
			Class<? extends TestJPF> testCls, int setModifiers,
			int unsetModifiers, String[] annotationNames) {
		List<Method> list = new ArrayList<Method>();

		for (Method m : testCls.getMethods()) {
			if (isMatchingMethod(m, setModifiers, unsetModifiers,
					annotationNames)) {
				list.add(m);
			}
		}
	...
	protected static boolean isMatchingMethod(Method m, int setModifiers,
			int unsetModifiers, String[] annotationNames) {
			...
			if (m.getParameterTypes().length == 0) {
				if (annotationNames != null) {
					...
					for (int i = 0; i < annotations.length; i++) {
						String annotType = annotations[i].annotationType()
								.getName();
	...
	protected static List<Method> getContextMethods(
			Class<? extends TestJPF> testCls, int setModifiers,
	...
	protected static List<Method> getBeforeMethods(
			Class<? extends TestJPF> testCls) {
		return getContextMethods(testCls, Modifier.PUBLIC, Modifier.STATIC,
				"org.junit.Before");
	}

	protected static List<Method> getAfterMethods(
			Class<? extends TestJPF> testCls) {
		return getContextMethods(testCls, Modifier.PUBLIC, Modifier.STATIC,
				"org.junit.After");
	}

	protected static List<Method> getBeforeClassMethods(
			Class<? extends TestJPF> testCls) {
		return getContextMethods(testCls, Modifier.PUBLIC | Modifier.STATIC, 0,
				"org.junit.BeforeClass");
	}

	protected static List<Method> getAfterClassMethods(
			Class<? extends TestJPF> testCls) {
		return getContextMethods(testCls, Modifier.PUBLIC | Modifier.STATIC, 0,
	...
	protected static List<Method> getTestMethods(
			Class<? extends TestJPF> testCls, String[] args) {
		String[] testAnnotations = { "org.junit.Test",
		...
		if (haveTestMethodSpecs(args)) { // test methods specified as arguments
			...
			for (String test : args) {
				if (test != null) {
					...
					try {
						...
						if (isMatchingMethod(m, Modifier.PUBLIC,
								Modifier.STATIC, null /* testAnnotations */)) {
							list.add(m);
						} else {
						...
						throw new RuntimeException("method: " + test
								+ "() not in test class: " + testCls.getName(),
								x);
					}
				}
			}
			...
			return getMatchingMethods(testCls, Modifier.PUBLIC,
					Modifier.STATIC, testAnnotations);
		}
	}
	...
	protected static void reportResults(String clsName, int nTests,
			int nFailures, int nErrors, List<String> results) {
		System.out.println();
		System.out
				.print("......................................... execution of testsuite: "
						+ clsName);
		...
		System.out.print(".........................................");
		System.out.println(" tests: " + nTests + ", failures: " + nFailures
				+ ", errors: " + nErrors);
	}

	static void invoke(Method m, Object testObject)
	 ...
	 */
	public static void runTests(Class<? extends TestJPF> testCls,
			String... args) {
		int nTests = 0;
		int nFailures = 0;
		int nErrors = 0;
		...
		try {
			...
			for (Method testMethod : testMethods) {
				...
				try {
					...
					if (cause instanceof AssertionError) {
						nFailures++;
						reportTestFinished("test method failed with: "
								+ cause.getMessage());
						result += ": Failed";
					} else {
						nErrors++;
						reportTestFinished("unexpected error while executing test method: "
								+ cause.getMessage());
						result += ": Error";
					}
			...
			cause.printStackTrace();
			nErrors++;
			reportTestFinished("TEST ERROR: @BeforeClass,@AfterClass method failed: "
					+ x.getMessage());

		} catch (InstantiationException x) {
			nErrors++;
			reportTestFinished("TEST ERROR: cannot instantiate test class: "
					+ x.getMessage());
		} catch (IllegalAccessException x) { // can't happen if getTestMethods()
												// worked
			nErrors++;
			reportTestFinished("TEST ERROR: default constructor or test method not public: "
					+ testMethodName);
		} catch (IllegalArgumentException x) { // can't happen if
												// getTestMethods() worked
			nErrors++;
			reportTestFinished("TEST ERROR: illegal argument for test method: "
					+ testMethodName);
		...
		if (!hideSummary) {
			reportResults(testCls.getName(), nTests, nFailures, nErrors,
					results);
		}
	...
	protected static void runTestsOfThisClass(String[] testMethods) {
		...
		Class<? extends TestJPF> testClass = Reflection
				.getCallerClass(TestJPF.class);
	...
	protected JPF assertionError(StackTraceElement testMethod, String... args) {
		return unhandledException(testMethod, "java.lang.AssertionError", null,
				args);
	}

	protected JPF assertionError(String... args) {
		return unhandledException(getCaller(), "java.lang.AssertionError",
				null, args);
	}

	protected JPF assertionErrorDetails(StackTraceElement testMethod,
			String details, String... args) {
		return unhandledException(testMethod, "java.lang.AssertionError",
				details, args);
	}
	...
	protected boolean verifyAssertionErrorDetails(String details,
			String... args) {
		if (runDirectly) {
			return true;
		} else {
			unhandledException(getCaller(), "java.lang.AssertionError",
					details, args);
			return false;
		}
	}
	...
	protected boolean verifyAssertionError(String... args) {
		if (runDirectly) {
			...
			unhandledException(getCaller(), "java.lang.AssertionError", null,
					args);
			return false;
		}
	}
	 ...
	 */
	protected JPF noPropertyViolation(StackTraceElement testMethod,
			String... args) {
		JPF jpf = null;
		...
		if ((errors != null) && (errors.size() > 0)) {
			fail("JPF found unexpected errors: "
	 ...
	 *            JPF arguments
	 */
	protected JPF unhandledException(StackTraceElement testMethod,
			String xClassName, String details, String... args) {
		JPF jpf = null;
		...
		if (error != null) {
			...
			if (errorProperty instanceof NoUncaughtExceptionsProperty) {
				ExceptionInfo xi = ((NoUncaughtExceptionsProperty) errorProperty)
						.getUncaughtExceptionInfo();
				String xn = xi.getExceptionClassname();
				if (!xn.equals(xClassName)) {
					fail("JPF caught wrong exception: " + xn + ", expected: "
							+ xClassName);
				}
				...
				fail("JPF failed to catch exception executing: ", args,
						("expected " + xClassName));
			}
		} else { // no error
			fail("JPF failed to catch exception executing: ", args,
	...
	protected JPF unhandledException(String xClassName, String details,
			String... args) {
		return unhandledException(getCaller(), xClassName, details, args);
	}

	protected boolean verifyUnhandledExceptionDetails(String xClassName,
			String details, String... args) {
		if (runDirectly) {
			return true;
		} else {
	...
	protected boolean verifyUnhandledException(String xClassName,
			String... args) {
		if (runDirectly) {
			return true;
		} else {
	 ...
	 *            JPF main() arguments
	 */
	protected JPF jpfException(StackTraceElement testMethod,
			Class<? extends Throwable> xCls, String... args) {
		JPF jpf = null;
		...
		if (exception != null) {
			if (!xCls.isAssignableFrom(exception.getClass())) {
				fail("JPF produced wrong exception: " + exception
						+ ", expected: " + xCls.getName());
			}
		} else {
	...
	protected boolean verifyJPFException(TypeRef xClsSpec, String... args) {
		if (runDirectly) {
			...
			try {
				Class<? extends Throwable> xCls = xClsSpec
						.asNativeSubclass(Throwable.class);
	 ...
	 *            JPF main() arguments
	 */
	protected JPF propertyViolation(StackTraceElement testMethod,
			Class<? extends Property> propertyCls, String... args) {
		JPF jpf = null;
	...
	protected JPF propertyViolation(Class<? extends Property> propertyCls,
			String... args) {
		return propertyViolation(getCaller(), propertyCls, args);
	}

	protected boolean verifyPropertyViolation(TypeRef propertyClsSpec,
			String... args) {
		if (runDirectly) {
			return true;
			...
			try {
				Class<? extends Property> propertyCls = propertyClsSpec
						.asNativeSubclass(Property.class);
	...
	public static void assertEquals(double expected, double actual, double delta) {
		assertEquals("Math.abs(expected - actual) > delta : " + "Math.abs("
				+ expected + " - " + actual + ") > " + delta, expected, actual,
				delta);
	}
	...
	public static void assertArrayEquals(byte[] expected, byte[] actual) {
		...
		for (int i = 0; i < expected.length; i++) {
			if (expected[i] != actual[i]) {
				fail("array element" + i + " different, expected "

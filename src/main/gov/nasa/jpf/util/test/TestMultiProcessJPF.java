...
public abstract class TestMultiProcessJPF extends TestJPF {
	...
	protected void setTestTargetKeys(Config conf, StackTraceElement testMethod) {
		...
		conf.put("vm.scheduler_factory.class",
				"gov.nasa.jpf.vm.DistributedSchedulerFactory");
	}

	protected native int getProcessId();

	protected boolean mpVerifyAssertionErrorDetails(int prcNum, String details,
			String... args) {
		if (runDirectly) {
			return true;
		} else {
	...
	protected boolean mpVerifyUnhandledExceptionDetails(int prcNum,
	...
	protected boolean mpVerifyUnhandledException(int prcNum, String xClassName,
			String... args) {
		if (runDirectly) {
			return true;
		} else {
	...
	protected boolean mpVerifyJPFException(int prcNum, TypeRef xClsSpec,
			String... args) {
		if (runDirectly) {
			return true;
	...
	protected boolean mpVerifyPropertyViolation(int prcNum,
			TypeRef propertyClsSpec, String... args) {
		if (runDirectly) {
			return true;

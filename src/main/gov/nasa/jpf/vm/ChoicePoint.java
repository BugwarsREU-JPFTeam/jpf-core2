 ...
 * are little more than just a list of ChoiceGenerator classnames and choice
 * indexes stored in a previous run)
 */
public class ChoicePoint {
	...
	public static void storeTrace(String fileName, String sutName,
			String comment, ChoiceGenerator[] trace, boolean verbose) {
		int i;
		if (fileName != null) {
			try {
				...
				// store the main app class and args here, so that we can do at
				// least some checking
				pw.print("application: ");
				pw.println(sutName);

				// keep a String->id map so that we don't have to store
				// thousands of redundant strings
	...
	static void match(StreamTokenizer scanner, String s) throws IOException {
		if ((scanner.ttype == StreamTokenizer.TT_WORD)
				&& (scanner.sval.equals(s))) {
			scanner.nextToken();
		} else {
			throw new JPFException("tracefile error - expected " + s
					+ ", got: " + scanner.sval);
		}
	}
		...
		if (scanner.ttype == StreamTokenizer.TT_WORD) {
			...
			if (s.length() == 0) {
				throw new JPFException(
						"tracefile error - non-empty string expected");
			}
			scanner.nextToken();
			return s;
		} else {
			throw new JPFException("tracefile error - word expected, got: "
					+ scanner.sval);
		}
	}
	...
	static void matchChar(StreamTokenizer scanner, char c) throws IOException {
		if (scanner.ttype == c) {
			...
			throw new JPFException("tracefile error - char '" + c
					+ "' expected, got: " + scanner.sval);
		}
	}
	...
	static int matchNumber(StreamTokenizer scanner) throws IOException {
		...
		throw new JPFException("tracefile error - number expected, got: "
				+ scanner.sval);
	}
		...
		try {
			...
			while (scanner.ttype != StreamTokenizer.TT_EOF) {
				...
				if (cpClass.charAt(0) == '#') { // it's a CG class id
					...
					if (cpClass == null) {
						throw new JPFException(
								"tracefile error - unknown ChoicePoint class id: "
										+ cpClass);
					}
				} else {
					String id = "#" + clsId++;

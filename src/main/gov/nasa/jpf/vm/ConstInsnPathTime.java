...
/**
 * time model that uses instruction count along current path to deduce the
 * execution time.
 * 
...
public class ConstInsnPathTime extends ListenerAdapter implements TimeModel {
	...
	public void choiceGeneratorRegistered(VM vm, ChoiceGenerator<?> nextCG,
		...
		if (nextCG.isCascaded()) {
			// there's got to be a previous one, and its associated with the
			// same insn
			tv = cgPrev.getAttr(TimeVal.class);

		} else {
			if (cgPrev != null) {
				TimeVal tvPrev = cgPrev.getAttr(TimeVal.class); // there has to
																// be one

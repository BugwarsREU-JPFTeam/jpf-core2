...
/**
 * a JenkinsStateSet that stores program state information in a readable and
 * diffable format.
 * 
 * Storing states as readable text is enabled by setting vm.storage.class to
 * this class
 * 
 * Note: this automatically sets/overrides the serializer to
 * Debug<serializer-class>
 */
public class DebugJenkinsStateSet extends JenkinsStateSet {
	...
	public DebugJenkinsStateSet(Config conf) {
		...
		if (serializerCls != null) {
			...
			if (!cname.startsWith("Debug")) {
				if (idx > 0) {
					serializerCls = serializerCls.substring(0, idx) + "Debug"
							+ cname;
				} else {
			...
			serializer = conf.getInstance(null, serializerCls,
					DebugStateSerializer.class);
			if (serializer == null) {
				throw new JPFConfigException(
						"Debug StateSet cannot instantiate serializer: "
								+ serializerCls);
			}
		}
		...
		if (!outputDir.isDirectory()) {
			if (!outputDir.mkdirs()) {
				throw new JPFConfigException(
						"Debug StateSet cannot create output dir: "
	...
	public void attach(VM vm) {
		...
		// <2do> this is a bit hack'ish - why does the VM keep the serializer
		// anyways,
	...
	public int addCurrent() {
		...
		try {
			...
			throw new JPFException(
					"cannot create Debug state set output file: "
		...
		try {
			...
			throw new JPFException("cannot write Debug state set output file: "
					+ outputFile.getAbsolutePath());
		}

		// if this is a new state, store it under its id, otherwise throw it
		// away
		if (stateId > maxId) {

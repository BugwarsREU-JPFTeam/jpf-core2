...
/**
 * default Attributor implementation to set method and fiel attributes at class
 * load time. Note this is critical functionality, esp. with respect to
 * threading
 */
public class DefaultAttributor implements Attributor {
	...
	public void setMethodInfoAttributes(MethodInfo mi) {
		...
		if (atomicMethods != null && atomicMethods.matchesAny(uniqueName)) {
			// but exclude the known blocking methods (Object.wait()/notify(),
			// Thread.join())
	...
	/**
	 * answer the type based object attributes for this class. See ElementInfo
	 * for valid choices
	 */
	@Override
	...
	public void setFieldInfoAttributes(FieldInfo fi) {
		...
		if (fi.isFinal()) {
			// <2do> Hmm, finals are not really immutable, they only can be set
			// once
			// <2do> same here - FieldInfo does not even have a isImmutable()
			attr |= ElementInfo.ATTR_IMMUTABLE;
		}
		// <2do> what about other immutable fields? don't say there are none -
		// try to set one

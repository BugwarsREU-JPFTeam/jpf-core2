 ...
 * status), but we don't change any thread or lock status. This has to happen in
 * the instruction before calling the factory
 */
...
public class DefaultSchedulerFactory implements SchedulerFactory {
	...
	protected ChoiceGenerator<ThreadInfo> getSyncCG(String id, ElementInfo ei,
			ThreadInfo ti) {
	...
	protected ThreadInfo[] getRunnablesIfChoices(ThreadInfo ti) {
		int n = vm.getThreadList().getMatchingCount(
				vm.getTimedoutRunnablePredicate());
	...
	protected ChoiceGenerator<ThreadInfo> createEnterCG(String id,
			ElementInfo ei, ThreadInfo ti) {
	...
	protected ChoiceGenerator<ThreadInfo> createExitCG(String id,
			ElementInfo ei, ThreadInfo ti) {
		if (breakSyncExit) {
			if (!ss.isAtomic()) {
				return getSyncCG(id, ei, ti);
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createSyncMethodEnterCG(ElementInfo ei,
			ThreadInfo ti) {
		return createEnterCG(SYNC_METHOD_ENTER, ei, ti);
	}

	@Override
	public ChoiceGenerator<ThreadInfo> createSyncMethodExitCG(ElementInfo ei,
			ThreadInfo ti) {
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createNotifyAllCG(ElementInfo ei,
			ThreadInfo ti) {
		return null; // no choice here
	}

	@Override
	public ChoiceGenerator<ThreadInfo> createSharedFieldAccessCG(
			ElementInfo ei, ThreadInfo ti) {
		if (ss.isAtomic()) {
			return null;
		}
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createSharedObjectExposureCG(
			ElementInfo ei, ThreadInfo ti) {
		if (ss.isAtomic()) {
			return null;
		}
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createParkCG(ElementInfo ei,
			ThreadInfo tiPark, boolean isAbsoluteTime, long timeOut) {
		// we treat this like a wait, but don't differentiate between absolute
		// and relative timeout. Note it has to be a right mover
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createSharedArrayAccessCG(
			ElementInfo ei, ThreadInfo ti) {
		...
		if (breakArrayAccess) {
			 ...
			 * ArrayInstruction ainsn = (ArrayInstruction)ti.getPC(); boolean
			 * isRead = ainsn.isRead(); int aref = ei.getThreadInfoForId();
			 * 
			 * for (ChoiceGenerator<?> cg = ss.getChoiceGenerator(); cg != null;
			 * cg = cg.getPreviousChoiceGenerator()){ if
			 * (cg.getThreadInfoForId() != ti || cg.getChoiceType() !=
			 * ThreadInfo.class){ break; // different thread or different choice
			 * type -> we need a CG }
			 * 
			 * Instruction cgInsn = cg.getInsn(); if (!(cgInsn instanceof
			 * ArrayInstruction)){ break; // not an aload/astore -> we need a CG
			 * } ArrayInstruction cgAinsn = (ArrayInstruction)cgInsn; // this is
			 * only an approximation since the array ref stored in the insn //
			 * might have changed. Note this only works if the insn arrayRef is
			 * // stored AFTER this gets executed if (cgAinsn.getArrayRef(ti) !=
			 * aref){ break; }
			 * 
			 * if (cgAinsn.isRead() == isRead){ return null; // same op on same
			 * array in same thread -> no new CG required }
			 * 
			 * // if we get here, this is a complement op on the same array in
			 * the same thread // -> skip over all prev. CG insns of the same
			 * type }
			 **/
			ChoiceGenerator<ThreadInfo> cg = getSyncCG(SHARED_ARRAY_ACCESS, ei,
					ti);
			return cg;
	...
	public ChoiceGenerator<ThreadInfo> createThreadStartCG(ThreadInfo newThread) {
		...
		// we've been going forth & back a number of times with this. The idea
		// was
		// that it would be more intuitive to see a transition break every time
		// we
		// start a new thread, but this causes significant state bloat in case
		// of
		...
		// It should be safe to go on, since the important thing is to set the
		// new thread
		// runnable.

		if (breakStart) {
			if (ss.isAtomic()) {
				// this is dangerous - POR now depends on Thread.start() being a
				// right mover
				// If the current thread doesn't have a scheduling point before
				// termination,
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createBeginAtomicCG(
			ThreadInfo atomicThread) {
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createThreadYieldCG(
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createInterruptCG(
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createThreadSleepCG(
			ThreadInfo sleepThread, long millis, int nanos) {
		if (breakSleep) {
			if (ss.isAtomic()) {
				return null;
			}
	...
	public ChoiceGenerator<ThreadInfo> createThreadTerminateCG(
		...
		// NOTE returning null does not directly define an end state - that's up
		// to
	...
	public ChoiceGenerator<ThreadInfo> createThreadStopCG(
			ThreadInfo stoppedThread) {
		return null; // left mover, there will be still a terminateCG
		// return getRunnableCG( THREAD_STOP);
	}

	/**
	 * used to break the transition for reasons not related to the executing
	 * instruction type (e.g. max transition length exceeded).
	 */
	@Override
	public ChoiceGenerator<ThreadInfo> createBreakTransitionCG(String reason,
	...
	@Override
	public ChoiceGenerator<ThreadInfo> createPreFinalizeCG(
	...
	public ChoiceGenerator<ThreadInfo> createPostFinalizeCG(
			ThreadInfo finalizerThread) {
		return new ThreadChoiceFromSet(POST_FINALIZE,
				getRunnablesWithout(finalizerThread), true);
	}
}

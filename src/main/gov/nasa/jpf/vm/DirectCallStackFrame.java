...
/**
 * DirectCallStackFrames are only used for overlay calls (from native code),
 * i.e. there is no corresponding INVOKE instruction. The associated MethodInfos
 * are synthetic, their only code is (usually) a INVOKEx and a DIRECTCALLRETURN.
 * NOTE: such MethodInfos do not belong to any class
 * 
 * Arguments for the invoke insn have to be pushed explicitly by the caller
 * 
 * direct calls do not return any values themselves, but they do get the return
 * values of the called method pushed onto their own operand stack. If the
 * DirectCallStackFrame user needs such return values, it has to do so via
 * ThreadInfo.getReturnedDirectCall()
 * 
 */
public abstract class DirectCallStackFrame extends StackFrame implements
		SystemAttribute {
	...
	protected DirectCallStackFrame(MethodInfo miDirectCall, MethodInfo callee) {
		super(miDirectCall, miDirectCall.getMaxLocals(), miDirectCall
				.getMaxStack());
		this.callee = callee;
	}
	...
	/*
	 * those set the callee arguments for the invoke insn - the returned value
	 * is the slot index OFFSET (not the slot index itself) for the next
	 * argument. This has to be used in a pattern like int argOffset =
	 * frame.setArgument( 0, firstArg, a0); argOffset = frame.setLongArgument(
	 * argOffset, secondArg, a1); ...
	 */

...
public abstract class ElementInfo implements Cloneable {
	...
	// --- the lower 2 bytes of the attribute field are sticky (state
	// stored/restored)
	...
	// this ElementInfo is not allowed to be modified anymore since it has been
	// state stored
	// ElementInfos are constructed in a non-frozen state, and will be frozen
	// upon
	// heap store (usuall in the heap or ElementInfo memento ctor)
	// This is the basis for lifting the change management from the fields level
	// (fields,monitor,attributes)
	...
	// The constructor for the object has returned. Final fields can no longer
	// break POR
	// This attribute is set in gov.nasa.jpf.vm.bytecode.RETURN.enter().
	// If ThreadInfo.usePorSyncDetection() is false, then this attribute is
	// never set.
	...
	// ATTR_SHARED is frozen (has to be changed explicitly, will not be updated
	// by checkUpdatedSharedness)
	public static final int ATTR_FREEZE_SHARED = 0x8000;

	// --- the upper two bytes are for transient (heap internal) use only, and
	// are not stored

	// BEWARE if you add or change values, make sure these are not used in
	// derived classes !
	// <2do> this is efficient but fragile

	public static final int ATTR_TREF_CHANGED = 0x40000; // referencingThreads
															// have changed
	public static final int ATTR_ATTRIBUTE_CHANGED = 0x80000; // refers only to
																// sticky bits
	...
	// transient flag set if object is reachable from root object, i.e. can't be
	// recycled
	public static final int ATTR_IS_MARKED = 0x80000000;

	// this bit is set/unset by the heap in order to identify live objects that
	// have
	// already been unmarked. This is to avoid additional passes over the whole
	// heap in
	// order to clean up dangling references etc.
	// NOTE - this bit should never be state stored - restored ElementInfo
	// should never have it set
	...
	// the set of threads using this object. Note this is not used for state
	// matching
	// so that order or thread id do not have a direct impact on heap symmetry
	protected ThreadInfoSet referencingThreads;

	// this is the reference value for the object represented by this
	// ElementInfo
	// (note this is a slight misnomer for StaticElementInfos, which don't
	// really
	// represent objects but collections of static fields belonging to the same
	// class)
	...
	// info in the future.
	// We distinguish between propagates and private object attributes, the
	// first
	// one stored in the lower 2 bytes
	protected int attributes;

	// --- the following fields are transient or search global, i.e. their
	// values
	// are not state-stored, but might be set during state restoration

	// FieldLockInfos are never state-stored/backtracked! They are set in the
	// order of
	...
	// cache for unchanged ElementInfos, so that we don't have to re-create
	// cachedMemento
	// objects all the time
	protected Memento<ElementInfo> cachedMemento;

	// cache for a serialized representation of the object, which can be used
	// by state-matching. Value interpretation depends on the configured
	// Serializer
	protected int sid;

	// helpers for state storage/restore processing, to avoid explicit iterators
	// on
	...
	protected ElementInfo(int id, ClassInfo c, Fields f, Monitor m,
			ThreadInfo ti) {
	...
	public String toString() {
		return ((ci != null ? ci.getName() : "ElementInfo") + '@' + Integer
				.toHexString(objRef));
	}
	...
	public void processReleaseActions() {
		...
		if (fields.hasObjectAttr()) {
			for (ReleaseAction action : fields
					.objectAttrIterator(ReleaseAction.class)) {
				action.release(this);
			}
		}
	}
	...
	/**
	 * post transition live object cleanup update all non-fields references used
	 * by this object. This is only called at the end of the gc, and recycled
	 * objects should be either null or not marked
	 * 
	 ...
	 * a previous phase1. This one is called on all 'root'-marked objects after
	 * phase1 is completed. ElementInfo is not an ideal place for this method,
	 * as it has to access some innards of both ClassInfo (FieldInfo container)
	 * and Fields. But on the other hand, we want to keep the whole heap
	 * traversal business as much centralized in ElementInfo and Heap
	 * implementors
	 */
	void markRecursive(Heap heap) {
		...
		if (isArray()) {
			...
			do {
				...
				boolean isRef = isWeakRef && ci.isReferenceClassInfo(); // is
																		// this
																		// the
				...
				for (i = 0; i < n; i++) {
					...
					if (fi.isReference()) {
						if ((i == 0) && isRef) {
							// we need to reset the ref field once the
							// referenced object goes away
							// NOTE: only the *first* WeakReference field is a
							// weak ref
							// (this is why we have our own implementation)
							heap.registerWeakReference(this);
						} else {
							int objref = fields.getReferenceValue(fi
									.getStorageOffset());
							if (objref != MJIEnv.NULL) {
								heap.queueMark(objref);
							}
	...
	public void setShared(boolean isShared) {
		if (isShared) {
			if ((attributes & ATTR_SHARED) == 0) {
				...
				// note we don't report the thread here since this is explicitly
				// set via Verify.setShared
	 ...
	 * 
	 * NOTE - this might return a new (cloned) ElementInfo in case the state
	 * stored/restored flag has been changed and/or the SharedObjectPolicy in
	 * effect uses persistent ThreadInfoSet objects (i.e. replaces them upon
	 * add). Use only from system code that is aware of the potential
	 * ElementInfo identity change (i.e. does not keep a reference to the old
	 * one)
	 */
	public ElementInfo getInstanceWithUpdatedSharedness(ThreadInfo ti) {
		...
		// NOTE - it is up to the policy to decide if this creates a new set or
		// destructively updates
		// the existing one (which translates into carry-over between paths,
		// i.e. search global state)
		ThreadInfoSet newReferencingThreads = referencingThreads.add(ti);
		if (newReferencingThreads != referencingThreads) { // policy uses
															// persistent
															// (invariant)
															// ThreadInfoSets
		...
		// the thread might already have been in referencingThreads, but we
		// might get here after
		// backtracking. In this case the ATTR_SHARED attribute might have been
		// reset and we have
		// to check if it needs updating (in case sharedness is not frozen)
		if ((updatedEi.attributes & ATTR_FREEZE_SHARED) == 0) {
			// note that we can only go from non-shared to shared, but not vice
			// versa
	...
	public void setExposed(ThreadInfo ti, ElementInfo eiFieldOwner) {
		// we actually have to add this to the attributes to avoid endless loops
		// by
	...
	/**
	 * this is called before the system attempts to reclaim the object. If we
	 * return 'false', the object will *not* be removed
	 */
	protected boolean recycle() {
		...
		// but the offending object goes out of scope before the system
		// backtracks
	...
	abstract protected FieldInfo getDeclaredFieldInfo(String clsBase,
			String fname);
	...
	public <T> ObjectList.TypedIterator<T> fieldAttrIterator(FieldInfo fi,
			Class<T> type) {
	...
	public void replaceElementAttrNoClone(int idx, Object oldAttr,
			Object newAttr) {
		fields.replaceFieldAttr(idx, oldAttr, newAttr);
	}
	...
	public <T> ObjectList.TypedIterator<T> elementAttrIterator(int idx,
			Class<T> type) {
	...
	public Object getFieldValueObject(String fname) {
		...
		if (fi != null) {
			...
			if (eiEnclosing != null) {
				...
				// we should check static fields in enclosing scopes, but there
				// is no
	...
	public void setDeclaredReferenceField(String fname, String clsBase,
			int value) {
	...
	public int getDeclaredIntField(String fname, String clsBase) {
		// be aware of that static fields are not flattened (they are unique),
		// i.e.
		// the FieldInfo might actually refer to another
		// ClassInfo/StaticElementInfo
		FieldInfo fi = getDeclaredFieldInfo(clsBase, fname);
	...
	/**
	 * note this only holds for instance fields, and hence the method has to be
	 * overridden in StaticElementInfo
	 */
	...
	protected void checkArray(int index) {
		if (fields instanceof ArrayFields) { // <2do> should check for !long
												// array
	...
	/**
	 * this is the backend for System.arraycopy implementations, but since it
	 * only throws general exceptions it can also be used in other contexts that
	 * require type and objRef checking
	 * 
	 * note that we have to do some additional type checking here because we
	 * store reference arrays as int[], i.e. for reference arrays we can't rely
	 * on System.arraycopy to do the element type checking for us
	 * 
	 * @throws java.lang.ArrayIndexOutOfBoundsException
	 * @throws java.lang.ArrayStoreException
	 */
	public void copyElements(ThreadInfo ti, ElementInfo eiSrc, int srcIdx,
			int dstIdx, int length) {

		if (!isArray()) {
			throw new ArrayStoreException("destination object not an array: "
					+ ci.getName());
		}
		if (!eiSrc.isArray()) {
			throw new ArrayStoreException("source object not an array: "
					+ eiSrc.getClassInfo().getName());
		}

		boolean isRefArray = isReferenceArray();
		if (eiSrc.isReferenceArray() != isRefArray) {
			throw new ArrayStoreException("array types not compatible: "
					+ eiSrc.getClassInfo().getName() + " and " + ci.getName());
		}

		// since the caller has to handle normal ArrayStoreExceptions and
		// ArrayIndexOutOfBoundsExceptions, we don't have to explicitly check
		// array length here

		// if we copy reference arrays, we first have to check element type
		// compatibility
		// (the underlying Fields type is always int[], hence we have to do this
		// explicitly)
		if (isRefArray) {
				...
				if (eref != MJIEnv.NULL) {
					...
					if (!srcElementCi.isInstanceOf(dstElementCi)) {
						throw new ArrayStoreException(
								"incompatible reference array element type (required "
										+ dstElementCi.getName() + ", found "
										+ srcElementCi.getName());
					}
				}
			}
		}

		// NOTE - we have to clone the fields even in case System.arraycopy
		// fails, since
		// the caller might handle ArrayStore/IndexOutOfBounds, and partial
		// changes
		...
		// this might throw ArrayIndexOutOfBoundsExceptions and
		// ArrayStoreExceptions
		System.arraycopy(srcVals, srcIdx, dstVals, dstIdx, length);

		// now take care of the attributes
		// <2do> what in case arraycopy did throw - we should only copy the
		// changed element attrs
	...
	public ElementInfo getDeclaredObjectField(String fname, String referenceType) {
		return VM.getVM().getHeap()
				.get(getDeclaredReferenceField(fname, referenceType));
	}
	 ...
	 * dependent, but we can give an upper bound for the fields/elements, and
	 * that should be good in terms of application specific properties)
	 */
	...
	/**
	 * is this a Number, a Boolean or a Character object Note these classes are
	 * all final, so we don't have to check for subtypes
	 * 
	...
	void updateLockingInfo() {
		...
		if (ti != null) {
			...
			// NOTE - the threads need to be restored *before* the ElementInfo
			// containers,
			...
			// assert area.ks.tl.get(ti.objRef) == ti; // covered by
			// verifyLockInfo
	...
	public void checkArrayBounds(int index)
		...
		if (fields instanceof ArrayFields) {
			if (index < 0 || index >= ((ArrayFields) fields).arrayLength()) {
				throw new ArrayIndexOutOfBoundsExecutiveException(ThreadInfo
						.getCurrentThread().createAndThrowException(
	...
	public ElementInfo deepClone() {
		try {
			...
			// referencingThreads is at least subtree global, hence doesn't need
			// to be cloned
	...
	public void registerLockContender(ThreadInfo ti) {
		...
		assert ti.lockRef == MJIEnv.NULL || ti.lockRef == objRef : "thread "
				+ ti + " trying to register for : " + this
		...
		// remove itself from there once it resumes: lock() calls
	...
	void blockLockContenders() {
		// check if there are any other threads that have to change status
		// because they
	...
	/**
	 * from a MONITOR_ENTER or sync INVOKExx if we cannot acquire the lock note:
	 * this is not called from a NOTIFIED_UNBLOCKED state, so we don't have to
	 * restore NOTIFIED
	 */
	public void block(ThreadInfo ti) {
		assert (monitor.getLockingThread() != null)
				&& (monitor.getLockingThread() != ti) : "attempt to block "
				+ ti.getName() + " on unlocked or own locked object: " + this;
	...
	public void lock(ThreadInfo ti) {
		// if we do unlock consistency checks with JPFExceptions, we should do
		// the same here
		if ((monitor.getLockingThread() != null)
				&& (monitor.getLockingThread() != ti)) {
			throw new JPFException("thread " + ti.getName()
					+ " tries to lock object: " + this
					+ " which is locked by: "
		...
		// before we enter anything else, mark this thread as not being blocked
		// anymore
		ti.resetLockRef();
		...
		// don't re-add if we are recursive - the lock count is avaliable in the
		// monitor
		if (monitor.getLockCount() == 1) {
			ti.addLockedObject(this);
		}

		// this might set other threads blocked - make sure we lock first or the
		// sequence
		// of notifications is a bit screwed (i.e. the lock would appear *after*
		// the block)
		blockLockContenders();
	}

	/**
	 * from a MONITOR_EXIT or sync method RETURN release a possibly recursive
	...
	public void unlock(ThreadInfo ti) {
		 ...
		 * generate balanced monitorenter and monitorexit instructions for all
		 * code paths. The VM is being used for more non-Java languages. Some of
		 * these compilers might be experimental and might generate unbalanced
		 * instructions. In a more likely case, dynamically generated bytecode
		 * is more likely to make a mistake and miss a code path.
		 */
		if ((monitor.getLockCount() <= 0) || (monitor.getLockingThread() != ti)) {
			throw new JPFException("thread " + ti.getName()
					+ " tries to release non-owned lock for object: " + this);
		}
		...
		if (monitor.getLockCount() == 1) {
			...
			for (int i = 0; i < lockedThreads.length; i++) {
				...
				case TIMEOUT_WAITING:
					// nothing to do yet, thread has to timeout, get notified,
					// or interrupted
					break;

				default:
					assert false : "Monitor.lockedThreads<->ThreadData.status inconsistency! "
							+ lockedThreads[i].getStateName();
					// why is it in the list - when someone unlocks, all others
					// should have been blocked
				}
			}

			// leave the contenders - we need to know whom to block on
			// subsequent lock
	...
	/**
	 * notifies one of the waiters. Note this is a potentially non-deterministic
	 * action if we have several waiters, since we have to try all possible
	 * choices. Note that even if we notify a thread here, it still remains in
	 * the lockedThreads list until the lock is released (notified threads
	 * cannot run right away)
	 */
	...
	private void notifies0(ThreadInfo tiWaiter) {
		if (tiWaiter.isWaiting()) {
			if (tiWaiter.getLockCount() > 0) {
				// waiter did hold the lock, but gave it up in the wait, so it
				// can't run yet
	...
	public void notifies(SystemState ss, ThreadInfo ti, boolean hasToHoldLock) {
		if (hasToHoldLock) {
			assert monitor.getLockingThread() != null : "notify on unlocked object: "
					+ this;
		}
		...
		if (nWaiters == 0) {
			...
			ThreadChoiceGenerator cg = ss
					.getCurrentChoiceGeneratorOfType(ThreadChoiceGenerator.class);
	...
	/**
	 * notify all waiters. This is a deterministic action all waiters remain in
	 * the locked list, since they still have to be unblocked, which happens in
	 * the unlock (monitor_exit or sync return) following the notifyAll()
	 */
	public void notifiesAll() {
		assert monitor.getLockingThread() != null : "notifyAll on unlocked object: "
				+ this;

		ThreadInfo[] locked = monitor.getLockedThreads();
		for (int i = 0; i < locked.length; i++) {
			// !!!! if there is more than one BLOCKED thread (sync call or
			// monitor enter), only one can be
			// unblocked
			notifies0(locked[i]);
		}
	...
	/**
	 * wait to be notified. thread has to hold the lock, but gives it up in the
	 * wait. Make sure lockCount can be restored properly upon notification
	 */
	public void wait(ThreadInfo ti, long timeout) {
		wait(ti, timeout, true);
	}

	// this is used from a context where we don't require a lock, e.g.
	// Unsafe.park()/unpark()
	...
	/**
	 * re-acquire lock after being notified. This is the notified thread, i.e.
	 * the one that will come out of a wait()
	 */
	public void lockNotified(ThreadInfo ti) {
		assert ti.isUnblocked() : "resume waiting thread " + ti.getName()
		...
		// ThreadInfo.lockedObjects set, and then restore from the saved heap),
		// the
		// lock set would not include the lock when we continue to enter this
		// thread
	...
	/**
	 * updates a pinDown counter. If it is > 0 the object is kept alive
	 * regardless if it is reachable from live objects or not.
	 * 
	 * @return true if the new counter is 1, i.e. the object just became pinned
	 *         down
	 * 
	 *         NOTE - this is not a public method, pinning down an object is now
	 *         done through the Heap API, which updates the counter here, but
	 *         might also have to update internal caches
	 */
	boolean incPinDown() {
	...
	public void restore(int index, int attributes, Fields fields,
			Monitor monitor) {
		markUnchanged();
	...
	public boolean isMarkedOrAlive(boolean liveBitValue) {
		return ((attributes & ATTR_IS_MARKED) != 0)
				| (((attributes & ATTR_LIVE_BIT) == 0) ^ liveBitValue);
	}
	...
	public void checkConsistency() {
		/**
		 * ThreadInfo ti = monitor.getLockingThread(); if (ti != null) { //
		 * object has to be in the lockedObjects list of this thread
		 * checkAssertion( ti.getLockedObjects().contains(this),
		 * "locked object not in thread: " + ti); }
		 * 
		 * if (monitor.hasLockedThreads()) { checkAssertion(
		 * refTid.cardinality() > 1,
		 * "locked threads without multiple referencing threads");
		 * 
		 * for (ThreadInfo lti : monitor.getBlockedOrWaitingThreads()){
		 * checkAssertion( lti.lockRef == objRef,
		 * "blocked or waiting thread has invalid lockRef: " + lti); }
		 * 
		 * // we can't check for having lock contenders without being shared,
		 * since this can happen // in case an object is behind a FieldInfo
		 * shared-ness firewall (e.g. ThreadGroup.threads), or // is kept/used
		 * in native code (listener, peer) }
		 **/
	}

	protected void checkAssertion(boolean cond, String failMsg) {
		if (!cond) {
			System.out.println("!!!!!! failed ElementInfo consistency: " + this
					+ ": " + failMsg);

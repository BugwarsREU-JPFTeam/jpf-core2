 ...
 * Used by on-the-fly partial order reduction in FieldInstruction to determine
 * if a GET/PUT_FIELD/STATIC insn has to be treated as a boundary step
 * (terminates a transition). If the field access is always protected by a lock,
 * only the corresponding sync (INVOKExx or MONITORENTER) are boundary steps,
 * thus the number of states can be significantly reduced.
 * 
 * FieldLockInfos are only used if vm.por.sync_detection is set
 * 
 * NOTE this might involve assumptions that can be violated in subsequent paths,
 * and might cause potential races to go undetected
 */
public abstract class FieldLockInfo implements Cloneable {
	...
	ThreadInfo tiLastCheck; // the thread this FieldLockInfo was last checked
							// for

	public abstract FieldLockInfo checkProtection(ThreadInfo ti,
	...
	void lockAssumptionFailed(ThreadInfo ti, ElementInfo ei, FieldInfo fi) {
		String src = ti.getTopFrameMethodInfo().getClassInfo()
				.getSourceFileName();
		int line = ti.getLine();
...
/**
 * FieldLockSet implementation for fields that are terminally considered to be
 * unprotected
 */

 ...
 * 
 * The concrete Heap implementors have to provide the ElementInfo collection and
 * associated getters, allocators and iterators
 */
public abstract class GenericHeap implements Heap, Iterable<ElementInfo> {
	...
	static abstract class GenericHeapMemento implements Memento<Heap> {
		...
		public Heap restore(Heap inSitu) {
			...
			heap.liveBitValue = false; // always start with false after a
										// restore
			return inSitu;
		}
	}
	...
	protected class ElementInfoMarker implements Processor<ElementInfo> {
		...
		public void process(ElementInfo ei) {
			ei.markRecursive(GenericHeap.this); // this might in turn call
												// queueMark
		}
	}
	...
	// list of pinned down references (this is only efficient for a small number
	// of objects)
	...
	static final int ATTR_ANY_CHANGED = (ATTR_ELEMENTS_CHANGED
			| ATTR_PINDOWN_CHANGED | ATTR_INTERN_CHANGED | ATTR_ATTRIBUTE_CHANGED);

	// --- these objects are only used during gc

	// used to keep track of marked WeakRefs that might have to be updated (no
	// need to restore, only transient use during gc)
	...
	protected DynamicElementInfo createElementInfo(int objref, ClassInfo ci,
			Fields f, Monitor m, ThreadInfo ti) {
	...
	public void registerPinDown(int objref) {
		...
		if (ei != null) {
			...
			throw new JPFException("pinDown reference not a live object: "
					+ objref);
		}
	}
	 ...
	 * NOTE: this implementation requires our own Reference/WeakReference
	 * implementation, to make sure the 'ref' field is the first one
	 */
	protected void cleanupWeakRefs() {
		if (weakRefs != null) {
			for (ElementInfo ei : weakRefs) {
				Fields f = ei.getFields();
				int ref = f.getIntValue(0); // watch out, the 0 only works with
											// our own WeakReference impl
				if (ref != MJIEnv.NULL) {
					ElementInfo refEi = get(ref);
					if ((refEi == null) || (refEi.isNull())) {
						ei = ei.getModifiableInstance();
						// we need to make sure the Fields are properly state
						// managed
	...
	// NOTE - this is where to assert if this index isn't occupied yet, since
	// only concrete classes know
	// if there can be collisions, and how elements are stored

	protected abstract AllocationContext getSUTAllocationContext(ClassInfo ci,
			ThreadInfo ti);

	protected abstract AllocationContext getSystemAllocationContext(
			ClassInfo ci, ThreadInfo ti, int anchor);

	/**
	 * this is called for newXX(..) allocations that are SUT thread specific,
	 * i.e. in response to a explicit NEW or xNEWARRAY instruction that should
	 * take the allocating thread into account
	 */
	...
	protected ElementInfo createArray(String elementType, int nElements,
			ClassInfo ci, ThreadInfo ti, int objref) {

		Fields f = ci.createArrayFields(ci.getName(), nElements,
				Types.getTypeSize(elementType), Types.isReference(elementType));
		Monitor m = new Monitor();
	...
	protected ClassInfo getArrayClassInfo(ThreadInfo ti, String elementType) {
		...
		if (!ciArray.isInitialized()) {
			// we do this explicitly here since there are no clinits for array
			// classes
	...
	@Override
	public ElementInfo newSystemArray(String elementType, int nElements,
	...
	protected ElementInfo newString(ClassInfo ciString, ClassInfo ciChars,
			String str, ThreadInfo ti, AllocationContext ctx) {
		SystemClassLoaderInfo sysCl = ti.getSystemClassLoaderInfo();
	...
	public ElementInfo newSystemString(String str, ThreadInfo ti, int anchor) {
		if (str != null) {
			...
			AllocationContext ctx = getSystemAllocationContext(ciString, ti,
					anchor);
			return newString(ciString, ciChars, str, ti, ctx);
	...
	@Override
	public ElementInfo newSystemThrowable(ClassInfo ciThrowable,
			String details, int[] stackSnapshot, int causeRef, ThreadInfo ti,
			int anchor) {
		SystemClassLoaderInfo sysCl = ti.getSystemClassLoaderInfo();
		ClassInfo ciString = sysCl.getStringClassInfo();
		ClassInfo ciChars = sysCl.getCharArrayClassInfo();
		...
		AllocationContext ctx = getSystemAllocationContext(ciThrowable, ti,
				anchor);
		...
		if (details != null) {
			...
			ElementInfo eiMsg = newString(ciString, ciChars, details, ti,
					ctxString);
			eiThrowable
					.setReferenceField("detailMessage", eiMsg.getObjectRef());
		}
		...
		ElementInfo eiSnap = createArray("I", stackSnapshot.length, ciSnap, ti,
				snapRef);
		...
		// --- the cause field
		eiThrowable.setReferenceField("cause",
				(causeRef != MJIEnv.NULL) ? causeRef : xRef);
	...
	/*
	 * these methods abstract away the container type used in GenericHeap
	 * subclasses
	 */
	...
	/**
	 * called during non-recursive phase1 marking of all objects reachable from
	 * static fields
	 * 
	 * @aspects: gc
	 */
	...
	/**
	 * this implementation uses a generic ElementInfo iterator, it can be
	 * replaced with a more efficient container specific version
	 */
	protected void sweep() {
		...
		// now go over all objects, purge the ones that are not live and reset
		// attrs for rest
		for (ElementInfo ei : this) {

			if (ei.isMarked()) { // live object, prepare for next transition &
									// gc cycle
				ei.setUnmarked();
	...
	protected void markFinalizableObjects() {
		// Phase 1: find unmarked objects with finalizers that haven't been
		// processed yet
		// and mark them recursively to avoid them from being GCed. Their
		// finalizers must
	 ...
	 * called from KernelState to process live ElementInfos after GC has
	 * finished and only live objects remain in the heap.
	 * 
	 * <2do> full heap enumeration is BAD - check if this can be moved into the
	 * sweep loop
	 */
	@Override
	...
	/**
	 * check if object is alive. This is here and not in ElementInfo because we
	 * might own the liveness bit. In fact, the generic implementation uses
	 * bit-toggle to avoid iteration over all live objects at the end of GC
	 */
	@Override
	...
	// since we can't provide generic implementations, we force concrete
	// subclasses to

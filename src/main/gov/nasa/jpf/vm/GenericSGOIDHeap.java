...
public abstract class GenericSGOIDHeap extends GenericHeap {
	...
	// NOTE - this has to be included in the mementos of concrete Heap
	// implementations
	...
	// --- to be overridden by subclasses that use different AllocationContext
	// implementations
	...
	// these are always called directly from the allocation primitive, i.e. the
	// allocating site is at a fixed
	// stack offset (callers caller)
	@Override
	protected AllocationContext getSUTAllocationContext(ClassInfo ci,
			ThreadInfo ti) {
	...
	@Override
	protected AllocationContext getSystemAllocationContext(ClassInfo ci,
			ThreadInfo ti, int anchor) {
		return HashedAllocationContext.getSystemAllocationContext(ci, ti,
				anchor);
		// return PreciseAllocationContext.getSystemAllocationContext(ci, ti,
		// anchor);
	}
	...
	protected int getNewElementInfoIndex(AllocationContext ctx) {
		...
		// sanity check - we do this here (and not in our super class) since we
		// know how elements are stored
		// assert get(idx) == null;

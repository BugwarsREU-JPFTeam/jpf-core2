...
/**
 * native peer class for programmatic JPF interface (that can be used inside of
 * apps to verify - if you are aware of the danger that comes with it)
 * 
 * this peer is a bit different in that it only uses static fields and methods
 * because its use is supposed to be JPF global (without classloader namespaces)
 */
public class JPF_gov_nasa_jpf_vm_Verify extends NativePeer {
	...
	// those are used to store search global int values (e.g. from TestJPF
	// derived classes)
	static int[] counter;
	...
	@MJI
	public static int getValue__Ljava_lang_String_2__I(MJIEnv env,
			int clsObjRef, int keyRef) {
		if (map == null) {
			return NO_VALUE;
		} else {
	...
	@MJI
	public static void putValue__Ljava_lang_String_2I__V(MJIEnv env,
			int clsObjRef, int keyRef, int val) {
		if (map == null) {
	...
	public static int getCounter__I__I(MJIEnv env, int clsObjRef, int counterId) {
		if ((counter == null) || (counterId < 0)
				|| (counterId >= counter.length)) {
			return 0;
		}
	...
	private static void ensureCounterCapacity(int counterId) {
		if (counter == null) {
			counter = new int[(counterId >= MAX_COUNTERS) ? counterId + 1
					: MAX_COUNTERS];
	...
	@MJI
	public static void resetCounter__I__V(MJIEnv env, int clsObjRef,
			int counterId) {
		if ((counter == null) || (counterId < 0)
				|| (counterId >= counter.length)) {
			return;
	...
	@MJI
	public static void setCounter__II__V(MJIEnv env, int clsObjRef,
			int counterId, int val) {
		if (counterId < 0) {
			return;
		}
	...
	@MJI
	public static int incrementCounter__I__I(MJIEnv env, int clsObjRef,
			int counterId) {
		if (counterId < 0) {
			return 0;
		}
	...
	@MJI
	public static void setBitInBitSet__IIZ__V(MJIEnv env, int clsObjRef,
			int id, int bitNum, boolean value) {
		checkBitSetId(id);
		bitSets[id].set(bitNum, value);
	}

	@MJI
	public static boolean getBitInBitSet__II__Z(MJIEnv env, int clsObjRef,
			int id, int bitNum) {
		checkBitSetId(id);
	...
	@MJI
	public static void addComment__Ljava_lang_String_2__V(MJIEnv env,
			int clsObjRef, int stringRef) {
	...
	@MJI
	public static void breakTransition__Ljava_lang_String_2__V(MJIEnv env,
			int clsObjRef, int reasonRef) {
	...
	@MJI
	public static boolean isCalledFromClass__Ljava_lang_String_2__Z(MJIEnv env,
			int clsObjRef, int clsNameRef) {
	...
	static <T extends ChoiceGenerator<?>> T createChoiceGenerator(
			Class<T> cgClass, SystemState ss, String id) {
		T gen = null;

		cgArgs[0] = config;
		cgArgs[1] = id; // good thing we are not multithreaded (other fields are
						// const)
	...
	static <T> T registerChoiceGenerator(MJIEnv env, SystemState ss,
			ThreadInfo ti, ChoiceGenerator<T> cg, T dummyVal) {
	...
	static <T, C extends ChoiceGenerator<T>> T getNextChoice(SystemState ss,
			String id, Class<C> cgClass, Class<T> choiceClass) {
	...
	public static boolean getBoolean____Z(MJIEnv env, int clsObjRef) {
		...
		if (!ti.isFirstStepInsn()) { // first time around
			...
			return getNextChoice(ss, "verifyGetBoolean",
					BooleanChoiceGenerator.class, Boolean.class);
		}
	}

	@MJI
	public static boolean getBoolean__Z__Z(MJIEnv env, int clsObjRef,
			boolean falseFirst) {
		ThreadInfo ti = env.getThreadInfo();
		SystemState ss = env.getSystemState();
		ChoiceGenerator<?> cg;
		...
		if (!ti.isFirstStepInsn()) { // first time around
			...
			return getNextChoice(ss, "verifyGetBoolean(Z)",
					BooleanChoiceGenerator.class, Boolean.class);
		}
	}
	...
	public static int getInt__II__I(MJIEnv env, int clsObjRef, int min, int max) {
		...
		if (!ti.isFirstStepInsn()) { // first time around
			...
			IntChoiceGenerator cg = new IntIntervalGenerator(
					"verifyGetInt(II)", min, max);
			return registerChoiceGenerator(env, ss, ti, cg, 0);

		} else {
			return getNextChoice(ss, "verifyGetInt(II)",
					IntChoiceGenerator.class, Integer.class);
		}
	}
	...
	static int getIntFromList(MJIEnv env, int[] values) {
		...
		if (!ti.isFirstStepInsn()) { // first time around
			ChoiceGenerator<Integer> cg = new IntChoiceFromSet(
					"verifyGetIntSet([I)", values);
			return registerChoiceGenerator(env, ss, ti, cg, 0);

		} else {
			return getNextChoice(ss, "verifyGetIntSet([I)",
					IntChoiceGenerator.class, Integer.class);
		}
	}

	@MJI
	public static int getIntFromList___3I__I(MJIEnv env, int clsObjRef,
			int valArrayRef) {
	...
	@MJI
	public static int getInt__Ljava_lang_String_2__I(MJIEnv env, int clsObjRef,
			int idRef) {
		ThreadInfo ti = env.getThreadInfo();
		SystemState ss = env.getSystemState();
		...
		if (!ti.isFirstStepInsn()) { // first time around
			...
			IntChoiceGenerator cg = createChoiceGenerator(
					IntChoiceGenerator.class, ss, id);
			return registerChoiceGenerator(env, ss, ti, cg, 0);

		} else {
			String id = env.getStringObject(idRef);
			return getNextChoice(ss, id, IntChoiceGenerator.class,
					Integer.class);
		}
	}
	...
	static long getLongFromList(MJIEnv env, long[] values) {
		...
		if (!ti.isFirstStepInsn()) { // first time around
			ChoiceGenerator<Long> cg = new LongChoiceFromList(
					"verifyLongList([J)", values);
			return registerChoiceGenerator(env, ss, ti, cg, 0L);

		} else {
			return getNextChoice(ss, "verifyLongList([J)",
					LongChoiceGenerator.class, Long.class);
		}
	}

	@MJI
	public static long getLongFromList___3J__J(MJIEnv env, int clsObjRef,
			int valArrayRef) {
	...
	@MJI
	public static int getObject__Ljava_lang_String_2__Ljava_lang_Object_2(
			MJIEnv env, int clsObjRef, int idRef) {
		ThreadInfo ti = env.getThreadInfo();
		SystemState ss = env.getSystemState();

		if (!ti.isFirstStepInsn()) { // first time around
			String id = env.getStringObject(idRef);
			ReferenceChoiceGenerator cg = createChoiceGenerator(
					ReferenceChoiceGenerator.class, ss, id);
			return registerChoiceGenerator(env, ss, ti, cg, 0);

		} else {
			String id = env.getStringObject(idRef);
			return getNextChoice(ss, id, ReferenceChoiceGenerator.class,
					Integer.class);
		}
	}

	@MJI
	public static double getDouble__Ljava_lang_String_2__D(MJIEnv env,
			int clsObjRef, int idRef) {
		ThreadInfo ti = env.getThreadInfo();
		SystemState ss = env.getSystemState();

		if (!ti.isFirstStepInsn()) { // first time around
			String id = env.getStringObject(idRef);
			DoubleChoiceGenerator cg = createChoiceGenerator(
					DoubleChoiceGenerator.class, ss, id);
			return registerChoiceGenerator(env, ss, ti, cg, 0.0);

		} else {
			String id = env.getStringObject(idRef);
			return getNextChoice(ss, id, DoubleChoiceGenerator.class,
					Double.class);
		}
	}
	...
	public static double getDoubleFromList(MJIEnv env, double[] values) {
		...
		if (!ti.isFirstStepInsn()) { // first time around
			ChoiceGenerator<Double> cg = new DoubleChoiceFromList(
					"verifyDoubleList([D)", values);
			return registerChoiceGenerator(env, ss, ti, cg, 0.0);

		} else {
			return getNextChoice(ss, "verifyDoubleList([D)",
					DoubleChoiceFromList.class, Double.class);
		}
	}

	@MJI
	public static double getDoubleFromList___3D__D(MJIEnv env, int clsObjRef,
			int valArrayRef) {
	...
	public static float getFloatFromList(MJIEnv env, float[] values) {
		...
		if (!ti.isFirstStepInsn()) { // first time around
			ChoiceGenerator<Float> cg = new FloatChoiceFromList(
					"verifyFloatList([F)", values);
			return registerChoiceGenerator(env, ss, ti, cg, 0.0f);

		} else {
			return getNextChoice(ss, "verifyFloatList([F)",
					FloatChoiceFromList.class, Float.class);
		}
	}

	@MJI
	public static float getFloatFromList___3F__F(MJIEnv env, int clsObjRef,
			int valArrayRef) {
	...
	@MJI
	public static void print__Ljava_lang_String_2I__V(MJIEnv env, int clsRef,
			int sRef, int val) {
	...
	@MJI
	public static void print__Ljava_lang_String_2Z__V(MJIEnv env, int clsRef,
			int sRef, boolean val) {
		String s = env.getStringObject(sRef);
		System.out.println(s + " : " + val);
	}

	@MJI
	public static void print___3Ljava_lang_String_2__V(MJIEnv env, int clsRef,
			int argsRef) {
	...
	@MJI
	public static void print__Ljava_lang_String_2__V(MJIEnv env, int clsRef,
			int sRef) {
		String s = env.getStringObject(sRef);
		System.out.print(s);
	}
	...
	@MJI
	public static void println__Ljava_lang_String_2__V(MJIEnv env, int clsRef,
			int sRef) {
		String s = env.getStringObject(sRef);
		...
		if (a != null) {
			if (a instanceof Integer) {
				...
				env.throwException("java.lang.RuntimeException",
						"element attribute not an Integer: " + a);
			}
		}
	...
	@MJI
	public static void setObjectAttribute__Ljava_lang_Object_2I__V(MJIEnv env,
			int clsRef, int oRef, int attr) {
		if (oRef != MJIEnv.NULL) {
	...
	@MJI
	public static int getObjectAttribute__Ljava_lang_Object_2__I(MJIEnv env,
			int clsRef, int oRef) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
	...
	@MJI
	public static void addObjectAttribute__Ljava_lang_Object_2I__V(MJIEnv env,
			int clsRef, int oRef, int attr) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
	...
	@MJI
	public static int getObjectAttributes__Ljava_lang_Object_2___3I(MJIEnv env,
			int clsRef, int oRef) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
	...
	@MJI
	public static void setFieldAttribute__Ljava_lang_Object_2Ljava_lang_String_2I__V(
			MJIEnv env, int clsRef, int oRef, int fnRef, int attr) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
			if (ei != null) {
				...
				if (fi != null) {
					...
					env.throwException("java.lang.NoSuchFieldException", ei
							.getClassInfo().getName() + '.' + fname);
				}
			} else {
				env.throwException("java.lang.RuntimeException",
						"illegal reference value: " + oRef);
			}
		}
	}

	@MJI
	public static int getFieldAttribute__Ljava_lang_Object_2Ljava_lang_String_2__I(
			MJIEnv env, int clsRef, int oRef, int fnRef) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
			if (ei != null) {
				String fname = env.getStringObject(fnRef);
	...
	@MJI
	public static void addFieldAttribute__Ljava_lang_Object_2Ljava_lang_String_2I__V(
			MJIEnv env, int clsRef, int oRef, int fnRef, int attr) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
			if (ei != null) {
	...
	@MJI
	public static int getFieldAttributes__Ljava_lang_Object_2Ljava_lang_String_2___3I(
			MJIEnv env, int clsRef, int oRef, int fnRef) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
			if (ei != null) {
	...
	@MJI
	public static void setLocalAttribute__Ljava_lang_String_2I__V(MJIEnv env,
			int clsRef, int varRef, int attr) {
		String slotName = env.getStringObject(varRef);
		StackFrame frame = env.getModifiableCallerStackFrame(); // we are
																// executing in
																// a
																// NativeStackFrame

		if (!frame.getMethodInfo().isStatic() && slotName.equals("this")) {
			frame.setLocalAttr(0, Integer.valueOf(attr)); // only for instance
															// methods of course
			...
			if (slotIdx >= 0) {
				...
				env.throwException("java.lang.RuntimeException",
						"local variable not found: " + slotName);
			}
		}
	}

	@MJI
	public static int getLocalAttribute__Ljava_lang_String_2__I(MJIEnv env,
			int clsRef, int varRef) {
		String slotName = env.getStringObject(varRef);
		ThreadInfo ti = env.getThreadInfo();
	...
	@MJI
	public static void addLocalAttribute__Ljava_lang_String_2I__V(MJIEnv env,
			int clsRef, int varRef, int attr) {
		String slotName = env.getStringObject(varRef);
		StackFrame frame = env.getModifiableCallerStackFrame(); // we are
																// executing in
		...
		if (!frame.getMethodInfo().isStatic() && slotName.equals("this")) {
			frame.addLocalAttr(0, Integer.valueOf(attr)); // only for instance
															// methods of course
	...
	@MJI
	public static int getLocalAttributes__Ljava_lang_String_2___3I(MJIEnv env,
			int clsRef, int varRef) {
		String slotName = env.getStringObject(varRef);
		ThreadInfo ti = env.getThreadInfo();
		StackFrame frame = env.getCallerStackFrame();
	...
	@MJI
	public static void setElementAttribute__Ljava_lang_Object_2II__V(
			MJIEnv env, int clsRef, int oRef, int idx, int attr) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
	...
	@MJI
	public static int getElementAttribute__Ljava_lang_Object_2I__I(MJIEnv env,
			int clsRef, int oRef, int idx) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
	...
	@MJI
	public static void addElementAttribute__Ljava_lang_Object_2II__V(
			MJIEnv env, int clsRef, int oRef, int idx, int attr) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
	...
	@MJI
	public static int getElementAttributes__Ljava_lang_Object_2I___3I(
			MJIEnv env, int clsRef, int oRef, int idx) {
		if (oRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(oRef);
			if (ei != null) {
				if (ei.isArray()) {
	...
	@MJI
	public static void storeTrace__Ljava_lang_String_2Ljava_lang_String_2__V(
			MJIEnv env, int clsObjRef, int filenameRef, int commentRef) {
		...
		env.getVM().storeTrace(fileName, comment,
				config.getBoolean("trace.verbose", false));
	}
	...
	@MJI
	public static boolean isShared__Ljava_lang_Object_2__Z(MJIEnv env,
			int clsObjRef, int objRef) {
	...
	@MJI
	public static void setShared__Ljava_lang_Object_2Z__V(MJIEnv env,
			int clsObjRef, int objRef, boolean isShared) {
		if (objRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(objRef);
			if (ei != null) {
				if (ei.getClassInfo() == ClassLoaderInfo
						.getCurrentSystemClassLoader().getClassClassInfo()) {
	...
	@MJI
	public static void freezeSharedness__Ljava_lang_Object_2Z__V(MJIEnv env,
			int clsObjRef, int objRef, boolean freeze) {
		if (objRef != MJIEnv.NULL) {
			ElementInfo ei = env.getElementInfo(objRef);
			if (ei != null) {
				if (ei.getClassInfo() == ClassLoaderInfo
	...
	@MJI
	public static void setProperties___3Ljava_lang_String_2__V(MJIEnv env,
			int clsObjRef, int argRef) {
	...
	public static int getProperty__Ljava_lang_String_2__Ljava_lang_String_2(
			MJIEnv env, int clsObjRef, int keyRef) {
	...
	@MJI
	public static void printPathOutput__ZLjava_lang_String_2__V(MJIEnv env,
			int clsObjRef, boolean cond, int msgRef) {
		if (cond) {
	...
	@MJI
	public static void printPathOutput__Ljava_lang_String_2__V(MJIEnv env,
			int clsObjRef, int msgRef) {
		VM vm = env.getVM();
		...
		if (msgRef != MJIEnv.NULL) {
			...
			System.out
					.println("~~~~~~~~~~~~~~~~~~~~~~~ begin program output at: "
							+ msg);
		} else {
		...
		// we might be in the middle of a transition that isn't stored yet in
		// the path
	...
	public static int createFromJSON__Ljava_lang_Class_2Ljava_lang_String_2__Ljava_lang_Object_2(
		...
		if (jsonObject != null) {
			...
			// check if we need any class init (and hence reexecution) before
			// creating any CGs
			...
			if (!ti.isFirstStepInsn()) {
				// Top half - get and register CGs we need to set to fill object
				// from JSON
				List<ChoiceGenerator<?>> cgList = CGCall
						.createCGList(jsonObject);
				if (cgList.isEmpty()) {
	...
	public static int readObjectFromFile__Ljava_lang_Class_2Ljava_lang_String_2__Ljava_lang_Object_2(
		...
		try {
			...
			String readObjectTypeName = javaObject.getClass()
					.getCanonicalName();

			int readObjRef = ObjectConverter.JPFObjectFromJavaObject(env,
					javaObject);
			...
			throw new JPFException("failure reading object from file: "
					+ fileName, iox);
	...
	private static void log(JPFLogger logger, int logLevel, String msg) {
		...
		case FINEST:
			...
			break;
		default:
			throw new JPFException("unknown log level " + logLevel
					+ " for logger " + logger.getName());
		}
	}

	@MJI
	public static void log__Ljava_lang_String_2ILjava_lang_String_2__V(
			MJIEnv env, int clsObjRef, int loggerIdRef, int logLevel, int msgRef) {
	...
	@MJI
	public static void log__Ljava_lang_String_2ILjava_lang_String_2Ljava_lang_String_2__V(
			MJIEnv env, int clsObjRef, int loggerIdRef, int logLevel,
			int arg1Ref, int arg2Ref) {
		String loggerId = env.getStringObject(loggerIdRef);
		String msg = env.getStringObject(arg1Ref)
				+ env.getStringObject(arg2Ref);
		JPFLogger logger = JPF.getLogger(loggerId);
	...
	@MJI
	public static void log__Ljava_lang_String_2ILjava_lang_String_2_3Ljava_lang_Object_2__V(
			MJIEnv env, int clsObjRef, int loggerIdRef, int logLevel,
			int fmtRef, int argsRef) {
		String loggerId = env.getStringObject(loggerIdRef);

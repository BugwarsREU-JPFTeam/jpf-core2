...
public class JenkinsStateSet extends SerializingStateSet {
	...
	public int add(int[] val) {
		...
		if (lastStateId >= nextRehash) { // too full
			...
			while (hashtable[idx] != 0) { // we know enough slots exist and
											// state is
											// new
				idx = (idx + delta) & mask;
			}
		...
		try { // this happens rarely enough to save on nominal branch
				// instructions
	...
	void growFingerprint(int minSize) {
		...
		System.arraycopy(fingerprints, 0, newFingerprints, 0,
				fingerprints.length);

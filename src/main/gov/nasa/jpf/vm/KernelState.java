...
public class KernelState implements Restorable<KernelState> {
	...
	public KernelState(Config config) {
		...
		heap = config.getEssentialInstance("vm.heap.class", Heap.class,
				argTypes, args);
		threads = config.getEssentialInstance("vm.threadlist.class",
				ThreadList.class, argTypes, args);
	}
	...
	/**
	 * called by internals to indicate a change in KernelState. list of
			...
			for (ChangeListener l : listeners) {
				if (l instanceof IncrementalChangeTracker) {
					throw new IllegalStateException(
							"Only one IncrementalChangeTracker allowed!");
				}
			}
		}
		listeners.push(cl);
	...
	public void gc() {
		...
		// <2do> get rid of this by storing objects instead of ref/id values
		// that are reused

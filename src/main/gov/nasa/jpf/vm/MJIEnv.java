...
/**
 * MJIEnv is the call environment for "native" methods, i.e. code that is
 * executed by the VM, not by JPF.
 * 
 * Since library abstractions are supposed to be "user code", we provide this
 * class as a (little bit of) insulation towards the inner JPF workings.
 * 
 ...
 * gov.nasa.jpf.vm NativePeer classes). The rest is package-default and can be
 * used to fiddle around as much as you like to (if you are in the ..jvm
 * package)
 * 
 * Note that MJIEnv objects are now per-ThreadInfo (i.e. the variable call
 * envionment only includes MethodInfo and ClassInfo), which means MJIEnv can be
 * used in non-native methods (but only carefully, if you don't need mi or
 * ciMth).
 * 
 * Note also this only works because we are not getting recursive in native
 * method calls. In fact, the whole DirectCallStackFrame / repeatTopInstruction
 * mechanism is there to turn logial recursion (JPF calling native, calling JPF,
 * calling native,..) into iteration. Otherwise we couldn't backtrack
 */
public class MJIEnv {
	...
	// an ElementInfo reference, it should use that one to
	// retrieve/enumerate/set
	...
	public void setDeclaredIntField(int objref, String refType, String fname,
			int val) {
		ElementInfo ei = heap.getModifiable(objref);
	...
	// these two are the workhorses
	public void setDeclaredReferenceField(int objref, String refType,
			String fname, int val) {
		ElementInfo ei = heap.getModifiable(objref);
	...
	// public void setLongField (int objref, String refType, String fname, long
	// val) {
	// ElementInfo ei = heap.get(objref);
	...
	// <2do> this uses the current system CL, we should probably use an explicit
	// CL argument
	...
	/**
	 * turn JPF String object into a VM String object (this is a method
	 * available for non gov..jvm NativePeer classes)
	 */
	...
	// danger - the returned arrays could be used to modify contents of stored
	// objects
	...
	/**
	 * check if the ClassInfo is properly initialized if yes, create a new
	 * instance of it but don't call any ctor if no, throw a ClinitRequired
	 * exception
	 */
	 ...
	 * initialization. This is useful in a context that already is aware and
	...
	public int newBoolean(boolean b) {
		return getStaticReferenceField("java.lang.Boolean", b ? "TRUE"
				: "FALSE");
	}

	public int newInteger(int n) {
		ElementInfo ei = heap
				.newObject(ClassLoaderInfo
						.getSystemResolvedClassInfo("java.lang.Integer"), ti);
		ei.setIntField("value", n);
		return ei.getObjectRef();
	}

	public int newLong(long l) {
		ElementInfo ei = heap.newObject(
				ClassLoaderInfo.getSystemResolvedClassInfo("java.lang.Long"),
				ti);
	...
	public int newDouble(double d) {
		ElementInfo ei = heap.newObject(
				ClassLoaderInfo.getSystemResolvedClassInfo("java.lang.Double"),
				ti);
	...
	public int newFloat(float f) {
		ElementInfo ei = heap.newObject(
				ClassLoaderInfo.getSystemResolvedClassInfo("java.lang.Float"),
				ti);
	...
	public int newByte(byte b) {
		ElementInfo ei = heap.newObject(
				ClassLoaderInfo.getSystemResolvedClassInfo("java.lang.Byte"),
				ti);
	...
	public int newShort(short s) {
		ElementInfo ei = heap.newObject(
				ClassLoaderInfo.getSystemResolvedClassInfo("java.lang.Short"),
				ti);
	...
	public int newCharacter(char c) {
		ElementInfo ei = heap.newObject(ClassLoaderInfo
				.getSystemResolvedClassInfo("java.lang.Character"), ti);
	...
	/**
	 * use this whenever a peer performs an operation on a class that might not
	 * be initialized yet Do a repeatInvocation() in this case
	 */
	...
	/**
	 * repeat execution of the instruction that caused a native method call NOTE
	 * - this does NOT mean it's the NEXT executed insn, since the native method
	 * might have pushed direct call frames on the stack before asking us to
	 * repeat it.
	 */
	...
	/**
	 * return attr list of all arguments. Use ObjectList to retrieve values from
	 * this list
	 * 
	...
	void setCallEnvironment(MethodInfo mi) {
		...
		// at the beginning of the transition. It gets reset upon return from
		// the
		// native method
		// exceptionRef = NULL;
	}
	...
	public boolean hasPendingInterrupt() {
		return (exceptionRef != NULL && isInstanceOf(exceptionRef,
	...
	/**
	 * It returns the ClassLoaderInfo corresponding to the given classloader
	 * object reference
	 */
	...
	// <2do> that's not correct - it should return the current
	// SystemClassLoader, NOT the startup SystemClassLoader
	...
	/**
	 * NOTE - callers have to be prepared this might return null in case the
	 * thread got already terminated
	 */
	...
	void initAnnotationProxyField(int proxyRef, FieldInfo fi, Object v)
		...
		if (v instanceof String) {
			...
			ClassInfo cci = ClassLoaderInfo
					.getCurrentResolvedClassInfo(clsName);
			...
			if (ftype.equals("java.lang.String[]")) {
				...
				for (int i = 0; i < a.length; i++) {
					setReferenceArrayElement(aref, i,
				...
				for (int i = 0; i < a.length; i++) {
					setBooleanArrayElement(aref, i,
							((Boolean) a[i]).booleanValue());
				}
	...
	int newAnnotationProxies(AnnotationInfo[] ai) throws ClinitRequired {
		...
		if ((ai != null) && (ai.length > 0)) {
			int aref = newObjectArray("Ljava/lang/annotation/Annotation;",
					ai.length);
			for (int i = 0; i < ai.length; i++) {
				ClassInfo aci = ClassLoaderInfo
						.getCurrentResolvedClassInfo(ai[i].getName());
			...
			int aref = getStaticReferenceField("java.lang.Class",
					"emptyAnnotations");
			if (aref == NULL) {
				aref = newObjectArray("Ljava/lang/annotation/Annotation;", 0);
				setStaticReferenceField("java.lang.Class", "emptyAnnotations",
						aref);
			}
			return aref;
		}

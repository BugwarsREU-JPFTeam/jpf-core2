...
/**
 * information associated with a method. Each method in JPF is represented by a
 * MethodInfo object
 */
public class MethodInfo extends InfoObject implements GenericSignatureHolder {
	...
	protected static final ArrayList<MethodInfo> mthTable = new ArrayList<MethodInfo>(
			INIT_MTH_SIZE);
	...
	static final int FIREWALL = 0x40000; // firewall any unhandled
											// exceptionHandlers
											// (turn into UnhandledException
											// throws)
	static final int IS_CLINIT = 0x80000;
	static final int IS_INIT = 0x100000;

	static final int IS_REFLECTION = 0x200000; // this is a reflection direct
												// call
	...
	/**
	 * this is a lazy evaluated mangled name consisting of the name and arg type
	 * signature
	 */
	...
	/**
	 * Table used for line numbers this assigns a line number to every
	 * instruction index, instead of using an array of ranges. Assuming we have
	 * 2-3 insns per line on average, this should still require less memory than
	 * a reference array with associated range objects, and allows faster access
	 * of instruction line numbers, which we might need for location specs
	 */
	...
	public static MethodInfo create(ClassInfo ci, String name,
			String signature, int modifiers) {
		return new MethodInfo(ci, name, signature, modifiers);
	}

	static MethodInfo create(ClassInfo ci, String name, String signature,
			int modifiers, int maxLocals, int maxStack) {
		return new MethodInfo(ci, name, signature, modifiers, maxLocals,
				maxStack);
	}

	/**
	 * for direct call construction Note: this is only a partial initialization,
	 * the code still has to be created/installed by the caller
	 */
	public MethodInfo(MethodInfo callee, int nLocals, int nOperands) {
		globalId = DIRECT_CALL;
		// we don't want direct call methods in the mthTable (would be a memory
		// leak) so don't register

		ci = callee.ci;
		name = "[" + callee.name + ']'; // it doesn't allocate anything, so we
		...
		// we need to preserve the ClassInfo so that class resolution for static
		// method calls works
		ci = callee.ci;
	...
	public MethodInfo(MethodInfo mi) {
		...
		localVars = null; // there are no StackFrame localVarInfos, this is
							// native
		// code still has to be installed by caller
	}
	...
	// since some listeners might call this on every method invocation, we
	// should do a little optimization
	...
	static AnnotationInfo[][] NO_PARAMETER_ANNOTATIONS_2 = {
			new AnnotationInfo[0], new AnnotationInfo[0] };
	static AnnotationInfo[][] NO_PARAMETER_ANNOTATIONS_3 = {
			new AnnotationInfo[0], new AnnotationInfo[0], new AnnotationInfo[0] };

	public AnnotationInfo[][] getParameterAnnotations() {
		if (parameterAnnotations == null) { // keep this similar to
											// getAnnotations()
			int n = getNumberOfArguments();
			switch (n) {
			case 0:
				return NO_PARAMETER_ANNOTATIONS_0;
			case 1:
				return NO_PARAMETER_ANNOTATIONS_1;
			case 2:
				return NO_PARAMETER_ANNOTATIONS_2;
			case 3:
				return NO_PARAMETER_ANNOTATIONS_3;
			default:
	...
	/**
	 * turn unhandled exceptionHandlers at the JPF execution level into
	 * UnhandledException throws at the host VM level this is useful to
	 * implement firewalls for direct calls which should not let
	 * exceptionHandlers permeate into bytecode/ application code
	 */
	...
	/**
	 * yet another name - this time with a non-mangled, but abbreviated
	 * signature and without return type (e.g. like "main(String[])"
	 */
	...
	/**
	 * return the minimal name that has to be unique for overloading used as a
	 * lookup key NOTE: with the silent introduction of covariant return types
	...
	/**
	 * return only the LocalVarInfos for arguments, in order of definition or
	 * null if there are no localVarInfos. throw a JPFException if there are
	 * more immediately in scope vars than args
	 * 
	 * NOTE - it is perfectly legal for a method to have arguments but no
	 * LocalVarInfos, which are code attributes, clients have to check for a
	 * non-null return value even if the method has arguments. Note also that
	 * abstract / interface methods don't have code and hence no LocalVarInfos
	 */
	public LocalVarInfo[] getArgumentLocalVars() {
		if (localVars == null) { // shortcut in case we don't have args or
									// localVars;
			return null;
		}
		...
		if (nArgs == 0) {
			return new LocalVarInfo[0]; // rare enough so that we don't use a
										// static
		}
			...
			if (lvi.getStartPC() == 0) {
				if (n == nArgs) { // ARGH - more in-scope vars than args
					throw new JPFException(
							"inconsistent localVar table for method "
									+ getFullName());
				}

				// order with respect to slot index - since this might get
				// called
				// frequently, we don't use java.util.Arrays.sort() but sort in
				// on-the-fly. Note that we can have several localVar entries
				// for the
	...
	public boolean includesLine(int line) {
		int len = code.length;
		return (code[0].getLineNumber() <= line)
				&& (code[len].getLineNumber() >= line);
	}
	...
	public Instruction[] getInstructionsForLineInterval(int l1, int l2) {
		...
		// instruction line numbers don't have to be monotonic (they can
		// decrease for loops)
		...
		if (c != null) {
			...
			if (matchingInsns == null) {
				...
				return matchingInsns.toArray(new Instruction[matchingInsns
						.size()]);
			}
	...
	public Instruction[] getMatchingInstructions(LocationSpec lspec) {
		return getInstructionsForLineInterval(lspec.getFromLine(),
				lspec.getToLine());
	}
	...
	public boolean containsLineNumber(int n) {
		if (lineNumbers != null) {
			return (lineNumbers[0] <= n)
					&& (lineNumbers[lineNumbers.length - 1] <= n);
		}
	...
	public boolean intersectsLineNumbers(int first, int last) {
		if (lineNumbers != null) {
			if ((last < lineNumbers[0])
					|| (first > lineNumbers[lineNumbers.length - 1])) {
				return false;
			}
			return true;
		}
	...
	public ExceptionHandler getHandlerFor(ClassInfo ciException,
			Instruction insn) {
		if (exceptionHandlers != null) {
			int position = insn.getPosition();
			for (int i = 0; i < exceptionHandlers.length; i++) {
				ExceptionHandler handler = exceptionHandlers[i];
				if ((position >= handler.getBegin())
						&& (position < handler.getEnd())) {
					// checks if this type of exception is caught here (null
					// means 'any')
	...
	/**
	 * note that this might contain duplicates for variables with multiple scope
	 * entries
	 */
	...
	/**
	 * Returns the size of the arguments. This returns the number of parameters
	 * passed on the stack, incl. 'this'
	 */
	...
	public int getNumberOfCallerStackSlots() {
		return Types.getNumberOfStackSlots(signature, isStatic()); // includes
																	// return
																	// type
	}
	...
	public boolean isJPFInternal() {
		...
		// is defined in VM spec 4.7.8. What we mean here is that this
		// MethodInfo
		// is not associated with any class (such as direct call MethodInfos),
		// but
	...
	protected void setException(int index, String exceptionType) {
		thrownExceptionClassNames[index] = Types
				.getClassNameFromTypeName(exceptionType);
	}
	...
	protected void setExceptionHandler(int index, int startPc, int endPc,
			int handlerPc, String catchType) {
	...
	protected void setLocalVar(int index, String varName, String descriptor,
			int scopeStartPc, int scopeEndPc, int slotIndex) {
		localVars[index] = new LocalVarInfo(varName, descriptor, "",
	...
	/**
	 * note - this depends on that we already have a code array and that the
	 * lines/startPcs are sorted (monotonic increasing)
	 */
	...
	/**
	 * this version takes an already expanded line number array which has to be
	 * of the same size as the code array
	 */
	...
	public void dump() {
		...
		for (int i = 0; i < code.length; i++) {
			System.out.printf("%2d [%d]: %s\n", i, code[i].getPosition(),
					code[i].toString());
		}
	}

	/**
	 * Creates a method for a given class, by cloning this MethodInfo and all
	 * the instructions belong to the method
	 */

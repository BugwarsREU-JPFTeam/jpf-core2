...
public class Monitor implements Cloneable {
	...
	/**
	 * the list of threads that try to acquire the lock (can be in blocked,
	 * waiting, interrupted or running state).
	 */
	...
	public Monitor clone() {
		try {
			// no need to clone the empty set (which should be the majority of
			// cases)
	...
	static ThreadInfo[] add(ThreadInfo[] list, ThreadInfo ti) {
		...
		if (containsLocked(list, ti)) {
			...
			// re-park/join from their respective handlers (they don't hold
			// locks)
			return list;
		}

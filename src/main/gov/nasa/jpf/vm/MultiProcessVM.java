...
/**
 * A VM implementation that simulates running multiple applications within the
 * same JPF process (centralized model checking of distributed applications).
 * This is achieved by executing each application in a separate thread group,
 * using separate SystemClassLoader instances to ensure proper separation of
 * types / static fields.
 * 
 * 
 * @author Nastaran Shafiei <nastaran.shafiei@gmail.com>
 * 
 *         To use this jpf.properties includes, vm.class =
...
public class MultiProcessVM extends VM {
	...
	public MultiProcessVM(JPF jpf, Config conf) {
		...
		appDaemonRunnablePredicate = new MultiProcessPredicate() {
			@Override
			public boolean isTrue(ThreadInfo t) {
				return (this.appCtx == t.appCtx && t.isRunnable() && t
						.isDaemon());
			}
		};
	...
	/**
	 * <2do> this should also handle command line specs such as "jpf ... tgt1
	 * tgt1_arg ... -- tgt2 tgt2_arg ...
	 */
	ApplicationContext[] createApplicationContexts() {
		...
		if (targets == null) {
			throw new JPFConfigException(
					"no applications specified, check 'target.N' settings");
		}

		ArrayList<ApplicationContext> list = new ArrayList<ApplicationContext>(
				targets.length);
		for (int i = 0; i < targets.length; i++) {
			if (targets[i] != null) { // there might be holes in the array
				...
				if (!isValidClassName(clsName)) {
					throw new JPFConfigException(
							"main class not a valid class name: " + clsName);
				}
				...
				String mainEntry = config.getString(entryKey,
						"main([Ljava/lang/String;)V");

				String host = config.getString(hostKey, "localhost");

				SystemClassLoaderInfo sysCli = createSystemClassLoaderInfo(list
						.size());

				ApplicationContext appCtx = new ApplicationContext(i, clsName,
						mainEntry, args, host, sysCli);
				list.add(appCtx);
			}
		}
	...
	protected ChoiceGenerator<?> getInitialCG() {
		ThreadInfo[] runnables = getThreadList().getAllMatching(
	...
	public String getSUTDescription() {
		...
		for (int i = 0; i < appCtxs.length; i++) {
			if (i > 0) {
				sb.append('+'); // "||" would be more fitting, but would screw
								// up filenames
			}
	...
	public boolean isEndState() {
		boolean hasNonTerminatedDaemon = getThreadList().hasAnyMatching(
				getUserLiveNonDaemonPredicate());
		boolean hasRunnable = getThreadList().hasAnyMatching(
				getUserTimedoutRunnablePredicate());
		boolean isEndState = !(hasNonTerminatedDaemon && hasRunnable);

		if (processFinalizers) {
			if (isEndState) {
				int n = getThreadList().getMatchingCount(
						systemRunnablePredicate);
				if (n > 0) {
					return false;
				}
			}
	...
	@Override
	// Note - for now we just check for global deadlocks not the local ones
	...
	public boolean isDeadlocked() {
		...
		for (int i = 0; i < len; i++) {
			...
			if (ti.isAlive()) {
				...
				// shortcut - if there is at least one runnable, we are not
				// deadlocked
	...
	public void terminateProcess(ThreadInfo ti) {
		...
		ThreadInfo[] appThreads = getThreadList().getAllMatching(
				getAppPredicate(ti));
		...
		for (int i = 0; i < appThreads.length; i++) {
			...
			// if finalizers have to be processed, FinalizerThread is not killed
			// at this
			// point. We need to keep it around in case fianlizable objects are
			// GCed after
			// System.exit() returns.
			if (processFinalizers && t.isSystemThread()) {
				finalizerTi = t;
			} else {
				// keep the stack frames around, so that we can inspect the
				// snapshot
				t.setTerminated();
			}
		}
		...
		if (tl.hasAnyMatching(getAlivePredicate())) {
			...
			cg = new MultiProcessThreadChoice("PROCESS_TERMINATE", runnables,
					true);
		} else {
		...
		// if there is a finalizer thread, we have to run the last GC, to queue
		// finalizable objects, if any
		if (finalizerTi != null) {
	...
	/**
	 * Returns a predicate used to obtain all the threads that belong to the
	 * same application as ti
	 */

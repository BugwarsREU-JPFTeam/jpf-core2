...
public class NativeMethodInfo extends MethodInfo {
	...
	public Instruction executeNative(ThreadInfo ti) {
		...
		try {
			...
			if (env.hasException()) {
				// even though we should prefer throwing normal
				// exceptionHandlers,
			...
			if (top == nativeFrame) { // no roundtrips, straight return
				...
				if (env.isInvocationRepeated()) {
					...
					// we don't have to do a ti.topClone() because the last insn
					// left
					// is NATIVERETURN. Even if a listener creates a CG on it,
					// it won't
				...
				// direct calls from within the native method, i.e. nativeFrame
				// is not
				// on top anymore, but its current instruction (invoke) will be
				// reexecuted
				// because DirectCallStackFrames don't advance the pc of the new
				// top top upon return
				return top.getPC();
			}
			...
			if (itx.getTargetException() instanceof ClassInfoException) {
				ClassInfoException cie = (ClassInfoException) itx
						.getTargetException();
				return ti.createAndThrowException(cie.getExceptionClass(),
						cie.getMessage());
			}

...
/**
 * native peer classes are part of MJI and contain the code that is executed by
 * the host VM (i.e. outside the state-tracked JPF VM). Each class executed by
 * JPF that has native mehods must have a native peer class (which is looked up
 * and associated at class loadtime)
 */
public class NativePeer implements Cloneable {
	...
	static String[] getPeerPackages(Config conf) {
		String[] defPeerPackages = { MODEL_PACKAGE, "gov.nasa.jpf.vm",
				DEFAULT_PACKAGE };
		String[] packages = conf.getStringArray("peer_packages",
				defPeerPackages);
		...
		for (int i = 0; i < peerPackages.length; i++) {
			...
			try {
				...
				if ((peerCls.getModifiers() & Modifier.PUBLIC) == 0) {
					...
					continue; // pointless to use this one, it would just create
	 ...
	 * derived class object. Watch out - this gets called before the ClassInfo
	 * is fully initialized (we shouldn't rely on more than just its name here)
	 */
	...
	public static <T> T getInstance(Class<?> cls, Class<T> type)
			throws JPFException {
	...
	public static <T> T getInstance(Class<?> cls, Class<T> type,
			Class<?>[] argTypes, Object[] args) throws JPFException {
		Object o = null;
			...
			try {
				...
				if ((argTypes.length > 1)
					...
					throw new JPFException(
							"no suitable ctor found for the peer class "
									+ cls.getName());
				}
		...
		// check type
		if (!cls.isInstance(o)) {
			throw new JPFException("instance not of type: " + cls.getName());
		}
	...
	protected void initialize(Class<?> peerClass, ClassInfo ci,
		...
		try {
			...
			try {
				...
				throw new RuntimeException(
						"peer initialization method not accessible: "
								+ cls.getName());
	...
	private static boolean isMJICandidate(Method mth) {
		...
		if ((argTypes.length >= 2) && (argTypes[0] == MJIEnv.class)
	...
	/**
	 * look at all public static methods in the peer and set their corresponding
	 * model class MethodInfo attributes <2do> pcm - this is too long, break it
	 * down
	 */
	private void loadMethods(boolean cacheMethods) {
		...
		for (int i = 0; i < m.length; i++) {
			...
			if (isMJICandidate(mth)) {
				// Note that we can't mangle the name automatically, since we
				// loose the
				...
				// JNI doesn't allow <clinit> or <init> to be native, but MJI
				// does
				// (you should know what you are doing before you use that,
				// really)
				...
				// as 'peer-ed'
				// <2do> in case of <clinit>, it wouldn't be strictly required
				// to
				// have a MethodInfo upfront (we could create it). Might be
				// handy
				...
				if ((mi == null) && (sig == null)) {
					// nothing found, we have to do it the hard way - check if
					// there is
					// a single method with this name (still unsafe, but JNI
					// behavior)
					// Note there's no point in doing that if we do have a
					// signature
				...
				if (mi != null) {
					...
					NativeMethodInfo miNative = new NativeMethodInfo(mi, mth,
							this);
					miNative.replace(mi);

				} else {
					// we have an orphan method, i.e. a peer method that does
					// not map into any model method
					// (this is usually a signature typo or an out-of-sync peer)
					String message = "orphan NativePeer method: "
							+ ci.getName() + '.' + mname;
					...
					// issue a warning if we have a NativePeer native method w/o
					// a corresponding
					// method in the model class (this might happen due to
	...
	private static MethodInfo searchMethod(String mname, MethodInfo[] methods) {
		...
		for (int j = 0; j < methods.length; j++) {
			if (methods[j].getName().equals(mname)) {
				...
				// isn't the right choice, we would get an
				...
				if (idx == -1) {
					...
					throw new JPFException(
							"overloaded native method without signature: "
									+ mname);
				}
			}
		}

 ...
 * 
 * This is a special Stackframe to execute NativeMethodInfos, which are just a
 * wrapper around Java reflection calls. As required by the Java reflection API,
 * they can store argument and return values as object references
 * 
...
public abstract class NativeStackFrame extends StackFrame {
	...
	public boolean modifiesState() {
		// native stackframes don't do anything with their operands or locals
		// per se
		// they are executed atomically, so there is no need to ever restore
		// them
		return false;
	}
	...
	public void markThreadRoots(Heap heap, int tid) {
		// what if some listener creates a CG post-EXECUTENATIVE or
		// pre-NATIVERETURN?
		// and the native method returned an object?
		// on the other hand, we have to make sure we don't mark a return value
		// from
		// a previous transition

		if (pc instanceof NATIVERETURN) {
			if (ret != null && ret instanceof Integer
					&& mi.isReferenceReturnType()) {
	...
	// --- NativeStackFrames aren't called directly and have special return
	// value processing (in NATIVERETURN.execute())
	@Override
	public void setArgumentLocal(int idx, int value, Object attr) {
		throw new JPFException(
				"NativeStackFrames don't support setting argument locals");
	}
	...
	public void setExceptionReference(int exRef) {
		throw new JPFException(
				"NativeStackFrames don't support exception handlers");
	}
	...
	public int getResult() {
		...
		if (r instanceof Number) {
			if (r instanceof Double) {
				throw new JPFException("result " + ret
						+ " can't be converted into int");
			...
			throw new JPFException("result " + ret
					+ " can't be converted into raw int value");
		}
	}
	...
	public int getReferenceResult() {
		if (ret instanceof Integer) {
			return (Integer) ret; // MJI requires references to be returned as
									// 'int'
		} else {
			throw new JPFException("result " + ret
					+ " can't be converted into JPF refrence value");
		}
	}
	...
	public long getLongResult() {
		...
		if (r instanceof Long) {
			...
			throw new JPFException("result " + ret
					+ " can't be converted into raw long value");
		}
	}

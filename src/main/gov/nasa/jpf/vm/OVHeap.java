...
/**
 * a heap that implements search global object ids (SGOIDs) and uses a simple
 * ObjVector to store ElementInfos. This is only efficient for small heaps with
 * low fragmentation
 * 
 * SGOID computation uses HashedAllocationContext, which means there is a chance
 * of collisions, in which case a different heap type has to be used (we don't
 * try to resolve collisions here)
 * 
 * NOTE - a reference value of 0 represents NULL, but we rather waste one unused
 * element than doing a -1 on all gets/sets
 */

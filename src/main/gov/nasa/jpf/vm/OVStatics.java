...
/**
 * Statics implementation that uses a simple ObjVector as the underlying
 * container.
 * 
 * The ids used to retrieve ElementInfos are dense and search global,
 * computation is based on the assumption that each ClassLoader can only define
 * one class per binary class name
 */
public class OVStatics implements Statics {
	...
	// NOTE this is per instance so that each one is as dense as possible, but
	// since
	// it is search global it does NOT have to be restored and we can copy the
	// reference when cloning
	...
	protected StaticElementInfo createStaticElementInfo(int id, ClassInfo ci,
	...
	public StaticElementInfo get(int id) {
		// the cast sucks, but otherwise we run into the Processor covariance
		// problem
	...
	public StaticElementInfo getModifiable(int id) {
		...
		if (ei.isFrozen()) {
			...
			// freshly created ElementInfos are not frozen, so we don't have to
			// defreeze
			elementInfos.set(id, ei);
		}

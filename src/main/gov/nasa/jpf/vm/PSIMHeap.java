...
/**
 * heap implementation that uses a PersistentStagingMsbIntMap as the underlying
 * container
 * 
 * This is intended for large state spaces, to minimize store/restore costs.
 * While those are negligible for PersistentMaps, the per-change overhead is not
 * since the container has to duplicate the access path to the changed node on
 * every modification. Efficiency of PSIMHeap therefore relies on accessing
 * objects in a consecutive pattern, i.e. depends on clustered access.
 * 
 * <2do> this should not be a GenericSGOIDHeap derived class since this includes
 * a number of non-persistent data structures (allocCounts, intern strings,
 * pinDowns) that are not persistent, i.e. still requires a memento that
 * creates/restores snapshots of these and hence looses a lot of the benefit we
 * use a persistent map for
 * 
 * NOTE - a reference value of 0 represents null and therefore is not a valid
 * SGOID
 */
public class PSIMHeap extends GenericSGOIDHeap {

	/**
	 * this sucks - we need a memento in order to store/restore allocCounts,
	 * internStrings and pinDownList
	 */
	static class PSIMMemento extends GenericSGOIDHeapMemento {
		...
		PSIMMemento(PSIMHeap heap) {
			...
			eiSnap = heap.elementInfos; // no need to transform anything, it's
										// persistent
		}
	...
	class SweepPredicate implements Predicate<ElementInfo> {
		...
		public boolean isTrue(ElementInfo ei) {
			...
			if (ei.isMarked()) { // live object, prepare for next transition &
				...
				// <2do> still have to process finalizers here, which might make
				// the object live again
	...
	public ElementInfo getModifiable(int ref) {
		...
		if (ref <= 0) {
			...
			if (ei != null && ei.isFrozen()) {
				ei = ei.deepClone();
				// freshly created ElementInfos are not frozen, so we don't have
				// to defreeze

...
/**
 * class that captures execution context consisting of executing thread and pc's
 * of ti's current StackFrames
 * 
...
public class PreciseAllocationContext implements AllocationContext {
	...
	// this is search global, i.e. does not have to be backtracked, but has to
	// be
	...
	protected int hashCode; // computed once during construction (from
							// LookupContext)

	// a mutable ExecutionContext that is only used internally to avoid creating
	// superfluous new instances to
	...
	public static synchronized PreciseAllocationContext getSUTExecutionContext(
			ClassInfo ci, ThreadInfo ti) {
		...
		if (ec == null) {
			ec = new PreciseAllocationContext(ti,
					Arrays.copyOf(cc, stackDepth), h);
			ccCache.put(ec, ec);
		}
	...
	// we only construct this from a LookupContext, which already has all the
	// data
	private PreciseAllocationContext(ThreadInfo ti, Instruction[] cc,
			int hashCode) {
		this.ti = ti;
		this.cc = cc;
	...
	public boolean equals(Object o) {
		if (o == this) { // identity shortcut
			...
			if (o instanceof PreciseAllocationContext) {
				...
				if (hashCode == other.hashCode) { // we might get here because
													// of bin masking
	...
	public AllocationContext extend(ClassInfo ci, int anchor) {
		return new PreciseAllocationContext(ti, cc, OATHash.hash(hashCode,
				ci.hashCode()));
	}

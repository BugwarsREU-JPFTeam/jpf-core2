...
public class ReferenceFieldInfo extends SingleSlotFieldInfo {
	...
	String sInit; // <2do> pcm - just a temporary quirk to init from string
					// literals
	...
	public void setConstantValue(Object constValue) {
		// <2do> pcm - check what other constants we might encounter, this is
		// most
		// probably not just used for Strings.
		// Besides the type issue, there is an even bigger problem with
		// identities.
		// For instance, all String refs initialized via the same string literal
		// inside a single classfile are in fact refering to the same object.
		// This
		// means we have to keep a registry (hashtab) with string-literal
		// created
		// String objects per ClassInfo, and use this when we assign or init
		// String references.
		// For the sake of progress, we ignore this for now, but have to come
		// back
		...
		if (constValue instanceof String) {
			...
			throw new JPFException("unsupported reference initialization: "
					+ constValue);
		}
	}

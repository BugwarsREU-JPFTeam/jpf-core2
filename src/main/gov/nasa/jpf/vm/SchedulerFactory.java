...
/**
 * interface of factory object that creates all ChoiceGenerators required to
...
public interface SchedulerFactory {
	...
	ChoiceGenerator<ThreadInfo> createSyncMethodEnterCG(ElementInfo ei,
			ThreadInfo ti);

	/** user by Returns of sync methods */
	public static final String SYNC_METHOD_EXIT = "SYNC_METHOD_EXIT";

	ChoiceGenerator<ThreadInfo> createSyncMethodExitCG(ElementInfo ei,
			ThreadInfo ti);

	/** used by MonitorEnter insns */
	public static final String MONITOR_ENTER = "MONITOR_ENTER";

	ChoiceGenerator<ThreadInfo> createMonitorEnterCG(ElementInfo ei,
			ThreadInfo ti);

	/** used by MonitorExit insns */
	public static final String MONITOR_EXIT = "MONITOR_EXIT";

	ChoiceGenerator<ThreadInfo> createMonitorExitCG(ElementInfo ei,
			ThreadInfo ti);

	/** used by Object.wait() */
	public static final String WAIT = "WAIT";

	ChoiceGenerator<ThreadInfo> createWaitCG(ElementInfo ei, ThreadInfo ti,
			long timeOut);
	...
	ChoiceGenerator<ThreadInfo> createParkCG(ElementInfo ei, ThreadInfo tiPark,
			boolean isAbsoluteTime, long timeOut);
	...
	ChoiceGenerator<ThreadInfo> createSharedFieldAccessCG(ElementInfo ei,
			ThreadInfo ti);
	...
	ChoiceGenerator<ThreadInfo> createSharedArrayAccessCG(ElementInfo eiArray,
			ThreadInfo ti);

	/**
	 * used by PutField,PutStatic insns if previously unshared objects get
	 * stored in shared object fields
	 */
	public static final String SHARED_OBJECT_EXPOSURE = "SHARED_OBJECT_EXPOSURE";

	ChoiceGenerator<ThreadInfo> createSharedObjectExposureCG(ElementInfo ei,
			ThreadInfo ti);
	...
	ChoiceGenerator<ThreadInfo> createThreadSleepCG(ThreadInfo sleepThread,
	...
	ChoiceGenerator<ThreadInfo> createThreadTerminateCG(
	...
	/**
	 * used to break the transition for other reasons (e.g. max transition
	 * length exceeded) Although most of this happens from a mandatory CG
	 * context, we still route this through the scheduler policy object to give
	 * it a chance to select the scheduling candidates Since this is a generic
	 * method, we require an explicit id/reason argument
	 */
	ChoiceGenerator<ThreadInfo> createBreakTransitionCG(String reason,
			ThreadInfo ti);

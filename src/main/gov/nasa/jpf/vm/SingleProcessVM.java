 ...
 *         gov.nasa.jpf.vm.SingleProcessVM
 */
public class SingleProcessVM extends VM {
	...
	protected ApplicationContext createApplicationContext() {
		...
		clsName = config.getProperty("target"); // explicit 'target' takes
												// precedence

		if (clsName == null) {
			if (freeArgs != null) { // if it is non-null, there is at least one
									// entry
				// note that application property filenames have been removed by
				// Config since they are part of its initialization
				clsName = freeArgs[0];

				if (freeArgs.length > 1) { // if there is no 'target' command
											// line overrides 'target.args'
		...
		if (clsName == null) {
			throw new JPFConfigException(
					"no target class specified, terminating");
		}
		if (!isValidClassName(clsName)) {
			throw new JPFConfigException("main class not a valid class name: "
					+ clsName);
		}

		// can be any static method that has a (String[]), (String) or ()
		// signature
		String mainEntry = config.getProperty("target.entry",
	...
	/**
	 * The program is terminated if there are no alive threads, and there is no
	 * nonDaemon left.
	 * 
	 * NOTE - this is only approximated in real life. Daemon threads can still
	 * run for a few cycles after the last non-daemon died, which opens an
	 * interesting source of errors we actually might want to check for
	 */
	@Override
	public boolean isEndState() {
		// note this uses 'alive', not 'runnable', hence isEndStateProperty
		// won't
	...
	public void terminateProcess(ThreadInfo ti) {
		...
		for (int i = 0; i < liveThreads.length; i++) {
			if (!liveThreads[i].isSystemThread()) {
				...
				// FinalizerThread is not killed at this point. We need to keep
				// it around in
				// case fianlizable objects are GCed after System.exit()
				// returns.

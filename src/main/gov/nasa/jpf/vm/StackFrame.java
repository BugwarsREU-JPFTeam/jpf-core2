 ...
 * 
 * Java methods always have bounded local and operand stack sizes, computed at
 * compile time, stored in the classfile, and checked at runtime by the bytecode
 * verifier. Consequently, we combine locals and operands in one data structure
 * with the following layout
 * 
 * slot[0] : 'this' .. .. local vars slot[stackBase-1] : last local var
 * slot[stackBase] : first operand slot .. ^ .. | operand stack range .. v
...
public abstract class StackFrame implements Cloneable {
	...
	/**
	 * this StackFrame is not allowed to be modified anymore because it has been
	 * state stored. Set during state storage and checked upon each
	 * modification, causing exceptions on attempts to modify callerSlots frozen
	 * instance. The flag is reset in clones
	 */
	public static final int ATTR_IS_FROZEN = 0x100;
	static final int ATTR_IS_REFLECTION = 0x1000;
	/**
	 * the previous StackFrame (usually the caller, null if first). To be set
	 * when the frame is pushed on the ThreadInfo callstack
	 */
	protected StackFrame prev;

	/**
	 * state management related attributes similar to ElementInfo. The lower 16
	 * bits are stored/restored, the upper 16 bits are for transient use
	 */
	...
	protected int thisRef = MJIEnv.NULL; // slots[0] can change, but we have to
											// keep 'this'
	 ...
	 * dups etc.), so it's pretty much up to the VM listeners/peers what's
	 * stored
	 * 
	 * NOTE: attribute values are not restored upon backtracking per default,
	 * but attribute references are. If you need restoration of values, use
	 * copy-on-write in your clients
	 * 
	 ...
	 * operations NOTE - TESTING ONLY! this does not have a MethodInfo
	 */
	...
	protected FixedBitSet createReferenceMap(int nSlots) {
		if (nSlots <= 64) {
			...
			throw new JPFException("too many slots in " + mi.getFullName()
					+ " : " + nSlots);
		}
	}
	...
	/**
	 * return the object reference for an instance method to be called (we are
	 * still in the caller's frame). This only makes sense after all params have
	 * been pushed, before the INVOKEx insn is executed
	 */
	...
	/**
	 * return reference of called object in the context of the caller (i.e. we
	 * are in the caller frame)
	 */
	...
	/**
	 * to be set (by ThreadInfo) when the frame is pushed. Can also be used for
	 * non-local gotos, but be warned - that's tricky
	 */
	...
	public Object getLocalValueObject(LocalVarInfo lv) {
		if (lv != null) { // might not have been compiled with debug info
			...
			case 'J':
				return new Long(Types.intsToLong(slots[slotIdx + 1], v)); // Java
																			// is
																			// big
																			// endian,
																			// Types
			...
			case 'D':
				return new Double(Double.longBitsToDouble(Types.intsToLong(
						slots[slotIdx + 1], v)));
			default: // reference
				if (v >= 0) {
	 ...
	 * 'nTopSlots' is usually obtained from
	...
	/**
	 * does any of the operand slots hold callerSlots reference value of
	 * 'objRef'
	 */
	...
	/**
	 * is this StackFrame modifying the KernelState this is true unless this is
	 * callerSlots NativeStackFrame
	 */
	 ...
	 */
	public void processRefArguments(MethodInfo miCallee,
	 ...
	 * attribute at callerSlots time, or check/process result with ObjectList
	 */
	 ...
	 * attribute at callerSlots time - you obtained the value you set by
	 * callerSlots previous getXAttr() - you constructed callerSlots multi value
	 * list with ObjectList.createList()
	 */
	...
	public <T> ObjectList.TypedIterator<T> operandAttrIterator(int offset,
			Class<T> attrType) {
		int i = top - offset;
		assert (i >= stackBase);
		Object a = (attrs != null) ? attrs[i] : null;
	...
	public <T> ObjectList.TypedIterator<T> longOperandAttrIterator(
			Class<T> attrType) {
	...
	public <T> ObjectList.TypedIterator<T> localAttrIterator(int index,
			Class<T> attrType) {
		assert index < stackBase;
		Object a = (attrs != null) ? attrs[index] : null;
		return ObjectList.typedIterator(a, attrType);
	...
	/**
	 * helper to quickly find out if any of the locals slots holds an attribute
	...
	/**
	 * return an array of all argument attrs, which in turn can be lists. If you
	 * have to retrieve values, use the ObjectList APIs
	 * 
	 * this is here (and not in ThreadInfo) because we might call it on
	 * callerSlots cached/cloned StackFrame (caller stack might be already
	 * modified, e.g. for callerSlots native method). to be used from listeners.
	 */
	...
	/**
	 * check if there is any argument attr of the provided type on the operand
	 * stack this is far more efficient than retrieving attribute values (we
	 * don't care for argument types)
	 */
	...
	public void setLocalVariable(int index, int v, boolean ref) {
		...
		boolean activateGc = ref
				|| (isRef.get(index) && (slots[index] != MJIEnv.NULL));
	...
	public String getLocalVariableType(String name) {
		LocalVarInfo lv = mi.getLocalVar(name,
				pc.getPosition() + pc.getLength());
		if (lv != null) {
			return lv.getType();
		}
	...
	public String getLocalVariableType(int idx) {
		LocalVarInfo lv = mi
				.getLocalVar(idx, pc.getPosition() + pc.getLength());
		if (lv != null) {
			return lv.getType();
		}
	...
	// --- use with extreme care - don't modify outside of StackFrame
	// construction. THIS IS ONLY FOR INTERNAL USE
	/*
	 * ths main reason for breaking encapsulation with these methods is that we
	 * need an API that allows efficient initialization of slots from VM mode
	 * dependent invoke instructions. Argument passing differs between VMs
	 */
	public int[] getSlots() {
	...
	public void visitReferenceSlots(ReferenceProcessor visitor) {
		for (int i = isRef.nextSetBit(0); i >= 0 && i <= top; i = isRef
				.nextSetBit(i + 1)) {
			visitor.processReference(slots[i]);
		}
	}
	...
	/**
	 * if this is an instance method, return the reference of the corresponding
	 * object (note this only has to be in slot 0 upon entry)
	 */
	public int getThis() {
		return thisRef;
	}
	...
	protected void checkIsModifiable() {
		if ((attributes & ATTR_IS_FROZEN) != 0) {
			throw new JPFException("attempt to modify frozen stackframe: "
					+ this);
		}
	}
	...
	public int mixinExecutionStateHash(int h) {
		...
		if (pc != null) {
			...
			// we don't need the bytecode since there can only be one insn with
			// this index in this method
		}
	...
	/**
	 * mark all objects reachable from local or operand stack positions
	 * containing references. Done during phase1 marking of threads (the stack
	 * is one of the Thread gc roots)
	 */
	public void markThreadRoots(Heap heap, int tid) {

		/**
		 * for (int i = isRef.nextSetBit(0); i>=0 && i<=top; i =
		 * isRef.nextSetBit(i + 1)) { int objref = slots[i]; if (objref !=
		 * MJIEnv.NULL) { heap.markThreadRoot(objref, tid); } }
		 **/
		for (int i = 0; i <= top; i++) {
			if (isRef.get(i)) {
				int objref = slots[i];
	...
	// those set the local vars that are normally initialized from call
	// arguments

...
public final class StaticElementInfo extends ElementInfo {
	...
	// this is kind of dangerous - make sure these flags are still unused in
	// ElementInfo
	static final int ATTR_COR_CHANGED = 0x100000;
	static final int ATTR_STATUS_CHANGED = 0x200000;

	static final int ATTR_ANY_CHANGED = ElementInfo.ATTR_ANY_CHANGED
			| ATTR_COR_CHANGED | ATTR_STATUS_CHANGED;
	...
	public StaticElementInfo(int id, ClassInfo ci, Fields f, Monitor m,
		...
		// <2do> not ideal, should be in superclass, but we need the
		// classObjectRef for init
		referencingThreads = createThreadInfoSet(ti); // initialization depends
														// on subclass and
														// policy
	...
	/**
	 * public boolean isShared() { // static fields are always thread global
	 * return true; }
	 **/
	...
	protected FieldInfo getDeclaredFieldInfo(String clsBase, String fname) {
		ClassInfo ci = ClassLoaderInfo.getCurrentResolvedClassInfo(clsBase); // <2do>
																				// should
																				// use
																				// CL
																				// argument
	...
	public String toString() {
		return getClassInfo().getName(); // don't append objRef (useless and
											// misleading for statics)
	}

 ...
 * (the KernelState), but also the part of it's history that is required by VM
 * to backtrack, plus some potential annotations that can be used to control the
 * search (i.e. forward/backtrack calls)
 */
public class SystemState {
	 ...
	 * stripped SystemState down enough to just store the SystemState itself (so
	 * far, we don't change it's identity, there is only one) the KernelState is
	 * still stored separately (which seems to be another anachronism)
	 * 
	 * NOTE: this gets stored at the end of a transition, i.e. if we need a
	 * value to be restored to it's transition entry state (like atomicLevel),
	 * we have to do that explicitly. Alternatively we could create the Memento
	 * before we start to enter the step, but then we have to update the nextCg
	 * in the snapshot, since it's only set at the transition end (required for
	 * restore(), i.e. HeuristicSearches)
	 * 
	 * NOTE: the plain Memento doesn't deep copy the CGs, which means it can
	 * only be used for depth first search, where the parent CG states are
	 * always current if we encounter an error. If general state restoration is
	 * required (where the parent CGs might have been changed at the time we
	 * restore), we have to use a RestorableMemento <2do> this separation is
	 * error prone and fragile. It depends on correct ChoiceGenerator deepCopy()
	 * implementations and a separate state acquisition for restorable states.
	 * Currently, the gate for this is VM.getRestorableState(), but this could
	 * be bypassed.
	 */
	static class Memento {
		ChoiceGenerator<?> curCg; // the ChoiceGenerator for the current
									// transition
		...
		Memento(SystemState ss) {
			...
			atomicLevel = ss.entryAtomicLevel; // store the value we had when we
												// started the transition
			id = ss.id;
			execThread = ss.execThread;

			// we can just copy the reference since it is re-created in each
			// transition
			restorers = ss.restorers;
		}

		/**
		 * this one is used to restore to a state which will re-enter with the
		 * next choice of the same CG, i.e. nextCG is reset
		 */
		void backtrack(SystemState ss) {
			ss.nextCg = null; // this is important - the nextCG will be set by
								// the next Transition
			ss.curCg = curCg;
		...
		void restore(SystemState ss) {
			throw new JPFException(
					"can't restore a SystemState.Memento that was created for backtracking");

			/**
			 * ss.nextCg = nextCg; ss.curCg = curCg;
			 * 
			 * ss.atomicLevel = atomicLevel; ss.id = id; ss.execThread =
			 * execThread;
			 **/
		}
	}
	...
	/**
	 * a Memento that can be restored, not just backtracked to. Be aware this
	 * can be a lot more expensive since it has to deep copy CGs so that we have
	...
	static class RestorableMemento extends Memento {
		...
		/**
		 * this one is used if we restore and then advance, i.e. it might change
		 * the CG on the next advance (if nextCg was set)
		 */
		@Override
		void restore(SystemState ss) {
			// if we don't clone them on restore, it means we can only restore
			// this memento once
	...
	ChoiceGenerator<?> curCg; // the ChoiceGenerator used in the current
								// transition
	ThreadInfo execThread; // currently executing thread, reset by
							// ThreadChoiceGenerators

	// on-demand list of optional restorers that run if we backtrack to this
	// state
	...
	boolean retainAttributes; // as long as this is set, we don't reset
								// attributes
	...
	// this is an optimization - long transitions can cause a lot of
	// short-living
	 ...
	 * optimization that can be used by applications calling
	 * Verify.begin/endAtomic(), but be aware of that it now reports a deadlock
	 * property violation in case of a blocking op inside an atomic section Data
	 * CGs however are now allowed to be inside atomic sections
	 * 
	 * BEWARE - It is in the nature of atomic sections that they might loose
	 * paths that are relevant. This is esp. true for Thread.start() within AS
	 * if the starter runs to completion without further scheduling points
	 * (DiningPhil problem).
	 */
	int atomicLevel;
	...
	public SystemState(Config config, VM vm) {
		...
		schedulerFactory = config.getEssentialInstance(
				"vm.scheduler_factory.class", SchedulerFactory.class, argTypes,
				args);
		...
		// recordSteps is set later by VM, first we need a reporter (which
		// requires the VM)
	}
	...
	public ChoiceGenerator<?> getChoiceGenerator(String id) {
		for (ChoiceGenerator<?> cg = curCg; cg != null; cg = cg
	...
	public <T extends ChoiceGenerator<?>> T getCurrentChoiceGeneratorOfType(
			Class<T> cgType) {
		for (ChoiceGenerator<?> cg = curCg; cg != null; cg = cg
				.getCascadedParent()) {
			if (cgType.isAssignableFrom(cg.getClass())) {
	...
	public ChoiceGenerator<?> getCurrentChoiceGenerator(
	 ...
	 * also a scheduling point, or 'null' if there is none in the list of
	 * current CGs
	 */
	...
	public <T extends ChoiceGenerator<?>> T getInsnChoiceGeneratorOfType(
			Class<T> cgType, Instruction insn, ChoiceGenerator<?> cgPrev) {
		ChoiceGenerator<?> cg = cgPrev != null ? cgPrev
				.getPreviousChoiceGenerator() : curCg;

		if (cg != null && cg.getInsn() == insn
				&& cgType.isAssignableFrom(cg.getClass())) {
			return (T) cg;
		}
	 ...
	 * 
	 * @return true if there is a nextCg set after registration and listener
	 *         notification
	 */
	public boolean setNextChoiceGenerator(ChoiceGenerator<?> cg) {
		if (isIgnored) {
			// if this transition is already marked as ignored, we are not
			// allowed
			// to set nextCg because 'isIgnored' results in a shortcut backtrack
			// that
		...
		if (cg != null) {
			// first, check if we have to randomize it. Note this might change
			// the CG
			// instance since some algorithmic CG types need to be transformed
			// into
			...
			// do we already have a nextCG, which means this one is a cascaded
			// CG
			if (nextCg != null) {
				cg.setPreviousChoiceGenerator(nextCg);
				nextCg.setCascaded(); // note the last registered CG is NOT set
										// cascaded
			...
			execThread.getVM().notifyChoiceGeneratorRegistered(cg, execThread); // <2do>
																				// we
																				// need
																				// a
																				// better
	...
	/**
	 * remove the current 'nextCg' Note this has to be called in a loop if all
	 * cascaded CGs have to be removed
	 */
	...
	/**
	 * this can be called anywhere from within a transition, to revert it and go
	 * on with the next choice. This is mostly used explicitly in the app via
	 * Verify.ignoreIf(..)
	 * 
	 ...
	 * 
	 * @param key
	 *            usually the object this restorer encapsulates
	 * @param restorer
	 *            the ClosedMemento that restores the state of the object it
	 *            encapsulates once we backtrack/restore this program state
	 * 
	 *            Note that restorers are called in the order of registration,
	 *            but in general it is not a good idea to depend on order since
	 *            restorers can be set from different locations (listeners,
	 *            peers, instructions)
	 */
	 ...
	 * CG initialization from transition execution is to avoid KernelState
	 * storage in case the initialization does not produce a next choice and we
	 * have to backtrack.
	 * 
	 * @see VM.forward()
	 * 
	 * @return 'true' if there is a next choice, i.e. a next transition to
	 *         enter. 'false' if there is no next choice and the system has to
	 *         backtrack
	 */
	public boolean initializeNextTransition(VM vm) {
		...
		// 'nextCg' got set at the end of the previous transition (or a
		// preceding
		// choiceGeneratorSet() notification).
		// Be aware of that 'nextCg' is only the *last* CG that was registered,
		// i.e.
		// there can be any number of CGs between the previous 'curCg' and
		// 'nextCg'
		...
		while (nextCg != null) {
			...
			// Hmm, that's a bit late (could be in setNextCG), but we keep it
			// here
			// for the sake of locality, and it's more consistent if it just
			// refers
	...
	public void executeNextTransition(VM vm) {
		...
		assert execThread.isRunnable() : "next transition thread not runnable: "
				+ execThread.getStateDescription();
	...
	protected boolean advanceCurCg(VM vm) {
		...
		if (cg.hasMoreChoices()) {
			// check if this is the first time, for which we also have to
			// advance our parents

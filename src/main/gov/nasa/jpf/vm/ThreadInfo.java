...
/**
 * Represents a thread. It contains the state of the thread, static information
 * about the thread, and the stack frames. Race detection and lock order also
 * store some information in this data structure.
 * 
 * Note that we preserve identities according to their associated
 * java.lang.Thread object (oref). This esp. means along the same path, a
 * ThreadInfo reference is kept invariant
 * 
 * <2do> remove EXECUTENATIVE,INVOKESTATIC .bytecode dependencies
 */
public class ThreadInfo extends InfoObject implements Iterable<StackFrame>,
		Comparable<ThreadInfo>, Restorable<ThreadInfo> {
	...
	public enum State {
		...
		RUNNING, BLOCKED, // waiting to acquire a lock
		UNBLOCKED, // was BLOCKED but can acquire the lock now
		WAITING, // waiting to be notified
		TIMEOUT_WAITING, NOTIFIED, // was WAITING and got notified, but is still
									// blocked
	...
	protected class InvokedStackIterator extends StackIterator implements
			Iterator<StackFrame> {
		...
		public StackFrame next() {
			if (frame != null) {
				StackFrame ret = frame;
				frame = null;
				for (StackFrame f = ret.getPrevious(); f != null; f = f
						.getPrevious()) {
	...
	// transient, not state stored. This is reset when backtracking or starting
	// a new transition
	...
	// something that tells the ThreadList how to look this up efficiently (e.g.
	// index)
	...
	// note also that the ThreadList is allowed to move this thread around, in
	// which
	...
	/**
	 * a listener or peer request for throwing an exception into the SUT, to be
	 * processed in executeInstruction
	 */
	...
	/**
	 * not so nice we cross-couple the NativePeers with ThreadInfo, but to carry
	 * on with the JNI analogy, a MJIEnv is clearly owned by a thread (even
	 * though we don't have much ThreadInfo state dependency in here (yet),
	 * hence the simplistic init)
	 */
	MJIEnv env;

	/**
	 * the VM we are running on. Bad backlink, but then again, we can't really
	 * test a ThreadInfo outside a VM context anyways. <2do> If we keep 'list'
	 * as a field, 'vm' might be moved over there (all threads in the list share
	 * the same VM)
	 */
	VM vm;
	 ...
	 * after each backtrack (we don't want to duplicate state storage) list of
	 * lock objects currently held by this thread. unfortunately, we cannot
	 * organize this as a stack, since it might get restored (from the heap) in
	 * random order
	 */
	int[] lockedObjectReferences;

	/**
	 * !! this is also volatile -> has to be reset after backtrack the reference
	 * of the object if this thread is blocked or waiting for
	 */
	...
	static class TiMemento implements Memento<ThreadInfo> {
		// note that we don't have to store the invariants (id, oref,
		// runnableRef, ciException)
		ThreadInfo ti;
		...
		TiMemento(ThreadInfo ti) {
			...
			threadData = ti.threadData; // no need to clone - it's copy on first
										// write
			top = ti.top; // likewise
			stackDepth = ti.stackDepth; // we just copy this for efficiency
										// reasons
	...
	// if we want to change this at runtime, or POR might make sense on a
	// per-thread
	// basis
	...
	/**
	 * do we treat access of fields referring to objects that are referenced
	 * from different threads as transition breaks?
	 */
	...
	/**
	 * do we treat ctor calls as unshared. If set to true, field access in init
	 * methods is not checked for shared access. While this is generally true
	 * since the object is still in construction, it is possible to leak the
	 * object reference to another thread before the ctor returns
	 */
	static boolean porUnsharedInit;

	/**
	 * break the current transition after this number of instructions. This is a
	 * safeguard against paths that won't break because potentially shared
	 * fields are not yet accessed by a second thread (existence of such paths
	 * is the downside of our access tracking). Note that we only break on
	...
	static boolean init(Config config) {
		...
		ignoreUncaughtHandlers = config.getBoolean(
				"vm.ignore_uncaught_handler", true);
		passUncaughtHandler = config.getBoolean("vm.pass_uncaught_handler",
				true);

		// --- POR related configuration options
		porInEffect = config.getBoolean("vm.por");
		porFieldBoundaries = porInEffect
				&& config.getBoolean("vm.por.field_boundaries");
		porBreakOnExposure = porFieldBoundaries
				&& config.getBoolean("vm.por.break_on_exposure");

		porSyncDetection = porInEffect
				&& config.getBoolean("vm.por.sync_detection");

		porUnsharedInit = porInEffect
				&& config.getBoolean("vm.por.unshared_init", true);
	...
	/*
	 * search global cache for dense ThreadInfo ids. We could just use oref
	 * since those are guaranteed to be global, but not dense. The ids are
	 * search global, i.e. there is no need to store/restore, but it needs to be
	 * (re)set during init()
	 */
	...
	/**
	 * mainThread ctor called by the VM. Note we don't have a thread object yet
	 * (hen-and-egg problem since we can't allocate objects without a
	 * ThreadInfo)
	 */
	protected ThreadInfo(VM vm, int id, ApplicationContext appCtx) {
		...
		init(vm);
		// we don't have the group yet, no Runnable or parent, and the name is
		// fixed
	...
	/**
	 * the ctor for all explicitly (bytecode) created threads. At this point,
	 * there is at least a mainThread and we have a corresponding
	 * java.lang.Thread object
	 */
	protected ThreadInfo(VM vm, int objRef, int groupRef, int runnableRef,
			int nameRef, ThreadInfo parent) {
		id = computeId(objRef);
		this.appCtx = parent.getApplicationContext();

		init(vm); // this is only partial, we still have to link/set the
					// references
		...
		// note the thread is not yet in the ThreadList, we have to register
		// from the caller
	}

	protected void init(VM vm) {
		// 'gid' is set by the factory method
		// we can't set the 'id' field of the corresponding java.lang.Thread
		// object until we have one
	 ...
	 * This is mostly used to tell the top- from the bottom-half of a native
	 * method or Instruction.enter(), so that only one (the top half) registers
	 * the CG (top = register CG and reschedule insn, bottom = re-enter insn and
	 * process choice at beginning of new transition)
	 * 
	 * This can be used in both pre- and post-exec notification listeners, the
	 * executedInstructions number is incremented before notifying
	 * instructionExecuted()
	 * 
	 * this method depends on the sequence of operations in
	 * ThreadInfo.executeInstruction, which is: nextPc = null notify
	 * executeInstruction nextPc = insn.enter increment executedInstructions
	...
	public boolean isFirstStepInsn() {
		...
		if (nInsn == 0) {
			...
			// that is for setting the CG in executeInsn or enter, and then
			// testing in
			// instructionExecuted. Note that nextPc is reset before pre-exec
			// notification
			// and hence should only be non-null from insn.enter() up to the
			// next
	 ...
	 * gets incremented after calling Instruction.enter(), i.e. before notifying
	 * instructionExecuted() listeners
	 */
	...
	public void setState(State newStatus) {
		...
		if (oldStatus != newStatus) {
			...
			assert (oldStatus != State.TERMINATED) : "can't resurrect thread "
					+ this + " to " + newStatus.name();
	...
	void setBlockedState(int objref) {
		...
		default:
			assert false : "thread " + this + "can't be blocked in state: "
					+ currentState.name();
		}
	}
	...
	void setNotifiedState() {
		...
		default:
			assert false : "thread " + this + "can't be notified in state: "
					+ currentState.name();
		}
	}
	...
	public boolean isRunnable() {
		...
		case UNBLOCKED:
			return true;
		case SLEEPING:
			return true; // that's arguable, but since we don't model time we
							// treat it like runnable
		case TIMEDOUT:
			return true; // would have been set to blocked if it couldn't
							// reacquire the lock
		default:
			return false;
		}
	...
	public boolean willBeRunnable() {
		...
		case UNBLOCKED:
			return true;
		case TIMEOUT_WAITING: // it's not yet, but it will be at the time it
								// gets scheduled
		case SLEEPING:
			return true;
		default:
	...
	public boolean isTimeoutRunnable() {
		...
		case TIMEOUT_WAITING:
			...
			// assert lockRef != MJIEnv.NULL :
			// "timeout waiting but no blocked object";
	...
	public void setStopped(int throwableRef) {
		...
		if (!hasBeenStarted()) {
			// that one is easy - just remember the state so that a subsequent
			// start()
			// does nothing
			return;
		}

		// for all other cases, we need to have a proper stopping Throwable that
		// does not
		// fall victim to GC, and that does not cause
		// NoUncaughtExcceptionsProperty violations
		if (throwableRef == MJIEnv.NULL) {
			// if no throwable was provided (the normal case), throw a
			// java.lang.ThreadDeath Error
			ClassInfo cix = ClassInfo.getInitializedSystemClassInfo(
					"java.lang.ThreadDeath", this);
			throwableRef = createException(cix, null, MJIEnv.NULL);
		}

		// now the tricky part - this thread is alive but might be blocked,
		// notified
		// or waiting. In any case, exception action should not take place
		// before
		// the thread becomes scheduled again, which
		// means we are not allowed to fiddle with its state in any way that
		// changes
		// scheduling/locking behavior. On the other hand, if this is the
		// currently
		// executing thread, take immediate action

		if (isCurrentThread()) { // we are suicidal
			if (isInNativeMethod()) {
				// remember the exception to be thrown when we return from the
				// native method
		...
		} else { // this thread is not currently running, this is an external
					// kill

			// remember there was a pending exception that has to be thrown the
			// next
	...
	public boolean isCalleeThis(ElementInfo r) {
		...
		return getCalleeThis(Types.getArgumentsSize(call
				.getInvokedMethodSignature()) + 1) == r.getObjectRef();
	}
	...
	/**
	 * path local unique id for live threads. This is what we use for the public
	 * java.lang.Thread.getId() that can be called from SUT code. It is NOT what
	 * we use for our canonical root set
	 */
	public int getId() {
		return id;
	}
	...
	/**
	 * this is our internal, search global id that is used for the canonical
	 * root set
	 */
	...
	/**
	 * thread is not blocked anymore needs to be public since we have to use it
	 * from INVOKECLINIT (during call skipping)
	 */
	...
	public boolean resume() {
		return (threadData.suspendCount > 0)
				&& (--threadDataClone().suspendCount == 0);
	}
	...
	public ClassInfo resolveReferencedClass(String clsName) {
		...
		// return
		// ClassLoaderInfo.getCurrentClassLoader(this).getResolvedClassInfo(clsName);
	}
	...
	/**
	 * get the current stack trace of this thread this is called during creation
	 * of a Throwable, hence we should skip all throwable ctors in here <2do>
	 * this is only a partial solution,since we don't catch exceptionHandlers in
	 * Throwable ctors yet
	 */
	 ...
	 * 
	 * <2do> pcm - not clear to me how lower stack frames can contribute to a
	 * different threadinfo state hash - only the current one can be changed by
	...
	public boolean atMethod(String mname) {
		return top != null
				&& getTopFrameMethodInfo().getFullName().equals(mname);
	}
	...
	public Object clone() {
		try {
			// threadData and top StackFrame are copy-on-write, so we should not
			// have to clone them
			// lockedObjects are state-volatile and restored explicitly after a
			// backtrack
			return super.clone();
	...
	/**
	 * get a stack snapshot that consists of an array of {mthId,pc} pairs. strip
	 * stackframes that enter instance methods of the exception object
	 */
	...
	/**
	 * turn a snapshot into an JPF array of StackTraceElements, which means a
	 * lot of objects. Do this only on demand
	 */
	public int createStackTraceElements(int[] snapshot) {
		...
		Heap heap = vm.getHeap();
		ElementInfo eiArray = heap.newArray("Ljava/lang/StackTraceElement;",
				nVisible, this);
	...
	/**
	 * the reason why this is kind of duplicated (there is also a
	 * StackFrame.getPositionInfo) is that this might be working off a
	 * StackTraceElement[] that is created when the exception is created. At the
	 * time printStackTrace() is called, the StackFrames in question are most
	 * likely already be unwinded
	 */
	public void printStackTrace(PrintWriter pw, int objRef) {
		// 'env' usage is not ideal, since we don't know from what context we
		// are called, and
		// hence the MJIEnv calling context might not be set (no Method or
		// ClassInfo)
		// on the other hand, we don't want to re-implement all the MJIEnv
		// accessor methods
		...
		if (aRef != MJIEnv.NULL) {
			...
			for (int i = 0; i < len; i++) {
				...
				if (steRef != MJIEnv.NULL) { // might be ignored (e.g. direct
												// call)
			...
			for (int i = 0, j = 0; i < len; i++) {
				int methodId = snapshot[j++];
				int pcOffset = snapshot[j++];
				StackTraceElement ste = new StackTraceElement(methodId,
						pcOffset);
				ste.printOn(pw);
			}
		}
	...
	class StackTraceElement {
		...
		StackTraceElement(int sRef) {
			clsName = env.getStringObject(env
					.getReferenceField(sRef, "clsName"));
			mthName = env.getStringObject(env
					.getReferenceField(sRef, "mthName"));
			fileName = env.getStringObject(env.getReferenceField(sRef,
					"fileName"));
		...
		int createJPFStackTraceElement() {
			if (ignore) {
				...
				ClassInfo ci = ClassLoaderInfo
						.getSystemResolvedClassInfo("java.lang.StackTraceElement");
				ElementInfo ei = heap.newObject(ci, ThreadInfo.this);

				ei.setReferenceField("clsName",
						heap.newString(clsName, ThreadInfo.this).getObjectRef());
				ei.setReferenceField("mthName",
						heap.newString(mthName, ThreadInfo.this).getObjectRef());
				ei.setReferenceField("fileName",
						heap.newString(fileName, ThreadInfo.this)
								.getObjectRef());
		...
		void printOn(PrintWriter pw) {
			if (!ignore) {
				// the usual behavior is to print only the filename, strip the
				// path
				if (fileName != null) {
	...
	/**
	 * this is a helper class to store listener generated exception requests
	 * that are checked before and after calling Instruction.execute(). This is
	 * a safe way to raise SUT exceptions from listener code without
	 * compromising consistency of executes() that are not prepared to cut short
	 * by means of re-execution or host VM exceptions
	 */
	...
	public void requestSUTException(String exceptionClsName, String details) {
		pendingSUTExceptionRequest = new SUTExceptionRequest(exceptionClsName,
				details);
	...
	protected void processPendingSUTExceptionRequest() {
		if (pendingSUTExceptionRequest != null) {
			// <2do> we could do more specific checks for
			// ClassNotFoundExceptions here
			nextPc = createAndThrowException(
					pendingSUTExceptionRequest.getExceptionClassName(),
					pendingSUTExceptionRequest.getDetails());
			pendingSUTExceptionRequest = null;
		}
	}

	/**
	 * <2do> pcm - this is only valid for java.* and our own Throwables that
	 * don't need ctor execution since we only initialize the Throwable fields.
	 * This method is here to avoid round trips in case of exceptions
	 */
	int createException(ClassInfo ci, String details, int causeRef) {
		int[] snap = getSnapshot(MJIEnv.NULL);
		return vm.getHeap()
				.newSystemThrowable(ci, details, snap, causeRef, this, 0)
				.getObjectRef();
	}
	...
	public Instruction createAndThrowException(String cname, String details) {
		try {
			...
			try {
				...
				if (cie.getExceptionClass().equals(
			...
			if (!cname.equals(cie.getExceptionClass())) {
				ClassInfo ci = ClassLoaderInfo.getCurrentResolvedClassInfo(cie
						.getExceptionClass());
	...
	/**
	 * enter instructions until there is none left or somebody breaks the
	 * transition (e.g. by registering a CG)
	 * 
	...
	protected void executeTransition(SystemState ss) throws JPFException {
		...
		// insns until someone registered a ChoiceGenerator, there are no insns
		// left,
		// the transition was explicitly marked as ignored, or we have reached a
		// max insn count and preempt the thread upon the next available
		// backjump
		while (pc != null) {
			...
			if (ss.breakTransition()) {
				...
				if (executedInstructions >= maxTransitionLength) { // try to
																	// preempt
																	// the
																	// current
																	// thread
					if (pc.isBackJump() && (pc != nextPc)
							&& (top != null && !top.isNative())) {
						log.info(
								"max transition length exceeded, breaking transition on ",
								nextPc);
	...
	public Instruction executeInstruction() {
		...
		// the default, might be changed by the insn depending on if it's the
		// first
		// time we exec the insn, and whether it does its magic in the top
		// (before break)
		...
		// note that we don't reset pendingSUTExceptionRequest since it could be
		// set outside executeInstruction()

		if (log.isLoggable(Level.FINER)) {
			log.fine(pc.getMethodInfo().getFullName() + " " + pc.getPosition()
					+ " : " + pc);
		}

		// this is the pre-execution notification, during which a listener can
		// perform
		// on-the-fly instrumentation or even replace the instruction
		// alltogether
		...
		if (!skipInstruction) {
			...
			try {
				...
				nextPc = this.createAndThrowException(cie.getExceptionClass(),
						cie.getMessage());
			}
		}
		...
		// since this is part of the inner execution loop, it is a convenient
		// place to check for probes
		...
		// set+return the next insn to enter if we did not return from the last
		// stack frame.
		// Note that 'nextPc' might have been set by a listener, and/or 'top'
		// might have
	...
	/**
	 * enter instruction hidden from any listeners, and do not record it in the
	 * path
	 */
	public Instruction executeInstructionHidden() {
		...
		nextPc = null; // reset in case pc.execute() blows (this could be behind
						// an exception firewall)
		...
		// since this is part of the inner execution loop, it is a convenient
		// place to check probe notifications
		vm.getSearch().checkAndResetProbeRequest();
	...
	/**
	 * is this after calling Instruction.enter() used by instructions and
	 * listeners
	 */
	...
	/**
	 * skip and continue with the following instruction. This is deprecated
	 * because callers should explicitly provide the next instruction (depending
	 * on the skipped insn)
	 */
	@Deprecated
	...
	public boolean setNextPC(Instruction insn) {
		if (nextPc == null) {
			// this is pre-execution, if we don't skip the next insn.execute()
			// is going
			...
			if (top != null && nextPc != top.getPC()) { // this needs to be
														// re-executed
				nextPc = insn;
				return true;
			}
	...
	/**
	 * Executes a method call. Be aware that it executes the whole method as one
	 * atomic step. Arguments have to be already on the provided stack
	 * 
	 * This only works for non-native methods, and does not allow any choice
	 * points, so you have to know very well what you are doing.
	 * 
	 * Instructions executed by this method are still fully observable and
	 * stored in the path
	 */
	public void executeMethodAtomic(StackFrame frame) {
		...
		ss.incAtomic(); // to shut off avoidable context switches (MONITOR_ENTER
						// and wait() can still block)
		...
		while (depth <= countStackFrames()) {
			...
			if (ss.getNextChoiceGenerator() != null) {
				...
				// createAndThrowException("java.lang.AssertionError",
				// "choice point in sync executed method: " + frame);
				throw new JPFException(
						"choice point in atomic method execution: " + frame);
			} else {
				pc = nextPC;
			}
		}
		...
		// the frame was already removed by the RETURN insn of the frame's
		// method
	}
	 ...
	 * prevents such a method from changing the program state, and we wouldn't
	 * know for what reason by looking at the trace
	 * 
	 * this method should only be used if we have to enter test application code
	 * like hashCode() or equals() from native code, e.g. to silently check
	 * property violations
	 * 
	 * executeMethodHidden also acts as an exception firewall, since we don't
	 * want any silently executed code fall back into the visible path (for no
	 * observable reason)
	 */
	public void executeMethodHidden(StackFrame frame) {
		...
		int depth = countStackFrames(); // this includes the
										// DirectCallStackFrame
		Instruction pc = frame.getPC();

		vm.getSystemState().incAtomic(); // to shut off avoidable context
											// switches (MONITOR_ENTER and
											// wait() can still block)
		...
		while (depth <= countStackFrames()) {
			...
			if (pendingException != null) {
				...
				if (nextPC == pc) {
					...
					// "choice point in sync executed method: " + frame);
					throw new JPFException(
							"choice point in hidden method execution: " + frame);
				} else {
					pc = nextPC;
				}
			}
	...
	public ElementInfo getBlockedObject(MethodInfo mi, boolean isBeforeCall,
		...
		if (mi.isSynchronized()) {
			if (mi.isStatic()) {
				...
				// NOTE 'inMethod' doesn't work for natives, because getThis()
				// pulls 'this' from the stack frame,
				// which we don't have (and don't need) for natives
				objref = isBeforeCall ? getCalleeThis(mi) : getThis();
			}

			ei = (isModifiable) ? getModifiableElementInfo(objref)
					: getElementInfo(objref);
	...
	/**
	 * note - this assumes the stackframe of the method to enter is already
	 * initialized and on top (pushed)
	 */
	public void enter() {
		MethodInfo mi = top.getMethodInfo();

		if (mi.isSynchronized()) {
			int oref = mi.isStatic() ? mi.getClassInfo().getClassObjectRef()
					: top.getThis();
	...
	public void leave() {
		...
		// <2do> - that's not really enough, we might have suspicious bytecode
		// that fails
		// to release locks acquired by monitor_enter (e.g. by not having a
		// handler that
		// monitor_exits & re-throws). That's probably shifted into the bytecode
		// verifier
		// in the future (i.e. outside JPF), but maybe we should add an explicit
		// test here
		// and report an error if the code does asymmetric locking (according to
		// the specs,
		// VMs are allowed to silently fix this, so it might run on some and
		// fail on others)
		...
		if (mi.isSynchronized()) {
			...
			if (mi.isClinit()) {
				// we just released the lock on the class object, returning from
				// a clinit
				// now we can consider this class to be initialized.
				// NOTE this is still part of the RETURN insn of clinit, so
	...
	/**
	 * this should only be called from the top half of the last DIRECTCALLRETURN
	 * of a thread.
	 * 
	 * @return true - if the thread is done, false - if instruction has to be
	 *         re-executed
	 */
	public boolean exit() {
		...
		if (!ei.canLock(this)) {
			...
			// if we can't acquire the lock, it means there needs to be another
			// thread alive,
			// but it might not be runnable (deadlock) and we don't want to mask
			// that error
			ChoiceGenerator<ThreadInfo> cg = ss.getSchedulerFactory()
					.createMonitorEnterCG(ei, this);
			ss.setMandatoryNextChoiceGenerator(cg,
					"blocking thread termination without CG: ");

			return false; // come back once we can obtain the lock to notify our
							// waiters

		} else { // Ok, we can get the lock, time to die

			// if this is the last non-daemon and there are only daemons left
			// (which
			// would be killed by our termination) we have to give them a chance
			// to
			// run BEFORE we terminate, to catch errors in those daemons we
			// might have
			// triggered in our last transition. Even if a daemon has a proper
			// CG
			// on the trigger that would expose the error subsequently, it would
			// not be
			// scheduled anymore but hard terminated. This is even true if the
			// trigger
			...
			if (vm.getThreadList().hasOnlyMatchingOtherThan(this,
					vm.getDaemonRunnablePredicate())) {
				if (yield()) {
					return false;
				}
			}
			...
			if (!holdsLock(ei)) {
				// we only need to increase the lockcount if we don't own the
				// lock yet,
				// as is the case for synchronized run() in anonymous threads
				// (otherwise
			...
			// we don't unregister this thread yet, this happens when the
			// corresponding
			...
			// give the thread tracking policy a chance to remove this thread
			// from
			// object/class thread sets
			SharedObjectPolicy.getPolicy().cleanupThreadTermination(this);

			if (vm.getThreadList().hasAnyMatchingOtherThan(this,
					getRunnableNonDaemonPredicate())) {
				ChoiceGenerator<ThreadInfo> cg = ss.getSchedulerFactory()
						.createThreadTerminateCG(this);
				ss.setMandatoryNextChoiceGenerator(cg,
						"thread terminated without CG: ");
			}

			popFrame(); // we need to do this *after* setting the CG (so that we
						// still have a CG insn)
	...
	/**
	 * this is called upon ThreadInfo.exit() and corresponds to the private
	 * Thread.exit()
	 */
	void cleanupThreadObject(ElementInfo ei) {
		// ideally, this should be done by calling Thread.exit(), but this
		// does a ThreadGroup.remove(), which does a lot of sync stuff on the
		// shared
		// ThreadGroup object, which might create lots of states. So we just
		// nullify
	...
	/**
	 * this is called upon ThreadInfo.exit() and corresponds to
	 * ThreadGroup.remove(t), which is called from Thread.exit()
	 * 
	 * ideally this should be in the ThreadGroup peer, but we don't want to
	 * reference concrete peers from core (which is the lowest layer). Since we
	 * already depend on ThreadGroup fields during VM initialization we just
	 * keep all Thread/ThreadGroup related methods here
	 */
				...
				if (eiThreads.isArray()) {
					...
					for (int i = 0; i < nthreads; i++) {
						...
						if (tref == threadRef) { // compact the threads array
							int n1 = nthreads - 1;
							for (int j = i; j < n1; j++) {
								eiThreads.setReferenceElement(j,
										eiThreads.getReferenceElement(j + 1));
							}
							...
							// <2do> we should probably also check if we have to
							// set it destroyed
							return;
						}
					}
	...
	/**
	 * this is called by the VM upon initialization of the main thread. At this
	 * point, we have a tiMain and a java.lang.Thread object, but there is no
	 * ThreadGroup yet
	 * 
	 * This method is here to keep all Thread/ThreadGroup field dependencies in
	 * one place. The downside of not keeping this in VM is that we can't
	 * override in order to have specialized ThreadInfos, but there is no
	 * factory for them anyways
	 */
	protected void createMainThreadObject(SystemClassLoaderInfo sysCl) {
		...
		ClassInfo ciPermit = sysCl
				.getResolvedClassInfo("java.lang.Thread$Permit");
	...
	/**
	 * this creates and inits the main ThreadGroup object, which we have to do
	 * explicitly since we can't execute bytecode yet
	 */
	protected ElementInfo createMainThreadGroup(SystemClassLoaderInfo sysCl) {
		...
		// 'threads' and 'nthreads' will be set later from
	...
	protected void addToThreadGroup(ElementInfo eiGroup) {
		...
		if (eiGroup.getBooleanField("destroyed")) {
			...
			if (threadsRef == MJIEnv.NULL) {
				...
				int newThreadsRef = env.newObjectArray("Ljava/lang/Thread;",
						nThreads + 1);
			...
			/**
			 * <2do> we don't model this yet FieldInfo finUnstartedThreads =
			 * eiGroup.getFieldInfo("nUnstartedThreads"); int nUnstarted =
			 * eiGroup.getIntField(finUnstartedThreads);
	...
	public void interrupt() {
		...
		case TIMEOUT_WAITING:
			...
			if (eiLock.canLock(this)) {
				...
				// we cannot yet remove this thread from the Monitor lock
				// contender list
				// since it still has to re-acquire the lock before becoming
				// runnable again

				// NOTE: this can lead to attempts to reenter the same thread to
				// the
				// lock contender list if the interrupt handler of the
				// interrupted thread
	...
	void markRoots(Heap heap) {
		...
		// 3. if we have a pending exception that wasn't handled, make sure the
		// exception object is not collected before we match states
		if (pendingException != null) {
	...
	/**
	 * replace the top frame - this is a dangerous method that should only be
	 * used from Restoreres and to restore operators and locals in
	 * post-execution notifications to their pre-execution contents
	 */
	public void setTopFrame(StackFrame frame) {
		top = frame;

		// since we have swapped the top frame, the stackDepth might have
		// changed
		int n = 0;
	...
	public void pushFrame(StackFrame frame) {
		...
		// a new frame cannot have been stored yet, so we don't need to clone on
		// the next mod
		// note this depends on not pushing a frame in the top half of a CG
		// method
		markTfChanged(top);
	...
	public boolean hasReturnedFromDirectCall(String directCallId) {
		return (returnedDirectCall != null && returnedDirectCall
				.getMethodName().equals(directCallId));
	}
	...
	boolean haltOnThrow(String exceptionClassName) {
		if ((haltOnThrow != null)
				&& (haltOnThrow.matchesAny(exceptionClassName))) {
			return true;
		}
	...
	Instruction throwStopException() {
		...
		if (insn instanceof EXECUTENATIVE) {
			// we only get here if there was a CG in a native method and we
			// might
			// have to reacquire a lock to go on

			// <2do> it would be better if we could avoid to enter the native
			// method
			// since it might have side effects like overwriting the exception
			// or
			// doing roundtrips in its bottom half, but we don't know which lock
			// that
	...
	/**
	 * this is basically a side-effect free version of throwException to
	 * determine if a given exception will be handled.
	 */
	public HandlerContext getHandlerContextFor(ClassInfo ciException) {
		ExceptionHandler matchingHandler = null; // the matching handler we
													// found (if any)

		for (StackFrame frame = top; frame != null; frame = frame.getPrevious()) {
			// that means we have to turn the exception into an
			// InvocationTargetException
			if (frame.isReflection()) {
				ciException = ClassInfo.getInitializedSystemClassInfo(
						"java.lang.reflect.InvocationTargetException", this);
			}

			matchingHandler = frame.getHandlerFor(ciException);
			if (matchingHandler != null) {
				return new HandlerContext(this, ciException, frame,
						matchingHandler);
			}
		}
		...
		if (!ignoreUncaughtHandlers && !isUncaughtHandlerOnStack()) {
			...
			if ((uchRef = getInstanceUncaughtHandler()) != MJIEnv.NULL) {
				return new HandlerContext(this, ciException,
						HandlerContext.UncaughtHandlerType.INSTANCE, uchRef);
			}

			int grpRef = getThreadGroupRef();
			if ((uchRef = getThreadGroupUncaughtHandler(grpRef)) != MJIEnv.NULL) {
				return new HandlerContext(this, ciException,
						HandlerContext.UncaughtHandlerType.GROUP, uchRef);
			}

			if ((uchRef = getGlobalUncaughtHandler()) != MJIEnv.NULL) {
				return new HandlerContext(this, ciException,
						HandlerContext.UncaughtHandlerType.GLOBAL, uchRef);
			}
		}
	...
	/**
	 * unwind stack frames until we find a matching handler for the exception
	 * object
	 */
	public Instruction throwException(int exceptionObjRef) {
		...
		StackFrame handlerFrame = null; // the stackframe that has a matching
										// handler (if any)
		ExceptionHandler matchingHandler = null; // the matching handler we
													// found (if any)

		// first, give the VM a chance to intercept (we do this before changing
		// anything)
		...
		// we don't have to store the stacktrace explicitly anymore, since that
		// is now
		// done in the Throwable ctor (more specifically the native
		// fillInStackTrace)
		...
		if (haltOnThrow(exceptionName)) {
			// shortcut - we don't try to find a handler for this one but bail
			// immediately
		...
		for (StackFrame frame = top; (frame != null) && (handlerFrame == null); frame = frame
				.getPrevious()) {
			// that means we have to turn the exception into an
			// InvocationTargetException
			if (frame.isReflection()) {
				ciException = ClassInfo.getInitializedSystemClassInfo(
						"java.lang.reflect.InvocationTargetException", this);
				exceptionObjRef = createException(ciException, exceptionName,
						exceptionObjRef);
			...
			if ((handlerFrame == null) && frame.isFirewall()) {
				// this method should not let exceptionHandlers pass into lower
				// level stack frames
		...
		if (handlerFrame == null) {
			// we still have to check if there is a
			// Thread.UncaughtExceptionHandler in effect,
			// and if we already enter within one, in which case we don't
			// reenter it
			if (!ignoreUncaughtHandlers && !isUncaughtHandlerOnStack()) {
				// we use a direct call instead of exception handlers within the
				// run()/main()
				// direct call methods because we want to preserve the whole
				// stack in case
				// we treat returned (report-only) handlers as
				// NoUncaughtExceptionProperty
				// violations (passUncaughtHandler=false)
				insn = callUncaughtHandler(pendingException);
				if (insn != null) {
					// we only do this if there is a UncaughtHandler other than
					// the standard
					// ThreadGroup, hence we have to check for the return value.
					// If there is
					// only ThreadGroup.uncaughtException(), we put the system
					// out of its misery
					return insn;
				}
			}

			// there was no overridden uncaughtHandler, or we already executed
			// it
			if ("java.lang.ThreadDeath".equals(exceptionName)) { // gracefully
																	// shut down
				unwindToFirstFrame();
				pendingException = null;
			...
			// according to the VM spec, before transferring control to the
			// handler we have
			// to reset the operand stack to contain only the exception
			// reference
			...
			// jump to the exception handler and set pc so that listeners can
			// see it
	...
	/**
	 * is there any UncaughHandler in effect for this thread? NOTE - this
	 * doesn't check if we are already executing one (i.e. it would still handle
	 * an exception) or if uncaughtHandlers are enabled within JPF
	 */
	...
	/**
	 * this explicitly models the standard ThreadGroup.uncaughtException(), but
	 * we want to save us a roundtrip if that's the only handler we got. If we
	 * would use a handler block in the run()/main() direct call stubs that just
	 * delegate to the standard ThreadGroup.uncaughtException(), we would have
	 * trouble mapping this to NoUncaughtExceptionProperty violations (which is
	 * just a normal printStackTrace() in there).
	 */
	protected Instruction callUncaughtHandler(ExceptionInfo xi) {
		Instruction insn = null;

		// 1. check if this thread has its own uncaughtExceptionHandler set. If
		// not,
		...
		if (hRef != MJIEnv.NULL) {
			...
			// 2. check if any of the ThreadGroup chain has an overridden
			// uncaughtException
			int grpRef = getThreadGroupRef();
			hRef = getThreadGroupUncaughtHandler(grpRef);

			if (hRef != MJIEnv.NULL) {
				insn = callUncaughtHandler(xi, hRef,
						"[threadGroupUncaughtHandler]");
	...
	protected int getGlobalUncaughtHandler() {
		...
		// the field is in our java.lang.Thread, but the concrete thread object
		// class might differ
	...
	/**
	 * using an attribute to mark DirectCallStackFrames executing
	 * uncaughtHandlers is not ideal, but the case is so exotic that we don't
	 * want another concrete StackFrame subclass that would have to be created
	 * through the ClassInfo factory. Apart from retrieving the ExceptionInfo
	 * this is just a normal DirectCallStackFrame. We could directly use
	 * ExceptionInfo, but it seems more advisable to have a dedicated, private
	 * type. This could also store execution state
	 */
	...
	protected Instruction callUncaughtHandler(ExceptionInfo xi, int handlerRef,
			String id) {
		ElementInfo eiHandler = getElementInfo(handlerRef);
		ClassInfo ciHandler = eiHandler.getClassInfo();
		MethodInfo miHandler = ciHandler.getMethod(
				"uncaughtException(Ljava/lang/Thread;Ljava/lang/Throwable;)V",
				true);

		// we have to clear this here in case there is a CG while executing the
		// uncaughtHandler
		pendingException = null;

		DirectCallStackFrame frame = miHandler.createDirectCallStackFrame(this,
				0);
	...
	protected StackFrame popUncaughtHandlerFrame() {
		...
		// reporting this is probably false and we want JPF to report the
		// defect.
		// If this is a fail-safe handler that tries to clean up so that other
		// threads can
		...
		if (passUncaughtHandler) {
			...
			getModifiableTopFrame().advancePC();
			assert top.getPC() instanceof ReturnInstruction : "topframe PC not a ReturnInstruction: "
					+ top.getPC();
			return top;

		} else {
			// treat this still as an NoUncaughtExceptionProperty violation
			UncaughtHandlerAttr ctx = returnedDirectCall
					.getFrameAttr(UncaughtHandlerAttr.class);
			pendingException = ctx.getExceptionInfo();
			// NoUncaughtExceptionsProperty.setExceptionInfo(pendingException);
			throw new UncaughtException(this,
					pendingException.getExceptionReference());
		}
	}

	protected void unwindTo(StackFrame newTopFrame) {
		for (StackFrame frame = top; (frame != null) && (frame != newTopFrame); frame = frame
				.getPrevious()) {
	...
	protected StackFrame unwindToFirstFrame() {
		...
		for (frame = top; frame.getPrevious() != null; frame = frame
				.getPrevious()) {
			leave(); // that takes care of releasing locks
			vm.notifyExceptionBailout(this); // notify before we pop the frame
			popFrame();
	...
	/**
	 * watch out - just clearing it might cause an infinite loop if we don't
	 * drop frames and/or advance the pc
	 */
	...
	/**
	 * Returns a clone of the thread data. To be called every time we change
	 * some ThreadData field (which unfortunately includes lock counts, hence
	...
	/**
	 * this is a generic request to reschedule that is not based on instruction
	 * type Note that we check for a mandatory CG, i.e. the policy has to return
	 * a CG to make sure there is a transition break. We still go through the
	 * policy object for selection of scheduling choices though
	 * 
	 */
	public void reschedule(String reason) {
		SystemState ss = vm.getSystemState();
		ChoiceGenerator<ThreadInfo> cg = ss.getSchedulerFactory()
				.createBreakTransitionCG(reason, this);
		ss.setMandatoryNextChoiceGenerator(cg,
				"rescheduling request without CG: ");
	}

	/**
	 * this is a version that unconditionally breaks the current transition
	 * without really adding choices. It only goes on with the same thread (to
	 * avoid state explosion). Since we require a break, and there is no choice
	 * (current thread is supposed to continue), there is no point going through
	 * the SchedulerFactory
	 * 
	 * if the current transition is already marked as ignored, this method does
	 * nothing
	 */
	...
	/**
	 * this is like a reschedule request, but gives the scheduler an option to
	 * skip the CG
	 */
	public boolean yield() {
		...
		if (!ss.isIgnored()) {
			ThreadInfo[] choices = vm.getThreadList().getAllMatching(
					vm.getAppTimedoutRunnablePredicate());
			ThreadChoiceFromSet cg = new ThreadChoiceFromSet("yield", choices,
					true);
	...
	/**
	 * this breaks the current transition with a CG that forces an end state
	 * (i.e. has no choices) this only takes effect if the current transition is
	 * not already marked as ignored
	 */
	public void breakTransition(boolean isTerminator) {
		SystemState ss = vm.getSystemState();

		if (!ss.isIgnored()) {
			BreakGenerator cg = new BreakGenerator("breakTransition", this,
					isTerminator);
			ss.setNextChoiceGenerator(cg); // this breaks the transition
		}
	}
	...
	public boolean checkPorFieldBoundary() {
		return (executedInstructions == 0) && porFieldBoundaries
				&& hasOtherRunnables();
	}

	public boolean hasOtherRunnables() {
		return vm.getThreadList().hasAnyMatchingOtherThan(this,
				vm.getRunnablePredicate());
	}
	 ...
	 * 
	 * NOTE - you can't use this AFTER getModifiableTopFrame() since it changes
	 * top. Because it is inherently unsafe, it should be used with care and you
	 * have to make sure nothing else has stored top references, or respective
	 * references are updated after this call.
	 * 
	 * NOTE also that we assume there is no frozen frame on top of an unfrozen
	 * one <2do> that should probably be reported as an error since it is a
	...
	/**
	 * note that we don't provide a modifiable version of this. All
	 * modifications should only happen in the executing (top) frame
	 */
	...
	public String toString() {
		return "ThreadInfo [name=" + getName() + ",id=" + id + ",state="
				+ getStateName() + ']';
	}
	...
	public void setPriority(int newPrio) {
		if (threadData.priority != newPrio) {
			...
			// values are just used directly from the Thread ctors (from where
			// we pull
			// it out in our ThreadInfo ctor), and henceforth only via our
			// intercepted
			// native getters
		}
	}
	...
	public void checkConsistency(boolean isStore) {
		...
		if (isRunnable()) {
			checkAssertion(lockRef == MJIEnv.NULL,
					"runnable thread with non-null lockRef: " + lockRef);
		}

		// every thread that has been started and is not terminated has to have
		// a stackframe with a next pc
		...
		// if we are timedout, the top pc has to be either on a native
		// Object.wait() or Unsafe.park()
		if (isTimedOut()) {
			Instruction insn = top.getPC();
			checkAssertion(insn instanceof EXECUTENATIVE,
					"thread timedout outside of native method");
			...
			checkAssertion(
					mname.equals("wait(J") || mname.equals("park(ZJ"),
					"timedout thread outside timeout method: "
							+ mi.getFullName());
		}
		...
		if (lockRef != MJIEnv.NULL) {
			...
			checkAssertion(ei != null, "thread locked on non-existing object: "
					+ lockRef);

			// we have to be in the lockedThreads list of that objects monitor
			checkAssertion(ei.isLocking(this),
					"thread blocked on non-locking object: " + ei);

			// can't be blocked on a lock we own (but could be in waiting before
			// giving it up)
			if (!isWaiting() && lockedObjectReferences.length > 0) {
				for (ElementInfo lei : lockedObjects) {
					checkAssertion(lei.getObjectRef() != lockRef,
							"non-waiting thread blocked on owned lock: " + lei);
				}
			}

			// the state has to be BLOCKED, NOTIFIED, WAITING or TIMEOUT_WAITING
			checkAssertion(isWaiting() || isBlockedOrNotified(),
					"locked thread not waiting, blocked or notified");

		} else { // no lockRef set
			checkAssertion(!isWaiting() && !isBlockedOrNotified(),
					"non-locked thread is waiting, blocked or notified");
		}

		// if we have locked objects, we have to be the locking thread of each
		// of them
		if (lockedObjects != null && !lockedObjects.isEmpty()) {
			for (ElementInfo ei : lockedObjects) {
				ThreadInfo lti = ei.getLockingThread();
				if (lti != null) {
					checkAssertion(lti == this,
							"not the locking thread for locked object: " + ei
									+ " owned by " + lti);
				} else {
	...
	protected void checkAssertion(boolean cond, String failMsg) {
		if (!cond) {
			System.out.println("!!!!!! failed thread consistency: " + this
					+ ": " + failMsg);
			vm.dumpThreadStates();
			assert false;
		}

 ...
 * 
 * builtin type: byte - T_BOOLEAN and the like type name: String - according to
 * JLS 6.7 ("int", "x.Y[]") type signature: String - like JNI ("I", "[Lx/Y;")
 * type classname: String - e.g. "int", "[I", "x.Y", "[Lx.Y;"
 */
public class Types {

	// these have the same values as the BCEL Constants since we don't want to
	// break compiled code
	...
	public static final byte T_ARRAY = 13; // <2do> do we need this in addition
											// to T_REFERENCE?
	...
	public static String getArgumentSignature(String[] typeNames,
			boolean qualified) {
	...
	public static String getJNIMangledMethodName(String cls, String name,
		...
		for (i = 1; i < slen; i++) {
			...
			case ')':
				// the return type part - note this is not JNI, but saves us a
				// lot of trouble with
	 ...
	 * 
	 * int -> int int[] -> [I String -> java.lang.String String[] ->
	...
	public static String getJNITypeCode(String type) {
		...
		if (type.equals("int") || type.equals("I")) {
			...
			sb.append('F');
		} else if (type.equals("void") || type.equals("V")) { // for return
																// types
			sb.append('V');
		} else { // reference type
			if (type.charAt(0) != 'L') {
	...
	/**
	 * get the canonical representation of a type name, which happens to be (1)
	 * the name of the builtin type (e.g. "int") (2) the normal dot name for
	 * ordinary classes (e.g. "java.lang.String") (3) the coded dot name for
	 * arrays (e.g. "[B", "[[C", or "[Ljava.lang.String;")
	 * 
	...
	public static boolean isBasicType(String typeName) {
		return ("boolean".equals(typeName) || "byte".equals(typeName)
				...
				|| "long".equals(typeName) || "double".equals(typeName)
	...
	/**
	 * return the qualified signature name according to JLS 6.7 (e.g. "int",
	 * "x.Y[]")
	 */
	...
	/**
	 * thoses are according to the arrayType codes of the newarray JVMS
	 * definition
	 */
	...
	/**
	 * what would be the info size in bytes, not words (we ignore 64bit machines
	 * for now)
	 */
	public static int getTypeSizeInBytes(String signature) {
		...
		case 'Z': // that's a stretch, but we assume boolean uses the smallest
					// addressable size
		case 'B':
			return 1;
	...
	public static boolean instanceOf(String type, String ofType) {
		...
		case T_REFERENCE:
			ClassInfo ci = ClassLoaderInfo
					.getCurrentResolvedClassInfo(getTypeName(type));
	...
	/**
	 * return the JPF internal representation of a method signature that is
	 * given in dot-notation (like javap),
	 * 

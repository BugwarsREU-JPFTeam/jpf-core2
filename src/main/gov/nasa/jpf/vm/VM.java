...
public abstract class VM {
	...
	/**
	 * this is a debugging aid to control compilation of expensive consistency
	 * checks (we don't control these with class-wise assertion enabling since
	 * we do use unconditional assertions for mandatory consistency checks)
	 */
	...
	/**
	 * The number of errors saved so far. Used to generate the name of the error
	 * trail file.
	 */
	...
	/** buffer to store output along path execution */

	/**
	 * various caches for VMListener state acquisition. NOTE - these are only
	 * valid during notification
	 * 
	 * <2do> get rid of the 'lasts' in favor of queries on the insn, the
	 * executing thread, and the VM. This is superfluous work to for every
	 * notification (even if there are no listeners using it) that can easily
	 * lead to inconsistencies
	 */
	protected Transition lastTrailInfo;

	protected boolean isTraceReplay; // can be set by listeners to indicate this
										// is a replay
	...
	/**
	 * this was the last stateId - note this is also used for stateless model
	 * checking
	 */
	...
	// a list of actions to be run post GC. This is a bit redundant to
	// VMListener,
	// but in addition to avoid the per-instruction execution overhead of a
	// VMListener
	...
	public VM(JPF jpf, Config conf) {
		...
		// we have to defer setting pathOutput until we have a reporter
		// registered
		...
		userliveNonDaemonPredicate = new Predicate<ThreadInfo>() {
			...
			public boolean isTrue(ThreadInfo ti) {
				return (!ti.isDaemon() && !ti.isTerminated() && !ti
						.isSystemThread());
			}
		};
	...
	/**
	 * just here for unit test mockups, don't use as implicit base ctor in VM
	 * derived classes
	 */
	protected VM() {
	}
	...
	public void initFields(Config config) {
		...
		backtracker = config.getEssentialInstance("vm.backtracker.class",
				Backtracker.class);
	...
	protected void initTimeModel(Config config) {
		...
		timeModel = config.getEssentialInstance("vm.time.class",
				TimeModel.class, argTypes, args);
	}
	...
	/**
	 * do we see our model classes? Some of them cannot be used from the
	 * standard CLASSPATH, because they are tightly coupled with the JPF core
	 * (e.g. java.lang.Class, java.lang.Thread, java.lang.StackTraceElement
	 * etc.) Our strategy here is kind of lame - we just look into
	 * java.lang.Class if we find the 'uniqueId' field (that's a true '42')
	 */
	static boolean checkSystemClassCompatibility(
			SystemClassLoaderInfo systemLoader) {
	...
	protected ThreadInfo createMainThreadInfo(int id, ApplicationContext appCtx) {
		...
		ThreadInfo.currentThread = tiMain; // we still need this for listeners
											// that process startup class
											// loading events
	...
	protected ThreadInfo createThreadInfo(int objRef, int groupRef,
			int runnableRef, int nameRef) {
		ThreadInfo tiCurrent = ThreadInfo.getCurrentThread();
		ThreadInfo tiNew = new ThreadInfo(this, objRef, groupRef, runnableRef,
				nameRef, tiCurrent);

		// note that we have to register here so that subsequent native peer
		// calls can use the objRef
		// to lookup the ThreadInfo. This is a bit premature since the thread is
		// not runnable yet,
		// but chances are it will be started soon, so we don't waste another
		// data structure to do the mapping
	...
	protected ThreadInfo createFinalizerThreadInfo(int id,
	...
	protected List<String> getStartupSystemClassNames() {
		...
		// we could be more fancy and use wildcard patterns and the current
		// classpath
		// to specify extra classes, but this could be VERY expensive. Projected
		// use
		// is mostly to avoid static init of single classes during the search
		String[] extraStartupClasses = config
				.getStringArray("vm.extra_startup_classes");
		...
		// the main class has to be handled separately since it might be VM
		// specific
	 ...
	 * 
	 * If system classes are not found, or are not valid JPF model classes, we
	 * throw a JPFConfigException and exit
	 * 
	 * returned ClassInfos are not yet registered in Statics and don't have
	 * class objects
	 */
	protected List<ClassInfo> getStartupSystemClassInfos(
			SystemClassLoaderInfo sysCl, ThreadInfo tiMain) {
		...
		try {
			for (String clsName : getStartupSystemClassNames()) {
				...
				ci.registerStartupClass(tiMain, list); // takes care of
														// superclasses and
														// interfaces
			}
		} catch (ClassInfoException e) {
			throw new JPFConfigException("cannot load system class "
					+ e.getFailedClass());
		}
	...
	/**
	 * this adds the application main class and its supers to the list of
	 * startup classes
	 */
	protected ClassInfo getMainClassInfo(SystemClassLoaderInfo sysCl,
			String mainClassName, ThreadInfo tiMain, List<ClassInfo> list) {
		try {
			ClassInfo ciMain = sysCl.getResolvedClassInfo(mainClassName);
			ciMain.registerStartupClass(tiMain, list); // this might add a
														// couple more

			return ciMain;

		} catch (ClassInfoException e) {
			throw new JPFConfigException("cannot load application class "
					+ e.getFailedClass());
		}
	}

	/*
	 * these are called when creating ApplicationContexts and can be overridden
	 * by concrete VM types
	 */
	protected SystemClassLoaderInfo createSystemClassLoaderInfo(int appId) {
		...
		SystemClassLoaderInfo sysCli = config.getEssentialInstance(
				"vm.classloader.class", SystemClassLoaderInfo.class, argTypes,
				args);
		return sysCli;
	}

	protected void createSystemClassLoaderObject(SystemClassLoaderInfo sysCl,
			ThreadInfo tiMain) {
		Heap heap = getHeap();

		// create ClassLoader object for the ClassLoader type defined by this
		// SystemClassLoaderInfo
		// NOTE - this requires the SystemClassLoaderInfo cache to be
		// initialized
	...
	protected void pushMainEntryArgs(MethodInfo miMain, String[] args,
			ThreadInfo tiMain, DirectCallStackFrame frame) {
		String sig = miMain.getSignature();
		Heap heap = getHeap();

		if (sig.contains("([Ljava/lang/String;)")) {
			ElementInfo eiArgs = heap.newArray("Ljava/lang/String;",
					args.length, tiMain);
			...
			throw new JPFException("unsupported main entry signature: "
					+ miMain.getFullName());
		}
	}

	protected void pushMainEntry(MethodInfo miMain, String[] args,
			ThreadInfo tiMain) {
		DirectCallStackFrame frame = miMain.createDirectCallStackFrame(tiMain,
				0);
	...
	protected MethodInfo getMainEntryMethodInfo(String mthName, ClassInfo ciMain) {
		...
		if (miMain == null || !miMain.isStatic()) {
			throw new JPFConfigException("no static entry method: "
					+ ciMain.getName() + '.' + mthName);
		}
	...
	protected void pushClinits(List<ClassInfo> startupClasses, ThreadInfo tiMain) {
		for (ClassInfo ci : startupClasses) {
			...
			if (mi != null) {
				DirectCallStackFrame frame = mi.createDirectCallStackFrame(
						tiMain, 0);
				tiMain.pushFrame(frame);
			} else {
				ci.setInitialized();
	...
	/**
	 * this is the main initialization point that sets up startup objects
	 * threads and callstacks. If this returns false VM initialization cannot
	 * proceed and JPF will terminate
	 */
	public abstract boolean initialize();

	/**
	 * create and initialize the main thread for the given ApplicationContext.
	 * This is called from VM.initialize() implementations, the caller has to
	 * handle exceptions that should be reported differently
	 * (JPFConfigException, ClassInfoException)
	 */
	protected ThreadInfo initializeMainThread(ApplicationContext appCtx, int tid) {
		...
		List<ClassInfo> startupClasses = getStartupSystemClassInfos(sysCl,
				tiMain);
		ClassInfo ciMain = getMainClassInfo(sysCl, appCtx.mainClassName,
				tiMain, startupClasses);

		if (!checkSystemClassCompatibility(sysCl)) {
			throw new JPFConfigException(
					"non-JPF system classes, check classpath");
		}
	...
	protected void initializeFinalizerThread(ApplicationContext appCtx, int tid) {
		if (processFinalizers) {
			...
			finalizerTi = (FinalizerThreadInfo) createFinalizerThreadInfo(tid,
					app);
	...
	protected void initSystemState(ThreadInfo mainThread) {
		...
		// the first transition probably doesn't have much choice (unless there
		// were
		// threads started in the static init), but we want to keep it uniformly
		// anyways
	...
	/**
	 * to be called from the Heap after GC is completed (i.e. only live objects
	 * remain)
	 */
	...
	/*
	 * while some of these can be called from various places, the calls that
	 * happen from within Instruction.execute() should happen right before
	 * return since listeners might do things such as
	 * ThreadInfo.createAndThrowException(..), i.e. cause side effects that
	 * would violate consistency requirements of successive operations (e.g. by
	 * assuming we are still executing in the same StackFrame - after throwing
	 * an exception)
	 */
	...
	protected void notifyVMInitialized() {
		try {
			...
			throw new JPFListenerException(
					"exception during vmInitialized() notification", t);
		}
	}
	...
	protected void notifyChoiceGeneratorRegistered(ChoiceGenerator<?> cg,
		...
		try {
			for (int i = 0; i < listeners.length; i++) {
				listeners[i]
						.choiceGeneratorRegistered(this, cg, ti, ti.getPC());
			}
		} catch (UncaughtException x) {
			throw x;
		} catch (JPF.ExitException x) {
			...
			throw new JPFListenerException(
					"exception during choiceGeneratorRegistered() notification",
					t);
		}
	}
	...
	protected void notifyChoiceGeneratorSet(ChoiceGenerator<?> cg) {
		try {
			...
			throw new JPFListenerException(
					"exception during choiceGeneratorSet() notification", t);
		}
	}
	...
	protected void notifyChoiceGeneratorAdvanced(ChoiceGenerator<?> cg) {
		try {
			...
			throw new JPFListenerException(
					"exception during choiceGeneratorAdvanced() notification",
					t);
		}
	}
	...
	protected void notifyChoiceGeneratorProcessed(ChoiceGenerator<?> cg) {
		try {
			...
			throw new JPFListenerException(
					"exception during choiceGeneratorProcessed() notification",
					t);
		}
	}
	...
	protected void notifyThreadStarted(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during threadStarted() notification", t);
		}
	}

	// NOTE: the supplied ThreadInfo does NOT have to be the running thread, as
	// this
	// notification can occur as a result of a lock operation in the current
	// thread
	protected void notifyThreadBlocked(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during threadBlocked() notification", t);
		}
	}
	...
	protected void notifyThreadWaiting(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during threadWaiting() notification", t);
		}
	}
	...
	protected void notifyThreadNotified(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during threadNotified() notification", t);
		}
	}
	...
	protected void notifyThreadInterrupted(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during threadInterrupted() notification", t);
		}
	}
	...
	protected void notifyThreadTerminated(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during threadTerminated() notification", t);
		}
	}
	...
	protected void notifyThreadScheduled(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during threadScheduled() notification", t);
		}
	}
	...
	protected void notifyLoadClass(ClassFile cf) {
		try {
			...
			throw new JPFListenerException(
					"exception during classLoaded() notification", t);
		}
	}
	...
	protected void notifyObjectCreated(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectCreated() notification", t);
		}
	}
	...
	protected void notifyObjectReleased(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectReleased() notification", t);
		}
	}
	...
	protected void notifyObjectLocked(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectLocked() notification", t);
		}
	}
	...
	protected void notifyObjectUnlocked(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectUnlocked() notification", t);
		}
	}
	...
	protected void notifyObjectWait(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectWait() notification", t);
		}
	}

	protected void notifyObjectExposed(ThreadInfo ti, ElementInfo eiShared,
		...
		try {
			...
			throw new JPFListenerException(
					"exception during objectExposed() notification", t);
		}
	}
	...
	protected void notifyObjectShared(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectShared() notification", t);
		}
	}
	...
	protected void notifyObjectNotifies(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectNotifies() notification", t);
		}
	}
	...
	protected void notifyObjectNotifiesAll(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during objectNotifiesAll() notification", t);
		}
	}
	...
	protected void notifyGCBegin() {
		try {
			...
			throw new JPFListenerException(
					"exception during gcBegin() notification", t);
		}
	}
	...
	protected void notifyGCEnd() {
		try {
			...
			throw new JPFListenerException(
					"exception during gcEnd() notification", t);
		}
	}
	...
	protected void notifyExceptionThrown(ThreadInfo ti, ElementInfo ei) {
		try {
			...
			throw new JPFListenerException(
					"exception during exceptionThrown() notification", t);
		}
	}
	...
	protected void notifyExceptionBailout(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during exceptionBailout() notification", t);
		}
	}
	...
	protected void notifyExceptionHandled(ThreadInfo ti) {
		try {
			...
			throw new JPFListenerException(
					"exception during exceptionHandled() notification", t);
		}
	}
	...
	protected void notifyMethodEntered(ThreadInfo ti, MethodInfo mi) {
		try {
			...
			throw new JPFListenerException(
					"exception during methodEntered() notification", t);
		}
	}
	...
	protected void notifyMethodExited(ThreadInfo ti, MethodInfo mi) {
		try {
			...
			throw new JPFListenerException(
					"exception during methodExited() notification", t);
		}
	}
	 ...
	 * 
	 * The path only contains all states when queried from a stateAdvanced()
	 * notification. If this is called from an instructionExecuted() (or other
	 * VMListener), and you need the ongoing transition in it, you have to call
	 * updatePath() first
	 */
	public Path getPath() {
		return path;
	...
	/**
	 * this is the ongoing transition. Note that it is not yet stored in the
	 * path if this is called from a VMListener notification
	 */
	...
	public <T> StateRestorer<T> getRestorer() {
		if (restorer == null) {
			if (serializer instanceof StateRestorer) {
				...
				restorer = config.getInstance("vm.restorer.class",
						StateRestorer.class);
			}
	...
	/**
	 * return the last registered SystemState's ChoiceGenerator object NOTE:
	 * there might be more than one ChoiceGenerator associated with the current
	 * transition (ChoiceGenerators can be cascaded)
	 */
	...
	public void setMandatoryNextChoiceGenerator(ChoiceGenerator<?> cg,
			String failMsg) {
		ss.setMandatoryNextChoiceGenerator(cg, failMsg);
	}

	/**
	 * return the latest registered ChoiceGenerator used in this transition that
	 * matches the provided 'id' and is of 'cgType'.
	 * 
	 * This should be the main getter for clients that are cascade aware
	 */
	public <T extends ChoiceGenerator<?>> T getCurrentChoiceGenerator(
			String id, Class<T> cgType) {
	...
	public <T extends ChoiceGenerator<?>> T[] getChoiceGeneratorsOfType(
			Class<T> cgType) {
		return ss.getChoiceGeneratorsOfType(cgType);
	}

	public <T extends ChoiceGenerator<?>> T getLastChoiceGeneratorOfType(
			Class<T> cgType) {
	...
	/**
	 * print call stacks of all live threads this is also used for debugging
	 * purposes, so we can't move it to the Reporter system (it's also using a
	 * bit too many internals for that)
	 */
	 ...
	 * used by the search object. Note this is called with the state that caused
	 * the backtrack still being on the stack, so we have to remove that one
	 * first (i.e. popping two states and restoring the second one)
	 */
	...
	/**
	 * store the current SystemState's Trail in our path, after updating it with
	 * whatever annotations the VM wants to add. This is supposed to be called
	 * after each transition we want to keep
	 */
	public void updatePath() {
		Transition t = ss.getTrail();
		Transition tLast = path.getLast();

		// NOTE: don't add the transition twice, this is public and might get
		// called
		// from listeners, so the transition object might get changed

		if (tLast != t) {
			// <2do> we should probably store the output directly in the
			// TrailInfo,
	 ...
	 * 
	 * forward() and backtrack() are the two primary interfaces towards the
	 * Search driver. note that the caller still has to check if there is a next
	 * state, and if the executed instruction sequence led into a new or already
	 * visited state
	 * 
	 * @return 'true' if there was an un-executed sequence out of the current
	 *         state, 'false' if it was completely explored
	 * 
	 */
	public boolean forward() {

		// the reason we split up CG initialization and transition execution
		// is that program state storage is not required if the CG
		// initialization
		...
		// CG listener intervention (i.e. not just after backtracking). For a
		// large
		// number of matched or end states and ignored transitions this can be a
		// huge saving.
		// The downside is that CG notifications are NOT allowed anymore to
		// change the
		// KernelState (modify fields or thread states) since those changes
		// would
		// happen before storing the KernelState, and hence would make
		// backtracking
		// inconsistent. This is advisable anyways since all program state
		// changes
		...
		if (transitionOccurred) {
			...
			try {
				...
				// we don't pass this up since it means there were insns
				// executed and we are
			...
			if (!isIgnoredState()) {
				...
				// matching states out of ignored transitions is also not a good
				// idea
				...
				} else { // this is 'state-less' model checking, i.e. we don't
							// match states
					ss.setId(++newStateId); // but we still should have states
											// numbered in case listeners use
											// the id
				}
			}
	...
	// --- various state attribute getters and setters (mostly forwarding to
	// SystemState)
	 ...
	 * the heap or stacks. use this with care, since it prunes whole search
	 * subtrees
	 */
	 ...
	 * 
	 * this returns true if no state has been produced yet, and false if no
	 * transition occurred after a forward call
	 */
	public boolean isNewState() {
		...
		} else { // stateless model checking - each transition leads to a new
					// state
			return true;
		}
	}
	 ...
	 * since for MultiprcessesVM one can decide when to terminate (after the the
	 * termination of all processes or only one process). todo - that needs to
	 * be specified through the properties file
	 */
	...
	/**
	 * get the numeric id for the current state Note: this can be called several
	 * times (by the search and observers) for every forward()/backtrack(), so
	 * we want to cache things a bit
	 */
	...
	/**
	 * <2do> this is a band aid to bundle all these legacy reference chains from
	 * JPFs past. The goal is to replace them with proper accessors (normally
	 * through ThreadInfo, MJIEnv or VM, which all act as facades) wherever
	 * possible, and use VM.getVM() where there is no access to such a facade.
	 * Once this has been completed, we can start refactoring the users of
	 * VM.getVM() to get access to a suitable facade.
	 */
	 ...
	 * any better if listeners have to dig this out from JPF Note - this isn't
	 * set during initialization, since the VM object is created first
	 */
	...
	/**
	 * given an object reference, it returns the ApplicationContext of the
	 * process to which this object belongs
	 */
	...
	/**
	 * Add a given finalizable object to the finalizeQueue array of
	 * java.lang.ref.Finalizer model class. This is invoked from the sweep()

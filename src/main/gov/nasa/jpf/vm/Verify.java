...
public class Verify {
	...
	static int[] counter; // only here so that we don't pull in all JPF classes
							// at RT
	 ...
	 * enable encapsulation of JPF specific types so that they only get pulled
	 * in on demand, and we otherwise can still use the same Verify class for
	 * JPF-external execution. We use a class object to make sure it doesn't get
	 * recycled once JPF is terminated.
	 */
	static Class<?> peer;
	 ...
	 * The sequence usually is JPF(Verify) -> VM(JPF_gov_nasa_jpf_vm_Verify) ->
	 * VM(Verify)
	 */
	...
	// note this is NOT marked native because we might also call it from host VM
	// code
	...
	public static int incrementCounter(int id) {
		if (peer != null) {
			return JPF_gov_nasa_jpf_vm_Verify.incrementCounter__I__I(null, 0,
					id);
		} else {
			if (counter == null) {
				counter = new int[(id >= MAX_COUNTERS) ? id + 1 : MAX_COUNTERS];
	...
	public static void putValue(String key, int value) {
		throw new UnsupportedOperationException(
				"putValue requires JPF execution");
	}

	public static int getValue(String key) {
		throw new UnsupportedOperationException(
				"getValue requires JPF execution");
	}
	...
	public static void setBitInBitSet(int id, int bit, boolean value) {
		if (peer != null) {
			...
			JPF_gov_nasa_jpf_vm_Verify.setBitInBitSet__IIZ__V(null, 0, id, bit,
					value);
		} else {
	...
	public static boolean getBitInBitSet(int id, int bit) {
		if (peer != null) {
			// this is executed if we did run JPF
			return JPF_gov_nasa_jpf_vm_Verify.getBitInBitSet__II__Z(null, 0,
					id, bit);
	...
	/**
	 * Marks the beginning of an atomic block. THIS IS EVIL, DON'T USE IT FOR
	 * OPTIMIZATION - THAT'S WHAT POR IS FOR! (it's mostly here to support model
	 * classes that need to execute atomic)
	 */
	public static void beginAtomic() {
	}

	/**
	 * Marks the end of an atomic block. EVIL - see beginAtomic()
	 */
	...
	/**
	 * simple debugging aids to imperatively print the current path output of
	 * the SUT (to be used with vm.path_output)
	 */
	 ...
	 * propagated correctly, w/o having to write a listener), since attributes
	 * are supposed to be created at the native side, and hence can't be
	...
	public static int[] getFieldAttributes(Object o, String fieldName) {
		return new int[0];
	}
	...
	/**
	 * this is the new boolean choice generator. Since there's no real heuristic
	 * involved with boolean values, we skip the id (it's a hardwired "boolean")
	 */
	...
	/**
	 * new boolean choice generator that also tells jpf which value to use first
	 * by default in a search.
	 */
	 ...
	 * both the corresponding ChoiceGenerator subclass, and the application
	 * specific ctor parameters from the normal JPF configuration mechanism
	 */
	...
	/**
	 * this is the API for double value choice generators. 'id' is used to
	 * identify both the corresponding ChoiceGenerator subclass, and the
	 * application specific ctor parameters from the normal JPF configuration
	 * mechanism
	 */
	...
	/**
	 * Returns a random boolean value, true or false. Note this gets handled by
	 * the native peer, and is just here to enable running instrumented
	 * applications w/o JPF
	 */
	...
	/**
	 * USE CAREFULLY - returns true if the virtual machine this code is running
	 * under is doing state matching. This can be used as a hint as to whether
	 * data structures (that are known to be correct!) should be configured to
	 * use a canonical representation. For example,
	 * 
	 * <pre>
	 * <code>
	 * Vector v = new Vector();
	 ...
	 * </pre>
	 * 
	 * To the programmer, the states are (almost certainly) the same. To the VM,
	 * they could be different (capacity inside the Vector). For the sake of
	 * speed, Vector does not store things canonically, but this can cause
	 * (probably mild) state explosion if matching states. If you know whether
	 * states are being matched, you can choose the right structure--as long as
	 * those structures aren't what you're looking for bugs in!
	 */
	...
	public static void storeTraceIf(boolean cond, String fileName,
			String comment) {
		if (cond) {
	...
	public static void storeTraceAndTerminateIf(boolean cond, String fileName,
			String comment) {
		if (cond) {
			storeTrace(fileName, comment);
			terminateSearch();
	...
	/*
	 * we add these here so that we don't need to pull in any java.util.logging
	 * classes Note - these need to be kept in sync with our native peer
	 */
	...
	public static void log(String loggerId, int logLevel, String format,
			Object... args) {

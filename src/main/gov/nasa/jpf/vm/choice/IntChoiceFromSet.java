 ...
 * @author cartho
 * 
 *         choose from a set of values provided in configuration as xxx.class =
 *         IntChoiceFromSet xxx.values = {1, 2, 3, 400} where "xxx" is the
 *         choice id.
 * 
...
public class IntChoiceFromSet extends IntChoiceFromList {
	...
	/*
	 * Remove duplicate values; currently implemented as iteration on array to
	 * avoid heavyweight TreeSet
	 */
	private void removeDuplicates() {
		...
		for (int i = 0; i < len; i++) {
			int j = i + 1;
			while (j < len) {
				if (values[i] - values[j] == 0) { // strange comparison to avoid
													// unboxing
					values[j] = values[len - 1];
					len--;
					// don't increment j as new element has been placed there

 ...
 */
public abstract class NumberChoiceFromList<T extends Number> extends
	...
	public NumberChoiceFromList(Config conf, String id) {
		...
		// get the choice values here because otherwise successive
		// getNextChoice()
		// calls within the same transition could see different values when
		// looking
		// up fields and locals
		values = createValueArray(vals.length);
		StackFrame resolveFrame = ThreadInfo.getCurrentThread()
	...
	protected T parse(String varSpec, StackFrame resolveFrame) {
		...
		if (varSpec.isEmpty()) {
			throw new JPFException("illegal value spec for IntChoiceFromList "
					+ id);
		}
		...
		if (Character.isDigit(c)) { // its an integer literal
			...
			if (o == null) {
				throw new JPFException("no local or field '" + varSpec
						+ "' value found for NumberChoiceFromList " + id);
			}
			if (o instanceof Number) {
				return newValue((Number) o, sign);
			} else {
				throw new JPFException("non-numeric local or field '" + varSpec
						+ "' value for NumberChoiceFromList " + id);
			}
		}
	}

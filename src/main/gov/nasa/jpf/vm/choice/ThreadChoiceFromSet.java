...
public class ThreadChoiceFromSet extends ChoiceGeneratorBase<ThreadInfo>
		implements ThreadChoiceGenerator {
	...
	public ThreadChoiceFromSet(String id, ThreadInfo[] set,
		...
		/**
		 * if (isSchedulingPoint){ // do a sanity check to see if the candidates
		 * are acutally runnable for (int i = 0; i < set.length; i++) { if
		 * (!set[i].isTimeoutRunnable()) { throw new
		 * JPFException("trying to schedule non-runnable: " + set[i]); } } }
		 **/
	}
	 ...
	 * only change their status to TIMEDOUT when they are picked as the next
	 * choice)
	 * 
	 * <2do> this should be in SystemState.nextSuccessor - there might be other
	 * ThreadChoiceGenerators, and we should handle this consistently
	 */
	@Override
	public void advance() {

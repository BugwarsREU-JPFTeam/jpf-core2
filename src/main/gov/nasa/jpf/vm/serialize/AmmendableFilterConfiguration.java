...
public class AmmendableFilterConfiguration implements FilterConfiguration {
	...
	public interface FieldAmmendment extends InstanceAmmendment,
			StaticAmmendment {
	}
	...
	protected final ObjVector<FrameAmmendment> frameAmmendments = new ObjVector<FrameAmmendment>(
			0);

	protected final ObjVector<InstanceAmmendment> instanceAmmendments = new ObjVector<InstanceAmmendment>(
			0);

	protected final ObjVector<StaticAmmendment> staticAmmendments = new ObjVector<StaticAmmendment>(
			0);

	protected final ObjVector<InstanceOverride> instanceOverrides = new ObjVector<InstanceOverride>(
			0);
	...
	protected void appendConfiguredFrameAmmendments(Config config) {
		ArrayList<FrameAmmendment> fas = config.getInstances(
				"filter.frame_ammendments", FrameAmmendment.class);
		if (fas != null)
			frameAmmendments.append(fas);
	}

	protected void appendConfiguredInstanceAmmendments(Config config) {
		ArrayList<InstanceAmmendment> ias = config.getInstances(
				"filter.instance_ammendments", InstanceAmmendment.class);
		if (ias != null)
			instanceAmmendments.append(ias);
	}

	protected void appendConfiguredStaticAmmendments(Config config) {
		ArrayList<StaticAmmendment> sas = config.getInstances(
				"filter.static_ammendments", StaticAmmendment.class);
		if (sas != null)
			staticAmmendments.append(sas);
	}

	protected void appendConfiguredInstanceOverrides(Config config) {
		ArrayList<InstanceOverride> ios = config.getInstances(
				"filter.instance_overrides", InstanceOverride.class);
		if (ios != null)
			instanceOverrides.append(ios);
	}
	...
	// not implemented
	/*
	 * public BitArray getFrameLocalInclusion(MethodInfo mi) { BitArray cur =
	 * new BitArray(mi.getMaxLocals()); cur.setAll(); // by default include
	 * everything for (FrameAmmendment fa : frameAmmendments) { cur =
	 * fa.ammendLocalInclusion(mi, cur); } return cur; }
	 */

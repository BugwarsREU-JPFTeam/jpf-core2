 ...
 * achieve heap symmetry. It does so by storing the order in which objects are
 * referenced, not the reference values themselves.
 * 
 * Use this serializer if the Heap implementation does not provide sufficient
 * symmetry, i.e. reference values depend on the order of thread scheduling.
 * 
 * Ad hoc heap symmetry is hard to achieve in the heap because of static
 * initialization. Each time a thread loads a class all the static init (at
 * least the class object and its fields) are associated with this thread, hence
 * thread reference values depend on which classes are already loaded by other
 * threads. Associating all allocations from inside of clinits to one address
 * range doesn't help either because then this range will experience scheduling
 * dependent orders. A hybrid approach in which only this segment is
 * canonicalized might work, but it is questionable if the overhead is worth the
 * effort.
 */
public class CFSerializer extends FilteringSerializer {
	...
	// the sid to 0 upon backtrack, and counting either upwards from 1 or
	// downwards
	...
	protected void serializeFrame(StackFrame frame) {
		...
		// would use concrete reference values as hash data, i.e. break heap
		// symmetry

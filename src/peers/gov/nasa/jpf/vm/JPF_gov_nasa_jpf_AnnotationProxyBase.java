...
/**
 * native peer for Annotation Proxies (saves us some bytecode interpretation
 * shoe leather)
 */
public class JPF_gov_nasa_jpf_AnnotationProxyBase extends NativePeer {
	...
	public int annotationType____Ljava_lang_Class_2(MJIEnv env, int objref) {
		...
		// we could also pull it out from the interfaces, but we know the naming
		// scheme
		...
		if (fields.length > 0) {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (ft.equals("int")) {
					...
					sb.append(env.getStringObject(env.getReferenceField(objref,
							fn)));
					...
					if (ft.equals("java.lang.String[]")) {
						for (int j = 0; j < n; j++) {
							if (j > 0)
								sb.append(',');
							sb.append(env.getStringObject(env
									.getReferenceArrayElement(ar, j)));
						}

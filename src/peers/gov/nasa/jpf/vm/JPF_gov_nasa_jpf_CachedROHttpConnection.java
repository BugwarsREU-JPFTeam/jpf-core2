...
/**
 * very simple URLConnection model that can be used for reading static URL
 * contents
 * 
 * The data can be stored in/read from the file system, and is cached to avoid
 * DOS by means of model checking
 */
public class JPF_gov_nasa_jpf_CachedROHttpConnection extends NativePeer {
	...
	public JPF_gov_nasa_jpf_CachedROHttpConnection(Config conf) {
		...
		if (cacheDirPath != null) {
			...
			if (!cacheDir.isDirectory()) {
				throw new JPFConfigException("illegal http.cache_dir entry: "
						+ cacheDirPath);
			}
		}
	...
	private byte[] getDataFromCachedFile(String url) {
		...
		if (cacheFile.isFile()) {
			try {
				...
				logger.warning("can't read http data from cached file ",
						cacheFile.getPath());
			}

			if (data != null) {
				logger.info("reading contents of ", url, " from file ",
						cacheFile.getPath());
	...
	private byte[] getDataFromURL(String surl) {
		...
		try {
			...
			if (is != null) {
				...
				if (cacheDir != null) {
					...
					try {
						...
						logger.info("storing contents of ", surl, " to file ",
								cacheFile.getPath());
					} catch (IOException iox) {
						logger.warning("can't store to cache directory ",
								cacheFile.getPath());
					}
				}
			...
			logger.warning("reading URL data ", surl, " failed with ",
					ex.getMessage());
		}
	...
	@MJI
	public int getContents__Ljava_lang_String_2___3B(MJIEnv env, int objRef,
			int surlRef) {

 ...
 * 
 *         A native peer for FinalizerThread. This is also an interface between
 *         the FinalizerThread object at the SUT level and it corresponding
 *         FinalizerThreadInfo object at the JPF level.
 */
public class JPF_gov_nasa_jpf_FinalizerThread extends NativePeer {

	@MJI
	public void runFinalizer__Ljava_lang_Object_2__V(MJIEnv env, int tiRef,
			int objRef) {
	...
	// removes the very first element in the list, which is the last finalizable
	// objects processed
	void removeElement(MJIEnv env, int tiRef, int objRef) {
		...
		ElementInfo newQueue = env.getHeap().newArray("Ljava/lang/Object;",
				len, ti);
		int[] newValues = newQueue.asReferenceArray();

		System.arraycopy(oldValues, 1, newValues, 0, len);
		env.getModifiableElementInfo(tiRef).setReferenceField("finalizeQueue",
				newQueue.getObjectRef());
	}

	// a predicate to obtain all alive, non-finalizer threads within the ti
	// process
	...
	public void manageState____V(MJIEnv env, int objref) {
		...
		// check for termination - Note that the finalizer thread has to be the
		// last alive thread
		// of the process
		if (!vm.getThreadList().hasAnyMatching(
				getAppAliveUserPredicate(finalizerTi))) {
			shutdown(env, objref);
		}
		// make the thread wait until more objects are added to finalizerQueue
			...
			ChoiceGenerator<ThreadInfo> cg = env.getSystemState()
					.getSchedulerFactory().createPostFinalizeCG(finalizerTi);
			env.getSystemState().setMandatoryNextChoiceGenerator(cg,
					"finalizeQueue processe without CG: ");
		}
	}

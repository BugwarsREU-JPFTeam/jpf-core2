...
public class JPF_gov_nasa_jpf_SerializationConstructor extends NativePeer {
	...
	public int newInstance___3Ljava_lang_Object_2__Ljava_lang_Object_2(
		...
		int superCtorRef = env.getReferenceField(mthRef,
				"firstNonSerializableCtor");
		MethodInfo miCtor = JPF_java_lang_reflect_Constructor.getMethodInfo(
				env, superCtorRef);
		...
		if (frame == null) { // first time
			...
			frame.setLocalReferenceVariable(0, objRef); // (1) we store the
														// reference as a local
														// var for retrieval
														// during reexec
			ti.pushFrame(frame);

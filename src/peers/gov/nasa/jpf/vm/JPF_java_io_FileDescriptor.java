...
/**
 * native peer for file descriptors, which are our basic interface to access
 * file contents. The implementation used here just forwards to
 * FileInputStreams, which is terribly inefficient for frequent restores (in
 * which case a simple byte[] buffer would be more efficient)
 */
public class JPF_java_io_FileDescriptor extends NativePeer {
	...
	public int open__Ljava_lang_String_2I__I(MJIEnv env, int objref,
		...
		if (mode == FD_READ) {
			...
			env.throwException("java.io.IOException", "illegal open mode: "
					+ mode);
			return -1;
		}
	}
	...
	void reopen(MJIEnv env, int objref) throws IOException {
		...
		if (content.get(fd) == null) {
			...
			if (mode == FD_READ) {
				...
				env.throwException("java.io.IOException", "illegal mode: "
						+ mode);
			}
		}
	}
			...
			if (fs != null) {
				if (fs instanceof FileOutputStream) {
					...
					env.throwException("java.io.IOException",
							"write attempt on file opened for read access");
				}
	...
	@MJI
	public void write___3BII__(MJIEnv env, int objref, int bref, int offset,
			int len) {
		int fd = env.getIntField(objref, "fd");
		long off = env.getLongField(objref, "off");
		...
		try {
			...
			if (fs != null) {
				if (fs instanceof FileOutputStream) {
					...
					byte[] buf = new byte[len]; // <2do> make this a permanent
												// buffer
		...
		try {
			...
			if (fs != null) {
				if (fs instanceof FileInputStream) {
					...
					env.throwException("java.io.IOException",
							"read attempt on file opened for write access");
					return -1;
				}
	...
	@MJI
	public int read___3BII__I(MJIEnv env, int objref, int bufref, int offset,
			int len) {
		int fd = env.getIntField(objref, "fd");
		long off = env.getLongField(objref, "off");
		...
		try {
			...
			if (fs != null) {
				...
				if (env.getIntField(objref, "state") == FD_OPENED) { // backtracked
					reopen(env, objref);
					return read___3BII__I(env, objref, bufref, offset, len); // try
																				// again
				} else {
					env.throwException("java.io.IOException",
							"read attempt on closed file");
		...
		try {
			...
			if (fs != null) {
				if (fs instanceof FileInputStream) {
					...
					env.throwException("java.io.IOException",
							"skip attempt on file opened for write access");
					return -1;
				}

			} else {
				env.throwException("java.io.IOException",
						"skip attempt on closed file");
				return -1;
			}
		...
		try {
			...
			if (fs != null) {
				if (fs instanceof FileInputStream) {
					...
					env.throwException("java.io.IOException",
							"available() on file opened for write access");
					return -1;
				}

			} else {
				env.throwException("java.io.IOException",
						"available() on closed file");
				return -1;
			}

...
public class JPF_java_io_RandomAccessFile extends NativePeer {
	...
	static ClassInfo getDataRepresentationClassInfo(MJIEnv env) {
		...
		ClassInfo ci = ClassLoaderInfo
				.getSystemResolvedClassInfo(DataRepresentation);
	...
	public void writeByte__I__V(MJIEnv env, int this_ptr, int data) {
		...
		int chunk = findDataChunk(env, this_ptr, current_posn, chunk_size);
		...
		if (current_posn >= current_len) {
			...
			env.setLongField(getMapping(env, this_ptr), current_length,
					current_posn + 1);
		}
	}
	...
	public int read___3BII__I(MJIEnv env, int this_ptr, int data_array,
		...
		while (i < len && current_posn < current_len) {
			env.setByteArrayElement(data_array, start + i,
					readByte____B(env, this_ptr));
			i += 1;
			current_posn += 1;
		}
		if (i == 0) {
	...
	private static final String DataRepresentation = RandomAccessFile
			+ "$DataRepresentation";
	...
	private static void setDataValue(MJIEnv env, int chunk_obj, long position,
		...
		env.setIntArrayElement(int_array, index,
				(old_value & ~(0xff << bit_shift)) | data_value << bit_shift);
	}

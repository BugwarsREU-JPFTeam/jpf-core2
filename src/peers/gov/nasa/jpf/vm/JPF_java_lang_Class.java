...
public class JPF_java_lang_Class extends NativePeer {
	...
	public static boolean init(Config conf) {
		// we create Method and Constructor objects, so we better make sure
		// these
	...
	@MJI
	public boolean isAssignableFrom__Ljava_lang_Class_2__Z(MJIEnv env,
			int rcls, int r1) {
	...
	@MJI
	public int getAnnotations_____3Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int robj) {
		ClassInfo ci = env.getReferredClassInfo(robj);
	...
	@MJI
	public int getAnnotation__Ljava_lang_Class_2__Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int robj, int annoClsRef) {
		ClassInfo ci = env.getReferredClassInfo(robj);
	...
	@MJI
	public int getPrimitiveClass__Ljava_lang_String_2__Ljava_lang_Class_2(
			MJIEnv env, int rcls, int stringRef) {
		...
		// note this does NOT return the box class (e.g. java.lang.Integer),
		// which
		// is a normal, functional class, but a primitive class (e.g. 'int')
		// that
		// is rather a strange beast (not even Object derived)

		ClassLoaderInfo scli = env.getSystemClassLoaderInfo(); // this is the
																// one
																// responsible
																// for builtin
																// classes
		String primClsName = env.getStringObject(stringRef); // always
																// initialized
	...
	/**
	 * this is an example of a native method issuing direct calls - otherwise
	 * known as a round trip. We don't have to deal with class init here
	 * anymore, since this is called via the class object of the class to
	 * instantiate
	 */
	@MJI
	public int newInstance____Ljava_lang_Object_2(MJIEnv env, int robj) {
		...
		MethodInfo miCtor = ci.getMethod("<init>()V", true); // note there
																// always is one
																// since
																// something
																// needs to call
		...
		if (frame == null) { // first time around
			...
			// note that we don't set this as a reflection call since it is
			// supposed to propagate exceptions
			frame.setReferenceArgument(0, objRef, null);
			frame.setLocalReferenceVariable(0, objRef); // (1) store ref for
														// retrieval during
														// re-exec
			ti.pushFrame(frame);
			...
			int objRef = frame.getLocalVariable(0); // that's the object ref we
													// set in (1)
			return objRef;
		}
	}
	...
	int getMethod(MJIEnv env, int clsRef, ClassInfo ciMethod, String mname,
			int argTypesRef, boolean isRecursiveLookup, boolean publicOnly) {

		ClassInfo ci = env.getReferredClassInfo(clsRef);

		StringBuilder sb = new StringBuilder(mname);
		sb.append('(');
		int nParams = argTypesRef != MJIEnv.NULL ? env
				.getArrayLength(argTypesRef) : 0;
		...
		if (mi == null || (publicOnly && !mi.isPublic())) {
			env.throwException("java.lang.NoSuchMethodException", ci.getName()
					+ '.' + fullMthName);
			return MJIEnv.NULL;
	...
	int createMethodObject(MJIEnv env, ClassInfo objectCi, MethodInfo mi) {
		...
		if (mi.isCtor()) {
			return JPF_java_lang_reflect_Constructor.createConstructorObject(
					env, objectCi, mi);
		} else {
			return JPF_java_lang_reflect_Method.createMethodObject(env,
					objectCi, mi);
		}
	}

	@MJI
	public int getDeclaredMethod__Ljava_lang_String_2_3Ljava_lang_Class_2__Ljava_lang_reflect_Method_2(
			MJIEnv env, int clsRef, int nameRef, int argTypesRef) {
	...
	@MJI
	public int getDeclaredConstructor___3Ljava_lang_Class_2__Ljava_lang_reflect_Constructor_2(
			MJIEnv env, int clsRef, int argTypesRef) {
		...
		int ctorRef = getMethod(env, clsRef, mci, "<init>", argTypesRef, false,
				false);
		return ctorRef;
	}

	@MJI
	public int getMethod__Ljava_lang_String_2_3Ljava_lang_Class_2__Ljava_lang_reflect_Method_2(
			MJIEnv env, int clsRef, int nameRef, int argTypesRef) {
		ClassInfo mci = getInitializedClassInfo(env, METHOD_CLASSNAME);
		if (mci == null) {
			env.repeatInvocation();
	...
	private void addDeclaredMethodsRec(HashMap<String, MethodInfo> methods,
			ClassInfo ci) {
		ClassInfo sci = ci.getSuperClass();
		if (sci != null) {
		...
		for (String ifcName : ci.getDirectInterfaceNames()) {
			ClassInfo ici = cl.getResolvedClassInfo(ifcName); // has to be
																// already
																// defined, so
																// no exception
	...
	@MJI
	public int getMethods_____3Ljava_lang_reflect_Method_2(MJIEnv env,
			int objref) {
		ClassInfo mci = getInitializedClassInfo(env, METHOD_CLASSNAME);
		if (mci == null) {
			env.repeatInvocation();
	...
	@MJI
	public int getDeclaredMethods_____3Ljava_lang_reflect_Method_2(MJIEnv env,
			int objref) {
		ClassInfo mci = getInitializedClassInfo(env, METHOD_CLASSNAME);
		if (mci == null) {
			env.repeatInvocation();
	...
	int getConstructors(MJIEnv env, int objref, boolean publicOnly) {
		...
		int aref = env
				.newObjectArray("Ljava/lang/reflect/Constructor;", nCtors);

		for (int i = 0; i < nCtors; i++) {
			env.setReferenceArrayElement(aref, i,
					createMethodObject(env, mci, ctors.get(i)));
		}
	...
	@MJI
	public int getConstructors_____3Ljava_lang_reflect_Constructor_2(
			MJIEnv env, int objref) {
		return getConstructors(env, objref, true);
	}

	@MJI
	public int getDeclaredConstructors_____3Ljava_lang_reflect_Constructor_2(
			MJIEnv env, int objref) {
		return getConstructors(env, objref, false);
	}

	@MJI
	public int getConstructor___3Ljava_lang_Class_2__Ljava_lang_reflect_Constructor_2(
			MJIEnv env, int clsRef, int argTypesRef) {
		ClassInfo mci = getInitializedClassInfo(env, CONSTRUCTOR_CLASSNAME);
		if (mci == null) {
			env.repeatInvocation();
	...
	@MJI
	public int getDeclaredFields_____3Ljava_lang_reflect_Field_2(MJIEnv env,
			int objRef) {
		...
		int aref = env.newObjectArray("Ljava/lang/reflect/Field;", nInstance
				+ nStatic);
		int i, j = 0;

		for (i = 0; i < nStatic; i++) {
			FieldInfo fi = ci.getStaticField(i);
			env.setReferenceArrayElement(aref, j++,
					createFieldObject(env, fi, fci));
		}
	...
	public int getFields_____3Ljava_lang_reflect_Field_2(MJIEnv env, int clsRef) {
		...
		for (; ci != null; ci = ci.getSuperClass()) {
			// the host VM returns them in order of declaration, but the spec
			// says there is no guaranteed order so we keep it simple
		...
		int aref = env.newObjectArray("Ljava/lang/reflect/Field;",
				fiList.size());
		int j = 0;
	...
	@MJI
	public int getDeclaredField__Ljava_lang_String_2__Ljava_lang_reflect_Field_2(
			MJIEnv env, int clsRef, int nameRef) {
		return getField(env, clsRef, nameRef, false);
	}

	@MJI
	public int getField__Ljava_lang_String_2__Ljava_lang_reflect_Field_2(
			MJIEnv env, int clsRef, int nameRef) {
	...
	public int getInterfaces_____3Ljava_lang_Class_2(MJIEnv env, int clsRef) {
		...
		// <2do> this is not exactly correct, since the interfaces should be
		// ordered
	...
	/**
	 * <2do> needs to load from the classfile location, NOT the MJIEnv (native)
	 * class
	 * 
	...
	public int getEnclosingClass____Ljava_lang_Class_2(MJIEnv env, int clsRef) {
		ClassInfo ciEncl = env.getReferredClassInfo(clsRef)
	...
	public int getDeclaredClasses_____3Ljava_lang_Class_2(MJIEnv env, int clsRef) {
		...
		MethodInfo mi = ti.getTopFrame().getPrevious().getMethodInfo();
		// class of the method that includes the invocation of
		...
		for (int i = 0; i < length; i++) {
			try {
				resolvedInnerClass[i] = cls
						.resolveReferencedClass(innerClassNames[i]);
			} catch (LoadOnJPFRequired lre) {
				env.repeatInvocation();
				return MJIEnv.NULL;
			}
	...
	@MJI
	public boolean isAnnotationPresent__Ljava_lang_Class_2__Z(MJIEnv env,
			int clsObjRef, int annoClsObjRef) {
		ClassInfo ci = env.getReferredClassInfo(clsObjRef);
	...
	@MJI
	public int getDeclaredAnnotations_____3Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int robj) {
		ClassInfo ci = env.getReferredClassInfo(robj);
	...
	@MJI
	public int getEnclosingConstructor____Ljava_lang_reflect_Constructor_2(
			MJIEnv env, int robj) {
		ClassInfo mci = getInitializedClassInfo(env, CONSTRUCTOR_CLASSNAME);
		if (mci == null) {
			env.repeatInvocation();
	...
	@MJI
	public int getEnclosingMethod____Ljava_lang_reflect_Method_2(MJIEnv env,
			int robj) {
		ClassInfo mci = getInitializedClassInfo(env, METHOD_CLASSNAME);
		if (mci == null) {
			env.repeatInvocation();
	...
	public boolean isMemberClass____Z(MJIEnv env, int robj) {
		ClassInfo ci = env.getReferredClassInfo(robj);
		return (ci.getEnclosingClassInfo() != null)
				&& !isLocalOrAnonymousClass(ci);
	}

	/**
	 * Append the package name prefix of the class represented by robj, if the
	 * name is not absolute. OW, remove leading "/".
	 */
	@MJI
	public int getResolvedName__Ljava_lang_String_2__Ljava_lang_String_2(
			MJIEnv env, int robj, int resRef) {

...
public class JPF_java_lang_System extends NativePeer {
	...
	@MJI
	public void arraycopy__Ljava_lang_Object_2ILjava_lang_Object_2II__V(
			MJIEnv env, int clsObjRef, int srcArrayRef, int srcIdx,
			int dstArrayRef, int dstIdx, int length) {
		...
		try {
			eiDst.copyElements(env.getThreadInfo(), eiSrc, srcIdx, dstIdx,
					length);
	...
	@MJI
	public int getenv__Ljava_lang_String_2__Ljava_lang_String_2(MJIEnv env,
	...
	int createPrintStream(MJIEnv env, int clsObjRef) {
		...
		ClassInfo ci = ClassLoaderInfo
				.getSystemResolvedClassInfo("gov.nasa.jpf.ConsoleOutputStream");
	...
	@MJI
	public int createSystemOut____Ljava_io_PrintStream_2(MJIEnv env,
			int clsObjRef) {
		return createPrintStream(env, clsObjRef);
	}

	@MJI
	public int createSystemErr____Ljava_io_PrintStream_2(MJIEnv env,
			int clsObjRef) {
		return createPrintStream(env, clsObjRef);
	}
	...
	public String getSUTJavaClassPath(VM vm) {
		ClassInfo system = ClassLoaderInfo
				.getSystemResolvedClassInfo("java.lang.System");

		ThreadInfo ti = vm.getCurrentThread();
		Heap heap = vm.getHeap();
		ElementInfo eiClassPath = heap.newString(JAVA_CLASS_PATH, ti);

		MethodInfo miGetProperty = system.getMethod(
				"getProperty(Ljava/lang/String;)Ljava/lang/String;", true);
		DirectCallStackFrame frame = miGetProperty.createDirectCallStackFrame(
				ti, 0);
	...
	int getSelectedSysPropsFromHost(MJIEnv env) {
		...
		if (keys == null) {
			String[] defKeys = { "path.separator", "line.separator",
					"file.separator", "user.name", "user.dir", "user.timezone",
					"user.country", "java.home", "java.version",
					"java.io.tmpdir", JAVA_CLASS_PATH
		...
		for (String s : keys) {
			...
			int idx = s.indexOf('/');
			if (idx > 0) { // this one is an explicit key/value pair from
							// vm.system.properties
				v = s.substring(idx + 1);
				s = s.substring(0, idx);

			} else {
				// the special beasts first (if they weren't overridden with
				// vm.system.properties)
				if (s == JAVA_CLASS_PATH) {
					// maybe we should just use vm.classpath
					// NOTE: the curent classloader at the point it has to be a
					// system classloader.
					ClassPath cp = ClassLoaderInfo.getCurrentClassLoader()
							.getClassPath();
					// <2do> should be consistent with path.separator (this is
					// host VM notation)
					v = cp.toString();
	...
	@MJI
	public int getKeyValuePairs_____3Ljava_lang_String_2(MJIEnv env,
			int clsObjRef) {
		Config conf = env.getConfig();
		SystemPropertyPolicy sysPropSrc = conf.getEnum("vm.sysprop.source",
				SystemPropertyPolicy.values(), SystemPropertyPolicy.SELECTED);
	...
	@MJI
	public int identityHashCode__Ljava_lang_Object_2__I(MJIEnv env,

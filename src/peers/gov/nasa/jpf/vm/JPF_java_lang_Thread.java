 ...
 * 
 * NOTE - this implementation depends on all live thread objects being in
 * ThreadList
 */
public class JPF_java_lang_Thread extends NativePeer {
	 ...
	 */
	@MJI
	public void init0__Ljava_lang_ThreadGroup_2Ljava_lang_Runnable_2Ljava_lang_String_2J__V(
			MJIEnv env, int objRef, int groupRef, int runnableRef, int nameRef,
			long stackSize) {
		VM vm = env.getVM();

		// we only need to create the ThreadInfo - its initialization will take
		// care
	...
	public void dumpStack____V(MJIEnv env, int clsObjRef) {
		...
		ti.printStackTrace(); // this is not correct, we should go through
								// VM.print
	}

	@MJI
	public void setName0__Ljava_lang_String_2__V(MJIEnv env, int objref,
			int nameRef) {
	...
	@MJI
	public boolean holdsLock__Ljava_lang_Object_2__Z(MJIEnv env, int clsObjRef,
			int objref) {
		ThreadInfo ti = env.getThreadInfo();
	...
	public void interrupt____V(MJIEnv env, int objref) {
		...
		if (!ti.isFirstStepInsn()) {
			...
			// the interrupted thread can re-acquire the lock and therefore is
			// runnable again
			// hence we should give it a chance to do so (Thread.interrupt()
			// does not require
			// holding a lock)
			if (interruptedThread.isUnblocked()) {
				ChoiceGenerator<?> cg = ss.getSchedulerFactory()
						.createInterruptCG(interruptedThread);
	...
	public void start____V(MJIEnv env, int objref) {
		...
		if (!tiCurrent.isFirstStepInsn()) { // first time we see this (may be
											// the only time)
			if (tiStartee.isStopped()) {
				// don't do anything but set it terminated - it hasn't acquired
				// any resources yet.
				// note that apparently host VMs don't schedule this thread, so
				// it never
			...
			// check if this thread was already started. If it is still running,
			// this
			// is a IllegalThreadStateException. If it already terminated, it
			// just gets
			// silently ignored in Java 1.4, but the 1.5 spec explicitly marks
			// this
			// as illegal, so we adopt this by throwing an IllegalThreadState,
			// too
			if (!tiStartee.isNew()) {
			...
			if (runnableRef == MJIEnv.NULL) {
				// note that we don't set the 'target' field, since
				// java.lang.Thread doesn't
				runnableRef = objref;
			}

			// vm.registerThread(tiStartee);

			// we don't do this during thread creation because the thread isn't
			// in
			...
			// exit point (DIRECTCALLRETURN) in case the thread is stopped or
			// there is
			// a fail-safe UncaughtExceptionHandler set
			DirectCallStackFrame runFrame = miRun
					.createRunStartStackFrame(tiStartee);
			...
			ChoiceGenerator<?> cg = ss.getSchedulerFactory()
					.createThreadStartCG(tiStartee);
			if (ss.setNextChoiceGenerator(cg)) {
				env.repeatInvocation();
			} else {
				Instruction insn = tiCurrent.getPC();
				log.info(tiStartee.getName(),
						" start not a scheduling point in ", insn
								.getMethodInfo().getFullName());
			}
	...
	public void yield____V(MJIEnv env, int clsObjRef) {
		...
		if (!ti.isFirstStepInsn()) { // first time we see this (may be the only
										// time)
			ChoiceGenerator<?> cg = ss.getSchedulerFactory()
					.createThreadYieldCG(ti);
			if (ss.setNextChoiceGenerator(cg)) {
				env.repeatInvocation();
			}
		} else {
	...
	public void sleep__JI__V(MJIEnv env, int clsObjRef, long millis, int nanos) {
		...
		if (!ti.isFirstStepInsn()) { // first time we see this (may be the only
										// time)
			ChoiceGenerator<?> cg = ss.getSchedulerFactory()
					.createThreadSleepCG(ti, millis, nanos);
			if (ss.setNextChoiceGenerator(cg)) {
				env.repeatInvocation();
				ti.setSleeping();
			}
		} else {
			// this seems asymmetric (since we only set it to sleeping when we
			// register
	...
	public void suspend____(MJIEnv env, int threadObjRef) {
		...
		if (!currentThread.isFirstStepInsn()) {
			if (target.suspend()) {
				ChoiceGenerator<?> cg = ss.getSchedulerFactory()
						.createThreadSuspendCG(currentThread);
				if (ss.setNextChoiceGenerator(cg)) {
					env.repeatInvocation();
					return;
				}
	...
	public void resume____(MJIEnv env, int threadObjRef) {
		...
		if (!currentThread.isFirstStepInsn()) {
			if (target.resume()) {
				ChoiceGenerator<?> cg = ss.getSchedulerFactory()
						.createThreadResumeCG(currentThread);
				if (ss.setNextChoiceGenerator(cg)) {
					env.repeatInvocation();
					return;
				}
	...
	/**
	 * this is here so that we don't have to break the transition on a
	 * synchronized call
	 */
	static void join0(MJIEnv env, int joineeRef, long timeout) {
		ThreadInfo tiJoiner = env.getThreadInfo(); // this is the CURRENT thread
													// (joiner)
		...
		SystemState ss = env.getSystemState();
		ElementInfo ei = env.getModifiableElementInfo(joineeRef); // the thread
																	// object to
																	// wait on

		if (tiJoiner.isInterrupted(true)) { // interrupt status is set, throw
											// and bail

			// since we use lock-free joins, we need to remove ourselves from
			// the
			// lock contender list
			ei.setMonitorWithoutLocked(tiJoiner);

			// note that we have to throw even if the thread to join to is not
			// alive anymore
			env.throwInterrupt();
			return;
		}
		...
		if (tiJoiner.isFirstStepInsn()) { // re-execution, we already have a CG
			...
			case UNBLOCKED:
				// Thread was owning the lock when it joined - we have to wait
				// until
				// we can reacquire it
			...
			if (isAlive) {
				...
				ChoiceGenerator<ThreadInfo> cg = ss.getSchedulerFactory()
						.createWaitCG(ei, tiJoiner, timeout);

				env.setMandatoryNextChoiceGenerator(cg,
						"no CG for blocking join()");
				env.repeatInvocation();
	...
	/*
	 * public static void join__ (MJIEnv env, int objref) { ThreadInfo tiStop =
	 * getThreadInfo(env,objref);
	 * 
	 * if (tiStop.isAlive()) { env.wait(objref); } }
	 */
	...
	@MJI
	public void stop__Ljava_lang_Throwable_2__V(MJIEnv env, int threadRef,
			int throwableRef) {
		ThreadInfo tiStop = env.getThreadInfoForObjRef(threadRef); // the thread
																	// to stop
		ThreadInfo tiCurrent = env.getThreadInfo(); // the currently executing
													// thread
		...
		if (!tiCurrent.isFirstStepInsn()) {
			// since this is usually not caught (it shouldn't, at least not
			// without
			// rethrowing), we might turn this into a right mover since it
			// terminates tiStop

			SystemState ss = env.getSystemState();
			ChoiceGenerator<?> cg = ss.getSchedulerFactory()
					.createThreadStopCG(tiCurrent);
			if (ss.setNextChoiceGenerator(cg)) {
				env.repeatInvocation();
				return;
			}

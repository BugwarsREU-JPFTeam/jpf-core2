 ...
 * 
 * Unfortunately, this is quite redundant to the Method peer, but Constructor is
 * not a Method subclass, and hence we can't rely on it's initialization
 */
public class JPF_java_lang_reflect_Constructor extends NativePeer {
	...
	static int createConstructorObject(MJIEnv env, ClassInfo ciCtor,
			MethodInfo mi) {
		// note - it is the callers responsibility to ensure Constructor is
	...
	@MJI
	public int newInstance___3Ljava_lang_Object_2__Ljava_lang_Object_2(
			MJIEnv env, int mthRef, int argsRef) {
		...
		if (frame == null) { // first time
			...
			frame.setLocalReferenceVariable(0, objRef); // (1) store the objRef
														// for retrieval during
														// re-exec

			int argOffset = frame.setReferenceArgument(0, objRef, null);
			if (!JPF_java_lang_reflect_Method.pushUnboxedArguments(env,
					miCallee, frame, argOffset, argsRef)) {
	...
	public int getParameterTypes_____3Ljava_lang_Class_2(MJIEnv env, int objRef) {
		// kind of dangerous, but we don't refer to any fields and the
		// underlying JPF construct
		// (MethodInfo) is the same, so we just delegate to avoid copying
		// non-trivial code
		return JPF_java_lang_reflect_Method.getParameterTypes(env,
				getMethodInfo(env, objRef));
	}
	...
	public int getAnnotations_____3Ljava_lang_annotation_Annotation_2(
		...
		return JPF_java_lang_reflect_Method.getAnnotations(env,
				getMethodInfo(env, objRef));
	}
	...
	public int getDeclaredAnnotations_____3Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int objRef) {
		return JPF_java_lang_reflect_Method.getDeclaredAnnotations(env,
				getMethodInfo(env, objRef));
	}
	...
	public int getAnnotation__Ljava_lang_Class_2__Ljava_lang_annotation_Annotation_2(
		...
		return JPF_java_lang_reflect_Method.getAnnotation(env,
				getMethodInfo(env, objRef), annotationClsRef);
	}

	@MJI
	public int getParameterAnnotations_____3_3Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int objRef) {
		return JPF_java_lang_reflect_Method.getParameterAnnotations(env,
				getMethodInfo(env, objRef));
	}
	...
	public int getDeclaringClass____Ljava_lang_Class_2(MJIEnv env, int objRef) {
		...
		// can't get a Constructor object w/o having initialized it's declaring
		// class first
	...
	public boolean equals__Ljava_lang_Object_2__Z(MJIEnv env, int objRef,
		...
		ClassInfo ci = ClassLoaderInfo
				.getSystemResolvedClassInfo(JPF_java_lang_Class.CONSTRUCTOR_CLASSNAME);

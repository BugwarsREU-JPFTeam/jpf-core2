...
public class JPF_java_lang_reflect_Field extends NativePeer {
	...
	// using Fields is fine, but creating them is not efficient until we fix
	// this
	...
	static ElementInfo getCheckedElementInfo(MJIEnv env, FieldInfo fi,
			int fobjRef, Class<?> fiType, String type, boolean isWrite) {
		ElementInfo ei;
		...
		if (fi.isStatic()) {
			...
			ei = isWrite ? fci.getModifiableStaticElementInfo() : fci
					.getStaticElementInfo();
		} else { // instance field
			ei = isWrite ? env.getModifiableElementInfo(fobjRef) : env
					.getElementInfo(fobjRef);
		}
		...
		if (fiType != null && !fiType.isInstance(fi)) {
			env.throwException("java.lang.IllegalArgumentException",
					"field type incompatible with " + type);
			return null;
		}
	...
	@MJI
	public boolean getBoolean__Ljava_lang_Object_2__Z(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				BooleanFieldInfo.class, "boolean", false);
		if (ei != null) {
	...
	@MJI
	public byte getByte__Ljava_lang_Object_2__B(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				ByteFieldInfo.class, "byte", false);
		if (ei != null) {
	...
	@MJI
	public char getChar__Ljava_lang_Object_2__C(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				CharFieldInfo.class, "char", false);
		if (ei != null) {
	...
	@MJI
	public short getShort__Ljava_lang_Object_2__S(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				ShortFieldInfo.class, "short", false);
		if (ei != null) {
	...
	@MJI
	public int getInt__Ljava_lang_Object_2__I(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				IntegerFieldInfo.class, "int", false);
		if (ei != null) {
	...
	@MJI
	public long getLong__Ljava_lang_Object_2__J(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				LongFieldInfo.class, "long", false);
		if (ei != null) {
	...
	@MJI
	public float getFloat__Ljava_lang_Object_2__F(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				FloatFieldInfo.class, "float", false);
		if (ei != null) {
	...
	@MJI
	public double getDouble__Ljava_lang_Object_2__D(MJIEnv env, int objRef,
			int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				DoubleFieldInfo.class, "double", false);
		if (ei != null) {
	...
	@MJI
	public void setBoolean__Ljava_lang_Object_2Z__V(MJIEnv env, int objRef,
			int fobjRef, boolean val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		}
		...
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				BooleanFieldInfo.class, "boolean", true);
		if (ei != null) {
	...
	@MJI
	public void setByte__Ljava_lang_Object_2B__V(MJIEnv env, int objRef,
			int fobjRef, byte val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		}
	...
	@MJI
	public void setChar__Ljava_lang_Object_2C__V(MJIEnv env, int objRef,
			int fobjRef, char val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		}
	...
	@MJI
	public void setShort__Ljava_lang_Object_2S__V(MJIEnv env, int objRef,
			int fobjRef, short val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		}
		...
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				ShortFieldInfo.class, "short", true);
		if (ei != null) {
	...
	@MJI
	public void setInt__Ljava_lang_Object_2I__V(MJIEnv env, int objRef,
			int fobjRef, int val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		...
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				IntegerFieldInfo.class, "int", true);
		if (ei != null) {
	...
	@MJI
	public void setLong__Ljava_lang_Object_2J__V(MJIEnv env, int objRef,
			int fobjRef, long val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		}
	...
	@MJI
	public void setFloat__Ljava_lang_Object_2F__V(MJIEnv env, int objRef,
			int fobjRef, float val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		}
		...
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				FloatFieldInfo.class, "float", true);
		if (ei != null) {
	...
	@MJI
	public void setDouble__Ljava_lang_Object_2D__V(MJIEnv env, int objRef,
			int fobjRef, double val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		if (!isAvailable(env, fi, fobjRef)) {
			return;
		}
		...
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef,
				DoubleFieldInfo.class, "double", true);
		if (ei != null) {
	...
	@MJI
	public int get__Ljava_lang_Object_2__Ljava_lang_Object_2(MJIEnv env,
			int objRef, int fobjRef) {
		FieldInfo fi = getFieldInfo(env, objRef);
		ElementInfo ei = getCheckedElementInfo(env, fi, fobjRef, null, null,
				false); // no type check here
		if (ei == null) {
			return 0;
		}

		if (!(fi instanceof ReferenceFieldInfo)) { // primitive type, we need to
													// box it
	...
	static FieldInfo getFieldInfo(MJIEnv env, int objRef) {
		...
		assert ((fidx >= 0) || (fidx < nRegistered)) : "illegal FieldInfo request: "
				+ fidx + ", " + nRegistered;
	...
	static boolean isAvailable(MJIEnv env, FieldInfo fi, int fobjRef) {
		if (fi.isStatic()) {
			...
			// class had obviously been initialized, otherwise we won't have an
			// instance of it
		}
	...
	public void set__Ljava_lang_Object_2Ljava_lang_Object_2__V(MJIEnv env,
			int objRef, int fobjRef, int val) {
		FieldInfo fi = getFieldInfo(env, objRef);
		...
		if (Modifier.isFinal(modifiers)) {
			env.throwException("java.lang.IllegalAccessException", "field "
					+ fi.getName() + " is final");
			return;
		}
		ClassInfo ci = fi.getClassInfo();
		...
		if (!setValue(env, fi, fobjRef, val, attr)) {
			env.throwException(
					...
					"Can not set "
							...
							+ fi.getFullName()
							+ " to "
							+ ((MJIEnv.NULL != val) ? env.getClassInfo(val)
									.getName() + " object " : "null"));
		}
	}

	private static boolean setValue(MJIEnv env, FieldInfo fi, int obj,
			int value, Object attr) {
		...
		if (tci.isPrimitive()) {
			...
			// checks whether unboxing can be done by accessing the field
			// "value"
	...
	public boolean equals__Ljava_lang_Object_2__Z(MJIEnv env, int objRef,
		...
		if (fidx >= 0 && fidx < nRegistered) {
			...
			return ((fi1.getClassInfo() == fi2.getClassInfo())
					&& fi1.getName().equals(fi2.getName()) && fi1.getType()
					.equals(fi2.getType()));
		}
		return false;
	}
	...
	public int getDeclaredAnnotations_____3Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int objRef) {
		return getAnnotations_____3Ljava_lang_annotation_Annotation_2(env,
				objRef);
	}
}

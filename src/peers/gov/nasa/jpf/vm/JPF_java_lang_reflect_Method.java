...
public class JPF_java_lang_reflect_Method extends NativePeer {
	...
	public static boolean init(Config conf) {
		...
		// native peers - this might also get explicitly called by the
		// java.lang.Class
	...
	static int createMethodObject(MJIEnv env, ClassInfo ciMth, MethodInfo mi) {
		// note - it is the callers responsibility to ensure Method is properly
		// initialized
	...
	static int getParameterTypes(MJIEnv env, MethodInfo mi) {
		...
		for (int i = 0; i < argTypeNames.length; i++) {
			ClassInfo ci = ClassLoaderInfo
					.getCurrentResolvedClassInfo(argTypeNames[i]);
			if (!ci.isRegistered()) {
				ci.registerClass(ti);
			}
	...
	int getExceptionTypes(MJIEnv env, MethodInfo mi) {
		...
		for (int i = 0; i < exceptionNames.length; i++) {
			ClassInfo ci = ClassLoaderInfo
					.getCurrentResolvedClassInfo(exceptionNames[i]);
			if (!ci.isRegistered()) {
				ci.registerClass(ti);
			}
		...
		int aRef = env.newObjectArray("Ljava/lang/Class;",
				exceptionNames.length);
		for (int i = 0; i < exceptionNames.length; i++) {
			env.setReferenceArrayElement(aRef, i, ar[i]);
		}
	...
	public int getReturnType____Ljava_lang_Class_2(MJIEnv env, int objRef) {
		...
		ClassInfo ci = ClassLoaderInfo.getCurrentResolvedClassInfo(mi
				.getReturnTypeName());
		if (!ci.isRegistered()) {
			ci.registerClass(ti);
		}
	...
	public int getDeclaringClass____Ljava_lang_Class_2(MJIEnv env, int objRef) {
		...
		// it's got to be registered, otherwise we wouldn't be able to acquire
		// the Method object
		return ci.getClassObjectRef();
	}

	static int createBoxedReturnValueObject(MJIEnv env, MethodInfo mi,
		...
		if (rt == Types.T_DOUBLE) {
			...
			ret = env.newObject(ClassLoaderInfo
					.getSystemResolvedClassInfo("java.lang.Double"));
			...
			ret = env.newObject(ClassLoaderInfo
					.getSystemResolvedClassInfo("java.lang.Float"));
			rei = env.getModifiableElementInfo(ret);
	...
	static boolean pushUnboxedArguments(MJIEnv env, MethodInfo mi,
			DirectCallStackFrame frame, int argIdx, int argsRef) {
		ElementInfo source;
		...
		// according to the API docs, passing null instead of an empty array is
		// allowed for no args
		passedCount = (argsRef != MJIEnv.NULL) ? env.getArrayLength(argsRef)
				: 0;

		if (nArgs != passedCount) {
			env.throwException(IllegalArgumentException.class.getName(),
					"Wrong number of arguments passed.  Actual = "
							+ passedCount + ".  Expected = " + nArgs);
			return false;
		}
		...
		for (int i = 0; i < nArgs; i++) {
			...
			if (sourceRef == MJIEnv.NULL) {
				if ((destTypes[i] != Types.T_REFERENCE)
						&& (destTypes[i] != Types.T_ARRAY)) {
					env.throwException(
			...
			sourceType = getSourceType(sourceClass, destTypes[i],
					destTypeNames[i]);

			Object attr = env.getElementInfo(argsRef).getFields()
					.getFieldAttr(i);
			if ((argIdx = pushArg(argIdx, frame, source, sourceType,
					destTypes[i], attr)) < 0) {
				env.throwException(IllegalArgumentException.class.getName(),
						"Wrong argument type at index " + i
								+ ".  Source Class = " + sourceClass.getName()
								+ ".  Dest Class = " + destTypeNames[i]);
				return false;
			}
		}
	...
	// this returns the primitive type in case we have to unbox, and otherwise
	// checks reference type compatibility
	private static byte getSourceType(ClassInfo ciArgVal, byte destType,
			String destTypeName) {
		switch (destType) {
		// the primitives
		...
		case Types.T_ARRAY:
		case Types.T_REFERENCE: // check if the source type is assignment
								// compatible with the destType
	...
	private static int pushArg(int argIdx, DirectCallStackFrame frame,
			ElementInfo eiArg, byte srcType, byte destType, Object attr) {
		switch (srcType) {
		case Types.T_DOUBLE: {
	...
	@MJI
	public int invoke__Ljava_lang_Object_2_3Ljava_lang_Object_2__Ljava_lang_Object_2(
			MJIEnv env, int mthRef, int objRef, int argsRef) {
		ThreadInfo ti = env.getThreadInfo();
			...
			if (!miCallee.isStatic()) {
				if (objRef == MJIEnv.NULL) {
					...
					if (!objClass.isInstanceOf(calleeClass)) {
						env.throwException(
								IllegalArgumentException.class.getName(),
								"Object is not an instance of declaring class.  Actual = "
										+ objClass + ".  Expected = "
										+ calleeClass);
						return MJIEnv.NULL;
					}
				...
				if (callerClass != calleeClass) {
					env.throwException(
							...
							"Class "
									...
									+ calleeClass.getName()
									+ " with modifiers \""
									+ Modifier.toString(miCallee.getModifiers()));
					return MJIEnv.NULL;
				}
			}
	...
	static int getAnnotations(MJIEnv env, MethodInfo mi) {
		...
		try {
			return env.newAnnotationProxies(aiList
					.toArray(new AnnotationInfo[aiList.size()]));
	...
	public int getAnnotations_____3Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int mthRef) {
		return getAnnotations(env, getMethodInfo(env, mthRef));
	}

	// the following ones consist of a package default implementation that is
	// shared with
	...
	public int getAnnotation__Ljava_lang_Class_2__Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int mthRef, int annotationClsRef) {
	...
	static int getParameterAnnotations(MJIEnv env, MethodInfo mi) {
		...
		// this should always return an array object, even if the method has no
		// arguments

		try {
			int paRef = env.newObjectArray(
					"[Ljava/lang/annotation/Annotation;", pa.length);
		...
		} catch (ClinitRequired x) { // be prepared that we might have to
										// initialize respective annotation
										// classes
			env.handleClinitRequest(x.getRequiredClassInfo());
			return MJIEnv.NULL;
		}
	...
	@MJI
	public int getParameterAnnotations_____3_3Ljava_lang_annotation_Annotation_2(
			MJIEnv env, int mthRef) {
	...
	public boolean equals__Ljava_lang_Object_2__Z(MJIEnv env, int objRef,
		...
		ClassInfo ci = ClassLoaderInfo
				.getSystemResolvedClassInfo(JPF_java_lang_Class.METHOD_CLASSNAME);

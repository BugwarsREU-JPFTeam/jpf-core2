...
public class JPF_java_text_DateFormatSymbols extends NativePeer {
	@MJI
	public void initializeData__Ljava_util_Locale_2__V(MJIEnv env, int objRef,
			int localeRef) {
		...
		env.setReferenceField(objRef, "shortMonths",
				env.newStringArray(shortMonths));

		String[] weekdays = dfs.getWeekdays();
		env.setReferenceField(objRef, "weekdays", env.newStringArray(weekdays));

		String[] shortWeekdays = dfs.getShortWeekdays();
		env.setReferenceField(objRef, "shortWeekdays",
				env.newStringArray(shortWeekdays));
		...
		int aaref = env.newObjectArray("[Ljava.lang.String;",
				zoneStrings.length);
		env.setReferenceField(objRef, "zoneStrings", aaref);
		for (int i = 0; i < zoneStrings.length; i++) {
			env.setReferenceArrayElement(aaref, i,
					env.newStringArray(zoneStrings[i]));
		}

		String localPatternChars = dfs.getLocalPatternChars();
		env.setReferenceField(objRef, "localPatternChars",
				env.newString(localPatternChars));

 ...
 * 
 * we need to intercept the initialization because it is requires file io
 * (properties) based on the Locale
 */
public class JPF_java_text_DecimalFormatSymbols extends NativePeer {
	@MJI
	public void initialize__Ljava_util_Locale_2__V(MJIEnv env, int objRef,
			int localeRef) {
		...
		env.setReferenceField(objRef, "infinity",
				env.newString(dfs.getInfinity()));
		env.setReferenceField(objRef, "NaN", env.newString(dfs.getNaN()));
		env.setReferenceField(objRef, "currencySymbol",
				env.newString(dfs.getCurrencySymbol()));
		env.setCharField(objRef, "monetarySeparator",
				dfs.getMonetaryDecimalSeparator());

		env.setCharField(objRef, "decimalSeparator", dfs.getDecimalSeparator());
		env.setCharField(objRef, "groupingSeparator",
				dfs.getGroupingSeparator());
		env.setCharField(objRef, "exponential", 'E'); // getExponentialSymbol()
														// is not public
	}
}

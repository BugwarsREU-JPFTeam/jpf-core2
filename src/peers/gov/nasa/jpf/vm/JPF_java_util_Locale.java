...
public class JPF_java_util_Locale extends NativePeer {
	...
	static Locale getLocale(MJIEnv env, int locref) {
		...
		// --- check first if it's one of the standard locales (ci is obviously
		// loaded at this point
		ClassInfo ci = env.getClassInfo(locref); // Locale is final, so we can
													// do this
		ElementInfo sei = ci.getStaticElementInfo();

		if (locref == sei.getReferenceField("US"))
			return Locale.US;
		if (locref == sei.getReferenceField("GERMAN"))
			return Locale.GERMAN;
		if (locref == sei.getReferenceField("ENGLISH"))
			return Locale.ENGLISH;
		if (locref == sei.getReferenceField("FRENCH"))
			return Locale.FRENCH;
		if (locref == sei.getReferenceField("JAPANESE"))
			return Locale.JAPANESE;
		if (locref == sei.getReferenceField("CHINESE"))
		...
		if (fiBase != null) { // Java >= 1.7
			...
			country = env.getStringObject(env.getReferenceField(baseLocref,
					"region"));
			language = env.getStringObject(env.getReferenceField(baseLocref,
					"language"));
			variant = env.getStringObject(env.getReferenceField(baseLocref,
					"variant"));

		} else { // Java < 1.7
			country = env.getStringObject(env.getReferenceField(locref,
					"country"));
			language = env.getStringObject(env.getReferenceField(locref,
					"language"));
			variant = env.getStringObject(env.getReferenceField(locref,
					"variant"));
		}
	...
	@MJI
	public int getDisplayName__Ljava_util_Locale_2__Ljava_lang_String_2(
	...
	@MJI
	public int getDisplayVariant__Ljava_util_Locale_2__Ljava_lang_String_2(
			MJIEnv env, int objref, int locref) {
		Locale locale = getLocale(env, locref);
	...
	@MJI
	public int getDisplayCountry__Ljava_util_Locale_2__Ljava_lang_String_2(
			MJIEnv env, int objref, int locref) {
		Locale locale = getLocale(env, locref);
	...
	@MJI
	public int getDisplayLanguage__Ljava_util_Locale_2__Ljava_lang_String_2(
			MJIEnv env, int objref, int locref) {
		Locale locale = getLocale(env, locref);

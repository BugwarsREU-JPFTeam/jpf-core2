 ...
 * 
 * model - peer delegation is done via restoring a singleton, which
 * unfortunately has to resort to the rather nasty Unsafe mechanism because the
 * required random internals are private. We could use per-instance native
 * objects stored as object attributes, but that would only partly solve the
 * problem because we still would have to backtrack the internal state of such
 * objects
 */
...
public class JPF_java_util_Random extends NativePeer {
	...
	// those are only used if enumerateRandom is not set, i.e. we delegate to
	// the host VM
	boolean reproducibleRandom;
	long constantSeed;
	int[] defaultIntSet; // in case we have an nextInt(), i.e. an unspecified
							// upper boundary
	long[] defaultLongSet;
	double[] defaultDoubleSet;
	float[] defaultFloatSet;

	// since peer methods are atomic, we just keep one delegator instead of
	// per-object,
	// which would have to rely on attributes and still require
	// storing/restoring
	// the seed state with nasty Unsafe
	static Delegatee delegatee = new Delegatee();

	// this is bad stuff we need to set/retrieve the Random.seed value. We only
	// have
	...
	static {
		try {
			// Unsafe.getUnsafe() can only be called from a
			...
			seedFieldOffset = unsafe.objectFieldOffset(Random.class
					.getDeclaredField("seed"));
		} catch (Exception ex) {
			throw new JPFException("cannot access java.util.Random internals: "
					+ ex);
		}
	}
	...
	public JPF_java_util_Random(Config conf) {
		...
		defaultIntSet = conf.getIntArray("vm.random_ints", Integer.MIN_VALUE,
				0, Integer.MAX_VALUE);
		defaultDoubleSet = conf.getDoubleArray("vm.random_doubles",
				Double.MIN_VALUE, 0, Double.MAX_VALUE);
		defaultLongSet = conf.getLongArray("vm.random_longs", Long.MIN_VALUE,
				0, Long.MAX_VALUE);
		defaultFloatSet = conf.getFloatArray("vm.random_floats",
				Float.MIN_VALUE, 0, Float.MAX_VALUE);
	}
	...
	long computeDefaultSeed() {
		Random rand = (reproducibleRandom) ? new Random(constantSeed)
				: new Random();
	...
	public void $init__J__V(MJIEnv env, int objRef, long seedStarter) {
		// note - the provided seedStarter is modified by java.util.Random, it
		// is
	...
	public int nextInt____I(MJIEnv env, int objRef) {
		if (enumerateRandom) {
			return JPF_gov_nasa_jpf_vm_Verify
					.getIntFromList(env, defaultIntSet);
	...
	public int next__I__I(MJIEnv env, int objRef, int nBits) {
		if (enumerateRandom) {
			// <2do> we can't do this with an interval since it most likely
			// would explode our state space
			return JPF_gov_nasa_jpf_vm_Verify
					.getIntFromList(env, defaultIntSet);
	...
	public void nextBytes___3B__V(MJIEnv env, int objRef, int dataRef) {
		// <2do> this one is an even worse state exploder. We could use cascaded
		// CGs,
		// but chances are this really kills us, so we just ignore
		// 'enumerateRandom' for now
	...
	public long nextLong____J(MJIEnv env, int objRef) {
		if (enumerateRandom) {
			return JPF_gov_nasa_jpf_vm_Verify.getLongFromList(env,
					defaultLongSet);
	...
	public float nextFloat____F(MJIEnv env, int objRef) {
		if (enumerateRandom) {
			return JPF_gov_nasa_jpf_vm_Verify.getFloatFromList(env,
					defaultFloatSet);
	...
	public double nextDouble____D(MJIEnv env, int objRef) {
		if (enumerateRandom) {
			return JPF_gov_nasa_jpf_vm_Verify.getDoubleFromList(env,
					defaultDoubleSet);
	...
	public double nextGaussian____D(MJIEnv env, int objRef) {
		...
		// delegation (which would require an additional 'haveNextGaussian'
		// state)
		restoreRandomState(env, objRef, delegatee);

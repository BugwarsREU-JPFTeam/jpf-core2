 ...
 * 
 * this is a (mostly) delegating implementation that became necessary because
 * TimeZone has been considerably changed in Java 1.7 (OpenJDK) and we need a
 * version that is compatible with pre and post 1.7 releases
 */
public class JPF_java_util_TimeZone extends NativePeer {

	// --- internals
	static TimeZone tz; // only used within (atomic) peer methods so that we
						// don't have to instantiate all the time

	// we have to keep optional defaults here so that we don't change the host
	// environment
	...
	// --- the factory methods
	@MJI
	public int getTimeZone__Ljava_lang_String_2__Ljava_util_TimeZone_2(
	...
	@MJI
	public int createDefaultZone____Ljava_util_TimeZone_2(MJIEnv env,
			int clsObjRef) {
	...
	@MJI
	public void setDefaultValues__Ljava_util_TimeZone_2(MJIEnv env,
			int clsObjRef, int tzRef) {
	...
	// --- the ID queries
	@MJI
	public int getAvailableIDs_____3Ljava_lang_String_2(MJIEnv env,
			int clsObjRef) {
	...
	@MJI
	public int getAvailableIDs__I___3Ljava_lang_String_2(MJIEnv env,
			int clsObjRef, int rawOffset) {
	...
	@MJI
	public int getOffset__IIIIII__I(MJIEnv env, int objRef, int era, int year,
			int month, int day, int dayOfWeek, int milliseconds) {
	...
	// unfortunately, this is not public in Java 1.7, so we can't delegate w/o
	// reflection
	@MJI
	public int getOffsets__J_3I__I(MJIEnv env, int objRef, long date,
			int offsetsRef) {
		TimeZone tz = getTimeZone(env, objRef);
	...
	@MJI
	public int getDisplayName__ZILjava_util_Locale_2__Ljava_lang_String_2(
			MJIEnv env, int objRef, boolean daylight, int style, int localeRef) {
		TimeZone tz = getTimeZone(env, objRef);

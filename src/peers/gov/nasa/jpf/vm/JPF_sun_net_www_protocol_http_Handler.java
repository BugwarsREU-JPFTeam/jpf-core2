 ...
 * 
 * We use model class configuration in case somebody wants to implement logic in
 * there that requires backtracking, and doesn't want to do this on the native
 * peer level
 * 
 * example config: http.connection= http://*.dtd =>
 * gov.nasa.jpf.CachedROHttpConnection, http://foo.com/* -- x.y.MyHttpConnection
 */
public class JPF_sun_net_www_protocol_http_Handler extends NativePeer {
			...
			for (int i = 0; i < specs.length; i++) {
				...
				if (idx > 0 && idx < (s.length() - 3)) {
					...
					if (!pattern.isEmpty() && !clsName.isEmpty()) {
						...
						logger.info("mapping URL pattern ", pattern, " to ",
								clsName);
					}
				}
				...
				if (e == null) {
					throw new JPFConfigException(
							"not a valid http.connection spec: " + s);
				}
	...
	@MJI
	public int getConnectionClass__Ljava_lang_String_2__Ljava_lang_Class_2(
			MJIEnv env, int objref, int surlRef) {

...
public class JPF_gov_nasa_jpf_test_basic_MJITest extends NativePeer {
	...
	@MJI
	public int nativeCreate2DimIntArray__II___3_3I(MJIEnv env, int robj,
			int size1, int size2) {
	...
	@MJI
	public int nativeInstanceMethod(MJIEnv env, int robj, double d, char c,
			boolean b, int i) {
		System.out.println("# entering nativeInstanceMethod() d=" + d + ", c="
				+ c + ", b=" + b + ", i=" + i);
	...
	@MJI
	public long nativeStaticMethod__JLjava_lang_String_2__J(MJIEnv env,
			int rcls, long l, int stringRef) {
	 ...
	 * the call chain is:
	 * 
	 * JPF: testRoundtripLoop VM: nativeRoundTripLoop x 3 JPF: roundtrip VM:
	...
	public int nativeRoundtripLoop__I__I(MJIEnv env, int robj, int a) {
		...
		MethodInfo mi = env.getClassInfo(robj)
				.getMethod("roundtrip(I)I", false);
		...
		if (frame == null) { // first time
			...
			// this method can't be executed unless the class is already
			// initialized,
			// i.e. we don't have to check for overlayed clinit calls and the
			// frame
			...
			// into
			// the native method
			int r = frame.getResult(); // the return value of the direct call
										// above
	...
	public int nativeHiddenRoundtrip__I__I(MJIEnv env, int robj, int a) {
		...
		MethodInfo mi = env.getClassInfo(robj).getMethod("atomicStuff(I)I",
				false);
		...
		try {
			...
			System.out
					.println("# hidden method execution failed, leaving nativeHiddenRoundtrip: "
							+ ux);
			ti.clearPendingException();
			ti.popFrame(); // this is still the DirectCallStackFrame, and we

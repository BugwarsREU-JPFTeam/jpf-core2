...
public class MJITest extends TestJPF {
	...
	static {
		...
		sdata = 0; // dummy insn required for the Eclipse compiler (skips empty
					// methods)
	}
	...
	public void testNativeCreate2DimIntArray() {
		if (verifyNoPropertyViolation()) {
			...
			assert (a.getClass().getComponentType().getName().equals("[I")) : "native int[][] creation failed: wrong component type";
	...
	public void testNativeCreateIntArray() {
		if (verifyNoPropertyViolation()) {
			...
			assert (a.getClass().getComponentType() == int.class) : "native int array creation failed: wrong component type";
	...
	public void testNativeCreateStringArray() {
		if (verifyNoPropertyViolation()) {
			...
			assert (a.getClass().isArray()) : "native String array creation failed: not an array";

			assert (a.getClass().getComponentType() == String.class) : "native String array creation failed: wrong component type";
	...
	public void testNativeCrash() {
		if (verifyJPFException(new TypeRef(
	...
	public void testHiddenRoundtrip() {
		if (verifyNoPropertyViolation()) {
			...
			int res = echo(20) + nativeHiddenRoundtrip(21); // get something on
															// the operand stack
	...
	public void testHiddenRoundtripException() {
		if (verifyNoPropertyViolation()) {
			...
			int res = echo(20) + nativeHiddenRoundtrip(-1); // get something on
															// the operand stack
	...
	int atomicStuff(int a) { // this is called from nativeAtomicRoundtrip()
		...
		if (a < 0) {
			System.out
					.println("## atomicStuff throwing IllegalArgumentException");

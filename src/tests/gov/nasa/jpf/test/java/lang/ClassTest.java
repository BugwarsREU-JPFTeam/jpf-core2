...
public class ClassTest extends TestJPF implements Cloneable, Serializable {
		...
		if (verifyNoPropertyViolation()) {
			...
			if (clazz == null) {
				throw new RuntimeException(
						"Class.forName() returned null object");
			}
	...
	public void testClassForNameInit() throws ClassNotFoundException {
		...
		if (verifyNoPropertyViolation()) {
			Class<?> cls = Class.forName(
					"gov.nasa.jpf.test.java.lang.ClassTest$X", true, this
	...
	@Test
	public void testNewInstanceFailAccess() throws IllegalAccessException,
	...
	@Test
	public void testNewInstanceFailAbstract() throws IllegalAccessException,
			InstantiationException {
	...
	public void testInterfaces() {
		if (verifyNoPropertyViolation()) {
			...
			if (ifc.length != 2) {
				throw new RuntimeException("wrong number of interfaces: "
						+ ifc.length);
			}

			int n = ifc.length;
			String[] ifcs = { Cloneable.class.getName(),
			...
			if (n != 0) {
				throw new RuntimeException("wrong interface types: "
						+ ifc[0].getName() + ',' + ifc[1].getName());
			}
		}
	}
	...
	public void localClassEnclosingClassTest() {
		if (verifyNoPropertyViolation()) {
			...
			assertEquals(testObj.foo.getClass().getEnclosingClass(),
					TestEnclosedClass.class);
		}
	}
	...
	public void getCanonicalNameTest() {
		if (verifyNoPropertyViolation()) {
			assertEquals(ArrayList.class.getCanonicalName(),
					"java.util.ArrayList");
	...
	public void getEnclosingMethod() throws SecurityException,
			NoSuchMethodException {
		if (verifyNoPropertyViolation()) {
			Class cls = (new ClassTest.TestEnclosedClass()).getLocalClassObj()
					.getClass();
			...
			Method m1 = ClassTest.TestEnclosedClass.class.getMethod(
					"getLocalClassObj", new Class[0]);
			Method m2 = cls.getEnclosingMethod();
			assertEquals(m1, m2);
			assertTrue(cls.getEnclosingMethod().equals(
					ClassTest.TestEnclosedClass.class.getMethod(
							"getLocalClassObj", new Class[0])));
		}
	}
	...
	public void isAnonymousClassTest() {
		if (verifyNoPropertyViolation()) {
			Class cls = (new ClassTest.TestEnclosedClass())
					.getAnonymousClassObj().getClass();
	...
	public void getDeclaringClassTest() {
		if (verifyNoPropertyViolation()) {
			...
			Class anonymousCls = (new ClassTest.TestEnclosedClass())
					.getAnonymousClassObj().getClass();
	...
	public void isMemberClassTest() {
		if (verifyNoPropertyViolation()) {
			...
			assertFalse(((new TestEnclosedClass()).getLocalClassObj()
					.getClass().isMemberClass()));
		}
	}
	...
	public void getResourceTest() {
		if (verifyNoPropertyViolation()) {
			...
			assertEquals(c.getResource("Class.class"),
					c.getResource("/java/lang/Class.class"));

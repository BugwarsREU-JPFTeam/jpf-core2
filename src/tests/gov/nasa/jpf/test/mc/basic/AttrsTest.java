...
public class AttrsTest extends TestJPF {
	...
	public void testIntPropagation() {
		if (verifyNoPropertyViolation("+listener=.test.mc.basic.AttrsTest$IntListener")) {
			...
			Verify.setLocalAttribute("i", 42); // this overwrites whatever the
												// ISTORE listener did set on
												// 'i'
			int attr = Verify.getLocalAttribute("i");
			Verify.println("'i' attribute after Verify.setLocalAttribute(\"i\",42): "
					+ attr);
			assertTrue(attr == 42);

			iInt = echoInt(i); // return val -> instance field
			sInt = iInt; // instance field -> static field
			int j = sInt; // static field -> local - now j should have the
							// initial i attribute, and value 42
			...
			if (executedInsn instanceof DSTORE) {
				if (mi.getName().equals("testDoublePropagation")) {
					...
					if (localName.equals("d")) {
						...
						/**
						 * get's overwritten in the model class if (a != ATTR){
						 * throw new
						 * JPFException("attribute propagation failed"); }
						 **/
					}
				}
	...
	public void testDoublePropagation() {
		if (verifyNoPropertyViolation("+listener=.test.mc.basic.AttrsTest$DoubleListener")) {
			...
			Verify.setLocalAttribute("d", 42); // this overwrites whatever the
												// DSTORE listener did set on
												// 'd'
		...
		public void instructionExecuted(VM vm, ThreadInfo ti,
			...
			if (executedInsn instanceof InvokeInstruction) {
				...
				if (mName.equals("goModel") || mName.equals("goNative")) {
					...
					System.out.println("listener notified of: " + mName
							+ "(), attributes= " + a[0] + ',' + a[1] + ','
							+ a[2]);

					// note - this is only acceptable if we know exactly there
					// are just
					// single attrs
	...
	public void testExplicitRef() {
		if (verifyNoPropertyViolation()) {
			...
			Verify.print("@ 'iDouble' attribute before set: ",
					Integer.toString(attr));
			Verify.println();

			Verify.setFieldAttribute(this, "iDouble", 42);

			attr = Verify.getFieldAttribute(this, "iDouble");
			Verify.print("@ 'iDouble' attribute after set: ",
					Integer.toString(attr));
			Verify.println();
	...
	public void testExplicitArrayRef() {
		if (verifyNoPropertyViolation()) {
			...
			Verify.print("@ 'myArray[5]' attribute before set: ",
					Integer.toString(attr));
			Verify.println();
			...
			attr = Verify.getElementAttribute(myArray, 5);
			Verify.print("@ 'myArray[5]' attribute after set: ",
					Integer.toString(attr));
			Verify.println();
	...
	public void testBacktrack() {
		if (verifyNoPropertyViolation()) {
			int v = 42; // need to init or the compiler does not add it to the
						// name table
	...
	public void testInteger() {
		if (verifyNoPropertyViolation()) {
			...
			// semi autoboxed
			j = o;
			boolean b = Verify.getBoolean(); // just cause some backtracking
												// damage
			attr = Verify.getLocalAttribute("j");
			assert attr == 4200;

			/**
			 * this does not work because of cached, preallocated Integer
			 * objects) // fully autoboxed Integer a = v; j = a; attr =
			 * Verify.getLocalAttribute("j"); assert attr == 4200;
			 **/
		}
	}
	...
	public static class MixedAttrTypeListener extends ListenerAdapter {
		...
		public void executeInstruction(VM vm, ThreadInfo ti,
			...
			if (insnToExecute instanceof INVOKEVIRTUAL) {
				MethodInfo callee = ((INVOKEVIRTUAL) insnToExecute)
						.getInvokedMethod();
				if (callee.getUniqueName().equals("foo(J)J")) {
					System.out
							.println("--- pre-exec foo() invoke interception, setting arg attrs");
				...
				if (mi.getUniqueName().equals("foo(J)J")) {
					System.out
							.println("--- pre-exec foo() return interception");
					StackFrame frame = ti.getModifiableTopFrame();
		...
		public void instructionExecuted(VM vm, ThreadInfo ti,
			...
			if (executedInsn instanceof INVOKEVIRTUAL) {
				MethodInfo callee = ((INVOKEVIRTUAL) executedInsn)
						.getInvokedMethod();
				if (callee.getUniqueName().equals("foo(J)J")) {
					System.out
							.println("--- post-exec foo() invoke interception");

					StackFrame frame = ti.getModifiableTopFrame(); // we are now
																	// in the
																	// callee
					int varIdx = frame.getLocalVariableSlotIndex("x");
					...
					assertTrue(frame.getNextLocalAttr(varIdx, String.class,
							sAttr) == null);

					Object lAttr = frame.getLocalAttr(varIdx, Long.class);
					assertTrue(lAttr != null
							&& lAttr.equals(frame.getLongLocalVariable(varIdx)));
					assertTrue(frame
							.getNextLocalAttr(varIdx, Long.class, lAttr) == null);
				...
				if (mi.getUniqueName().equals("foo(J)J")) {
					...
					System.out
							.println("--- post-exec foo() return interception");
					for (Object a : frame.longOperandAttrIterator()) {
						System.out.println(a);
					}

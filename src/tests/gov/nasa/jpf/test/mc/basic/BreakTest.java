...
public class BreakTest extends TestJPF {
	...
	public void testSimpleIgnore() {
		if (verifyNoPropertyViolation(
				"+listener=.test.mc.basic.BreakTest$FieldIgnorer",
	...
	public void testSimpleBreak() {
		if (verifyNoPropertyViolation(
				"+listener=.test.mc.basic.BreakTest$FieldBreaker",
				"+vm.max_transition_length=1000000")) {
			int i = 42;
	...
	public static class FooCallBreaker extends BreakListener {
		...
		public void instructionExecuted(VM vm, ThreadInfo ti,
			...
			if (executedInsn instanceof InvokeInstruction) { // break on method
																// call
				InvokeInstruction call = (InvokeInstruction) executedInsn;

				if ("foo()V".equals(call.getInvokedMethodName())) {
					System.out.println("# breaking & pruning after: "
							+ executedInsn);
					System.out.println("# registered (ignored) CG: "
							+ ss.getNextChoiceGenerator());
					ti.breakTransition("breakTest"); // not required since we
														// ignore
					ss.setIgnored(true);
				}
			}
	...
	public static class VerifyNextIntBreaker extends BreakListener {
		...
		public void choiceGeneratorRegistered(VM vm, ChoiceGenerator<?> nextCG,
			...
			if (cg.getId().equals("verifyGetInt(II)")) {
				...
				ss.setIgnored(true); // should reset the IntIntervalCG
										// registered by the native getInt()
	...
	public void testIgnoreAfterCG() {
		...
		if (verifyNoPropertyViolation("+listener=.test.mc.basic.BreakTest$VerifyNextIntBreaker")) {
			if (Verify.getBoolean(false)) {
				...
				int i = Verify.getInt(1, 2); // listener breaks & ignores post
												// exec
				fail("should never get here");

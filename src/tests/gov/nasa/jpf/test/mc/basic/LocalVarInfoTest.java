...
public class LocalVarInfoTest extends TestJPF {
	...
	public static class TestLookupListener extends ListenerAdapter {
		...
		public void methodEntered(VM vm, ThreadInfo ti, MethodInfo mi) {
			if (mi.getUniqueName().equals("testLookup()V")) {
				...
				System.out
						.println(" LocalVarInfos (should have {'this', 'x', 'y'} : ");
		...
		public void instructionExecuted(VM vm, ThreadInfo ti,
			...
			if (executedInsn.getMethodInfo() == logMethod) {
				System.out.printf(" %2d: %s", executedInsn.getPosition(),
						executedInsn);
				if (executedInsn instanceof LocalVariableInstruction) {
					...
					log.add(executedInsn.getClass().getSimpleName() + " "
							+ lv.getName());
				}
				System.out.println();
			}
	...
	static String[] expected = { "ALOAD this", "ISTORE x", "ILOAD x",
			"ISTORE y", "ILOAD y" };

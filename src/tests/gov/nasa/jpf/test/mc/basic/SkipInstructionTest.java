...
public class SkipInstructionTest extends TestJPF {
	...
	public static class GetFieldListener extends ListenerAdapter {
		...
		public void executeInstruction(VM vm, ThreadInfo ti,
			...
			if (pc instanceof GETFIELD) {
				...
				if (gf.getVariableId().equals(
						SkipInstructionTest.class.getName() + ".answer")) {
	...
	// this is not using skipInstruction but is setting the next one to enter,
	// which
	// means it can skip over a whole bunch of things. The use that comes to
	// mind is to
	// skip over method bodies, but still preserve the invoke processing so that
	// we
	// preserve the locking/sync (which we otherwise would have to do explicitly
	// from
	// a pre-exec listener
	...
	public static class InvokeListener extends ListenerAdapter {
		...
		public void classLoaded(VM vm, ClassInfo ci) {
			if (ci.getName().equals(
					"gov.nasa.jpf.test.mc.basic.SkipInstructionTest")) {
		...
		public void instructionExecuted(VM vm, ThreadInfo ti,
			...
			if (mi == interceptedMethod) {
				...
				if (!ti.isFirstStepInsn()) {
					...
					if (executedInsn instanceof InvokeInstruction) { // we just
																		// entered
																		// the
																		// interceptedMethod
						IntChoiceFromList cg = new IntChoiceFromList("Test",
								42, 43);
						if (vm.setNextChoiceGenerator(cg)) {
							return; // we are done here, next insn in this
									// method will skip to return
						}
					}
				} else {
					// note - this is the first insn WITHIN the
					// interceptedMethod
					System.out.println("in bottom half: " + executedInsn);
					IntChoiceFromList cg = vm.getCurrentChoiceGenerator("Test",
					...
					if (cg != null) {
						...
						StackFrame frame = ti.getModifiableTopFrame(); // we are
																		// modifying
																		// it
						System.out
								.println("listener is skipping method body of "
										+ mi + " returning " + choice);
						frame.push(choice);

...
public class TraceTest extends TestJPF {
	...
	public void foo() {
		...
		Verify.storeTraceAndTerminateIf(!Verify.isTraceReplay(), TRACE,
				"cut off here..");
	...
	public void testPartialTrace() {
		...
		try {
			...
			if (Verify.getCounter(0) != 1) {
				fail("wrong number of backtracks on non-replay run: "
						+ Verify.getCounter(0));
			}

			// second JPF run
			noPropertyViolation(setTestMethod(TEST_CLASS, "foo"),
					"+listener=.listener.ChoiceSelector", "+choice.use_trace="
							+ TRACE);

			if (Verify.getCounter(0) != 5) {
				fail("wrong number of backtracks on replay run: "
						+ Verify.getCounter(0));
			}
	...
	public void testErrorTrace() {
		...
		try {
			...
			assertionError(setTestMethod("bar"),
					"+listener=.listener.TraceStorer", "+trace.file=" + TRACE);
	...
	public void baz() {
		// note there always is an automatic thread-CG after static
		// initialization
	...
	public void testDepth() {
		...
		try {
			...
			noPropertyViolation("+listener=.listener.TraceStorer",
					"+trace.file=" + TRACE, "+trace.depth=3",
					"+search.depth_limit=3", TEST_CLASS, "baz");

			noPropertyViolation("+listener=.listener.ChoiceSelector",
					"+choice.use_trace=" + TRACE, "+search.depth_limit=3",
					TEST_CLASS, "baz");
		} finally {
			tf.delete();
		}

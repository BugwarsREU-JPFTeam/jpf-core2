...
public class VerifyTest extends TestJPF {
	...
	public void testBreak() {
		...
		if (verifyNoPropertyViolation()) {
			...
			while (x != y) { // JPF should state match on the backjump
				...
				Verify.breakTransition("testBreakTransition"); // this should
																// eventually
																// state match
			}
	...
	public void testChangeListener() {
		...
		if (verifyNoPropertyViolation()) {
			Verify.setProperties("listener=gov.nasa.jpf.listener.StateSpaceAnalyzer"); // This
																						// used
																						// to
																						// cause
																						// a
	...
	@Test
	public void testGetBooleanFalseFirst() {
	...
	/**
	 * This test ensures that stateBacktracked() is called even if the
	 * transistion is ignored. This is important for listeners that keep a state
	 * that must match the VM's state exactly and the state is updated in the
	 * middle of transitions. This is not possible if a backtrack happens on an
	 * ignored transition and the stateBacktracked is not called.
	 */
	@Test
	public void backtrackNotificationAfterIgnore() {
		if (verifyNoPropertyViolation(
				"+listener+=,gov.nasa.jpf.test.mc.basic.VerifyTest$CountBacktrack",

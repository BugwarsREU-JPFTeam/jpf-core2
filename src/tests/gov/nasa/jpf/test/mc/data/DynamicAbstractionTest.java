...
public class DynamicAbstractionTest extends TestJPF {
	...
	public void testMyClass() {
		...
		if (verifyNoPropertyViolation(
			...
			System.out.println("new state for myClass.data = "
					+ matchedObject.data);
	...
	public void testMixedFields() {
		...
		if (verifyNoPropertyViolation(
			...
			System.out.println("new state for myClass.data = "
					+ matchedObject.data + ", " + matchedObject.notAbstracted);
			Verify.incrementCounter(0);
		}
	...
	public void testClassFilter() {
		...
		if (verifyNoPropertyViolation(SERIALIZER_ARG,
				"+das.classes.include=*$MyClass", // only consider MyClass
													// instance and static data
				"+das.methods.exclude=*", // make sure we don't match this
											// stackframe ('i' changes)
			...
			for (int i = 0; i < 2; i++) {
				...
				Verify.breakTransition("testDataAbstraction"); // (2) matching
																// point for
																// someObject

				// if we get here we had a new state (i.e. wasn't matched)
				// NOTE we don't get here for matchedObject.data=0 because that
				// would match with the state before (1)
				System.out.printf(
						"         new state for matched=%d, ignored=%d\n",
						matchedObject.data, ignoredObject.data);
				Verify.incrementCounter(0); // should be only reached once for
											// matchedObject.data={1,2}
			}
		}

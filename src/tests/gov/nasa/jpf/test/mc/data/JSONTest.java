...
public class JSONTest extends TestJPF {
	...
	public void testFillFromJSONInheritance() {
		if (verifyNoPropertyViolation()) {
			MyClass sup = Verify.createFromJSON(MyClass.class,
					"{'j':123, 'i':321 }");
	...
	public void testFillIntArrayFromJSON() {
		if (verifyNoPropertyViolation()) {
			IntArr ia = Verify.createFromJSON(IntArr.class,
					"{'ints': [1, 2, 3]}");
	...
	public void testFillPrimitiveArrays() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "\"bools\" : [true, false, true],"
					+ "\"bytes\" : [-40, -30, -20],"
					+ "\"shorts\" : [2, 3, 4]," + "\"ints\" : [1, 2, 3],"
					+ "\"longs\" : [1000, 2000, 3000],"
					+ "\"floats\" : [12.34, 23.45, 34.56],"
					+ "\"doubles\" : [-12.34, -23.45, -34.56]" + "}";
			PrimitiveArrays pa = Verify.createFromJSON(PrimitiveArrays.class,
					json);
	...
	public void testMultiArrayFilling() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "\"intsInts\" : [[1, 2, 3], [4, 5, 6]]" + "}";
	...
	public void testArrayOfObjectsFilling() {
		if (verifyNoPropertyViolation()) {
			...
			ArrayOfObjects aoo = Verify.createFromJSON(ArrayOfObjects.class,
					json);
	...
	public void testFillingMultArrayOfObjects() {
		if (verifyNoPropertyViolation()) {
			...
			MultObjectsArr moa = Verify.createFromJSON(MultObjectsArr.class,
					json);
	...
	public void testFillStringValue() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "\"s1\" : \"val\"," + "\"s2\" : null" + "}";

			ClassWithString cws = Verify.createFromJSON(ClassWithString.class,
					json);
	...
	public void testSetBoolFromCG() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "'b' : TrueFalse()" + "}";

			Object[] expected = { new Bool(true), new Bool(false) };
	...
	public void testSetByteShortIntFromCG() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "'b' : IntSet(1, 2)," + "'s' : 2,"
					+ "'i' : IntSet(3, 4, 5)," + "'l' : IntSet(8)" + "}";

			Object[] expected = { new ByteShortIntLong(1, 2, 3, 8),
					new ByteShortIntLong(2, 2, 3, 8),
					new ByteShortIntLong(1, 2, 4, 8),
					new ByteShortIntLong(2, 2, 4, 8),
					new ByteShortIntLong(1, 2, 5, 8),
					new ByteShortIntLong(2, 2, 5, 8), };
			ByteShortIntLong bsil = Verify.createFromJSON(
					ByteShortIntLong.class, json);
	...
	public void testFillWithIntevalCG() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "'b' : 1," + "'s' : IntInterval(1, 3),"
					+ "'i' : 1," + "'l' : IntInterval(8, 10)" + "}";

			Object[] expected = { new ByteShortIntLong(1, 1, 1, 8),
					new ByteShortIntLong(1, 2, 1, 8),
					new ByteShortIntLong(1, 3, 1, 8),
					new ByteShortIntLong(1, 1, 1, 9),
					new ByteShortIntLong(1, 2, 1, 9),
					new ByteShortIntLong(1, 3, 1, 9),
					new ByteShortIntLong(1, 1, 1, 10),
					new ByteShortIntLong(1, 2, 1, 10),
					new ByteShortIntLong(1, 3, 1, 10), };
			ByteShortIntLong bsil = Verify.createFromJSON(
					ByteShortIntLong.class, json);
			checkValue(expected, bsil);
		}
	...
	public void testFillInnerClassCG() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "'inner' : {" + "'i' : IntSet(3, 4, 5)" + "}"
					+ "}";

			Object[] expected = { new O(3), new O(4), new O(5), };
	...
	public void testFillingObjectInArrayWithCG() {
		if (verifyNoPropertyViolation()) {
			...
			Object[] expected = { new ArrI(1, 4), new ArrI(2, 4),
					new ArrI(3, 4), new ArrI(1, 5), new ArrI(2, 5),
					new ArrI(3, 5), new ArrI(1, 6), new ArrI(2, 6),
					new ArrI(3, 6), };
	...
	public void testObjectFromCG() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "'bi' : IntSet(1, 2, 3)" + "}";

			Object[] expected = { new BoxedInteger(1), new BoxedInteger(2),
					new BoxedInteger(3), };
	...
	public void testBoxedDoubleFromCG() {
		if (verifyNoPropertyViolation()) {
			String json = "{" + "'d' : DoubleSet(1.1, 2.2, 3.3)" + "}";

			Object[] expected = { new BoxedDouble(1.1), new BoxedDouble(2.2),
					new BoxedDouble(3.3), };

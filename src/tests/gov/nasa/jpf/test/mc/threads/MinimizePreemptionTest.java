...
public class MinimizePreemptionTest extends TestJPF {
	...
	public static class A implements Runnable {
		...
		public void run() {
			int s = 0;
			for (int i = 1; i <= 5; i++) { // use loop var to make sure there
											// always is a state change
	...
	public void testNoPreemptions() {
		...
		if (verifyAssertionError(SEARCH_OPT)) {
			...
			for (int i = 0; i < 10; i++) { // use loop var to make sure there
											// always is a state change
	...
	public static class Listener extends ListenerAdapter {
		...
		int getPreemptions(Search search) {
			VM vm = search.getVM();
			int n = 0;
			ThreadChoiceGenerator cg = vm
					.getLastChoiceGeneratorOfType(ThreadChoiceGenerator.class);

			while (cg != null) {
				ThreadChoiceGenerator cgPrev = cg
		...
		public void stateRestored(Search search) {
			...
			System.out.println("   explore state with " + nPreemptions
					+ " preemptions");

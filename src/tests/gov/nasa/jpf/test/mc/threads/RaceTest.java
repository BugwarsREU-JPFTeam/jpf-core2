...
public class RaceTest extends TestJPF {
	...
	static final TypeRef PROPERTY = new TypeRef(
	...
	// this represents the case where the class loading thread is
	// non-deterministic
	...
	public void testSymmetricStaticRace() {
		if (verifyUnhandledExceptionDetails("java.lang.RuntimeException",
				"got race", "+vm.por.shared.class=.vm.GlobalTrackingPolicy")) {
	...
	public void testInstanceRaceListenerExclude() {
		if (verifyNoPropertyViolation(LISTENER, "+race.exclude="
				+ RaceTest.class.getName() + "*")) {
	...
	public void testInstanceRaceListenerInclude() {
		if (verifyPropertyViolation(PROPERTY, LISTENER, "+race.include="
				+ RaceTest.class.getName() + "*")) {
			testInstanceRaceNoThrow();
		}
	}
	...
	/*
	 * mostly the same as above except of that the race candidates are the same
	 * insn instance, i.e. use the same cached insn fields values
	 */

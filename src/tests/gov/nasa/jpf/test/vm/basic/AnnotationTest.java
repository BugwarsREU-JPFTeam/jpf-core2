...
public class AnnotationTest extends TestJPF {
	...
	public void testStringValueOk() {
		if (verifyNoPropertyViolation()) {
			try {
				java.lang.reflect.Method method = AnnotationTest.class
						.getMethod("testStringValueOk");
	...
	public void testStringArrayValueOk() {
		if (verifyNoPropertyViolation()) {
			try {
				java.lang.reflect.Method method = AnnotationTest.class
						.getMethod("testStringArrayValueOk");
	...
	public void testPartialDefaultParamsOk() {
		if (verifyNoPropertyViolation()) {
			try {
				java.lang.reflect.Method method = AnnotationTest.class
						.getMethod("testPartialDefaultParamsOk");
	...
	public void testSingleDefaultParamOk() {
		if (verifyNoPropertyViolation()) {
			try {
				java.lang.reflect.Method method = AnnotationTest.class
						.getMethod("testSingleDefaultParamOk");
	...
	@A6
	@Test
	public void testAnnotationClass() throws ClassNotFoundException,
			NoSuchMethodException {
		if (verifyNoPropertyViolation()) {
			Class clazz = Class
					.forName("gov.nasa.jpf.test.vm.basic.AnnotationTest");
	...
	@Test
	@A7(MyEnum.ONE)
	public void testEnumValue() throws ClassNotFoundException,
			NoSuchMethodException {
		if (verifyNoPropertyViolation()) {
			Class clazz = Class
					.forName("gov.nasa.jpf.test.vm.basic.AnnotationTest"); // Any
																			// class
																			// outside
																			// of
																			// this
																			...
																			// will
																			// do.
			Method method = clazz.getDeclaredMethod("testEnumValue"); // Any
																		// method
																		// with
																		// an
																		// annotation
	...
	public void testClassValue() throws ClassNotFoundException,
			NoSuchMethodException {
		if (verifyNoPropertyViolation()) {
			Class clazz = Class
					.forName("gov.nasa.jpf.test.vm.basic.AnnotationTest"); // Any
																			...
																			// will
																			// do.
			Method method = clazz.getDeclaredMethod("testClassValue"); // Any
																		// method
																		// with
																		// an
																		// annotation
	...
	public void testClassArrayValueOk() throws ClassNotFoundException,
		...
		public void executeInstruction(VM vm, ThreadInfo ti,
			...
			if (insnToExecute instanceof GETFIELD) {
				...
				if (fi.getName().equals("data")) {
					AnnotationInfo ai = fi
							.getAnnotation("gov.nasa.jpf.test.vm.basic.AnnotationTest$A1");
					System.out.println("annotation for " + fi.getFullName()
							+ " = " + ai);
		...
		public void executeInstruction(VM vm, ThreadInfo ti,
			...
			if (insnToExecute instanceof InvokeInstruction) {
				...
				if (mi.getName().equals("foo")) {
					...
					assert pai.length == 2 : "wrong number of parameter annotation arrays: "
							+ pai.length;
					assert pai[0] != null : "no parameter annotation for first argument found";
					assert pai[0].length == 1 : "wrong number of annotations for first argument: "
							+ pai[0].length;
					assert pai[1] != null : "no parameter annotation for second argument found";
					assert pai[1].length == 0 : "wrong number of annotations for first argument: "
							+ pai[1].length;

					for (int i = 0; i < pai.length; i++) {
						System.out
								.println("-- annotations for parameter: " + i);
						AnnotationInfo[] ai = pai[i];
						if (ai != null && ai.length > 0) {
							for (int j = 0; j < ai.length; j++) {
								assert (ai[i] != null) : "null annotation for paramter: "
										+ j;

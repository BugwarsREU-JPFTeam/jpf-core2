...
public class ArrayTest extends TestJPF {
	...
	public void test2DStringArray() {
		if (verifyNoPropertyViolation()) {
			...
			assert (a.getClass().getComponentType().getName()
	...
	public void testAoBX() {
		if (verifyNoPropertyViolation()) {
			...
			throw new RuntimeException(
					"array bounds check failed (no ArrayIndexOutOfBoundsException)");
		}
	}
	...
	public void testArrayStoreException() {
		if (verifyNoPropertyViolation()) {
			...
			throw new RuntimeException(
					"array element type check failed (no ArrayStoreException)");
		}
	}
	...
	public void testArrayStoreExceptionArraycopy() {
		if (verifyNoPropertyViolation()) {
			...
			try {
				System.arraycopy(new Object[] { new Integer(42) }, 0,
						new String[2], 0, 1);
			...
			if (!caught) {
				throw new RuntimeException(
						"array element type check failed (no ArrayStoreException: Integer[] -> String[])");
			}
			...
			if (!caught) {
				throw new RuntimeException(
						"array element type check failed (no ArrayStoreException: Integer[] -> int[])");
			}
			...
			if (!caught) {
				throw new RuntimeException(
						"array element type check failed (no ArrayStoreException: int[] -> long[])");
			}
			...
			if (!caught) {
				throw new RuntimeException(
						"array element type check failed (no ArrayStoreException: int[] -> float[])");
			}
			...
			if (!caught) {
				throw new RuntimeException(
						"array element type check failed (no ArrayStoreException: double[] -> long[])");
			}
		}
	}

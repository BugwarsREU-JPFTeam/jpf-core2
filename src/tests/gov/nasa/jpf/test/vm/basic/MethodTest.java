...
class TestMethodBase extends TestJPF implements TMI {
	...
	boolean baz(boolean p, byte b, char c, short s, int i, long l, float f,
			double d, Object o) {
		assert p;
		assert b == 4;
		assert c == '?';
...
public class MethodTest extends TestMethodBase {
	...
	double foo(boolean p, byte b, char c, short s, int i, long l, float f,
			double d, Object o) {
		assert p;
		assert b == 4;
		assert c == '?';
	...
	public void testCall() {
		if (verifyNoPropertyViolation()) {
			...
			assert o.foo(true, (byte) 4, '?', (short) 42, 4242, 424242, 4.2f,
					4.242, new Integer(42)) == 4.242;
			assert o.data == 43;
		}
	}
	...
	public void testInheritedCall() {
		if (verifyNoPropertyViolation()) {
			MethodTest o = new MethodTest();
			assert o.baz(true, (byte) 4, '?', (short) 42, 4242, 424242, 4.2f,
					4.242, new Integer(42));
	...
	/**
	 * this is tricky - both the allocations and the foo() calls have to be the
	 * same instructions, and we have to remove all 'a' references before
	 * forcing a GC. Otherwise we don't allocate 'a's with the same reference
	 * values
	 * 
	 */
	@Test

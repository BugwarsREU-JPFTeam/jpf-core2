...
public class InterruptTest extends TestJPF {
		...
		if (verifyNoPropertyViolation()) {
			Runnable r = new Runnable() {
				...
				public void run() {
					...
					// doesn't matter from where we interrupt (thread can
					// interrupt itself)
					Thread t = Thread.currentThread();
					t.interrupt();

					synchronized (this) {
						// if we don't enter here, this should be reported as a
						// deadlock

						System.out.println("T waiting");
						try {
							wait(); // this should immediately throw, i.e.
									// return
							assert false : "wait() did not throw InterruptedException";
						} catch (InterruptedException ix) {
							ix.printStackTrace(); // should include
													// Object.wait()
							System.out.println("T interrupted, terminating");
							assert !t.isInterrupted() : "throw didn't reset interrupt status";
							return;
						} catch (Throwable x) {
							assert false : "wait did throw wrong exception: "
									+ x;
							return;
						}
					}
	...
	public void testWaitAsyncInterrupt() {
		if (verifyNoPropertyViolation()) {
			...
			// no matter if this is executed before or after t1 enters
			// Object.wait()
			// it should terminate it by throwing. In fact, JPF should explore
			// both
			// paths

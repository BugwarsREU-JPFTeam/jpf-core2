...
public class JoinTest extends TestJPF {
	...
	static final String[] JPF_ARGS = { "+cg.threads.break_start=true",
			"+cg.threads.break_yield=true", "+vm.tree_output=false",
			"+vm.path_output=true" };
	...
	@Test
	public void testNoRunnableSimpleJoin() {
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			Thread t = new Thread() {
				@Override
	...
	static class SomeThread extends Thread {
		...
		public void run() {
			synchronized (this) {
				// this causes a transition break - write on a shared object
				// while
	...
	@Test
	public void testJoinHoldingLock() {
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			Runnable r = new Runnable() {
				@Override
				public void run() {
	...
	@Test
	public void testPreJoinInterrupt() {
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			Runnable r = new Runnable() {
				@Override
				public void run() {
	...
	@Test
	public void testJoinBeforeStart() {
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			try {
				Thread t = new Thread();
				t.join();
				System.out
						.println("join on not-yet-started thread has no effect");
			} catch (Exception ex) {
				fail(ex.getMessage());
			}
		}
	...
	@Test
	public void testJoinAfterNotify() {
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			try {
				final Thread t = new Thread() {
					...
					public void run() {
						synchronized (this) {
							...
							notifyAll(); // this should not get us out of the
											// join()
						}
	...
	@Test
	public void testJoinNotifyDeadlock() {
		if (verifyDeadlock(JPF_ARGS)) {
			try {
				final Thread t = new Thread() {
					@Override
	...
	@Test
	public void testRecursiveJoinThreadGroup() {
		if (!isJPFRun()) {
		...
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			...
			try {
				Thread[] actives = new Thread[10]; // the length is just a guess
													// here
				int nActives = workers.enumerate(actives, true);
				System.out.println("main joining " + nActives
						+ " active threads");

				while (nActives > 0) {
					assert nActives < actives.length; // it has to be strictly
														// less to know we've
														// got all
					...
					System.out.println("..main now joining " + nActives
							+ " active threads");
				}
			} catch (Throwable x) {
		...
		if (!isJPFRun()) {
			// not an ideal test since we don't know if the threads are still
			// alive
	...
	public void testInterruptThreadWaitingToJoin() {
				...
				class WaitingToJoinThread extends Thread {
					...
					public void run() {
						try {
							...
							System.out
									.println("-- parent interrupted while child continues to run");
							Verify.incrementCounter(0);
						}
					}
				}
		...
		if (!isJPFRun()) {
			// at least one execution interrupts parent while child continues to
			// run
			assert Verify.getCounter(0) > 0;
		}
	}
	...
	public void testTimeoutJoin() {
		...
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			...
			try {
				...
				// we should get here for both terminated and non-terminated
				// thread
				switch (t.getState()) {
				case TERMINATED:
	...
	@Test
	public void testZeroTimeoutJoin() {
		if (!isJPFRun()) {
			Verify.resetCounter(0);
			Verify.resetCounter(1);
		}
	...
	@Test
	public void testNegativeTimeoutJoin() {
		if (verifyNoPropertyViolation(JPF_ARGS)) {
			try {
				Thread t = new Thread();
				t.join(-1);
	...
	@Test
	public void testNestedLocksJoin() {
		if (verifyNoPropertyViolation(JPF_ARGS)) {

...
public class ThreadExceptionHandlerTest extends TestJPF {
	...
	class NPEHandler implements Thread.UncaughtExceptionHandler {
		...
		public void uncaughtException(Thread t, Throwable e) {
			 ...
			 * System.out.print("   "); System.out.println(e); assertTrue(e
	...
	public void checkDefaults() {
		if (verifyNoPropertyViolation()) {
			...
			assertEquals(ct.getUncaughtExceptionHandler(), ct.getThreadGroup());
	...
	/*
	 * The existing handlers should forward the exception to the newly set
	 * default handler.
	 */
	@Test
	...
	/*
	 * The uncaughtHandler has precedence over any other handlers, including the
	 * thread group a thread belongs to.
	 */
	@Test
	...
	/*
	 * The handler of a thread's ThreadGroup has precedence over the default
	 * handler.
	 */
	@Test
	public void testPrecedence2() {
		if (verifyNoPropertyViolation("+vm.ignore_uncaught_handler=false",
				"+vm.pass_uncaught_handler")) {
			n = 0;
			Thread w = new Thread(new NPEHandler2("test"), new TestRunnable()); // n
																				// =
																				// 2
			Thread.setDefaultUncaughtExceptionHandler(new NPEHandler()); // n =
																			// 1
			w.start();
			try {
				w.join();
	...
	/*
	 * The handler of a ThreadGroup's parent has precedence over the child
	 * ThreadGroup and the default handler.
	 */
	@Test
	public void testPrecedence3() {
		if (verifyNoPropertyViolation("+vm.ignore_uncaught_handler=false",
				"+vm.pass_uncaught_handler")) {
			n = 0;
			Thread w = new Thread(new NPEHandler4(new NPEHandler3("parent"),
					"child"), new TestRunnable());
			Thread.setDefaultUncaughtExceptionHandler(new NPEHandler());
			w.start();
			try {
				w.join();

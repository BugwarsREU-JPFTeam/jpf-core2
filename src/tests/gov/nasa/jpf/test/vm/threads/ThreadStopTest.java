...
public class ThreadStopTest extends TestJPF {
	...
	public void testStopNewThread() {
		...
		if (verifyNoPropertyViolation()) {
			...
			assertTrue("main did not get past starting stopped thread",
					Verify.getCounter(0) > 0);
		}
	}
	...
	public void testStopNewSyncThread() {
		...
		if (verifyNoPropertyViolation()) {
			...
			synchronized (t) {
				...
				System.out
						.println("# main got past start of stopped sync thread");
			}
	...
	public void testStopSelf() {
		...
		if (verifyNoPropertyViolation()) {
			...
			assertTrue("main did not get past joining stopped thread",
					Verify.getCounter(0) > 0);
		}
	}
	...
	public void testStopBlocked() {
		...
		if (verifyNoPropertyViolation()) {
			...
			synchronized (lock) {
				...
				assertTrue("t not blocked",
						t.getState() == Thread.State.BLOCKED);

				System.out.println("# main now stopping t");
				t.stop();

				assertTrue("t dead despite main not giving up lock",
						t.isAlive());
			...
			Thread t = new Thread() {
				...
				public synchronized void run() {
					...
					try {
						...
						// usually not a good style, but can happen for exit
						// processing
	...
	public void testStopJoin() {
		...
		if (verifyNoPropertyViolation()) {
			waitee = new Thread() {
				...
				public void run() {
					...
					synchronized (lock) {
						try {
							System.out
									.println("# waitee now waiting for main to signal..");
							lock.wait();
			...
			t.stop();
			assertTrue(
					"t should not have terminated since waitee not notified yet",
					t.isAlive());

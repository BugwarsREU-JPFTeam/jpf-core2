...
public class ThreadTest extends TestJPF {
	...
	public void testDaemon() {
		if (verifyNoPropertyViolation()) {
			...
			if ((didRunThread == null) || (didRunThread.equals(tname))) {
				throw new RuntimeException("thread did not execute: "
						+ didRunThread);
			}
		}
	}
	...
	public void testDaemonTermination() {
		if (verifyNoPropertyViolation("+cg.threads.break_start=true",
			...
			Runnable r = new Runnable() {
				...
				@FilterField
				// without this, we have a perfectly open state space and never
				// finish in JPF
				int n = 0;
				...
				public void run() {
					while (true) { // loop forever or until main finishes
						...
						// NOTE: this does not necessarily hold outside of JPF,
						// since the daemon might still run for a few cycles
			...
			Thread.yield();
			// finishing this thread should also (eventually) terminate the
			// daemon
	...
	public void testMain() {
		if (verifyNoPropertyViolation()) {
			...
			if (!name.equals(refName)) {
				throw new RuntimeException("wrong main thread name, is: "
						+ name + ", expected: " + refName);
			}
			...
			if (!name.equals(refName)) {
				throw new RuntimeException("Thread.setName() failed, is: "
						+ name + ", expected: " + refName);
			}
			...
			if (prio != refPrio) {
				throw new RuntimeException("main thread has no NORM_PRIORITY: "
						+ prio);
			}
			...
			if (prio != refPrio) {
				throw new RuntimeException("main thread setPriority failed: "
						+ prio);
			}
			...
			Runnable r = new Runnable() {
				...
				public void run() {
					...
					if (prio != (Thread.MIN_PRIORITY + 2)) {
						throw new RuntimeException("wrong Thread priority: "
								+ prio);
					}
	...
	public void testJoin() {
		if (verifyNoPropertyViolation()) {
			...
			synchronized (r) {
				...
				if (didRunThread != null) {
					throw new RuntimeException(
							"sync thread did execute before lock release");
				}
			}
			...
			if (didRunThread == null) {
				throw new RuntimeException(
						"sync thread did not run after lock release");
			}
		}
	}
		...
		if (verifyNoPropertyViolation()) {
			Runnable r = new Runnable() {
				...
				public void run() {
					...
					didRunThread = Thread.currentThread().getName(); // this
																		// causes
																		// a CG
			...
			try {
				...
				case RUNNABLE:
					...
					break;
				default:
					throw new RuntimeException("infeasible thread state: "
							+ t.getState());
				}

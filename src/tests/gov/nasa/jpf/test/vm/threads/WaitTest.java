...
public class WaitTest extends TestJPF {
	...
	@Test
	public void testVerySimpleWait() {
				...
				try {
					...
					if (!cond) {
						throw new RuntimeException(
								"'cond' not set, premature wait return");
					}
				} catch (InterruptedException ix) {
					throw new RuntimeException("got interrupted");
				}
	...
	/**
	 * that's a misnomer, since this one executes almost all of signal handling
	 * (except of mixed wait/timeout-wait and timeout joins) it should be called
	 * 'testAlmostAll'
	 * 
	 */
	@Test
	public void testNotifyAll() {
		if (verifyNoPropertyViolation()) {
			...
			try {
				...
				throw new RuntimeException(
						"main interrupted while waiting for thread1 to finish");
			}
			try {
				t2.join();
			} catch (InterruptedException ix) {
				throw new RuntimeException(
						"main interrupted while waiting for thread2 to finish");
			}

...
public class SplitInputStreamTest extends TestJPF {
	...
	private static final Random s_random = Verify.isRunningInJPF() ? null
			: new SecureRandom();
	...
	@Test
	public void ignoreClosedStream() throws IOException, NoSuchFieldException,
	...
	public void concurrentRead() throws IOException, InterruptedException {
		...
		if (verifyNoPropertyViolation(s_jpfArgs)) {
			source = new InputStream() {
				...
				public int read() {
					if (m_reader == null) {
						m_reader = Thread.currentThread(); // JPF will catch the
															// race condition if
															// 2 threads call
															// read
															// concurrently.
		...
		if (verifyNoPropertyViolation(s_jpfArgs)) {
			source = new InputStream() {
				...
				public int available() {
					...
					m_access = Thread.currentThread(); // JPF will catch the
														// race condition if 2
														// threads call
														// concurrently.
					m_access = null;
